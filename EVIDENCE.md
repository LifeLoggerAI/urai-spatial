...
- Tier/XR release enforcement: `scripts/check-tier-xr-release-matrix.mjs` added in commit `ceda0b7f69bf13cfcb5028c99acfda0bccf6cc99`. Root release scripts now run `check:tier-xr-release-matrix` during `lock:static` and run `xr:verify` during `verify:release:full` in commit `723deaca998973ac1bf16ea0c7856e26a4ef75aa`.
