...
export default function DeepStars() {
  ...
  useFrame(({clock}) => {
    if (!lastCameraPos.current.equals(camera.position)) {
      cameraVelocity.current.subVectors(lastCameraPos.current, camera.position);
    ...
    <>
      <group ref={nearLayer}>
        <points geometry={nearStars}>
          <pointsMaterial color="#bbbbbb" size={0.08} sizeAttenuation depthWrite={false} fog={false} />
        </points>
      </group>
      <group ref={midLayer} scale={2}>
        <points geometry={midStars}>
          <pointsMaterial color="#888888" size={0.05} sizeAttenuation depthWrite={false} fog={false} />
        </points>
      </group>
      <group ref={farLayer} scale={4}>
        <points geometry={farStars}>
          <pointsMaterial color="#555555" size={0.03} sizeAttenuation depthWrite={false} fog={false} />
        </points>
      </group>
      <points ref={dustLayer}>
        <bufferGeometry attach="geometry">
            <bufferAttribute
                ...
                count={1000}
                array={new Float32Array(3000).map(() => (Math.random() - 0.5) * 200)}
                itemSize={3}
            />
        </bufferGeometry>
        <pointsMaterial
