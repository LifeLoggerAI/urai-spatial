import { createSystemLoop, isSystemLoopState, type SystemLoopState } from "./kernel/SystemLoop";
...
async function main() {
  ...
  const loop = await createSystemLoop({ tickIntervalMs: 1000, replayLimit: 50, initialState: restoredState });
  const dashboard = new SimulationDashboard(loop.engine.bus, { enabled: true, logIntervalMs: 2000 });
  dashboard.attach();
