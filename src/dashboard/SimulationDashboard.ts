...
export type DashboardOptions = { enabled?: boolean; logIntervalMs?: number };
...
export class SimulationDashboard {
  ...
  private metrics = { ticks: 0, events: 0, lastMemoryNodes: 0, lastPredictions: 0, lastXRObjects: 0 };
  ...
  attach() {
    ...
    this.detachWildcard = this.bus.on("*", () => { this.metrics.events += 1; });
    this.detachTick = this.bus.on("system.tick", () => { this.metrics.ticks += 1; });
    this.detachSnapshot = this.bus.on("state.snapshot", (event: KernelEvent) => {
      const state = event.payload as { snapshot?: { totalNodes?: number }; prediction?: { candidates?: unknown[] }; frame?: { objects?: unknown[] } } | undefined;
      if (!state) return;
  ...
  private render() {
    console.log("\n[URAI DASHBOARD]");
    console.table({ runtime: "URAI Spatial Dashboard", ...this.metrics, timestamp: Date.now() });
  }
