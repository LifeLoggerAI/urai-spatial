...
type State = { stars: MemoryStar[]; activeStarId: string | null; activeChapterId: ChapterId | null; camera: LifeMapCamera; companionLine: string; phase: LifeMapPhase; reducedMotion: boolean };
type Action = { type: 'SET_REDUCED_MOTION'; value: boolean } | { type: 'SET_GLOWING_STARS'; ids: string[] } | { type: 'FOCUS_STAR'; starId: string } | { type: 'FOCUS_CLUSTER'; chapterId: ChapterId; camera: LifeMapCamera; companionLine: string } | { type: 'MARK_RESOLVED'; starId: string } | { type: 'SET_CAMERA'; camera: LifeMapCamera } | { type: 'CLEAR_FOCUS' } | { type: 'SET_COMPANION_LINE'; line: string };
...
export default function LifeMapScene() {
  ...
  useEffect(() => {
    ...
    const run = () => {
      const pool = state.stars.filter((s) => s.id !== state.activeStarId);
      const count = 1 + Math.floor(Math.random() * 3);
      const top = pool.map((s) => ({ id: s.id, score: 1 + s.recency * 2 + s.intensity * 2 + s.unresolvedWeight * 3 + eScore[s.emotion] - (s.state === 'resolved' ? 4 : 0) })).sort((a, b) => b.score - a.score).slice(0, Math.max(4, count * 2));
      const picked = top.sort(() => Math.random() - 0.5).slice(0, count).map((s) => s.id);
      ...
      timer = window.setTimeout(run, 8000 + Math.floor(Math.random() * 6000));
    };
  ...
  .connections{position:absolute;inset:0;width:100%;height:100%}.connection{opacity:.35;transition:opacity .45s ease}.connection.is-bright{opacity:1}.connection.is-dimmed{opacity:.2}.connection-line{stroke:rgba(190,220,255,.24);stroke-width:.2;stroke-dasharray:1 1.8;stroke-linecap:round}.connection-line.is-flowing{animation:constellationFlow 6s linear infinite}.connection-line.is-reverse{animation-direction:reverse}.connection-line.is-active{stroke:rgba(220,246,255,.86);stroke-width:.36;filter:drop-shadow(0 0 9px rgba(125,211,252,.75))}.connection-line.is-static{stroke-dasharray:none}.flow-particle{fill:rgba(208,241,255,.95);filter:drop-shadow(0 0 5px rgba(125,211,252,.8))}
  ...
  @media (prefers-reduced-motion:reduce){.memory-star,.lifemap-space{animation:none!important;transition-duration:.01ms!important}.connection-line{animation:none!important;stroke-dasharray:none}.connection{opacity:.42}.connection.is-bright{opacity:.8}.connection-line.is-active{stroke-width:.4}}
`}</style></main>;
}
