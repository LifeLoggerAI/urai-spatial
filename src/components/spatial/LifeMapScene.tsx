...
type TransitionTarget = { phase: LifeMapPhase; activeStarId: string | null; activeChapterId: ChapterId | null; camera: LifeMapCamera; companionLine?: string }
type Action = { type: 'SET_REDUCED_MOTION'; value: boolean } | { type: 'SET_GLOWING_STARS'; ids: string[] } | { type: 'TRANSITION_VIEW'; target: TransitionTarget } | { type: 'MARK_RESOLVED'; starId: string } | { type: 'SET_COMPANION_LINE'; line: string };
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case 'TRANSITION_VIEW': {
      const focusedStar = action.target.activeStarId ? state.stars.find((s) => s.id === action.target.activeStarId) : null;
      return {
        ...state,
        phase: action.target.phase,
        activeStarId: action.target.activeStarId,
        activeChapterId: action.target.activeChapterId,
        camera: action.target.camera,
        companionLine: action.target.companionLine ?? state.companionLine,
        stars: state.stars.map((s) => {
          if (s.state === 'resolved') return s;
          if (!focusedStar) return { ...s, state: s.state === 'glowing' && action.target.phase === 'living' ? s.state : 'idle' };
          if (s.id === focusedStar.id) return { ...s, state: 'active', lastActivatedAt: Date.now() };
          return focusedStar.connectedTo.includes(s.id) && s.state === 'glowing' ? s : { ...s, state: 'idle' };
        }),
      };
    }
...
export default function LifeMapScene() {
  ...
  useEffect(() => { if (typeof window === 'undefined') return; const onEsc = (e: KeyboardEvent) => e.key === 'Escape' && transitionTo({ phase: 'living', activeStarId: null, activeChapterId: null, camera: { x: 50, y: 50, zoom: 1 } }); window.addEventListener('keydown', onEsc); return () => window.removeEventListener('keydown', onEsc); }, []);
  ...
  const transitionTo = (target: TransitionTarget) => dispatch({ type: 'TRANSITION_VIEW', target });

  return <main className="life-map-shell" aria-label="URAI Spatial Life Map scene"><section className={`lifemap-space ${activeStar ? 'is-focused' : ''}`}><div className="starfield" style={{ ['--camera-x' as string]: `${state.camera.x}%`, ['--camera-y' as string]: `${state.camera.y}%`, ['--camera-zoom' as string]: String(state.camera.zoom) }}><svg className="connections" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden>{state.stars.flatMap((s) => s.connectedTo.map((to) => [s.id, to] as const)).filter(([a, b]) => a < b).map(([a, b]) => { const s1 = starById.get(a); const s2 = starById.get(b); if (!s1 || !s2) return null; const isActive = !!activeStar && (a === activeStar.id || b === activeStar.id || activeStar.connectedTo.includes(a) || activeStar.connectedTo.includes(b)); return <line key={`${a}-${b}`} className={`connection-line is-flowing ${isActive ? 'is-active' : activeStar ? 'is-dimmed' : 'is-glowing'}`} x1={s1.x} y1={s1.y} x2={s2.x} y2={s2.y} />; })}</svg>{state.stars.map((star) => { const connected = !!activeStar && activeStar.connectedTo.includes(star.id); const chapterFocused = state.phase === 'cluster' && star.chapterId === state.activeChapterId; const dimmed = state.phase === 'focus' ? (!!activeStar && star.id !== activeStar.id && !connected) : state.phase === 'cluster' ? !chapterFocused : false; return <button key={star.id} type="button" className={`memory-star state-${star.state} ${connected ? 'is-connected' : ''} ${chapterFocused ? 'is-chapter-focused' : ''} ${dimmed ? 'is-dimmed' : ''}`} style={{ left: `${star.x}%`, top: `${star.y}%`, width: `${star.size}px`, height: `${star.size}px` }} aria-label={`${star.title}, ${star.emotion}, ${star.state}`} onClick={() => { transitionTo({ phase: 'focus', activeStarId: star.id, activeChapterId: star.chapterId, camera: { x: star.x, y: star.y, zoom: 1.8 }, companionLine: star.narratorLine }); emitNarratorEvent({ event: 'lifemap.star.focus', starId: star.id, chapterId: star.chapterId, emotion: star.emotion }); emitTimelineSync({ phase: 'focus', activeStarId: star.id, activeChapterId: star.chapterId }); }}><span>{star.title}</span></button>; })}</div></section><aside className="panel export-panel" aria-label="Export panel"><button type="button">Export snapshot</button><button type="button">Export arc</button></aside><aside className="panel companion-panel" aria-label="Companion panel"><h2>Companion</h2><p>{state.companionLine}</p></aside>{activeStar && <aside className="panel detail" aria-live="polite"><h3>{activeStar.title}</h3><p>{activeStar.emotion} · {CHAPTERS.find((c) => c.id === activeStar.chapterId)?.title}</p><p>{activeStar.narratorLine}</p><div className="actions"><button type="button" onClick={() => { transitionTo({ phase: 'focus', activeStarId: activeStar.id, activeChapterId: activeStar.chapterId, camera: { x: activeStar.x, y: activeStar.y, zoom: 1.8 }, companionLine: 'Replaying the emotional thread.' }); emitNarratorEvent({ event: 'lifemap.star.focus', starId: activeStar.id, chapterId: activeStar.chapterId, emotion: activeStar.emotion, action: 'replay' }); }}>Replay</button><button type="button" onClick={() => { transitionTo({ phase: 'focus', activeStarId: activeStar.id, activeChapterId: activeStar.chapterId, camera: { x: activeStar.x, y: activeStar.y, zoom: 1.8 }, companionLine: 'Reflection mode is open.' }); emitNarratorEvent({ event: 'lifemap.star.focus', starId: activeStar.id, chapterId: activeStar.chapterId, emotion: activeStar.emotion, action: 'reflect' }); }}>Reflect</button><button type="button" onClick={() => { dispatch({ type: 'MARK_RESOLVED', starId: activeStar.id }); emitNarratorEvent({ event: 'lifemap.star.resolved', starId: activeStar.id, chapterId: activeStar.chapterId, emotion: activeStar.emotion, action: 'resolve' }); emitTimelineSync({ phase: 'focus', activeStarId: activeStar.id, activeChapterId: activeStar.chapterId }); }}>Mark resolved</button></div></aside>}<nav className="chapter-row" aria-label="Chapter anchors">{CHAPTERS.map((chapter) => <button type="button" key={chapter.id} className={`chapter-pill ${state.activeChapterId === chapter.id ? 'active' : ''}`} onClick={() => { const stars = state.stars.filter((s) => s.chapterId === chapter.id); const x = stars.reduce((a, s) => a + s.x, 0) / stars.length; const y = stars.reduce((a, s) => a + s.y, 0) / stars.length; transitionTo({ phase: 'cluster', activeStarId: null, activeChapterId: chapter.id, camera: { x, y, zoom: 1.45 }, companionLine: CHAPTER_LINES[chapter.id] }); emitNarratorEvent({ event: 'lifemap.cluster.focus', chapterId: chapter.id }); emitTimelineSync({ phase: 'cluster', activeChapterId: chapter.id }); }}><strong>{chapter.title}</strong><small>{chapter.subtitle}</small></button>)}</nav><style jsx>{`
