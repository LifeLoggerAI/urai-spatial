...
export default function SpatialScene() {
  ...
  return (
    <div data-testid="urai-spatial-stage" data-mode={mode} className={`spatial-stage mode-${mode}`}>
      ...
      <div className="bg-stars" aria-hidden style={{ transform: `translate3d(0, ${ascentProgress * -44}px, 0) scale(${1 + ascentProgress * 0.08})` }}>
      ...
      {mode === 'home' ? <section data-testid="urai-home-scene" className="home-scene">
        ...
        <button type="button" data-testid="urai-home-ground" className="home-ground" aria-label="Open Ground" onClick={() => { setGroundOpen(true); window.history.replaceState(null, '', '/?phase=ground'); window.dispatchEvent(new PopStateEvent('popstate')) }} />
      ...
      {mode === 'ground' ? <section data-testid="urai-ground-scene" className="ground-scene"><div className="ground-core" /><button type="button" className="ground-enter" onClick={() => setGroundOpen(true)}>Ascend to LifeMap</button></section> : null}

      {mode === 'ascent' ? <section data-testid="urai-ascent-cover" className="ascent-cover" style={{ opacity: 0.22 + ascentProgress * 0.78 }}>
        <div className="ascent-core" style={{ transform: `translate(-50%, -50%) scale(${1 + ascentProgress * 1.5})`, opacity: 1 - ascentProgress * 0.4 }} />
      </section> : null}
      ...
      .bg-sky,.bg-nebula,.bg-stars,.home-scene,.ascent-cover,.lifemap-scene{position:absolute;inset:0;transition:transform .3s ease}
      ...
      .home-ground{position:absolute;left:-15%;right:-15%;bottom:-35%;height:62%;border-radius:50%;background:radial-gradient(ellipse at top,rgba(15,23,42,.92) 0%,rgba(2,6,23,.98) 58%,#000 100%);box-shadow:inset 0 30px 90px rgba(125,211,252,.18);border:none;cursor:pointer}.ground-scene{display:grid;place-items:center;background:radial-gradient(circle at 50% 72%,rgba(15,23,42,.88),rgba(2,6,23,.95));backdrop-filter:blur(4px)}.ground-core{width:min(72vw,520px);height:min(34vh,280px);border-radius:50%;background:radial-gradient(ellipse at center,rgba(125,211,252,.22),rgba(2,6,23,.06) 70%);box-shadow:0 0 120px rgba(125,211,252,.28)}.ground-enter{position:absolute;bottom:13vh;border:1px solid rgba(191,219,254,.45);background:rgba(2,6,23,.7);color:#e0f2fe;border-radius:999px;padding:10px 16px;font-weight:700;cursor:pointer}
