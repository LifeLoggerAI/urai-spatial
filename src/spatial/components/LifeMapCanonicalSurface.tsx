...
type FlightState = { x: number; y: number; scale: number; vx: number; vy: number; targetScale: number; dragging: boolean; lastX: number; lastY: number; orbit: boolean; autopilot: boolean; autopilotT: number; autopilotFromX: number; autopilotFromY: number; autopilotFromScale: number; autopilotToX: number; autopilotToY: number; autopilotToScale: number; roll: number; targetRoll: number };
...
export default function LifeMapCanonicalSurface() {
  ...
  const flightRef = useRef<FlightState>({ x: 0, y: 0, scale: 1, vx: 0, vy: 0, targetScale: 1, dragging: false, lastX: 0, lastY: 0, orbit: true, autopilot: false, autopilotT: 0, autopilotFromX: 0, autopilotFromY: 0, autopilotFromScale: 1, autopilotToX: 0, autopilotToY: 0, autopilotToScale: 1, roll: 0, targetRoll: 0 });
  ...
  const [flight, setFlight] = useState({ x: 0, y: 0, scale: 1, speed: 0, roll: 0 });

  const selected = lifeNodes.find((n) => n.id === selectedId) || null;
  const stars = useMemo(() => Array.from({ length: 320 }, (_, i) => bgStar(i)), []);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      frame += 1;
      if (f.autopilot) {
        f.autopilotT = Math.min(1, f.autopilotT + 0.018);
        const e = easeInOutCubic(f.autopilotT);
        f.x = f.autopilotFromX + (f.autopilotToX - f.autopilotFromX) * e;
        f.y = f.autopilotFromY + (f.autopilotToY - f.autopilotFromY) * e;
        f.scale = f.autopilotFromScale + (f.autopilotToScale - f.autopilotFromScale) * e;
        f.targetScale = f.scale;
        if (f.autopilotT >= 1) f.autopilot = false;
      } else if (!f.dragging) {
        if (f.orbit && Math.abs(f.vx) < .02 && Math.abs(f.vy) < .02 && !selectedId) {
          f.x += Math.sin(frame / 180) * .012;
          f.y += Math.cos(frame / 210) * .01;
        }
        f.x += f.vx; f.y += f.vy; f.vx *= 0.91; f.vy *= 0.91;
        f.scale += (f.targetScale - f.scale) * 0.14;
      }
      f.x = clamp(f.x, -58, 58); f.y = clamp(f.y, -46, 46);
      f.targetRoll = clamp(f.vx * 1.7, -8, 8);
      f.roll += (f.targetRoll - f.roll) * .12;
      const speed = Math.min(1, Math.hypot(f.vx, f.vy) / 2.1);
      setFlight({ x: f.x, y: f.y, scale: f.scale, speed, roll: f.roll });
  ...
  function beginAutopilot(x: number, y: number, scale: number) { const f = flightRef.current; f.autopilot = true; f.autopilotT = 0; f.autopilotFromX = f.x; f.autopilotFromY = f.y; f.autopilotFromScale = f.scale; f.autopilotToX = x; f.autopilotToY = y; f.autopilotToScale = scale; f.vx = 0; f.vy = 0; }
  function focusNode(node: LifeNode) { setSelectedId(node.id); setMode("focus"); beginAutopilot(50 - node.x, 48 - node.y, 1.86 + node.z * .08); router.replace(routeForMode("focus", node.id), { scroll: false }); }
  function resetFlight() { const f = flightRef.current; f.orbit = true; beginAutopilot(0, 0, 1); setSelectedId(null); setMode("lifemap"); router.replace("/life-map", { scroll: false }); }
  function tour() { const next = selectedId ? lifeNodes[(lifeNodes.findIndex((n) => n.id === selectedId) + 1) % lifeNodes.length] : lifeNodes[0]; focusNode(next); }
  function onPointerDown(event: PointerEvent<HTMLDivElement>) { if (event.button !== 0) return; const f = flightRef.current; f.dragging = true; f.orbit = false; f.autopilot = false; f.lastX = event.clientX; f.lastY = event.clientY; (event.currentTarget as HTMLDivElement).setPointerCapture(event.pointerId); }
  ...
  function onWheel(event: WheelEvent<HTMLDivElement>) { event.preventDefault(); const f = flightRef.current; f.orbit = false; f.autopilot = false; f.targetScale = clamp(f.targetScale - event.deltaY * 0.0014, 0.68, 2.95); }
  ...
  const haloRadius = mode === "focus" && selected ? 260 : 160;
  const skyHaze = 0.2 + transitionProgress * 0.58;
  const starReveal = transitionProgress;
  const cameraStyle = { transform: `translate(${flight.x}%, ${flight.y}%) scale(${flight.scale}) rotate(${flight.roll}deg)` };
  const depthFog = clamp((flight.scale - .68) / 2.27, 0, 1);

  return <div className="lm-root" style={{ ["--orb-intensity" as string]: String(orbIntensity), ["--halo-radius" as string]: `${haloRadius}px`, ["--sky-haze" as string]: String(skyHaze), ["--star-reveal" as string]: String(starReveal), ["--focus-aura" as string]: selected?.auraColor ?? "#7dd3fc", ["--flight-x" as string]: `${flight.x}px`, ["--flight-y" as string]: `${flight.y}px`, ["--flight-speed" as string]: String(flight.speed), ["--depth-fog" as string]: String(depthFog) }} data-phase={phase} data-mode={mode}>
    <div className="sky" />
    <div className="depth-fog" />
    <div className="warp" />
    <div className="nebula nebula-a" /><div className="nebula nebula-b" /><div className="nebula nebula-c" />
    <div className="stars stars-far">{stars.map((s, i) => <i key={i} style={{ left: `${s.x}%`, top: `${s.y}%`, width: s.size, height: s.size, opacity: s.opacity, transform: `translate(${flight.x * s.depth * .28}px, ${flight.y * s.depth * .28}px) scale(${1 + s.depth * (flight.scale - 1) * .18})` }} />)}</div>
    <div className="flight-layer" onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={onPointerUp} onWheel={onWheel}>
      <div className="camera" style={cameraStyle}>
        ...
        <div className="nodes">{lifeNodes.map((n) => { const active = selectedId === n.id; const dim = selectedId && !active && mode !== "lifemap"; return <button key={n.id} className={`node ${active ? "active" : ""} ${dim ? "dim" : ""}`} style={{ left: `${n.x}%`, top: `${n.y}%`, "--aura": n.auraColor, "--size": `${18 + n.intensity * 30}px`, "--z": n.z } as CSSProperties} onClick={(e) => { e.stopPropagation(); focusNode(n); }} aria-label={`Focus ${n.title}`}><span className="orbit" /><span className="pulse" /><span className="core" /><strong>{n.title.split(" ").map((w) => w[0]).join("").slice(0, 3)}</strong></button>; })}</div>
      </div>
    </div>
    <section className="hero-card"><p>MIRROR OF BECOMING</p><h1>{selected && mode !== "lifemap" ? selected.title : "The full arc is visible now."}</h1><span>{selected && mode !== "lifemap" ? selected.narratorLine : "Drag to fly. Wheel or pinch to drift deeper. Use Tour to glide through the arc like a cinematic memory path."}</span></section>
    {selected && mode === "focus" && <aside className="panel focus-panel"><p className="eyebrow">FOCUS STAR</p><h2>{selected.title}</h2><small>{selected.subtitle} · {new Date(selected.timestamp).toDateString()}</small><p>{selected.description}</p><blockquote>{selected.narratorLine}</blockquote><div className="actions"><button onClick={() => navigate("replay", selected.id)}>Replay memory</button><button onClick={tour}>Next star</button><button onClick={resetFlight}>Back to map</button></div></aside>}
