...
function bgStar(i: number) { return { x: (i * 37 + 13) % 100, y: (i * 61 + 7) % 100, size: 1 + ((i * 11) % 4), opacity: 0.18 + (((i * 17) % 7) / 20) }; }
...
export default function LifeMapCanonicalSurface() {
  ...
  const stars = useMemo(() => Array.from({ length: 220 }, (_, i) => bgStar(i)), []);
  ...
  function focusNode(node: LifeNode) { setSelectedId(node.id); setMode("focus"); router.replace(routeForMode("focus", node.id), { scroll: false }); }

  useEffect(() => { const onEsc = (event: KeyboardEvent) => { if (event.key !== "Escape") return; event.preventDefault(); if (mode === "focus" || mode === "replay") navigate("lifemap"); else navigate("home"); }; window.addEventListener("keydown", onEsc); return () => window.removeEventListener("keydown", onEsc); });
  ...
  const cameraStyle = selected && mode !== "lifemap" ? { transform: `translate(${50 - selected.x}%, ${48 - selected.y}%) scale(1.72)` } : undefined;

  return <div className="lm-root" style={{ ["--orb-intensity" as string]: String(orbIntensity), ["--halo-radius" as string]: `${haloRadius}px`, ["--sky-haze" as string]: String(skyHaze), ["--star-reveal" as string]: String(starReveal), ["--focus-aura" as string]: selected?.auraColor ?? "#7dd3fc" }} data-phase={phase} data-mode={mode}>
    <div className="sky" />
    ...
    <div className="camera" style={cameraStyle}>
      ...
      <div className="nodes">{lifeNodes.map((n) => { const active = selectedId === n.id; const dim = selectedId && !active && mode !== "lifemap"; return <button key={n.id} className={`node ${active ? "active" : ""} ${dim ? "dim" : ""}`} style={{ left: `${n.x}%`, top: `${n.y}%`, "--aura": n.auraColor, "--size": `${18 + n.intensity * 30}px` } as CSSProperties} onClick={() => focusNode(n)} aria-label={`Focus ${n.title}`}><span className="pulse" /><span className="core" /><strong>{n.title.split(" ").map((w) => w[0]).join("").slice(0, 3)}</strong></button>; })}</div>
    </div>
    <section className="hero-card"><p>MIRROR OF BECOMING</p><h1>{selected && mode !== "lifemap" ? selected.title : "The full arc is visible now."}</h1><span>{selected && mode !== "lifemap" ? selected.narratorLine : "Life phases, repeated patterns, recovery cycles, relationship lessons, and purpose threads are connected in one zoom-out."}</span></section>
    {selected && mode === "focus" && <aside className="panel focus-panel"><p className="eyebrow">FOCUS STAR</p><h2>{selected.title}</h2><small>{selected.subtitle} · {new Date(selected.timestamp).toDateString()}</small><p>{selected.description}</p><blockquote>{selected.narratorLine}</blockquote><div className="actions"><button onClick={() => navigate("replay", selected.id)}>Replay memory</button><button onClick={() => navigate("mirror", selected.id)}>Mirror</button><button onClick={() => navigate("lifemap")}>Back to map</button></div></aside>}
