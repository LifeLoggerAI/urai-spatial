...
type Mode = "lifemap" | "focus" | "replay" | "home";
...
function routePairs(group: Group) {
  return group.nodeIds.slice(0, -1).map((from, index) => [from, group.nodeIds[index + 1]] as const);
}

export function LifeMapCompletionOverlay() {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const shouldOpen = pathname === "/life-map" || searchParams.get("phase") === "lifemap" || searchParams.get("phase") === "focus" || searchParams.get("phase") === "replay";
  const [mode, setMode] = useState<Mode>(shouldOpen ? "lifemap" : "home");
  ...
  const visible = nodes.filter((node) => filter === "all" || node.type === filter || node.id === selectedId);
  const activePatterns = nodes.filter((node) => node.type === "pattern").length;
  const phaseIndex = Math.min(replayPhases.length - 1, Math.floor(progress / 25));
  const activeEraGroup = groupForEra(era);

  const unwind = () => {
    if (mode === "replay") setMode("focus");
    else if (mode === "focus") { setMode("lifemap"); setSelectedId(null); }
    else if (mode === "lifemap") { setMode("home"); setSelectedId(null); router.push("/home"); }
  };

  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      if (event.key === "Escape" && mode !== "home") { event.preventDefault(); unwind(); }
    };
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  });

  if (!shouldOpen && mode === "home") return null;

  return (
    <div className={`lm-complete ${reducedMotion ? "reduced" : ""}`} data-mode={mode} data-testid="urai-lifemap-completion-overlay">
      ...
      <svg className="routes" aria-hidden="true">
        {groups.flatMap((group) => routePairs(group).map(([fromId, toId]) => {
          ...
          const locked = group.nodeIds.some((id) => nodes.find((node) => node.id === id)?.locked);
      ...
      <section className="hud hud-left" aria-label="LifeMap status">
        <p>LifeMap</p>
        <h2>{era}</h2>
        <div><b>{visible.length}</b><span>Visible Stars</span></div>
      ...
      <div className="nodes">
        {nodes.map((node) => {
          ...
          return <button key={node.id} type="button" data-testid={node.id === "pattern-01" ? "lifemap-node-pattern-01" : `lifemap-node-${node.id}`} className={`life-node ${node.type} ${selectedId === node.id ? "selected" : ""} ${filteredOut ? "filtered" : ""} ${eraDimmed ? "era-dim" : ""} ${focusedDimmed ? "focus-dim" : ""} ${node.locked ? "locked" : ""}`} aria-label={`${node.type} star: ${node.title}`} aria-pressed={selectedId === node.id} style={{ left: `${node.x}%`, top: `${node.y}%`, "--aura": node.auraColor, "--pulse": `${1.7 - node.intensity * 0.8}s` } as React.CSSProperties} onClick={() => { setSelectedId(node.id); setMode("focus"); setPanel(null); }}><span /><em>{node.locked ? "⌁" : node.type[0].toUpperCase()}</em></button>;
        })}
      </div>
      ...
      {selected && mode === "focus" ? <section className="focus-card" data-testid="urai-focus-card" role="dialog" aria-label={`${selected.title} focus card`}>
        <p>{selected.type.toUpperCase()} / {era}</p><h1>{selected.title}</h1><strong>{selected.subtitle}</strong><span>{selected.timestamp} · {selected.emotion} · {Math.round(selected.intensity * 100)}%</span><article>{selected.description}</article><blockquote>{selected.narratorLine}</blockquote>{selected.locked ? <b className="lock-message">This memory is still forming.</b> : null}<div><button type="button" disabled={selected.locked || !selected.replayAvailable} onClick={() => setMode("replay")}>Replay</button><button type="button" onClick={unwind}>Unwind</button><button type="button" onClick={() => { setMode("home"); router.push("/home"); }}>Return Home</button></div>
      </section> : null}
      ...
      {selected && mode === "replay" ? <section className="replay-overlay" data-testid="urai-replay-overlay" role="dialog" aria-label={`${selected.title} replay`}>
        <p>REPLAY STREAM</p><h1>{selected.title}</h1><div className="phase">{replayPhases[phaseIndex]}</div><div className="progress"><span style={{ width: `${progress}%` }} /></div><div className="wave" /><article>{selected.replayScript[phaseIndex] ?? selected.narratorLine}</article><blockquote>{selected.narratorLine}</blockquote><div><button type="button" onClick={unwind}>Collapse Replay / Unwind</button><button type="button" onClick={() => { setMode("home"); router.push("/home"); }}>Return Home</button></div>
      </section> : null}

      {panel === "filter" ? <div className="popover">{filters.map((item) => <button key={item.id} type="button" className={filter === item.id ? "active" : ""} onClick={() => setFilter(item.id)}>{item.label}</button>)}</div> : null}
      {panel === "era" ? <div className="popover era-pop">{eras.map((item) => <button key={item} type="button" className={era === item ? "active" : ""} onClick={() => setEra(item)}>{item}</button>)}</div> : null}

      <nav className="command" aria-label="LifeMap command ribbon">
        <button type="button" className={mode === "lifemap" ? "active" : ""} onClick={() => { setMode("lifemap"); setSelectedId(null); }}>Overview</button>
        {mode !== "lifemap" ? <button type="button" disabled={!selected || selected.locked || !selected.replayAvailable} onClick={() => setMode(mode === "replay" ? "focus" : "replay")}>{mode === "replay" ? "Collapse Replay" : "Replay"}</button> : null}
