import type { LifeMapStar } from '../lib/uraiCanon/starSchema'
'use client';
...
function HomeSky({
  ...
  return (
    <mesh
      position={[0, 14, -22]}
      visible={enabled}
      onPointerEnter={() => enabled && setPointerHot(true)}
      onPointerLeave={() => setPointerHot(false)}
      onPointerDown={(e) => {
        e.stopPropagation();
        if (enabled) onClick();
      }}
  ...
  return (
    <group visible={phase === 'lifemap' || phase === 'focus' || phase === 'replay'}>
      {stars.map((star) => {
        ...
        const selectedScale = isSelected ? 1.08 : 1;
        ...
        return (
          <group key={star.id} position={star.position}>
            ...
            >
              <sphereGeometry args={[baseRadius * hoverScale * selectedScale * (subdued ? 0.88 : 1), 20, 20]} />
              <meshStandardMaterial
                color={isSelected ? '#DEE7FF' : isHovered ? '#CFDAF7' : star.band === 'far' ? '#8B95B3' : '#B6C1DE'}
                emissive={isSelected ? '#BFCBF2' : isHovered ? '#9BA8CE' : star.band === 'far' ? '#56607B' : '#7E89A8'}
                emissiveIntensity={isSelected ? 0.86 : isHovered ? 0.52 : star.band === 'far' ? 0.14 : 0.33}
                roughness={0.7}
                metalness={0}
              />
            </mesh>
            ...
            <mesh>
              <sphereGeometry args={[haloRadius * (isHovered ? 1.15 : 1), 16, 16]} />
              <meshBasicMaterial color="#AAB6D8" transparent opacity={isHovered ? opacity * 1.35 : opacity} />
            </mesh>
          </group>
        );
      })}
  ...
  return (
    <group ref={group} visible={active} position={selectedStar.position}>
      <mesh
        visible={phase === 'focus'}
        onPointerEnter={() => {
          if (!inputLocked && phase === 'focus') setPointerHot(true);
        }}
        onPointerLeave={() => setPointerHot(false)}
        onPointerDown={(e) => {
          e.stopPropagation();
  ...
  return (
    <div
      style={{
        ...
        lineHeight: 1.45,
        pointerEvents: 'none',
        border: '1px solid rgba(165,177,210,0.18)',
        minWidth: 230,
      }}
    >
      <div>phase: {phase}</div>
      <div>selected: {selectedStar?.id ?? 'none'}</div>
      <div>hovered: {hoveredStarId ?? 'none'}</div>
      <div>interactive stars: {interactiveCount}</div>
      <div>replay ready: {replayReady ? 'yes' : 'no'}</div>
      <div>esc chain: replay → focus → lifemap → home</div>
    </div>
  );
}
...
function SceneCore() {
  ...
  const interactiveStars = useMemo(() => stars.filter((s) => s.band !== 'far'), [stars]);

  const [phase, setPhase] = useState<Phase>('home');
  const [selectedStar, setSelectedStar] = useState<StarNode | null>(interactiveStars[0] ?? stars[0] ?? null);
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <AuditHud
        ...
        interactiveCount={interactiveStars.length}
