...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ECO-SPAT-001 | Spatial | Record exact production deployed SHA, target Firebase project, environment, and rollback SHA | `EVIDENCE.md` deployment fields remain unrecorded | P0 | Passing release gates; deployment authority | `.github/workflows/spatial-live-deploy.yml`, `EVIDENCE.md` | Deployment receipt plus external domain smoke | Current release | BLOCKED | Firebase/GitHub deployment authority | `EVIDENCE.md` deployment table + workflow artifact |
| ECO-SPAT-002 | Spatial | Prove `/privacy-controls/` serves the dedicated source owner on the custom domain | Dedicated route exists with title and route fingerprint; live parity not reverified | P0 | Passing build/export/deploy | `urai-tier1/src/app/privacy-controls`, production-route gate | Browser assertion and content marker on custom domain | V1 | VERIFIED IN REPOSITORY; LIVE PENDING | Deployment/external browser | Spatial workflow receipt and screenshots |
| ECO-SPAT-003 | Spatial | Prove `/focus`, `/focus/`, and query variants render the same current chamber | Dedicated `FinalFocusChamber` owner and route fingerprint exist; live parity not reverified | P0 | Passing build/export/deploy | `urai-tier1/src/app/focus`, production-route gate | Deep-link browser matrix with `memoryId=quiet-reset` | V1 | VERIFIED IN REPOSITORY; LIVE PENDING | Deployment/external browser | Query-parity artifact and live smoke |
| ECO-SPAT-004 | Spatial | Prevent static hosting rewrites from masking missing pages as Home | Current `firebase.static.json` contains an empty rewrite list; release gate now enforces zero rewrites | P0 | Passing release run and live negative-route check | `firebase.static.json`, `scripts/check-production-route-exposure.mjs` | Missing-route test must return intentional 404/fallback, not Home | V1 | VERIFIED IN REPOSITORY; LIVE PENDING | Deployment/external browser | Hosting-config check + live negative-route receipt |
| ECO-SPAT-005 | Spatial | Use service-account deployment without legacy token precedence | Current workflow uses service-account JSON and `GOOGLE_APPLICATION_CREDENTIALS`; no legacy token variable is exported | P0 | Passing deploy dry gate | `.github/workflows/spatial-live-deploy.yml` | Service-account input validation and authorized deploy receipt | V1 | VERIFIED IN REPOSITORY; DEPLOY PENDING | GitHub Actions/Firebase credentials | Workflow run artifact |
| ECO-SPAT-006 | Spatial | Pass frozen install, full tier/XR release verification, browser and accessibility gates on current main | Gates wired; current-main run not recorded | P0 | ECO-SPAT-002–005 source gates; CI execution | Root scripts, `urai-tier1`, release matrix | `pnpm live:check`, browser/E2E, reduced-motion, accessibility | V1 | PARTIALLY IMPLEMENTED | CI runners | Release workflow run + artifact |
...
| --- | --- | --- | --- |
| V1 | Public spatial foundation and core journey | PARTIALLY IMPLEMENTED; partially live; NOT CERTIFIED | Current-main release run, deployed/rollback SHA, external route/query/negative-route parity, browser/mobile/a11y proof |
