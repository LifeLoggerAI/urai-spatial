...
const doorways = [
  { id: 'ground', destination: '/ground', href: '/ground/', name: 'Open Ground directly' },
  { id: 'life-map', destination: '/life-map', href: '/life-map/', name: 'Open Life Map directly' },
]
if (!/^[0-9a-f]{40}$/.test(exactSha)) throw new Error('Exact source SHA required')
const normalize = (value) => new URL(value, baseUrl).pathname.replace(/\/$/, '') || '/'
...
async function resolveTarget(page, doorway) {
  ...
  if (!href || normalize(href) !== doorway.destination) {
    throw new Error(`invalid native href ${JSON.stringify(href)} for ${doorway.destination}`)
  }
  return target
}
...
const errors = interactions
  ...
  .map((item) => `${item.device}:${item.activationMethod}:${item.destinationRoute}: ${item.failureReason}`)
const receipt = {
  schemaVersion: 5,
  exactSha,
  baseUrl,
  createdAt: new Date().toISOString(),
  persistentWorldCanon: true,
  directDestinationNavigationPermitted: true,
  nativeLinkNavigationRequired: true,
  interactions,
