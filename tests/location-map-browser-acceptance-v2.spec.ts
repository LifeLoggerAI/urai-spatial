...
function monitor(page) {
    const evidence = { consoleErrors: [], pageErrors: [], failedRequests: [] };
...
function isExpectedNavigationAbort(request) {
    return request.includes('net::ERR_ABORTED') && (request.includes('/location-map?')
...
async function gestureAnchor(page) {
...
async function nativeTouchTap(page, target) {
...
async function dispatchPointerDrag(page, pointerType, dx, dy) {
...
async function nativePinch(page) {
...
async function realWheelZoom(page) {
    const { x, y } = await gestureAnchor(page);
    await page.mouse.move(x, y);
...
test.describe('Location Map exact-head browser acceptance evidence v2', () => {
    test('desktop complete acceptance packet', async ({ page, context }, testInfo) => {
        ...
        const unexpectedFailedRequests = errors.failedRequests.filter(request => (!expectedOfflineRequests.includes(request)
