...
test('keyboard doorway activation proves native Tab focus followed by native Enter', () => {
  ...
  assert.match(proof, /function liveSemanticTarget/)
  assert.match(proof, /const active = document\.activeElement/)
  assert.match(proof, /active instanceof HTMLAnchorElement/)
  assert.match(proof, /active\.getAttribute\('aria-label'\) === name/)
  assert.match(proof, /active\.getAttribute\('href'\) === href/)
  assert.match(proof, /await target\.waitFor\(\{ state: 'visible', timeout: 45000 \}\)/)
