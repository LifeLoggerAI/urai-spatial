...
export default function CameraRig() {
  ...
  useFrame((state, delta) => {
    ...
    if ((mode === "focus" || mode === "replay") && selectedStar) {
      const focusLift = 1.15 + selectedStar.size * 0.11;
      const focusDistance = 5.4 + Math.min(selectedStar.size * 0.30, 1.15);
    ...
    if (mode === "replay" && selectedStar) {
      ...
      const baseRadius = 5.0 + Math.min(selectedStar.size * 0.36, 1.2);
