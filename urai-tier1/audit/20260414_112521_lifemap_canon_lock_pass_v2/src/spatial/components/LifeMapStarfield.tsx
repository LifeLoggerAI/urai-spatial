...
export default function LifeMapStarfield({
  ...
  const globalOpacity = clamp(1 - focusSuppression * 0.7, 0.18, 1)

  return (
    <group ref={root} visible={visible} position={[0, 0, -18]}>
      <mesh position={[0, 0, -54]}>
        <sphereGeometry args={[104, 28, 28]} />
        <meshBasicMaterial color="#04111f" transparent opacity={0.46} side={THREE.BackSide} />
      </mesh>

      <mesh position={[0, -14, -28]} rotation={[-0.28, 0, 0]}>
        <circleGeometry args={[66, 72]} />
        <meshBasicMaterial color="#0a1834" transparent opacity={0.16} />
      </mesh>

      {dust.map((p, i) => (
        <mesh key={"dust-" + i} position={p.toArray()}>
          <sphereGeometry args={[0.03 + (i % 5) * 0.004, 6, 6]} />
          <meshBasicMaterial color="#8fb8ff" transparent opacity={0.03} />
        </mesh>
      ))}
      ...
      {starData.map((star) => {
        ...
        const opacity = (selected ? 1 : star.depthClass === 'hero' ? 0.95 : star.depthClass === 'mid' ? 0.64 : 0.34) * depthFade * globalOpacity

        return (
          <group
            key={star.id}
            position={star.position.toArray()}
            onPointerOver={() => onHoverStar && onHoverStar(star.id)}
            onPointerOut={() => onHoverStar && onHoverStar(null)}
            onClick={() => onSelectStar && onSelectStar(star.id)}
          >
            <mesh>
            ...
            <mesh>
              ...
              <meshBasicMaterial color={star.color} transparent opacity={opacity * (selected ? 0.16 : 0.10)} />
            </mesh>
            {selected ? (
              <mesh>
