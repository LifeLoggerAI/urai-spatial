...
// Locked product authority; V386 is the current literal-pixel implementation:
...
function makeFocusPhotosphereTexture() {
  ...
  for (let y = 0; y < size; y += 1) for (let x = 0; x < size; x += 1) {
    ...
    const distance = Math.sqrt(dx * dx + dy * dy);
    const angle = Math.atan2(dy, dx);
    const boundary = THREE.MathUtils.clamp(.69
      + Math.sin(angle * 5 + .52) * .145
      + Math.sin(angle * 11 - .81) * .095
      + Math.sin(angle * 19 + .34) * .052
      + Math.sin(angle * 31 - .27) * .028, .42, .98);
    const offset = (y * size + x) * 4;
    ...
    const brightness = THREE.MathUtils.clamp(.16 + limb * .30 + granulation * .40 + activeRegion * .95, 0, 1);
...
function makeFocusPhotosphereGeometry() {
  ...
  for (let index = 0; index < position.count; index += 1) {
    ...
    const warp = 1
      + Math.sin(vertex.x * 17 + vertex.y * 11 + vertex.z * 7) * .105
      + Math.sin(vertex.x * 31 - vertex.y * 13 + vertex.z * 19) * .061
      + Math.sin((vertex.x + vertex.y - vertex.z) * 43) * .031
...
function FocusMemoryStar({
  ...
  return <group
    ...
    userData={{
      visualAuthority: 'selected-memory-star-resolving-through-memory-v386',
      lifeMapContinuity: 'same-selected-star-resolved-at-close-range',
      terrainOwner: false,
    }}
  >
    <>
    <sprite raycast={() => null} position={[-.04, .02, -.10]} scale={[2.18, 1.92, 1]} rotation={-.11} name="focus-memory-star-corona-glow">
      <spriteMaterial map={coronaTexture} color="#fff2c6" transparent opacity={memory ? .32 : .10} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[.02, .01, .06]} scale={[2.60, 2.16, 1]} rotation={.31} name="focus-memory-star-organic-streamer-field">
      <spriteMaterial map={rayTexture} color="#fff9dc" transparent opacity={memory ? .22 : .06} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[-.01, .01, .18]} scale={[1.88, 1.58, 1]} rotation={.07} name="focus-memory-star-photosphere-surface">
    ...
    <mesh raycast={() => null} geometry={photosphereGeometry} scale={[.32, .29, .27]} name="focus-memory-star-photosphere-core" rotation={[0.08, -0.18, 0]}>
      <meshBasicMaterial
        map={sphereTexture}
        color="#fff7d2"
        transparent
        opacity={memory ? 0.20 : 0.05}
        depthWrite={false}
...
export default function FocusChamberClient() {
  ...
  return <main
    ...
    data-focus-visual-revision="v386-non-globe-irregular-photosphere-contained-memory"
    ...
    data-focus-stellar-treatment="selected-corona-plasma-irregular-photosphere-stellar-field-v386"
