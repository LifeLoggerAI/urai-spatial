...
// Locked product authority; V367 is the current literal-pixel implementation:
...
function makeFocusPhotosphereTexture() {
  ...
  for (let y = 0; y < size; y += 1) for (let x = 0; x < size; x += 1) {
    ...
    const distance = Math.sqrt(dx * dx + dy * dy);
    const angle = Math.atan2(dy, dx);
    const boundary = THREE.MathUtils.clamp(.80
      + Math.sin(angle * 5 + .52) * .120
      + Math.sin(angle * 11 - .81) * .080
      + Math.sin(angle * 19 + .34) * .050, .60, .995);
    const offset = (y * size + x) * 4;
...
function FocusMemoryStar({
  ...
  return <group
    ...
    userData={{
      visualAuthority: 'selected-memory-star-resolving-through-memory-v367',
      lifeMapContinuity: 'same-selected-star-resolved-at-close-range',
      terrainOwner: false,
    }}
  >
    <>
    <sprite raycast={() => null} position={[-.08, .04, -.08]} scale={[3.06, 2.58, 1]} rotation={-.18} name="focus-memory-star-corona-glow">
      <spriteMaterial map={coronaTexture} color="#ffbd63" transparent opacity={memory ? .48 : .04} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[.06, -.02, .11]} scale={[4.34, 3.10, 1]} rotation={.23} name="focus-memory-star-photosphere-rays">
      <spriteMaterial map={rayTexture} color="#ffe09a" transparent opacity={memory ? .52 : .03} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[-.05, .06, .10]} scale={[2.92, 4.18, 1]} rotation={-.49} name="focus-memory-star-photosphere-rays-secondary">
      <spriteMaterial map={rayTexture} color="#fff7dc" transparent opacity={memory ? .36 : .02} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[-.02, .01, .12]} scale={[2.46, 1.94, 1]} rotation={.13} name="focus-memory-star-photosphere-surface">
      <spriteMaterial map={photosphereTexture} color="#ffe7aa" transparent opacity={memory ? .90 : .14} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[.03, -.02, .16]} scale={[2.20, 1.72, 1]} rotation={-.16} name="focus-memory-star-white-hot-limb">
    ...
    <mesh raycast={() => null} geometry={photosphereGeometry} scale={0.64} name="focus-memory-star-photosphere-core" rotation={[0.08, -0.18, 0]}>
      <meshBasicMaterial
        map={sphereTexture}
        color="#fff3b8"
        transparent
    ...
    <Html center transform position={[0.16, -0.05, 0.36]} distanceFactor={1.72} zIndexRange={[20, 10]}>
      <button
        type="button"
...
export default function FocusChamberClient() {
  ...
  return <main
    ...
    data-focus-visual-revision="v367-irregular-coronal-photosphere-memory-fracture"
    ...
    data-focus-stellar-treatment="selected-irregular-coronal-photosphere-owned-v367"
