...
function makeFocusCoronaTexture(power: number, rays = false) {
  ...
  for (let y = 0; y < size; y += 1) for (let x = 0; x < size; x += 1) {
    ...
    const diagonal = rays
      ...
      : 0;
    const alpha = Math.min(1, Math.pow(radial, power) + axial * .58 + diagonal * .34);
...
function FocusMemoryStar({
  ...
  >
    ...
    <sprite raycast={() => null} position={[-.08, .05, -.24]} scale={[4.15, 4.15, 1]} name="focus-memory-star-corona-glow">
      <spriteMaterial map={coronaTexture} color={accent} transparent opacity={memory ? .78 : .05} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[.06, -.03, -.10]} scale={[3.62, 3.62, 1]} rotation={.22} name="focus-memory-star-photosphere-rays">
      <spriteMaterial map={rayTexture} color={light} transparent opacity={memory ? .82 : .05} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <mesh raycast={() => null} scale={0.72} name="focus-memory-star-outer-corona">
      <sphereGeometry args={[1, 64, 40]} />
      <meshBasicMaterial color={accent} transparent opacity={memory ? 0.085 : 0.002} depthWrite={false} blending={THREE.AdditiveBlending} />
    </mesh>
    <mesh raycast={() => null} scale={0.54} name="focus-memory-star-inner-corona">
      <sphereGeometry args={[1, 64, 40]} />
      <meshBasicMaterial color={light} transparent opacity={memory ? 0.15 : 0.003} depthWrite={false} blending={THREE.AdditiveBlending} />
    </mesh>
    <mesh raycast={() => null} scale={0.46} name="focus-memory-star-photosphere-core">
      ...
      <meshStandardMaterial
        color={light}
        emissive={accent}
        emissiveIntensity={memory ? 4.2 : 0.20}
        roughness={0.30}
        metalness={0}
        transparent
        opacity={memory ? 0.74 : 0.18}
        depthWrite={false}
      />
    </mesh>
    </>
    ...
    <pointLight color={accent} intensity={memory ? 2.4 : .9} distance={7.2} decay={2} />
