...
function makeFocusCoronaTexture(power: number, rays = false) {
  ...
  for (let y = 0; y < size; y += 1) for (let x = 0; x < size; x += 1) {
    ...
    const alpha = distance >= 1
      ? 0
      : rays
        ? Math.min(1, outerCorona * (.24 + broadPlume * 1.78 + finePlume * 1.28))
...
function FocusMemoryStar({
  ...
  >
    ...
    <sprite raycast={() => null} position={[.04, -.03, .11]} scale={[4.18, 3.64, 1]} rotation={.19} name="focus-memory-star-photosphere-rays">
    ...
    <sprite raycast={() => null} position={[-.04, .04, .10]} scale={[3.72, 4.34, 1]} rotation={-.43} name="focus-memory-star-photosphere-rays-secondary">
    ...
    <mesh raycast={() => null} geometry={photosphereGeometry} scale={0.60} name="focus-memory-star-photosphere-core" rotation={[0.08, -0.18, 0]}>
      <meshBasicMaterial
        map={sphereTexture}
        color="#ffb25a"
        transparent
        opacity={memory ? 0.54 : 0.16}
        depthWrite={false}
    ...
    <Html center transform position={[0.02, -0.01, 0.34]} distanceFactor={1.72} zIndexRange={[20, 10]}>
      <button
        type="button"
