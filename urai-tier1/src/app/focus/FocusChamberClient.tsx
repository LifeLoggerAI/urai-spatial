...
// Locked product authority; V328 is the current literal-pixel implementation:
...
function FocusMemoryStar({
  ...
  const starScale = memory ? THREE.MathUtils.clamp(memory.star.scale, 0.76, 0.88) : 0.82
  ...
  return <group
    ...
    userData={{
      visualAuthority: 'selected-memory-star-with-contained-memory-v328',
      lifeMapContinuity: 'same-selected-star-resolved-at-close-range',
      terrainOwner: false,
    }}
  >
    <sprite raycast={() => null} position={[-.12, .08, -.22]} scale={[4.05, 4.05, 1]} name="focus-memory-star-corona-glow">
      <spriteMaterial map={coronaTexture} color={accent} transparent opacity={memory ? .34 : .05} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[.10, -.05, -.08]} scale={[3.62, 3.62, 1]} rotation={.22} name="focus-memory-star-photosphere-rays">
      <spriteMaterial map={rayTexture} color={light} transparent opacity={memory ? .27 : .05} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <mesh raycast={() => null} scale={1.02} name="focus-memory-star-outer-corona">
      <sphereGeometry args={[1, 64, 40]} />
      <meshBasicMaterial color={accent} transparent opacity={memory ? 0.085 : 0.006} depthWrite={false} blending={THREE.AdditiveBlending} />
    </mesh>
    <mesh raycast={() => null} scale={0.90} name="focus-memory-star-inner-corona">
      <sphereGeometry args={[1, 64, 40]} />
      <meshBasicMaterial color={light} transparent opacity={memory ? 0.12 : 0.008} depthWrite={false} blending={THREE.AdditiveBlending} />
    </mesh>
    <mesh raycast={() => null} scale={0.72} name="focus-memory-star-photosphere-core">
    ...
    <mesh raycast={() => null} scale={0.78} name="focus-memory-star-interior-depth">
      <sphereGeometry args={[1, 48, 36]} />
      <meshBasicMaterial color={accent} transparent opacity={memory ? 0.11 : 0.020} depthWrite={false} side={THREE.BackSide} blending={THREE.AdditiveBlending} />
    </mesh>
    ...
    <Html center transform position={[0, 0, 0.44]} distanceFactor={5.65} zIndexRange={[20, 10]}>
      <button
        type="button"
...
export default function FocusChamberClient() {
  ...
  return <main
    ...
    data-focus-visual-revision="v328-stellar-photosphere-memory-interior"
