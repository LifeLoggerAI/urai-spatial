...
// Locked product authority; V388 is the current literal-pixel implementation:
...
function makeFocusPhotosphereTexture() {
  ...
  for (let y = 0; y < size; y += 1) for (let x = 0; x < size; x += 1) {
    ...
    const distance = Math.sqrt(dx * dx + dy * dy);
    const angle = Math.atan2(dy, dx);
    const boundary = THREE.MathUtils.clamp(.62
      + Math.sin(angle * 5 + .52) * .180
      + Math.sin(angle * 11 - .81) * .120
      + Math.sin(angle * 19 + .34) * .070
      + Math.sin(angle * 31 - .27) * .040, .28, 1.0);
    const offset = (y * size + x) * 4;
    ...
    const brightness = THREE.MathUtils.clamp(.10 + limb * .20 + granulation * .48 + activeRegion, 0, 1);
    const edgeFade = THREE.MathUtils.clamp((boundary - distance) / .055, 0, 1);
    data[offset] = 255;
...
function FocusMemoryStar({
  ...
  return <group
    ...
    userData={{
      visualAuthority: 'selected-memory-star-resolving-through-memory-v388',
      lifeMapContinuity: 'same-selected-star-resolved-at-close-range',
      terrainOwner: false,
    }}
  >
    <>
    <sprite raycast={() => null} position={[-.04, .02, -.10]} scale={[1.98, 1.66, 1]} rotation={-.11} name="focus-memory-star-corona-glow">
      <spriteMaterial map={coronaTexture} color="#fff2c6" transparent opacity={memory ? .26 : .10} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[.02, .01, .06]} scale={[2.18, 1.78, 1]} rotation={.31} name="focus-memory-star-organic-streamer-field">
      <spriteMaterial map={rayTexture} color="#fff9dc" transparent opacity={memory ? .14 : .06} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[-.01, .01, .18]} scale={[2.02, 1.72, 1]} rotation={.07} name="focus-memory-star-photosphere-surface">
      <spriteMaterial map={photosphereTexture} color="#fffce8" transparent opacity={memory ? .94 : .05} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <mesh raycast={() => null} geometry={photosphereGeometry} scale={[.26, .23, .21]} name="focus-memory-star-photosphere-core" rotation={[0.08, -0.18, 0]}>
      <meshBasicMaterial
        map={sphereTexture}
        color="#fff7d2"
        transparent
        opacity={memory ? 0.04 : 0.05}
        depthWrite={false}
    ...
    <Html center transform position={[0.05, -0.02, 0.54]} distanceFactor={3.15} zIndexRange={[20, 10]}>
      <button
        type="button"
...
export default function FocusChamberClient() {
  ...
  return <main
    ...
    data-focus-visual-revision="v388-stellar-memory-no-planet-horizon"
    ...
    data-focus-stellar-treatment="selected-corona-plasma-irregular-photosphere-stellar-field-v388"
