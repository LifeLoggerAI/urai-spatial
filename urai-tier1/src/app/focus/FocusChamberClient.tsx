...
// Locked product authority; V376 is the current literal-pixel implementation:
...
function makeFocusCoronaFilamentGeometry(count: number, innerRadius: number, outerRadius: number, phase: number) {
  const positions: number[] = []
  const segments = 6
  for (let ray = 0; ray < count; ray += 1) {
    const baseAngle = (ray / count) * Math.PI * 2
    const seed = Math.abs(Math.sin((ray + 1) * 12.9898 + phase * 31.17))
    const reach = outerRadius * (.68 + seed * .42 + Math.sin(ray * .71 + phase) * .08)
    const bend = Math.sin(ray * .83 + phase * 2.1) * .085
    for (let segment = 0; segment < segments; segment += 1) {
      ...
      const a0 = baseAngle + bend * t0 * t0 + Math.sin(ray * 1.37 + t0 * 8 + phase) * .015
      const a1 = baseAngle + bend * t1 * t1 + Math.sin(ray * 1.37 + t1 * 8 + phase) * .015
      positions.push(
        Math.cos(a0) * r0, Math.sin(a0) * r0, .18 + Math.sin(ray * .47 + t0 * 2.4) * .018,
        Math.cos(a1) * r1, Math.sin(a1) * r1, .18 + Math.sin(ray * .47 + t1 * 2.4) * .018,
      )
    }
  }
...
function makeFocusPhotosphereTexture() {
  ...
  for (let y = 0; y < size; y += 1) for (let x = 0; x < size; x += 1) {
    ...
    const distance = Math.sqrt(dx * dx + dy * dy);
    const angle = Math.atan2(dy, dx);
    const boundary = THREE.MathUtils.clamp(.94
      + Math.sin(angle * 5 + .52) * .028
      + Math.sin(angle * 11 - .81) * .018
      + Math.sin(angle * 19 + .34) * .010, .88, .995);
    const offset = (y * size + x) * 4;
...
function FocusMemoryStar({
  ...
  const coronaFilamentsA = useMemo(() => makeFocusCoronaFilamentGeometry(132, .30, 1.92, .17), [])
  const coronaFilamentsB = useMemo(() => makeFocusCoronaFilamentGeometry(84, .38, 1.48, 1.41), [])
  useEffect(() => () => {
    ...
    coronaFilamentsA.dispose()
    coronaFilamentsB.dispose()
  }, [coronaFilamentsA, coronaFilamentsB, coronaTexture, photosphereGeometry, photosphereTexture, rayTexture, sphereTexture])
  ...
  return <group
    ...
    userData={{
      visualAuthority: 'selected-memory-star-resolving-through-memory-v376',
      lifeMapContinuity: 'same-selected-star-resolved-at-close-range',
      terrainOwner: false,
    }}
  >
    <>
    <lineSegments raycast={() => null} geometry={coronaFilamentsA} position={[0, 0, .22]} rotation={[0, 0, -.09]} name="focus-memory-star-corona-filaments-primary">
      <lineBasicMaterial color="#ffd48a" transparent opacity={memory ? .22 : .025} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </lineSegments>
    <lineSegments raycast={() => null} geometry={coronaFilamentsB} position={[0, 0, .21]} rotation={[0, 0, .31]} name="focus-memory-star-corona-filaments-secondary">
      <lineBasicMaterial color="#fff0c2" transparent opacity={memory ? .13 : .018} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </lineSegments>
    <sprite raycast={() => null} position={[-.08, .04, -.08]} scale={[5.20, 4.10, 1]} rotation={-.18} name="focus-memory-star-corona-glow">
      <spriteMaterial map={coronaTexture} color="#ffbd63" transparent opacity={memory ? .62 : .04} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[.06, -.02, .11]} scale={[6.80, 3.20, 1]} rotation={.23} name="focus-memory-star-photosphere-rays">
      <spriteMaterial map={rayTexture} color="#ffe09a" transparent opacity={memory ? .78 : .03} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[-.05, .06, .10]} scale={[3.10, 6.40, 1]} rotation={-.49} name="focus-memory-star-photosphere-rays-secondary">
      <spriteMaterial map={rayTexture} color="#fff7dc" transparent opacity={memory ? .72 : .02} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[.02, .01, .13]} scale={[5.70, 2.50, 1]} rotation={.72} name="focus-memory-star-coronal-arm-diagonal-a">
      <spriteMaterial map={rayTexture} color="#ffd07a" transparent opacity={memory ? .56 : .02} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[-.03, .02, .14]} scale={[5.30, 2.35, 1]} rotation={-.91} name="focus-memory-star-coronal-arm-diagonal-b">
      <spriteMaterial map={rayTexture} color="#fff0b8" transparent opacity={memory ? .50 : .02} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[-.02, .01, .12]} scale={[2.70, 2.15, 1]} rotation={.13} name="focus-memory-star-photosphere-surface">
      <spriteMaterial map={photosphereTexture} color="#ffe7aa" transparent opacity={memory ? .38 : .14} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[.03, -.02, .16]} scale={[2.45, 1.88, 1]} rotation={-.16} name="focus-memory-star-white-hot-limb">
      <spriteMaterial map={photosphereTexture} color="#fffbe8" transparent opacity={memory ? .16 : .08} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <group raycast={() => null} name="focus-memory-star-explicit-corona-rays" position={[0, 0, -0.04]} />
    <mesh raycast={() => null} geometry={photosphereGeometry} scale={[0.22, 0.17, 0.12]} name="focus-memory-star-photosphere-core" rotation={[0.08, -0.18, 0]}>
      <meshBasicMaterial
        map={sphereTexture}
        color="#fff3b8"
        transparent
    ...
    <Html center transform position={[0.09, -0.03, 0.42]} distanceFactor={1.92} zIndexRange={[20, 10]}>
      <button
        type="button"
...
export default function FocusChamberClient() {
  ...
  return <main
    ...
    data-focus-visual-revision="v376-filament-plasma-corona-photosphere-stellar-memory-field"
    ...
    data-focus-stellar-treatment="selected-filament-plasma-corona-photosphere-stellar-field-v376"
