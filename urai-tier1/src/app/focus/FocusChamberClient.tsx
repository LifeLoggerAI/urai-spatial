...
// Locked product authority; V391 is the current literal-pixel implementation:
...
function FocusMemoryStar({
  ...
  return <group
    ...
    userData={{
      visualAuthority: 'selected-memory-star-resolving-through-memory-v391',
      lifeMapContinuity: 'same-selected-star-resolved-at-close-range',
      terrainOwner: false,
    }}
  >
    <>
    <sprite raycast={() => null} position={[-.04, .02, -.10]} scale={[2.62, 2.18, 1]} rotation={-.11} name="focus-memory-star-corona-glow">
    ...
    <sprite raycast={() => null} position={[.02, .01, .06]} scale={[3.18, 2.58, 1]} rotation={.31} name="focus-memory-star-organic-streamer-field">
      <spriteMaterial map={rayTexture} color="#fff9dc" transparent opacity={memory ? .11 : .06} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[-.01, .01, .18]} scale={[1.58, 1.42, 1]} rotation={.07} name="focus-memory-star-photosphere-surface">
      <spriteMaterial map={photosphereTexture} color="#fffce8" transparent opacity={memory ? .88 : .05} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <mesh raycast={() => null} geometry={photosphereGeometry} scale={[.18, .16, .15]} name="focus-memory-star-photosphere-core" rotation={[0.08, -0.18, 0]}>
      <meshBasicMaterial
        map={sphereTexture}
        color="#fff7d2"
        transparent
        opacity={memory ? 0.07 : 0.05}
        depthWrite={false}
    ...
    <Html center transform position={[0, 0, 0.50]} distanceFactor={3.15} zIndexRange={[20, 10]}>
      <button
        type="button"
...
export default function FocusChamberClient() {
  ...
  return <main
    ...
    data-focus-visual-revision="v391-stellar-photosphere-contained-memory-no-orb"
    ...
    data-focus-stellar-treatment="selected-corona-plasma-irregular-photosphere-contained-memory-v391"
