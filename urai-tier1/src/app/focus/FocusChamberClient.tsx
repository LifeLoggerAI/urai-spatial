...
import { FocusSourceMedia } from './FocusSourceMedia'
...
function makeFocusPhotosphereGeometry() {
  ...
  for (let index = 0; index < position.count; index += 1) {
    ...
    const warp = 1
      + Math.sin(vertex.x * 17 + vertex.y * 11 + vertex.z * 7) * .045
      + Math.sin(vertex.x * 31 - vertex.y * 13 + vertex.z * 19) * .026
      + Math.sin((vertex.x + vertex.y - vertex.z) * 43) * .014
...
export function MemoryVisualContent({ memory, compact = false }: { memory: SelectedMemory | null; compact?: boolean }) {
  if (!memory) {
  ...
  if (media?.kind === 'image' || media?.kind === 'video') {
    return <FocusSourceMedia media={{ kind: media.kind, url: media.url, caption: media.caption }} title={memory.title} demo={Boolean(memory.demo)} compact={compact} />
  }

  return <>
    <span data-focus-media-state="missing"
  ...
  <span className="focusMediaDisclosure">Interpretive visualization · no recorded image</span>
  </>
}
...
export default function FocusChamberClient() {
  ...
  >
    ...
    <header className="focusHeading">
      ...
      {memory && memoryVisual === 'generated-memory-visualization' ? <span className="focusMemoryAvailability">Interpretive visualization · no recorded image</span> : null}
