...
function MemoryTraces({ memory, accent, reducedMotion }: { memory: SelectedMemory | null; accent: string; reducedMotion: boolean }) {
  ...
  return (
    <group name="focus-grounded-memory-traces">
      {positions.map((position, index) => <Trace key={index} position={position} accent={accent} reducedMotion={reducedMotion} delay={index * 0.7} />)}
    </group>
  )
}
...
function MemoryAperture({ memory, accent, light, reducedMotion, onActivate }: { memory: SelectedMemory | null; accent: string; light: string; reducedMotion: boolean; onActivate: () => void }) {
  ...
  useFrame(({ clock }, delta) => {
    ...
    const wanted = hovered ? 1.09 : pulse
    group.current.scale.setScalar(THREE.MathUtils.damp(group.current.scale.x, wanted, 5.5, delta))
...
function FocusScene({ memory, profile, recenterSignal, onActivate, controls }: { memory: SelectedMemory | null; profile: SpatialQualityProfile; recenterSignal: number; onActivate: () => void; controls: React.RefObject<OrbitControlsImpl | null> }) {
  ...
  return (
    <>
      ...
      <pointLight position={[-4, 2, 1]} intensity={1.8} color="#826cff" distance={11} />
...
export default function FocusChamberClient() {
  ...
  return (
    <main className="focusWorld" style={style} data-testid="urai-final-focus-chamber" data-focus-composition="living-memory-chamber" data-focus-spatial="explorable-observatory" data-memory-status={result.status} data-chamber-state={chamberState} data-memory-id={memory?.id} data-star-id={memory?.star.id} data-node={memory?.star.id} data-manifest-id={memory?.replayManifest.id} data-canonical-asset={focusAssets.primary.src} data-spatial-quality={profile.tier}>
