...
// V250 convergence authority: the selected living-memory manifestation owns the
// arrival composition while remaining grounded inside the authored chamber.
...
function FocusSanctuaryGround({ accent, selectedMemoryActive }: { accent: string; selectedMemoryActive: boolean }) {
  ...
  return <mesh name={selectedMemoryActive ? "focus-v317-three-radial-terrain-selected-memory-cavities" : "focus-v214-continuous-eroded-memory-ground"} geometry={geometry} receiveShadow userData={{ selectedMemoryCavityAuthority: selectedMemoryActive ? "v317-three-radial-terrain-basin-depth" : "neutral-ground-no-memory-rupture" }}>
...
function MemoryAperture({ memory, accent, onActivate }: { memory: SelectedMemory | null; accent: string; light: string; reducedMotion: boolean; onActivate: () => void }) {
  ...
  // V272 keeps the selected memory physically present and still. Hover changes
  ...
  return <group position={[FOCUS_MEMORY_WORLD_X, 0, FOCUS_MEMORY_WORLD_Z]} name="focus-memory-aperture" userData={{ artRevision: 'v317-three-radial-terrain-pressure-cavities', hierarchy: 'selected-memory-buried-closed-authority-plus-terrain-depression-plus-cavity-bottom-incision', materialAuthority: 'terrain-owned-three-radial-basin-depth-plus-separated-unlit-near-black-cavity-bottoms-subordinate-hairline-connector-no-panel' }}>
    <group scale={[1, 1, 1]} position={[0, 0, 0]} name="focus-v251-grounded-living-memory-manifestation">
      ...
      <mesh geometry={incision} castShadow={false} receiveShadow={false} renderOrder={2} name="focus-v317-three-radial-terrain-pressure-cavity-bottoms" onClick={(event) => { event.stopPropagation(); if (memory) onActivate() }} onPointerOver={(event) => pointer(event, true)} onPointerOut={(event) => pointer(event, false)}>
...
export default function FocusChamberClient() {
  ...
  return <main ref={shellRef} className="focusWorld" style={style} data-testid="urai-final-focus-chamber" data-focus-composition="authored-floor-with-dominant-single-living-memory-fold" data-focus-visual-revision="v272-single-connected-living-memory-fold" data-focus-spatial="explorable-observatory" data-focus-movement="walk-keyboard-orbit-touch" data-focus-pointer-lock="false" data-focus-camera-x="0.000" data-focus-camera-y="1.180" data-focus-camera-z="5.100" data-focus-distance="0.000" data-focus-moving="false" data-memory-status={result.status} data-chamber-state={chamberState} data-webgl-state={webglState} data-canonical-asset={focusAssets.primary.src} data-focus-physical-asset={FOCUS_CHAMBER_MODEL} data-spatial-quality={profile.tier} data-software-renderer={!rendererClassified ? 'detecting' : softwareRenderer ? 'true' : 'false'} data-render-cadence={boundedCadence ? 'bounded-demand-4fps' : 'continuous'} data-memory-id={memory?.id} data-manifest-id={memory?.replayManifest.id} data-star-id={memory?.star.id} data-node={memory?.star.id}>
    ...
    <div className="focusCanvas" aria-label="Explorable Focus. Drag to orbit, scroll or pinch to move through depth, and use W A S D or arrow keys to travel.">
      {webglAvailable === null ? <div className="focusFallback" role="status">Preparing Focus…</div> : webglUsable ? <Canvas camera={{ position: DEFAULT_CAMERA, fov: 47, near: 0.08, far: 120 }} dpr={[1, profile.pixelRatioMax]} shadows={profile.shadows} frameloop={profile.documentVisible ? 'demand' : 'never'} gl={{ antialias: profile.antialias, alpha: false, powerPreference: 'high-performance' }} onCreated={({ gl }) => { gl.outputColorSpace = THREE.SRGBColorSpace; gl.toneMapping = THREE.ACESFilmicToneMapping; gl.toneMappingExposure = .94; setSoftwareRenderer(isSoftwareWebGLRenderer(gl)); setRendererClassified(true) }}><FocusRenderCadence bounded={boundedCadence} documentVisible={profile.documentVisible} /><FocusScene memory={memory} profile={profile} recenterSignal={recenterSignal} onActivate={enterReplay} controls={controls} onWebGLState={setWebglState} shellRef={shellRef} /></Canvas> : <div className="focusFallback" role="status" data-focus-fallback="semantic"><strong>Spatial view unavailable</strong><span>Focus remains accessible through the controls and memory details.</span></div>}
    </div>
    <header className="focusHeading"><p>{memory ? (memory.demo ? 'DEMO FIXTURE · NOT PERSONAL DATA' : `${memory.privacy} memory`) : 'URAI · FOCUS OBSERVATORY'}</p><h2>{heading}</h2>{memory ? <span>{dateLabel(memory.occurredAt)}</span> : null}<div className="focusNarration"><small>{memory ? 'Selected memory' : 'Focus Observatory'}</small><strong>{description}</strong></div></header>
    ...
    <div className="focusStatus" role={result.status === 'loading' ? 'status' : 'note'} aria-live="polite">{memory ? 'Focus ready' : result.status === 'loading' ? 'Opening selected memory' : directEntry ? 'Neutral observatory' : result.message}</div>
