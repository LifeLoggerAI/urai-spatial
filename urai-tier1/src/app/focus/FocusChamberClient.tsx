...
// Locked product authority; V385 is the current literal-pixel implementation:
...
function makeFocusPhotosphereGeometry() {
  ...
  for (let index = 0; index < position.count; index += 1) {
    ...
    const warp = 1
      + Math.sin(vertex.x * 17 + vertex.y * 11 + vertex.z * 7) * .052
      + Math.sin(vertex.x * 31 - vertex.y * 13 + vertex.z * 19) * .031
      + Math.sin((vertex.x + vertex.y - vertex.z) * 43) * .017
...
function FocusMemoryStar({
  ...
  return <group
    ...
    userData={{
      visualAuthority: 'selected-memory-star-resolving-through-memory-v385',
      lifeMapContinuity: 'same-selected-star-resolved-at-close-range',
      terrainOwner: false,
    }}
  >
    <>
    <sprite raycast={() => null} position={[-.04, .02, -.10]} scale={[2.40, 2.18, 1]} rotation={-.11} name="focus-memory-star-corona-glow">
      <spriteMaterial map={coronaTexture} color="#fff2c6" transparent opacity={memory ? .42 : .10} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[.02, .01, .06]} scale={[3.05, 2.48, 1]} rotation={.31} name="focus-memory-star-organic-streamer-field">
      <spriteMaterial map={rayTexture} color="#fff9dc" transparent opacity={memory ? .30 : .06} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[-.01, .01, .18]} scale={[2.02, 1.82, 1]} rotation={.07} name="focus-memory-star-photosphere-surface">
      <spriteMaterial map={photosphereTexture} color="#fffce8" transparent opacity={memory ? .98 : .05} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <mesh raycast={() => null} geometry={photosphereGeometry} scale={[.46, .43, .40]} name="focus-memory-star-photosphere-core" rotation={[0.08, -0.18, 0]}>
      <meshBasicMaterial
        map={sphereTexture}
        color="#fff7d2"
        transparent
        opacity={memory ? 0.42 : 0.05}
        depthWrite={false}
...
export default function FocusChamberClient() {
  ...
  return <main
    ...
    data-focus-visual-revision="v385-irregular-photosphere-memory-through-stellar-body"
    ...
    data-focus-stellar-treatment="selected-corona-plasma-irregular-photosphere-stellar-field-v385"
