...
// Locked product authority; V379 is the current literal-pixel implementation:
...
function makeFocusPhotosphereTexture() {
  ...
  for (let y = 0; y < size; y += 1) for (let x = 0; x < size; x += 1) {
    ...
    const distance = Math.sqrt(dx * dx + dy * dy);
    const angle = Math.atan2(dy, dx);
    const boundary = THREE.MathUtils.clamp(.88
      + Math.sin(angle * 5 + .52) * .055
      + Math.sin(angle * 11 - .81) * .034
      + Math.sin(angle * 19 + .34) * .018, .74, .985);
    const offset = (y * size + x) * 4;
    ...
    const edgeFade = THREE.MathUtils.clamp((boundary - distance) / .082, 0, 1);
    data[offset] = 255;
    data[offset + 1] = Math.round(154 + brightness * 98);
    data[offset + 2] = Math.round(54 + brightness * 126);
...
function makeFocusSphereTexture() {
  ...
  for (let y = 0; y < height; y += 1) for (let x = 0; x < width; x += 1) {
    ...
    data[offset + 1] = Math.round(132 + brightness * 114);
    data[offset + 2] = Math.round(36 + brightness * 112);
    data[offset + 3] = 255;
  }
...
function FocusMemoryStar({
  ...
  return <group
    ...
    userData={{
      visualAuthority: 'selected-memory-star-resolving-through-memory-v379',
      lifeMapContinuity: 'same-selected-star-resolved-at-close-range',
      terrainOwner: false,
    }}
  >
    <>
    <sprite raycast={() => null} position={[-.05, .03, -.04]} scale={[4.80, 4.35, 1]} rotation={-.11} name="focus-memory-star-corona-glow">
      <spriteMaterial map={coronaTexture} color="#ffbf63" transparent opacity={memory ? .82 : .07} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[.03, .01, .08]} scale={[5.70, 4.15, 1]} rotation={.31} name="focus-memory-star-organic-streamer-field">
      <spriteMaterial map={rayTexture} color="#ffe1a0" transparent opacity={memory ? .28 : .035} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[-.01, .01, .14]} scale={[3.00, 2.78, 1]} rotation={.07} name="focus-memory-star-photosphere-surface">
      <spriteMaterial map={photosphereTexture} color="#fff0bd" transparent opacity={memory ? .70 : .12} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <mesh raycast={() => null} geometry={photosphereGeometry} scale={[1.12, 1.04, 0.96]} name="focus-memory-star-photosphere-core" rotation={[0.08, -0.18, 0]}>
      <meshBasicMaterial
        map={sphereTexture}
        color="#fff8dc"
        transparent
        opacity={memory ? 0.44 : 0.14}
        depthWrite={false}
...
export default function FocusChamberClient() {
  ...
  return <main
    ...
    data-focus-visual-revision="v379-corona-plasma-photosphere-contained-memory-field"
    ...
    data-focus-stellar-treatment="selected-corona-plasma-photosphere-stellar-field-v379"
