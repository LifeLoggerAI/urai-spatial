...
import { makeFocusPlasmaTexture } from './focusStellarTexture'
import { FocusMemoryMedia, type FocusMediaState } from './FocusMemoryMedia'

// Locked product authority; V395 contract lineage, V396 plasma/source repair:
// The V396 candidate requires fresh literal-pixel review; lineage is not acceptance.
...
function MemoryVisualContent({ memory, compact = false }: { memory: SelectedMemory | null; compact?: boolean }) {
  ...
  >
    ...
    <span className="generatedMemoryVolume" aria-hidden="true" />
...
function FocusMemoryStar({
  ...
  const [mediaState, setMediaState] = useState<FocusMediaState>('loading')
  const sourceMedia = memory?.sourceMedia.find((item) => item.kind === 'image')
    ?? memory?.sourceMedia.find((item) => item.kind === 'video')
  const starScale = memory ? THREE.MathUtils.clamp(memory.star.scale * 2.02, 1.86, 2.08) : 1.42
  const coronaTexture = useMemo(() => makeFocusCoronaTexture(2.25), [])
  const rayTexture = useMemo(() => makeFocusCoronaTexture(4.2, true), [])
  const photosphereTexture = useMemo(() => makeFocusPlasmaTexture(), [])
  useEffect(() => () => {
  ...
  return <group
    ...
    userData={{
      visualAuthority: 'selected-memory-star-resolving-through-memory-v395',
      lifeMapContinuity: 'same-selected-star-resolved-at-close-range',
      terrainOwner: false,
    }}
  >
    <>
    <sprite raycast={() => null} position={[-.04, .02, -.10]} scale={[3.72, 3.22, 1]} name="focus-memory-star-corona-glow" renderOrder={0}>
      <spriteMaterial rotation={-.11} map={coronaTexture} color="#ffc273" transparent opacity={memory ? .44 : .10} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[.02, .01, .06]} scale={[4.36, 3.58, 1]} name="focus-memory-star-organic-streamer-field" renderOrder={1}>
      <spriteMaterial rotation={.31} map={rayTexture} color="#ffd290" transparent opacity={memory ? .36 : .06} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[-.01, .01, .18]} scale={[1.60, 1.60, 1]} name="focus-memory-star-photosphere-surface" renderOrder={2}>
    ...
    {sourceMedia && memory && <FocusMemoryMedia key={memory.id + ':' + sourceMedia.kind + ':' + sourceMedia.url} media={sourceMedia} onState={setMediaState} />}
    <sprite raycast={() => null} position={[.01, -.01, .25]} scale={[1.32, 1.22, 1]} name="focus-memory-star-photosphere-core" renderOrder={4}>
      <spriteMaterial rotation={-.13} map={photosphereTexture} color="#fff3c9" transparent opacity={sourceMedia ? .10 : .30} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    </>
    <mesh
    ...
    <Html center position={[0, 0, .26]} zIndexRange={[20, 10]}>
      <button
        type="button"
      ...
      >
        <span className="srOnly">{memory ? 'Enter Replay for ' + memory.title : 'Select a memory in Life Map'}</span>
        <span className="focusSourceState" data-focus-source-state={sourceMedia ? mediaState : 'generated'}>
          {sourceMedia ? (mediaState === 'ready' ? 'Source moment' : mediaState === 'loading' ? 'Loading source moment' : 'Source moment unavailable') : memory ? 'Generated memory visualization' : 'Awaiting a selected star'}
        </span>
      </button>
    </Html>
  </group>
...
export default function FocusChamberClient() {
  ...
  return <main
    ...
    data-focus-visual-revision="v395-stellar-photosphere-visible-contained-memory-no-orb"
    data-focus-plasma-revision="v396-feathered-plasma-world-space-source"
    ...
    data-focus-stellar-treatment="selected-corona-plasma-irregular-photosphere-visible-contained-memory-v395"
