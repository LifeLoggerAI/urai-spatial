...
function MemoryAperture({ memory, accent, onActivate }: { memory: SelectedMemory | null; accent: string; light: string; reducedMotion: boolean; onActivate: () => void }) {
  ...
  return <group position={[FOCUS_MEMORY_WORLD_X, 0, FOCUS_MEMORY_WORLD_Z]} name="focus-memory-aperture" userData={{ artRevision: 'v305-ground-owned-jagged-fissure', hierarchy: 'selected-memory-buried-closed-authority-plus-coplanar-ground-incision', materialAuthority: 'ground-conforming-dark-incision-no-emissive-no-object-edge' }}>
    <group scale={[1, 1, 1]} position={[0, 0, 0]} name="focus-v251-grounded-living-memory-manifestation">
      ...
      <mesh geometry={incision} castShadow={false} receiveShadow={false} renderOrder={2} name="focus-v305-ground-owned-fissure" onClick={(event) => { event.stopPropagation(); if (memory) onActivate() }} onPointerOver={(event) => pointer(event, true)} onPointerOut={(event) => pointer(event, false)}>
