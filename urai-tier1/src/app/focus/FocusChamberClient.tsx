...
// Locked product authority; V326 is the current literal-pixel implementation:
...
function FocusMemoryStar({
  ...
  return <group
    ...
    userData={{
      visualAuthority: 'selected-memory-star-with-contained-memory-v326',
      lifeMapContinuity: 'same-selected-star-resolved-at-close-range',
      terrainOwner: false,
    }}
  >
    <sprite raycast={() => null} scale={[5.4, 5.4, 1]} name="focus-memory-star-corona-glow">
      <spriteMaterial map={coronaTexture} color={accent} transparent opacity={memory ? .34 : .10} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} scale={[4.2, 4.2, 1]} name="focus-memory-star-photosphere-rays">
      <spriteMaterial map={rayTexture} color={light} transparent opacity={memory ? .30 : .08} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <mesh raycast={() => null} scale={1.74} name="focus-memory-star-outer-corona">
      <sphereGeometry args={[1, 64, 40]} />
      <meshBasicMaterial color={accent} transparent opacity={memory ? 0.085 : 0.025} depthWrite={false} blending={THREE.AdditiveBlending} />
    </mesh>
    <mesh raycast={() => null} scale={1.40} name="focus-memory-star-inner-corona">
      <sphereGeometry args={[1, 64, 40]} />
      <meshBasicMaterial color={light} transparent opacity={memory ? 0.11 : 0.035} depthWrite={false} blending={THREE.AdditiveBlending} />
    </mesh>
    <mesh
    ...
    >
      ...
      <meshPhysicalMaterial
        color={light}
        emissive={accent}
        emissiveIntensity={memory ? (hovered ? 2.8 : 2.15) : 0.42}
        transmission={0.08}
        thickness={0.10}
        roughness={0.48}
        metalness={0}
        transparent
        opacity={memory ? 0.18 : 0.10}
        clearcoat={0.24}
    ...
    <mesh raycast={() => null} scale={0.985} name="focus-memory-star-interior-depth">
      ...
      <meshBasicMaterial color={accent} transparent opacity={memory ? 0.055 : 0.025} depthWrite={false} side={THREE.BackSide} blending={THREE.AdditiveBlending} />
    </mesh>
    <pointLight color={accent} intensity={memory ? 7.2 : 1.2} distance={10.5} decay={2} />
    <Html center transform position={[0, 0, 0.76]} distanceFactor={4.35} zIndexRange={[20, 10]}>
      <button
        type="button"
...
export default function FocusChamberClient() {
  ...
  return <main
    ...
    data-focus-visual-revision="v326-stellar-photosphere-corona-memory-interior"
