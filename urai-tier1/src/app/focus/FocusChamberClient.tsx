...
// Locked product authority; V339 is the current literal-pixel implementation:
...
function FocusMemoryStar({
  ...
  const starScale = memory ? THREE.MathUtils.clamp(memory.star.scale * 1.52, 1.34, 1.58) : 1.08
  ...
  return <group
    ...
    userData={{
      visualAuthority: 'selected-memory-star-photosphere-memory-interior-v339',
      lifeMapContinuity: 'same-selected-star-resolved-at-close-range',
      terrainOwner: false,
    }}
  >
    <>
    <sprite raycast={() => null} position={[-.12, .08, -.22]} scale={[4.2, 4.2, 1]} name="focus-memory-star-corona-glow">
    ...
    <sprite raycast={() => null} position={[.10, -.05, -.08]} scale={[3.7, 3.7, 1]} rotation={.22} name="focus-memory-star-photosphere-rays">
    ...
    <mesh raycast={() => null} scale={0.72} name="focus-memory-star-outer-corona">
      ...
      <meshBasicMaterial color={accent} transparent opacity={memory ? 0.10 : 0.002} depthWrite={false} blending={THREE.AdditiveBlending} />
    </mesh>
    <mesh raycast={() => null} scale={0.54} name="focus-memory-star-inner-corona">
      <sphereGeometry args={[1, 64, 40]} />
      <meshBasicMaterial color={light} transparent opacity={memory ? 0.13 : 0.003} depthWrite={false} blending={THREE.AdditiveBlending} />
    </mesh>
    <mesh raycast={() => null} scale={0.31} name="focus-memory-star-photosphere-core">
    ...
    >
      ...
      <meshPhysicalMaterial
        ...
        transmission={memory ? 0.16 : 0}
        thickness={0.18}
        roughness={0.28}
        metalness={0}
        transparent
        opacity={memory ? 0.11 : 0.09}
        clearcoat={0.18}
    ...
    <mesh raycast={() => null} scale={0.58} name="focus-memory-star-interior-depth">
      ...
      <meshBasicMaterial color={accent} transparent opacity={memory ? 0.045 : 0.020} depthWrite={false} side={THREE.BackSide} blending={THREE.AdditiveBlending} />
    </mesh>
    ...
    <Html center transform position={[0, 0, 0.18]} distanceFactor={3.75} zIndexRange={[20, 10]}>
      <button
        type="button"
...
export default function FocusChamberClient() {
  ...
  return <main
    ...
    data-focus-visual-revision="v339-selected-star-photosphere-memory-interior"
