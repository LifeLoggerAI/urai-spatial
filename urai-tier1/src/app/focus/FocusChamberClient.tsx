...
function MemoryVisualContent({ memory, compact = false }: { memory: SelectedMemory | null; compact?: boolean }) {
  ...
  >
    ...
    <span className="focusMemoryGlass" aria-hidden="true" />
    <span className="focusMemoryTruthLabel">{memory.demo ? 'Generated demo visualization' : 'Generated memory visualization'}</span>
  </span>
}
...
function FocusMemoryStar({
  ...
  const starScale = memory ? THREE.MathUtils.clamp(memory.star.scale, 0.92, 1.12) : 0.92

  useFrame((state, delta) => {
    if (!group.current) return
    if (!reducedMotion) {
      group.current.rotation.y += Math.min(delta, 0.05) * 0.075
      const breathe = 1 + Math.sin(state.clock.elapsedTime * 0.72) * 0.012
  ...
  return <group
    ...
    userData={{
      visualAuthority: 'selected-memory-star-with-contained-memory-v323',
      lifeMapContinuity: 'same-selected-star-resolved-at-close-range',
      terrainOwner: false,
    }}
  >
    <mesh raycast={() => null} scale={1.72} name="focus-memory-star-outer-corona">
      <sphereGeometry args={[1, 48, 32]} />
      <meshBasicMaterial color={accent} transparent opacity={memory ? 0.045 : 0.018} depthWrite={false} blending={THREE.AdditiveBlending} />
    </mesh>
    <mesh raycast={() => null} scale={1.43} name="focus-memory-star-inner-corona">
      <sphereGeometry args={[1, 48, 32]} />
      <meshBasicMaterial color={light} transparent opacity={memory ? 0.075 : 0.028} depthWrite={false} blending={THREE.AdditiveBlending} />
    </mesh>
    <mesh
    ...
    >
      ...
      <meshPhysicalMaterial
        color="#07101f"
        emissive={accent}
        emissiveIntensity={memory ? (hovered ? 0.92 : 0.58) : 0.18}
        transmission={0.72}
        thickness={0.82}
        roughness={0.075}
        metalness={0.03}
    ...
    <mesh raycast={() => null} scale={0.985} name="focus-memory-star-interior-depth">
      <sphereGeometry args={[1, 48, 36]} />
      <meshBasicMaterial color="#030817" transparent opacity={0.34} depthWrite={false} side={THREE.BackSide} />
    </mesh>
    <pointLight color={accent} intensity={memory ? 5.2 : 1.1} distance={8.5} decay={2} />
    <Html center transform position={[0, 0, 0.82]} distanceFactor={5.05} zIndexRange={[20, 10]}>
      <button
        type="button"
...
export default function FocusChamberClient() {
  ...
  return <main
    ...
    data-focus-visual-revision="v323-memory-star-interior"
