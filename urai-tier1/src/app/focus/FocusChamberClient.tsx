...
// Locked product authority; V343 is the current literal-pixel implementation:
...
function makeFocusCoronaTexture(power: number, rays = false) {
  ...
  for (let y = 0; y < size; y += 1) for (let x = 0; x < size; x += 1) {
    ...
    const broadPlume = rays
      ? Math.pow(Math.max(0, .54 * Math.cos(warpedAngle * 7 + .2) + .29 * Math.cos(warpedAngle * 13 - .8) + .17 * Math.cos(warpedAngle * 23 + .35)), 7)
        * Math.pow(radial, .78)
      : 0;
    const finePlume = rays
      ? Math.pow(Math.max(0, .58 * Math.cos(warpedAngle * 11 - .45) + .42 * Math.cos(warpedAngle * 17 + .73)), 11)
        * Math.pow(radial, 1.08) * .34
      : 0;
    const alpha = distance >= 1
      ? 0
      : rays
        ? Math.min(1, Math.pow(radial, power) * .28 + broadPlume * .56 + finePlume)
...
function makeFocusPhotosphereTexture() {
  ...
  for (let y = 0; y < size; y += 1) for (let x = 0; x < size; x += 1) {
    ...
    const granulation = .5
      + Math.sin(x * .47 + Math.sin(y * .19) * 2.2) * .16
      + Math.sin(y * .61 + Math.cos(x * .23) * 1.7) * .12
      + Math.sin((x + y) * .17) * .08;
    const activeRegion = Math.max(0, Math.sin(x * .11 - y * .07) + Math.sin(x * .031 + y * .13)) * .055;
    const brightness = THREE.MathUtils.clamp(.34 + limb * .5 + granulation * .22 + activeRegion, 0, 1);
    const edgeFade = THREE.MathUtils.clamp((1 - distance) / .075, 0, 1);
    data[offset] = 255;
    data[offset + 1] = Math.round(132 + brightness * 116);
    data[offset + 2] = Math.round(42 + brightness * 166);
    data[offset + 3] = Math.round(edgeFade * (0.78 + limb * .22) * 255);
  }
  const texture = new THREE.DataTexture(data, size, size, THREE.RGBAFormat);
  texture.minFilter = THREE.LinearFilter;
  texture.magFilter = THREE.LinearFilter;
...
function FocusMemoryStar({
  ...
  const starScale = memory ? THREE.MathUtils.clamp(memory.star.scale * 1.66, 1.48, 1.74) : 1.08
  ...
  return <group
    ...
    userData={{
      visualAuthority: 'selected-memory-star-resolving-through-memory-v343',
      lifeMapContinuity: 'same-selected-star-resolved-at-close-range',
      terrainOwner: false,
    }}
  >
    <>
    <sprite raycast={() => null} position={[-.08, .05, -.28]} scale={[3.12, 3.12, 1]} name="focus-memory-star-corona-glow">
      <spriteMaterial map={coronaTexture} color={accent} transparent opacity={memory ? .18 : .04} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[.04, -.02, -.18]} scale={[3.82, 3.58, 1]} rotation={.19} name="focus-memory-star-photosphere-rays">
      <spriteMaterial map={rayTexture} color="#ffd27a" transparent opacity={memory ? .38 : .04} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[0, 0, -.07]} scale={[2.26, 2.26, 1]} name="focus-memory-star-photosphere-surface">
      <spriteMaterial map={photosphereTexture} color="#fff7d1" transparent opacity={memory ? .94 : .12} depthWrite={false} toneMapped={false} />
    </sprite>
    ...
    <mesh raycast={() => null} scale={0.54} name="focus-memory-star-inner-corona">
      <sphereGeometry args={[1, 64, 40]} />
      <meshBasicMaterial color={light} transparent opacity={memory ? 0.045 : 0.003} depthWrite={false} blending={THREE.AdditiveBlending} />
    </mesh>
    <mesh raycast={() => null} scale={0.52} name="focus-memory-star-photosphere-core">
...
export default function FocusChamberClient() {
  ...
  return <main
    ...
    data-focus-visual-revision="v343-stellar-css-corona-memory-within-photosphere"
