...
function FocusMemoryStar({
  ...
  const starScale = memory ? THREE.MathUtils.clamp(memory.star.scale, 0.84, 0.98) : 0.88
  ...
  return <group
    ...
    userData={{
      visualAuthority: 'selected-memory-star-with-contained-memory-v327',
      lifeMapContinuity: 'same-selected-star-resolved-at-close-range',
      terrainOwner: false,
    }}
  >
    <sprite raycast={() => null} position={[-.12, .08, -.22]} scale={[3.15, 3.15, 1]} name="focus-memory-star-corona-glow">
      <spriteMaterial map={coronaTexture} color={accent} transparent opacity={memory ? .12 : .05} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[.10, -.05, -.08]} scale={[2.72, 2.72, 1]} rotation={.22} name="focus-memory-star-photosphere-rays">
      <spriteMaterial map={rayTexture} color={light} transparent opacity={memory ? .14 : .05} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <mesh raycast={() => null} scale={1.30} name="focus-memory-star-outer-corona">
      <sphereGeometry args={[1, 64, 40]} />
      <meshBasicMaterial color={accent} transparent opacity={memory ? 0.010 : 0.006} depthWrite={false} blending={THREE.AdditiveBlending} />
    </mesh>
    <mesh raycast={() => null} scale={1.16} name="focus-memory-star-inner-corona">
      <sphereGeometry args={[1, 64, 40]} />
      <meshBasicMaterial color={light} transparent opacity={memory ? 0.015 : 0.008} depthWrite={false} blending={THREE.AdditiveBlending} />
    </mesh>
    <mesh raycast={() => null} scale={1.03} name="focus-memory-star-photosphere-core">
      <sphereGeometry args={[1, 64, 48]} />
      <meshStandardMaterial
        color={light}
        emissive={accent}
        emissiveIntensity={memory ? 0.82 : 0.20}
        roughness={0.34}
        metalness={0}
        transparent
        opacity={memory ? 0.64 : 0.18}
        depthWrite={false}
      />
    </mesh>
    <mesh
    ...
    >
      <sphereGeometry args={[1.10, 64, 48]} />
      <meshPhysicalMaterial
        color={light}
        emissive={accent}
        emissiveIntensity={memory ? (hovered ? 1.18 : 0.76) : 0.24}
        transmission={0.48}
        thickness={0.62}
        roughness={0.18}
        metalness={0}
        transparent
        opacity={memory ? 0.23 : 0.09}
        clearcoat={0.92}
        clearcoatRoughness={0.10}
        depthWrite={false}
      />
    </mesh>
    <mesh raycast={() => null} scale={0.97} name="focus-memory-star-interior-depth">
      <sphereGeometry args={[1, 48, 36]} />
      <meshBasicMaterial color={accent} transparent opacity={memory ? 0.040 : 0.020} depthWrite={false} side={THREE.BackSide} blending={THREE.AdditiveBlending} />
    </mesh>
    <pointLight color={accent} intensity={memory ? 3.6 : .9} distance={8.6} decay={2} />
    <pointLight position={[-1.1, 1.25, 1.7]} color={light} intensity={memory ? 2.0 : .35} distance={6.4} decay={2} />
    <Html center transform position={[0, 0, 0.58]} distanceFactor={4.35} zIndexRange={[20, 10]}>
      <button
        type="button"
...
export default function FocusChamberClient() {
  ...
  return <main
    ...
    data-focus-visual-revision="v327-dimensional-stellar-volume-memory-interior"
