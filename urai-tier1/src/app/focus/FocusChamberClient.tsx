...
// Locked product authority; V348 is the current literal-pixel implementation:
...
function FocusMemoryStar({
  ...
  return <group
    ...
    userData={{
      visualAuthority: 'selected-memory-star-resolving-through-memory-v348',
  ...
  >
    ...
    <mesh raycast={() => null} geometry={coronaShellA} scale={0.93} rotation={[.08, -.16, .11]} name="focus-memory-star-outer-corona">
      <meshBasicMaterial color="#ff5b1f" transparent opacity={memory ? 0.12 : 0.012} depthWrite={false} depthTest side={THREE.BackSide} blending={THREE.AdditiveBlending} toneMapped={false} />
    </mesh>
    <mesh raycast={() => null} geometry={coronaShellB} scale={0.84} rotation={[-.11, .18, -.23]} name="focus-memory-star-inner-corona">
      <meshBasicMaterial color="#ffc05f" transparent opacity={memory ? 0.08 : 0.008} depthWrite={false} depthTest side={THREE.BackSide} blending={THREE.AdditiveBlending} toneMapped={false} />
    </mesh>
...
export default function FocusChamberClient() {
  ...
  return <main
    ...
    data-focus-visual-revision="v348-soft-corona-memory-legibility"
