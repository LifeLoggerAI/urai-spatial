...
import { useAdaptiveSpatialQuality, markFirstSpatialFrame, type SpatialQualityProfile } from '@/spatial/performance/useAdaptiveSpatialQuality'
...
function FocusCameraRig({ controls, reducedMotion, recenterSignal }: { controls: React.RefObject<OrbitControlsImpl | null>; reducedMotion: boolean; recenterSignal: number }) {
  ...
  useFrame((_, delta) => {
    ...
    target.copy(controls.current?.target ?? new THREE.Vector3(...DEFAULT_TARGET))
    target.add(movement.multiplyScalar(0.72))
...
function ChamberArchitecture({ accent, light, reducedMotion }: { accent: string; light: string; reducedMotion: boolean }) {
  ...
  return (
    <group>
      ...
      <group ref={inner} rotation={[Math.PI / 2.8, 0.35, 0.1]}>
        <mesh><torusGeometry args={[3.2, 0.014, 10, 140]} /><meshBasicMaterial color={accent} transparent opacity={0.3} depthWrite={false} /></mesh>
      </group>
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -1.55, -1.2]} receiveShadow>
        <circleGeometry args={[13, 160]} />
        <meshPhysicalMaterial color="#030813" roughness={0.26} metalness={0.72} transparent opacity={0.9} clearcoat={0.65} clearcoatRoughness={0.28} />
      </mesh>
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -1.51, -1.2]}>
        <ringGeometry args={[2.2, 11.4, 180]} />
        <meshBasicMaterial color={accent} transparent opacity={0.07} side={THREE.DoubleSide} depthWrite={false} />
      </mesh>
      {[-5.4, 5.4].map((x) => (
        <group key={x} position={[x, 0, -3.8]}>
          <mesh><cylinderGeometry args={[0.11, 0.22, 5.8, 18]} /><meshStandardMaterial color="#07111d" emissive={accent} emissiveIntensity={0.35} metalness={0.78} roughness={0.24} /></mesh>
          <pointLight position={[0, 1.8, 0]} color={accent} intensity={1.6} distance={7} />
        </group>
      ))}
    </group>
  )
...
function Trace({ position, accent, reducedMotion, delay }: { position: [number, number, number]; accent: string; reducedMotion: boolean; delay: number }) {
  ...
  return (
    <group ref={ref} position={position}>
      <mesh><octahedronGeometry args={[0.18, 1]} /><meshStandardMaterial color="#07121e" emissive={accent} emissiveIntensity={1.4} roughness={0.24} metalness={0.52} /></mesh>
      <mesh><sphereGeometry args={[0.46, 20, 20]} /><meshBasicMaterial color={accent} transparent opacity={0.045} depthWrite={false} blending={THREE.AdditiveBlending} /></mesh>
    </group>
  )
}
...
function MemoryAperture({ memory, accent, light, reducedMotion, onActivate }: { memory: SelectedMemory | null; accent: string; light: string; reducedMotion: boolean; onActivate: () => void }) {
  ...
  useFrame(({ clock }, delta) => {
    ...
    group.current.scale.setScalar(THREE.MathUtils.damp(group.current.scale.x, hovered ? 1.09 : pulse, 5.5, delta))
  ...
  return (
    <group ref={group} position={[0, 0.35, -1.55]} name="focus-memory-aperture">
      <mesh onClick={(event) => { event.stopPropagation(); if (memory) onActivate() }} onPointerOver={(event) => pointer(event, true)} onPointerOut={(event) => pointer(event, false)}>
        ...
        <meshPhysicalMaterial color="#07131f" emissive={accent} emissiveIntensity={memory ? (hovered ? 2.8 : 1.55) : 0.42} transmission={0.38} thickness={0.7} roughness={0.12} metalness={0.18} transparent opacity={memory ? 0.92 : 0.55} clearcoat={1} clearcoatRoughness={0.08} />
      </mesh>
      ...
      <Html center position={[0, -2.25, 0]} transform distanceFactor={7.6} occlude="blending">
        <button type="button" className="focus-spatial-aperture-button" disabled={!memory} onClick={onActivate} aria-label={memory ? `Open Replay for ${memory.title}` : 'Select a memory in Life Map to open Replay'}>{memory ? 'Enter Replay' : 'Awaiting a selected star'}</button>
      </Html>
    </group>
  )
}

function FocusScene({ memory, profile, recenterSignal, onActivate, controls }: { memory: SelectedMemory | null; profile: SpatialQualityProfile; recenterSignal: number; onActivate: () => void; controls: React.RefObject<OrbitControlsImpl | null> }) {
...
function AuthorizedMemoryIdentity({ memory }: { memory: SelectedMemory }) {
  return <span hidden data-memory-id={memory.id} data-star-id={memory.star.id} data-node={memory.star.id} data-manifest-id={memory.replayManifest.id} />
}
...
export default function FocusChamberClient() {
  ...
  const directEntry = typeof window !== 'undefined' && !new URLSearchParams(window.location.search).get('memoryId') && !new URLSearchParams(window.location.search).get('node')
  ...
  const style = {
    '--memory-accent': memory?.visuals.accent ?? '#79dfff',
    '--memory-light': memory?.visuals.light ?? '#e7fbff',
    '--memory-sky': memory?.visuals.sky ?? '#020712',
    '--memory-ground': memory?.visuals.ground ?? '#07121c',
  ...
  return (
    <main className="focusWorld" style={style} data-testid="urai-final-focus-chamber" data-focus-composition="living-memory-chamber" data-focus-spatial="explorable-observatory" data-memory-status={result.status} data-chamber-state={chamberState} data-canonical-asset={focusAssets.primary.src} data-spatial-quality={profile.tier}>
      <h1 className="srOnly">URAI Focus spatial memory observatory</h1>
      {memory ? <AuthorizedMemoryIdentity memory={memory} /> : null}
      ...
      <div className="focusCanvas" aria-label="Explorable Focus chamber. Drag to orbit, scroll or pinch to move through depth, and use W A S D or arrow keys to travel.">
        {webglAvailable ? (
          <Suspense fallback={<div className="focusFallback" role="status">Opening spatial chamber…</div>}>
            <Canvas camera={{ position: DEFAULT_CAMERA, fov: 48, near: 0.08, far: 120 }} dpr={[1, profile.pixelRatioMax]} shadows={profile.shadows} frameloop={profile.documentVisible ? 'always' : 'never'} gl={{ antialias: profile.antialias, alpha: false, powerPreference: 'high-performance' }}>
              <FocusScene memory={memory} profile={profile} recenterSignal={recenterSignal} onActivate={enterReplay} controls={controls} />
            </Canvas>
          </Suspense>
        ) : <div className="focusFallback" role="status"><strong>Spatial view unavailable</strong><span>The chamber remains accessible through the controls and memory details.</span></div>}
      </div>

      <header className="focusHeading">
        <p>{memory ? (memory.demo ? 'DEMO FIXTURE · NOT PERSONAL DATA' : `${memory.privacy} memory`) : 'URAI · FOCUS OBSERVATORY'}</p>
        <h2>{heading}</h2>
        {memory ? <span>{dateLabel(memory.occurredAt)}</span> : null}
        <div className="focusNarration"><small>{memory ? 'Selected memory' : 'Chamber threshold'}</small><strong>{description}</strong></div>
      </header>

      <section className="artifactStage" aria-label={memory ? `Selected memory ${memory.title}` : 'Neutral Focus observatory'}>
      ...
      <aside className="memoryMeaning" aria-label="Selected memory context">
        <p>{memory ? 'Held in context. Nothing leaves this chamber.' : result.status === 'loading' ? 'Opening the selected memory safely.' : 'No personal memory is displayed in this neutral observatory.'}</p>
        {memory ? <dl><div><dt>Emotion</dt><dd>{memory.emotionalState}</dd></div><div><dt>Place</dt><dd>{memory.place?.label ?? 'Not recorded'}</dd></div><div><dt>People</dt><dd>{memory.people.map((person) => person.relationship ? `${person.label} · ${person.relationship}` : person.label).join(', ') || 'Not recorded'}</dd></div><div><dt>Privacy</dt><dd>{memory.privacy}</dd></div></dl> : <div className="neutralActions"><button type="button" onClick={unwind}>Open Life Map</button><span>{result.status === 'loading' ? 'Loading' : result.message}</span></div>}
      </aside>

      <nav className="focusControls" aria-label="Focus chamber controls">
        <button type="button" onClick={() => setRecenterSignal((value) => value + 1)}>Recenter</button>
        {memory ? <button type="button" className="primary" disabled={committed} onClick={enterReplay} aria-label={`Open Replay for ${memory.title}`}>{committed ? 'Opening…' : 'Enter Replay'}</button> : null}
