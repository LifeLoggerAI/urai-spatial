...
// Locked product authority; V366 is the current literal-pixel implementation:
...
function makeFocusPhotosphereTexture() {
  ...
  for (let y = 0; y < size; y += 1) for (let x = 0; x < size; x += 1) {
    ...
    const distance = Math.sqrt(dx * dx + dy * dy);
    const angle = Math.atan2(dy, dx);
    const boundary = THREE.MathUtils.clamp(.90
      + Math.sin(angle * 7 + .52) * .070
      + Math.sin(angle * 13 - .81) * .045
      + Math.sin(angle * 23 + .34) * .025, .78, .995);
    const offset = (y * size + x) * 4;
...
function FocusMemoryStar({
  ...
  return <group
    ...
    userData={{
      visualAuthority: 'selected-memory-star-resolving-through-memory-v366',
  ...
  >
    ...
    <sprite raycast={() => null} position={[-.08, .04, -.08]} scale={[3.06, 2.58, 1]} rotation={-.18} name="focus-memory-star-corona-glow">
      <spriteMaterial map={coronaTexture} color="#ffbd63" transparent opacity={memory ? .42 : .04} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[.06, -.02, .11]} scale={[4.34, 3.10, 1]} rotation={.23} name="focus-memory-star-photosphere-rays">
      <spriteMaterial map={rayTexture} color="#ffe09a" transparent opacity={memory ? .38 : .03} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[-.05, .06, .10]} scale={[2.92, 4.18, 1]} rotation={-.49} name="focus-memory-star-photosphere-rays-secondary">
      <spriteMaterial map={rayTexture} color="#fff7dc" transparent opacity={memory ? .26 : .02} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    ...
    <sprite raycast={() => null} position={[.03, -.02, .16]} scale={[2.20, 1.72, 1]} rotation={-.16} name="focus-memory-star-white-hot-limb">
      <spriteMaterial map={photosphereTexture} color="#fffbe8" transparent opacity={memory ? .30 : .08} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <group raycast={() => null} name="focus-memory-star-explicit-corona-rays" position={[0, 0, -0.04]} />
    <mesh raycast={() => null} geometry={photosphereGeometry} scale={0.76} name="focus-memory-star-photosphere-core" rotation={[0.08, -0.18, 0]}>
      <meshBasicMaterial
        map={sphereTexture}
        color="#fff3b8"
        transparent
        opacity={memory ? 0.86 : 0.16}
        depthWrite={false}
...
export default function FocusChamberClient() {
  ...
  return <main
    ...
    data-focus-visual-revision="v366-3d-photosphere-owned-memory-window"
    ...
    data-focus-stellar-treatment="selected-3d-photosphere-owned-v366"
