...
// Locked product authority; V394 is the current literal-pixel implementation:
...
function makeFocusPhotosphereTexture() {
  ...
  for (let y = 0; y < size; y += 1) for (let x = 0; x < size; x += 1) {
    ...
    const granulation = .44
      + Math.sin(x * .47 + Math.sin(y * .19) * 2.2) * .22
      + Math.sin(y * .61 + Math.cos(x * .23) * 1.7) * .18
      + Math.sin((x + y) * .17) * .12
...
function makeFocusPhotosphereGeometry() {
  ...
  for (let index = 0; index < position.count; index += 1) {
    ...
    const warp = 1
      + Math.sin(vertex.x * 17 + vertex.y * 11 + vertex.z * 7) * .160
      + Math.sin(vertex.x * 31 - vertex.y * 13 + vertex.z * 19) * .090
      + Math.sin((vertex.x + vertex.y - vertex.z) * 43) * .055
...
function FocusMemoryStar({
  ...
  return <group
    ...
    userData={{
      visualAuthority: 'selected-memory-star-resolving-through-memory-v394',
      lifeMapContinuity: 'same-selected-star-resolved-at-close-range',
      terrainOwner: false,
    }}
  >
    <>
    <sprite raycast={() => null} position={[-.04, .02, -.10]} scale={[2.78, 2.34, 1]} rotation={-.11} name="focus-memory-star-corona-glow">
      <spriteMaterial map={coronaTexture} color="#fff2c6" transparent opacity={memory ? .30 : .10} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[.02, .01, .06]} scale={[3.34, 2.76, 1]} rotation={.31} name="focus-memory-star-organic-streamer-field">
      <spriteMaterial map={rayTexture} color="#fff9dc" transparent opacity={memory ? .15 : .06} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite raycast={() => null} position={[-.01, .01, .18]} scale={[1.82, 1.64, 1]} rotation={.07} name="focus-memory-star-photosphere-surface">
      <spriteMaterial map={photosphereTexture} color="#fffce8" transparent opacity={memory ? .96 : .05} depthWrite={false} depthTest={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <mesh raycast={() => null} geometry={photosphereGeometry} scale={[.38, .34, .31]} name="focus-memory-star-photosphere-core" rotation={[0.08, -0.18, 0]}>
      <meshBasicMaterial
        map={sphereTexture}
        color="#fff7d2"
        transparent
        opacity={memory ? 0.14 : 0.05}
        depthWrite={false}
...
export default function FocusChamberClient() {
  ...
  return <main
    ...
    data-focus-visual-revision="v394-stellar-photosphere-visible-contained-memory-no-orb"
    ...
    data-focus-stellar-treatment="selected-corona-plasma-irregular-photosphere-visible-contained-memory-v394"
