...
export default function FocusChamberClient() {
  ...
  return (
    <main
      ...
      data-node={node}
      style={{ '--focus-route-art': assetCssStack(focusAssets.primary) } as CSSProperties}
    >
      ...
      <style>{`
        ...
        .focusFragment{position:absolute;width:88px;aspect-ratio:1;border:1px solid rgba(188,230,239,.17);border-radius:44% 56% 52% 48%;background:linear-gradient(145deg,rgba(10,20,29,.5),rgba(30,20,33,.28));box-shadow:0 18px 54px rgba(0,0,0,.42);display:grid;place-items:center;color:rgba(227,250,255,.68);font-size:8px;font-weight:850;letter-spacing:.15em;text-transform:uppercase;backdrop-filter:blur(7px);animation:focusFragmentFloat 6s ease-in-out infinite alternate}.focusFragmentA{left:1%;top:19%;transform:rotate(-9deg)}.focusFragmentB{right:2%;top:24%;transform:rotate(8deg);animation-delay:-1.5s}.focusFragmentC{left:5%;bottom:17%;transform:rotate(11deg);animation-delay:-2.6s}.focusFragmentD{right:4%;bottom:18%;transform:rotate(-10deg);animation-delay:-3.4s}
        .focusInsight{position:absolute;right:max(18px,env(safe-area-inset-right));bottom:max(78px,calc(env(safe-area-inset-bottom) + 68px));width:min(300px,calc(100vw - 36px));padding:11px 13px;border:1px solid rgba(195,234,240,.14);border-radius:16px;background:rgba(2,7,14,.42);backdrop-filter:blur(12px);box-shadow:0 18px 54px rgba(0,0,0,.38)}.focusInsight p{margin:0 0 4px;font-size:12px;font-weight:760}.focusInsight span{display:block;color:rgba(224,244,248,.62);font-size:9px;line-height:1.4}
        .focusReturn{position:absolute;left:max(18px,env(safe-area-inset-left));bottom:max(18px,env(safe-area-inset-bottom));min-height:44px;border:1px solid rgba(197,238,245,.2);border-radius:999px;padding:8px 13px;background:rgba(2,8,18,.52);color:#eefcff;font:inherit;font-size:10px;font-weight:850;cursor:pointer;backdrop-filter:blur(10px)}.focusReturn:focus-visible{outline:2px solid white;outline-offset:3px}
        @keyframes focusArrive{0%{opacity:1;transform:scale(.72)}72%{opacity:.22}100%{opacity:0;transform:scale(1.72);visibility:hidden}}
        @keyframes focusSettle{0%{opacity:0;transform:scale(.72) translateZ(-180px)}100%{opacity:1;transform:scale(1) translateZ(0)}}
        @keyframes focusWorldBreathe{from{transform:scale(1)}to{transform:scale(1.035) translate3d(0,-.6%,0)}}
        @keyframes focusAtmosphereDrift{to{background-position:113px 56px,220px 173px}}
        @keyframes focusOrbitTurn{to{rotate:1 1 0 360deg}}
        @keyframes focusShellBreathe{from{transform:scale(.985) rotate(-1deg)}to{transform:scale(1.025) rotate(1.2deg)}}
        @keyframes focusDepthPulse{50%{opacity:.44;transform:scale(.94)}}
        @keyframes focusFragmentFloat{from{translate:0 -5px}to{translate:0 8px}}
        @media(max-width:720px){.focusIdentity{left:14px;top:14px}.focusMemoryArchitecture{width:96vw;transform:translateY(3svh)}.focusMemoryPortal{width:min(58vw,290px)}.focusFragment{width:65px}.focusInsight{left:14px;right:14px;bottom:78px;width:auto}.focusReturn{left:14px;bottom:14px}.focusPortalLabel{bottom:-34px;font-size:8px}}
        @media(prefers-reduced-motion:reduce){.focusMemorySurface *{animation:none!important;transition:none!important}.focusArrivalVeil{display:none}.focusMemoryPortal:hover{transform:none}}
      `}</style>
    </main>
  )
}
