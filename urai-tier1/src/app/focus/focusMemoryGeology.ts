...
//
// V307 responds to exact V306 retained pixels: the branch appendages corrected continuity but
// made the ground event first-read as a twig/root/cable, especially in portrait. V307 removes
// all dendritic appendages and instead forms one broader torn-earth footprint with three local
// rupture pockets, asymmetric scalloped edges, tapered terminals, a narrow near-black inner
...
//
// V309 responds to exact V308 retained pixels: object vocabulary is finally gone, but the
// selected-memory event is too close to a faint hairline crack and loses focal authority.
// Keep the hairline connector and exact terrain ownership, but deepen the first read inside
// the three local rupture pockets: widen only those pockets, broaden the near-black cavity
...
//
// V311 responds to exact V310 pixels: the pocket footprint is finally legible, but the warm
// rim fragments still own first read as a raised-looking brown strip. Keep the exact same
// zero-thickness terrain seating and pocket geometry while shifting visual ownership into the
// near-black void: widen and asymmetrically offset only the dark cavity core, desaturate and
// sharply sparsify mineral rim exposure, and keep outer disturbance close to sanctuary ground.
//
// V312 responds to exact V311 pixels: the warm rim is suppressed, but too much opaque pocket
// skin still reads as a low folded ridge or mound. Preserve the three local pressure events,
// hairline connector and exact grade registration while collapsing non-core pocket contrast
...
//
// V314 responds to exact V313 pixels: removing light ownership exposed the real remaining
// silhouette problem. The broad filled outer pocket skin itself still reads as a gray folded
// panel. V314 removes that outer visual body and makes the rendered skin approximate only the
// near-black cavity footprint plus its hairline connector; no broad ground-colored pocket panel.
//
// V315 responds to conflicting V314 literal inspection by making missing-ground depth physical:
// the sanctuary terrain and visible near-black cavity bottom share one deterministic shallow
// depression field. The maximum depression stays above the buried V295 body, the connector is
...
//
// V318 responds to exact V317 retained pixels: the terrain basins are radial, but one continuous
// near-black cavity-bottom skin still visually unifies them into a single strip. V318 preserves
// the three radial terrain basins while localizing near-black void ownership to the pressure wells,
...
//
// V321 responds to exact-head c8af53d7 retained pixels: V320 remained too small on desktop and
// ultrawide to own focal hierarchy. V321 increases only the three local radial mouth radii,
// depression depth, longitudinal pressure-window width, and near-black bottom footprint. The
// connector remains unchanged and subordinate; intact sanctuary ground must still visibly
// separate the three openings, with no continuous trench, raised slab, rim, panel or object read.
//
...
export function focusSelectedMemoryCavityDepth(worldX: number, worldZ: number) {
  ...
  const basinSpecs = [
    { t: -.48, radius: .58, depth: .108 },
    { t: -.02, radius: .64, depth: .122 },
    { t: .42, radius: .56, depth: .104 },
  ]
  let basinDepth = 0
  ...
  return -Math.min(.128, basinDepth + connectorDepth)
}
...
function createLivingMemoryFold() {
  ...
  geometry.userData.focusLiteralPixelSuccessorV321 = 'v321-focal-readable-separated-radial-cavities-subordinate-hairline-connector'
  geometry.userData.focusVisualAuthority = 'v295-sanctuary-memory-scar-volume'
  geometry.userData.focusCurrentVisualAuthority = 'v321-focal-readable-three-radial-terrain-cavities-localized-bottoms-ground-blended-hairline-connector-buried-closed-body'
  return geometry
}
...
export function createFocusGroundIncision() {
  ...
  const uvs: number[] = []
  const indices: number[] = []
  const connectorGround = new THREE.Color().setRGB(.030, .034, .027)
  const cavityEdge = new THREE.Color().setRGB(.012, .014, .010)
  const weatheredMineral = new THREE.Color().setRGB(.072, .060, .036)
  const innerFissure = new THREE.Color().setRGB(.00035, .0003, .00025)

  const pushColor = (lateral: number, sectionPhase: number, pocketStrength: number) => {
    const absLateral = Math.abs(lateral)
    const cavityCenter = .08 * Math.sin(sectionPhase * .63 + .4) * pocketStrength
    const coreWidth = .38 + .18 * pocketStrength
    const core = Math.exp(-Math.pow((lateral - cavityCenter) / coreWidth, 2))
    const pocketMix = THREE.MathUtils.smoothstep(pocketStrength, .10, .62)
    const oneSidedFleck = lateral < 0
      ? THREE.MathUtils.smoothstep(absLateral, .70, 1)
        * pocketStrength
        * Math.max(0, Math.sin(sectionPhase * 2.11 + absLateral * 6.7) - .88)
        * .15
      : 0
    const voidMix = THREE.MathUtils.clamp(.06 + .91 * pocketMix * core, .06, .97)
    const color = connectorGround.clone()
      .lerp(cavityEdge, .28 + .58 * pocketMix)
      .lerp(weatheredMineral, oneSidedFleck)
      .lerp(innerFissure, voidMix)
    colors.push(color.r, color.g, color.b)
  }
  ...
  for (let section = 0; section < MEMORY_SECTIONS; section += 1) {
    ...
    const nearPocket = Math.exp(-Math.pow((t + .48) / .098, 2))
    const centerPocket = Math.exp(-Math.pow((t + .02) / .108, 2))
    const farPocket = Math.exp(-Math.pow((t - .42) / .094, 2))
    const pocketStrength = Math.max(nearPocket, centerPocket, farPocket)
    const hairlineHalfWidth = .0027 + .00055 * Math.sin(section * .91 + .31)
    const pocketHalfWidth = .330 * nearPocket + .380 * centerPocket + .315 * farPocket
    ...
    for (let cross = 0; cross < INCISION_CROSS_POINTS; cross += 1) {
      ...
      const sidePocket = lateral < 0
        ? nearPocket * .014 + centerPocket * .006
        : farPocket * .012 + centerPocket * .007
      const scallop = absLateral > .46
        ? (.016 + .024 * pocketStrength)
          * Math.sin(section * 2.03 + cross * 1.29 + (lateral < 0 ? .18 : 1.71))
          + .011 * pocketStrength * Math.sin(section * 4.17 - cross * .67)
        : .0035 * Math.sin(section * 1.49 + cross * .81) * absLateral
      const edgeBite = absLateral > .68 && pocketStrength > .18
        ? -.007 * pocketStrength * Math.max(0, Math.sin(section * 2.73 + (lateral < 0 ? .7 : 2.4)))
        : 0
      ...
      const worldX = FOCUS_MEMORY_WORLD_X + x
      const worldZ = FOCUS_MEMORY_WORLD_Z + z
      const y = focusGroundHeight(worldX, worldZ) + focusSelectedMemoryCavityDepth(worldX, worldZ) + .00034
      positions.push(x, y, z)
      uvs.push(crossU, u)
  ...
  geometry.computeBoundingBox()
  geometry.computeBoundingSphere()
  geometry.userData.focusIncisionAuthority = 'v321-focal-readable-localized-near-black-cavity-bottoms-ground-blended-subordinate-hairline-connector'
