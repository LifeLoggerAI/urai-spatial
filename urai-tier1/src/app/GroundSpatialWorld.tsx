...
function GroundHotspot({ label, detail, position, color }: { label: string; detail: string; position: [number, number, number]; color: string }) {
  const ref = useRef<THREE.Group>(null)
  useFrame(({ clock }) => {
  ...
  return (
    <group ref={ref} position={position}>
      <mesh rotation={[-Math.PI / 2, 0, 0]}>
        <ringGeometry args={[0.22, 0.44, 64]} />
      ...
      <mesh position={[0, 0.25, 0]}>
        ...
        <meshStandardMaterial color="#07111f" emissive={color} emissiveIntensity={0.38} transparent opacity={0.82} />
      </mesh>
      <Html position={[0, 0.7, 0]} center distanceFactor={6.8}>
        <span className="ground-hotspot-label"><strong>{label}</strong><em>{detail}</em></span>
      </Html>
    </group>
  )
}
...
function GroundScene() {
  return (
    <>
      ...
      <Skyline />
      <MovingTraffic />
      <GroundHotspot label="Signal" detail="new context" position={[-1.6, -0.42, 0.3]} color="#67e8f9" />
      <GroundHotspot label="Consent" detail="approval gate" position={[1.55, -0.42, 0.1]} color="#a78bfa" />
      <GroundHotspot label="Tasks" detail="city actions" position={[-1.0, -0.42, 2.35]} color="#facc6b" />
      <GroundHotspot label="Memory" detail="place anchors" position={[1.05, -0.42, 2.65]} color="#86efac" />
...
export default function GroundSpatialWorld() {
  return (
    <main className="ground-spatial-root" aria-label="URAI Ground explorable city layer">
      ...
      <header className="ground-card ground-hero">
        ...
        <span>Orbit the lived layer. Inspect signals, consent, tasks, and memory anchors before anything acts.</span>
      ...
      <style jsx>{`
        ...
        .ground-card,.ground-status,.ground-rail{position:absolute;z-index:5;border:1px solid rgba(160,220,255,.15);background:linear-gradient(145deg,rgba(2,8,24,.52),rgba(10,9,31,.28));box-shadow:0 24px 90px rgba(0,0,0,.28);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px)}
        ...
        .ground-hotspot-label{display:grid;gap:2px;min-width:78px;padding:6px 8px;border:1px solid rgba(160,220,255,.22);border-radius:12px;background:rgba(2,8,24,.68);box-shadow:0 12px 40px rgba(0,0,0,.3);backdrop-filter:blur(12px);color:#eef6ff;text-align:center;line-height:1.05}
        .ground-hotspot-label strong{font-size:10px;text-transform:uppercase;letter-spacing:.12em}.ground-hotspot-label em{font-style:normal;font-size:9px;color:rgba(235,244,255,.62)}
        @media(max-width:720px){.ground-hero{left:14px;right:14px;top:14px;width:auto}.ground-status{display:none}.ground-rail{bottom:12px}.ground-hotspot-label{display:none}}
      `}</style>
    </main>
  )
}
