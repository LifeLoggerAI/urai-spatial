...
function MemorySymbol({ kind, color }: { kind: MemoryKind; color: string }) {
  ...
  if (kind === 'home') {
    return (
      <group scale={0.62} position={[0, -0.08, 0]}>
        <mesh position={[0, -0.06, 0]}>{/* house */}<boxGeometry args={[1.05, 0.72, 0.78]} />{material}</mesh>
        <mesh position={[0, 0.48, 0]} rotation={[0, Math.PI / 4, 0]}><coneGeometry args={[0.78, 0.62, 4]} />{material}</mesh>
        <mesh position={[0, -0.16, 0.405]}><boxGeometry args={[0.22, 0.4, 0.04]} /><meshStandardMaterial color="#07121d" emissive={color} emissiveIntensity={0.15} /></mesh>
      </group>
    )
  }
  ...
  if (kind === 'ride') {
    return (
      <group scale={0.58} position={[0, -0.05, 0]}>
        <mesh position={[-0.46, -0.2, 0]} rotation={[Math.PI / 2, 0, 0]}><torusGeometry args={[0.34, 0.055, 10, 28]} />{material}</mesh>
        <mesh position={[0.46, -0.2, 0]} rotation={[Math.PI / 2, 0, 0]}><torusGeometry args={[0.34, 0.055, 10, 28]} />{material}</mesh>
        <mesh rotation={[0, 0, -0.1]}><tubeGeometry args={[new THREE.CatmullRomCurve3([
          new THREE.Vector3(-0.46, -0.2, 0),
          new THREE.Vector3(-0.08, 0.18, 0),
          new THREE.Vector3(0.22, -0.2, 0),
          new THREE.Vector3(0.46, -0.2, 0),
        ]), 18, 0.035, 8, false]} />{material}</mesh>
        <mesh position={[-0.04, 0.23, 0]} rotation={[0, 0, Math.PI / 2]}><cylinderGeometry args={[0.035, 0.035, 0.48, 8]} />{material}</mesh>
      </group>
    )
  }
      ...
      <group position={[0, -0.14, 0]}>
        {[-0.42, 0, 0.42].map((x, index) => (
          <group key={x} position={[x, index === 1 ? 0.08 : 0, index === 1 ? -0.08 : 0]} scale={index === 1 ? 1.05 : 0.88}>
            <mesh position={[0, 0.3, 0]}><sphereGeometry args={[0.15, 18, 18]} />{material}</mesh>
            <mesh position={[0, -0.08, 0]}><capsuleGeometry args={[0.16, 0.45, 7, 12]} />{material}</mesh>
          </group>
        ))}
      </group>
    )
  ...
  if (kind === 'music') {
    return (
      <group scale={0.78} position={[0, -0.02, 0]}>
        <mesh position={[-0.24, -0.22, 0]}><sphereGeometry args={[0.22, 22, 22]} />{material}</mesh>
        <mesh position={[0.28, -0.08, 0]}><sphereGeometry args={[0.19, 22, 22]} />{material}</mesh>
        <mesh position={[0.03, 0.28, 0]} rotation={[0, 0, -0.08]}><boxGeometry args={[0.08, 0.9, 0.08]} />{material}</mesh>
        <mesh position={[0.3, 0.56, 0]} rotation={[0, 0, -0.08]}><boxGeometry args={[0.62, 0.08, 0.08]} />{material}</mesh>
      </group>
    )
  }
  ...
  return (
    <group position={[0, -0.2, 0]} scale={0.72}>
      <mesh position={[0, -0.22, 0]}><cylinderGeometry args={[0.09, 0.16, 0.85, 12]} /><meshStandardMaterial color="#7a5b45" roughness={0.78} /></mesh>
      {[
        [0, 0.34, 0],
        [-0.24, 0.2, 0.02],
        [0.26, 0.19, -0.02],
...
function SanctuaryArchitecture() {
  const columnZ = [5.6, 2.1, -1.4, -4.9, -8.2]
  const pathZ = [6.35, 4.95, 3.55, 2.15, 0.75, -0.65, -2.05, -3.45, -4.85, -6.25, -7.65]
  ...
  return (
    <group name="home-sanctuary-spatial-architecture">
      ...
      {columnZ.flatMap((z, row) => [-1, 1].map((side) => (
