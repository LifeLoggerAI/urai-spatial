...
export default function StatusReleaseAuthority({ totalRoutes }: { totalRoutes: number }) {
  ...
  return (
    <article className="rounded-[2rem] border border-cyan-100/15 bg-slate-950/60 p-7 shadow-2xl shadow-cyan-950/30 backdrop-blur-2xl" data-testid="urai-status-release-authority" data-authority-state={state.kind} aria-live="polite">
      ...
      {state.kind === 'preview' && <div className={`${panel} border-violet-200/25 bg-violet-200/[0.08]`}><span className="text-xs font-black uppercase tracking-[0.18em] text-violet-200">Tracked {totalRoutes} public routes</span><strong className="mt-2 block text-2xl">Production fingerprint is read only on urai.app.</strong><p className="mt-3 text-sm">This proof origin does not request or substitute a live or candidate SHA.</p></div>}
