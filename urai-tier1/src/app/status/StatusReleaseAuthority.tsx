...
type RouteState = 'certified-live' | 'preview' | 'gated'
...
const groups: ReadonlyArray<{ title: string; items: ReadonlyArray<RouteItem> }> = [
  { title: 'Launch spine', items: [['/', 'certified-live', 'Home threshold entry'], ['/home', 'certified-live', 'Canonical Home World'], ['/ground', 'certified-live', 'Private operating world'], ['/life-map', 'certified-live', 'Spatial memory galaxy'], ['/focus', 'certified-live', 'Selected Memory Star'], ['/replay', 'certified-live', 'Memory film route'], ['/mirror', 'certified-live', 'Reflection realm'], ['/passport', 'certified-live', 'Identity vault'], ['/status', 'certified-live', 'Route and certification room']] },
  { title: 'Trust and place', items: [['/privacy-controls', 'certified-live', 'Permission controls'], ['/location-map', 'certified-live', 'Place and emotional weather'], ['/ascent', 'certified-live', 'Sky ascent route'], ['/unwind', 'certified-live', 'Return route']] },
  { title: 'Showcase and XR', items: [['/demo', 'certified-live', 'Disclosed public walkthrough'], ['/demo/replay-film', 'certified-live', 'Disclosed Replay film proof surface'], ['/spatial/life-map', 'certified-live', 'Spatial Life Map'], ['/spatial/life-map-r3f', 'certified-live', 'R3F Life Map'], ['/spatial/ar-vr', 'gated', 'Conditional XR route remains hidden until governed XR release evidence is complete']] },
]
...
function validate(value: unknown): Fingerprint {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('Release fingerprint is missing or malformed.')
  ...
  for (const field of ['authoritySha', 'releaseSha', 'rollbackSha'] as const) {
    if (typeof item[field] !== 'string' || !shaPattern.test(item[field])) throw new Error(`Release fingerprint ${field} is invalid.`)
  }
...
function badgeClass(label: string) {
  ...
  if (label === 'gated') return 'border-slate-200/20 bg-slate-200/10 text-slate-100'
...
export default function StatusReleaseAuthority() {
  ...
  useEffect(() => {
    ...
    fetch(`/release-fingerprint.json?_=${Date.now()}`, { cache: 'no-store', headers: { accept: 'application/json', 'cache-control': 'no-cache, no-store, max-age=0' }, signal: controller.signal })
      .then((response) => {
  ...
  const unresolvedCopy = state.kind === 'preview'
    ...
    <div data-authority-state={state.kind}>
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_410px]">
        ...
        <article className="rounded-[2rem] border border-cyan-100/15 bg-slate-950/60 p-7 shadow-2xl shadow-cyan-950/30 backdrop-blur-2xl" data-testid="urai-status-release-authority" aria-live="polite">
          ...
          {state.kind === 'ready' && <div className="grid grid-cols-2 gap-3"><div className="rounded-2xl border border-white/10 bg-white/[0.06] p-5"><span className="text-xs">Tracked</span><strong className="mt-2 block text-2xl">{totalRoutes}</strong></div><div className="rounded-2xl border border-emerald-200/20 bg-emerald-200/[0.08] p-5"><span className="text-xs">Production</span><strong className="mt-2 block text-2xl">Verified live</strong></div><div className="rounded-2xl border border-white/10 bg-white/[0.06] p-5"><span className="text-xs">Release</span><strong className="mt-2 block font-mono" title={state.fingerprint.releaseSha}>{shortSha(state.fingerprint.releaseSha)}</strong></div><div className="rounded-2xl border border-white/10 bg-white/[0.06] p-5"><span className="text-xs">Rollback</span><strong className="mt-2 block font-mono" title={state.fingerprint.rollbackSha}>{shortSha(state.fingerprint.rollbackSha)}</strong></div><dl className="col-span-2 grid gap-2 rounded-2xl border border-white/10 bg-black/20 p-4 text-xs"><div className="flex justify-between"><dt>Authority</dt><dd className="font-mono">{shortSha(state.fingerprint.authoritySha)}</dd></div><div className="flex justify-between"><dt>Firebase</dt><dd>{state.fingerprint.firebaseProject}</dd></div><div className="flex justify-between"><dt>Scope</dt><dd>{state.fingerprint.deploymentScope}</dd></div><div className="flex justify-between"><dt>Run</dt><dd>{state.fingerprint.workflowRunId}</dd></div></dl></div>}
        </article>
      </div>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-3">{groups.map((group) => <section key={group.title} className="rounded-[2rem] border border-white/10 bg-slate-950/58 p-5 shadow-2xl shadow-black/30 backdrop-blur-2xl"><h2 className="text-xl font-black tracking-tight">{group.title}</h2><div className="mt-5 grid gap-3">{group.items.map(([route, routeState, note]) => { const label = routeState === 'gated' ? 'gated' : routeState === 'preview' ? 'preview' : isReady ? 'verified live' : 'authority unresolved'; return <article key={route} className="rounded-2xl border border-cyan-100/10 bg-white/[0.045] p-4"><div className="flex items-center justify-between gap-3"><code className="font-mono text-sm font-black text-cyan-100">{route}</code><span className={`rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-[0.16em] ${badgeClass(label)}`}>{label}</span></div><p className="mt-2 text-sm font-semibold leading-6 text-slate-200/76">{note}</p></article> })}</div></section>)}</div>
      ...
      <nav className="mt-6 flex flex-wrap gap-3" aria-label="Status route navigation"><Link className="inline-flex min-h-12 items-center justify-center rounded-full bg-white px-5 py-3 text-sm font-black text-slate-950 no-underline" href="/home">Open Home</Link><Link className="inline-flex min-h-12 items-center justify-center rounded-full border border-white/20 px-5 py-3 text-sm font-black text-white no-underline" href="/ground">Open Ground</Link><Link className="inline-flex min-h-12 items-center justify-center rounded-full border border-white/20 px-5 py-3 text-sm font-black text-white no-underline" href="/life-map">Open Life Map</Link><Link className="inline-flex min-h-12 items-center justify-center rounded-full border border-white/20 px-5 py-3 text-sm font-black text-white no-underline" href="/privacy-controls">Privacy Controls</Link></nav>
    </div>
  )
}
