...
const groups = [
  {
    ...
    items: [
      ...
      ['/status', 'pending proof', 'Route and certification room'],
    ],
  },
  {
    title: 'Trust and place',
    items: [
      ['/privacy-controls', 'pending proof', 'Permission controls'],
    ...
    >
      ...
      <section className="relative z-10 mx-auto max-w-[1480px]">
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_410px]">
          <article className="rounded-[2rem] border border-cyan-100/15 bg-slate-950/60 p-8 shadow-2xl shadow-black/40 backdrop-blur-2xl md:p-12">
            ...
            <p className="mt-6 max-w-3xl text-base font-semibold leading-8 text-slate-200/80">
              This room is wired to the launch truth source and distinguishes source implementation from current deployment proof. A route is not certified live until the canonical commit, deploy receipt, rollback target, route fingerprints, slash parity, and required checks are recorded together.
            </p>
          </article>
          <article className="rounded-[2rem] border border-cyan-100/15 bg-slate-950/60 p-7 shadow-2xl shadow-cyan-950/30 backdrop-blur-2xl">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[0.06] p-4"><span className="text-[10px] font-black uppercase tracking-[0.22em] text-cyan-200">Mode</span><strong className="mt-2 block text-2xl">Locked</strong></div>
        ...
        <section className="mt-6 rounded-[2rem] border border-amber-200/20 bg-amber-200/[0.07] p-6 text-sm font-semibold leading-7 text-amber-50/90">
          ...
          <p className="mt-2">Production certification remains pending until the exact deployed commit and rollback target are recorded and the post-deployment custom-domain smoke passes every route fingerprint, slash form, and query-preservation check.</p>
        </section>
