    ...
    >
      ...
      <section className="relative z-10 mx-auto max-w-[1480px]">
        ...
        <section className="mt-6 grid gap-3 rounded-[2rem] border border-white/10 bg-slate-950/58 p-5 md:grid-cols-2 lg:grid-cols-4">
          ...
          <EvidenceCard label="Release" value={releaseReceipt.releaseId} />
          <EvidenceCard label="Manifest" value={releaseReceipt.manifestSha ? releaseReceipt.manifestSha.slice(0, 12) : "Not recorded"} />
        </section>
            ...
            <section key={group.title} className="rounded-[2rem] border border-white/10 bg-slate-950/58 p-5 shadow-2xl shadow-black/30 backdrop-blur-2xl">
              ...
              <div className="mt-5 grid gap-3">
                {group.items.map((route) => (
                  <article key={route.path} className="rounded-2xl border border-cyan-100/10 bg-white/[0.045] p-4">
                    <div className="flex items-center justify-between gap-3">
                      <code className="font-mono text-sm font-black text-cyan-100">{route.path}</code>
                      <span className={`rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-[0.16em] ${badgeClass(route)}`}>
                        {displayState(route)}
                      </span>
                    </div>
                    <p className="mt-2 text-sm font-semibold leading-6 text-slate-200/76">{routeNotes[route.path] ?? "Tracked release surface"}</p>
                  </article>
                ))}
              </div>
            </section>
...
function EvidenceCard({ label, value, warning = false }: { label: string; value: string; warning?: boolean }) {
  return (
    <div className={`rounded-2xl border p-4 ${warning ? "border-amber-200/20 bg-amber-200/[0.08]" : "border-white/10 bg-white/[0.06]"}`}>
      <span className={`text-[10px] font-black uppercase tracking-[0.22em] ${warning ? "text-amber-200" : "text-cyan-200"}`}>{label}</span>
      <strong className="mt-2 block break-all text-lg">{value}</strong>
    </div>
  );
}
