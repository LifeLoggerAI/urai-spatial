...
const portals: readonly PortalSpec[] = [
  ...
  { id: 'mirror', label: 'Mirror', detail: 'Pattern realm', href: '/mirror', position: [-7.2, 0.05, 0.8], rotationY: 1.04, color: '#8fd7ef' },
  { id: 'passport', label: 'Passport', detail: 'Ownership vault', href: '/passport', position: [7.2, 0.05, 0.8], rotationY: -1.04, color: '#e5c274' },
  { id: 'xr', label: 'XR', detail: 'Enter the world', href: '/spatial/ar-vr', position: [0, 0.05, 6.2], rotationY: Math.PI, color: '#79dbe8' },
]
  ...
  return (
    <group>
      ...
      {portals.map((portal) => {
        ...
        return (
          <mesh key={`path-${portal.id}`} rotation={[-Math.PI / 2, 0, -angle]} position={[x, -0.025, z]} receiveShadow>
      ...
      {[
        [-10, -8, 6.4, 2.8], [-4, -13, 7.2, 3.2], [5, -13.5, 8.6, 3.7],
        [12, -7, 7.2, 3.1], [-13, 3, 6.2, 2.7], [13, 4, 6.4, 2.9],
...
function Landscape() {
  const trees: Array<[number, number, number, number]> = [
    [-8.6, 0, -7.8, 1.12], [-5.9, 0, -10.1, 0.92], [-1.2, 0, -11.6, 1.04],
    [3.4, 0, -11.2, 0.95], [8.1, 0, -8.6, 1.1], [10.7, 0, -3.1, 0.96],
    [-10.6, 0, -2.4, 1.02], [-10.1, 0, 5.1, 0.92], [10.2, 0, 5.4, 1.02],
  ]
  return <group>{trees.map(([x, y, z, scale], index) => <Tree key={index} position={[x, y, z]} scale={scale} />)}</group>
}
...
function Orb({ reducedMotion, onOpen }: { reducedMotion: boolean; onOpen: () => void }) {
  ...
  useFrame(({ clock }) => {
    ...
    if (!reducedMotion) {
      group.current.position.y = 1.85 + Math.sin(clock.elapsedTime * 1.15) * 0.08
  ...
  return (
    <group ref={group} position={[0, 1.85, 0]}>
      <mesh castShadow onClick={(event: ThreeEvent<MouseEvent>) => { event.stopPropagation(); onOpen() }} onPointerOver={() => { document.body.style.cursor = 'pointer' }} onPointerOut={() => { document.body.style.cursor = 'default' }}>
        ...
        <meshPhysicalMaterial color="#d8fbff" emissive="#42dbea" emissiveIntensity={2.4} roughness={0.12} metalness={0.18} clearcoat={1} clearcoatRoughness={0.08} />
      </mesh>
...
function Portal({ spec, reducedMotion, onNavigate }: { spec: PortalSpec; reducedMotion: boolean; onNavigate: (href: string) => void }) {
  ...
  return (
    <group ref={group} position={spec.position} rotation={[0, spec.rotationY, 0]}>
      ...
      <mesh position={[1.05, 1.55, 0]} castShadow receiveShadow><boxGeometry args={[0.34, 3.1, 0.58]} /><meshStandardMaterial color="#77756f" roughness={0.88} metalness={0.04} /></mesh>
      <mesh position={[0, 3.08, 0]} castShadow><boxGeometry args={[2.44, 0.34, 0.62]} /><meshStandardMaterial color="#77756f" roughness={0.88} metalness={0.04} /></mesh>
...
function Scene({ reducedMotion, onOrbOpen }: { reducedMotion: boolean; onOrbOpen: () => void }) {
  ...
  return (
    <>
      ...
      <directionalLight position={[-5, 10, 6]} intensity={2.3} color="#fff0d6" castShadow shadow-mapSize-width={1024} shadow-mapSize-height={1024} />
      ...
      <OrbitControls makeDefault enablePan={false} enableDamping={!reducedMotion} dampingFactor={0.07} rotateSpeed={0.42} zoomSpeed={0.62} minDistance={7.2} maxDistance={15.5} minPolarAngle={0.72} maxPolarAngle={1.48} minAzimuthAngle={-1.45} maxAzimuthAngle={1.45} target={[0, 1.35, -1.8]} />
  ...
  return (
    <div className="urai-home-spatial-canvas-shell" data-home-spatial-renderer="webgl" data-webgl-ready="true" aria-label="Interactive spatial Home world">
      <Suspense fallback={<div className="urai-home-spatial-canvas-loading" aria-hidden="true" />}>
        <Canvas className="urai-home-spatial-canvas" shadows dpr={[1, 1.55]} camera={{ position: [0, 4.35, 11.8], fov: 48, near: 0.1, far: 90 }} gl={{ antialias: true, alpha: false, powerPreference: 'high-performance' }} onCreated={({ gl }) => { gl.outputColorSpace = THREE.SRGBColorSpace; gl.toneMapping = THREE.ACESFilmicToneMapping; gl.toneMappingExposure = 1.05 }}>
