...
const PHASE_VISUALS: Record<ReplayPhaseId, { ambient: number; fill: number; source: number; fogNear: number; fogFar: number }> = {
  ...
  memory: { ambient: 0.54, fill: 0.94, source: 4.6, fogNear: 15, fogFar: 46 },
  emotion: { ambient: 0.5, fill: 0.88, source: 4.2, fogNear: 13, fogFar: 42 },
  pattern: { ambient: 0.52, fill: 0.91, source: 4.4, fogNear: 14, fogFar: 44 },
  return: { ambient: 0.56, fill: 0.98, source: 3.6, fogNear: 17, fogFar: 48 },
}
...
function ReplaySpatialScene({ memory, playing, progressMs, muteVideo }: { memory: SelectedMemory; playing: boolean; progressMs: number; muteVideo: boolean }) {
  ...
  return (
    <>
      ...
      <directionalLight position={[-4, 7, 6]} intensity={2.05} color={memory.visuals.light} castShadow />
      <directionalLight position={[4, 2, -3]} intensity={0.88} color={memory.visuals.accent} />
