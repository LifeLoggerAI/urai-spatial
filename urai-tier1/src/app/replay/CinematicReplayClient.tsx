...
export default function CinematicReplayClient() {
  ...
  if (!memory) return (
    <main className="replayState" data-testid="cinematic-replay-client" data-memory-status={result.status} data-replay-loading={result.status === 'loading' ? 'true' : 'false'} data-canonical-asset={replayAssets.primary.src} data-replay-neutral="memory-horizon" data-replay-spatial-owner="r3f-memory-theater">
      ...
      <section role={result.status === 'loading' ? 'status' : 'region'} aria-label="Replay memory horizon"><p>{result.status === 'loading' ? 'Entering Replay' : 'Memory horizon'}</p><h1>{result.status === 'loading' ? 'The selected memory is resolving.' : 'Choose a memory to enter its reconstruction.'}</h1><span>{result.status === 'loading' ? 'Replay will open as soon as the selected memory is ready.' : 'Replay begins from a memory in Life Map, so you always arrive with context.'}</span>{result.status === 'loading' ? null : <button type="button" onClick={chooseMemory}>Choose a memory</button>}</section>
...
const stateCss = `.replayState{position:fixed;inset:0;overflow:hidden;display:grid;place-items:center;padding:24px;background:#02060d;color:#fff;isolation:isolate}.replaySpatialCanvas{position:absolute!important;inset:0;width:100%!important;height:100%!important}.replayState:after{content:'';position:absolute;inset:0;background:radial-gradient(circle at 50% 45%,transparent 0 22%,rgba(1,5,12,.20) 48%,rgba(1,5,12,.68) 100%);pointer-events:none}.replayState section{z-index:2;text-align:center;max-width:620px;padding:28px 30px;border:1px solid rgba(220,248,255,.12);border-radius:28px;background:linear-gradient(145deg,rgba(2,8,16,.7),rgba(2,8,16,.24));backdrop-filter:blur(18px);text-shadow:0 3px 24px #000}.replayState[data-replay-loading="true"]{place-items:stretch}.replayState[data-replay-loading="true"] section{align-self:end;justify-self:start;max-width:min(520px,calc(100vw - 48px));margin:0 0 max(34px,env(safe-area-inset-bottom)) max(28px,env(safe-area-inset-left));padding:0;border:0;border-radius:0;background:transparent;backdrop-filter:none;text-align:left;pointer-events:none}.replayState[data-replay-loading="true"] section h1{max-width:12ch;font-size:clamp(1.45rem,3.2vw,2.6rem)}.replayState[data-replay-loading="true"] section span{max-width:430px;margin:8px 0 0}.replayState section p{margin:0 0 9px;color:#c9f7ff;font-size:10px;font-weight:900;letter-spacing:.22em;text-transform:uppercase}.replayState section h1{margin:0;font:500 clamp(1.7rem,4.6vw,3.6rem)/1.02 var(--font-sans);letter-spacing:-.045em}.replayState section span{display:block;max-width:520px;margin:12px auto 0;color:rgba(235,247,255,.72);font-size:13px;line-height:1.55}.replayState button{min-height:48px;margin-top:20px;padding:0 22px;border-radius:999px;border:1px solid rgba(210,248,255,.32);background:linear-gradient(135deg,#dffbff,#8fe5ef);color:#041019;font-weight:900}.replayState button:focus-visible{outline:3px solid #fff;outline-offset:4px}@media(max-width:700px){.replayState section{max-width:calc(100vw - 32px);padding:24px 20px}.replayState[data-replay-loading="true"] section{max-width:calc(100vw - 32px);margin:0 0 max(24px,env(safe-area-inset-bottom)) 16px;padding:0}}@media(prefers-reduced-motion:reduce){.replayState section{backdrop-filter:none}}@media(forced-colors:active){.replayState section,.replayState button{border:2px solid CanvasText}.replayState[data-replay-loading="true"] section{border:0}}`
