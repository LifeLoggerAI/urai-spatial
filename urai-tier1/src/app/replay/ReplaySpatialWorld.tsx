...
import { buildReplaySpatialScene, replayEvidenceDescription, type ReplaySpatialSceneModel, type ReplayWorldAnchor } from '@/spatial/replay/replaySpatialModel'
...
function ReplayFloor({ model, walkTarget, mode }: { model: ReplaySpatialSceneModel; walkTarget: MutableRefObject<THREE.Vector3 | null>; mode: ReplayNavigationMode }) {
  const choose = (event: ThreeEvent<MouseEvent>) => {
    ...
    walkTarget.current = new THREE.Vector3(THREE.MathUtils.clamp(event.point.x, model.bounds.minX, model.bounds.maxX), 0, THREE.MathUtils.clamp(event.point.z, model.bounds.minZ, model.bounds.maxZ))
  }
  return (
...
function MemoryArchitecture() {
  return (
    <group name="replay-world-environment" data-testid="replay-world-environment">
      <mesh position={[0, 3.4, -7.8]} castShadow receiveShadow><boxGeometry args={[16, 6.8, 0.45]} /><meshStandardMaterial color="#07131d" roughness={0.7} metalness={0.16} /></mesh>
      {[-6.7, -4.5, -2.3, 2.3, 4.5, 6.7].map((x) => <mesh key={x} position={[x, 2.8, -3.2]} castShadow><boxGeometry args={[0.28, 5.6, 8.8]} /><meshStandardMaterial color="#0a1d28" roughness={0.64} metalness={0.24} /></mesh>)}
      <mesh position={[0, 2.7, -7.46]}><torusGeometry args={[2.25, 0.08, 16, 128, Math.PI]} /><meshBasicMaterial color="#b6f4ff" transparent opacity={0.34} depthWrite={false} /></mesh>
      ...
      <mesh position={[0, 0.03, -7.2]} rotation={[-Math.PI / 2, 0, 0]}><circleGeometry args={[2.1, 96]} /><meshStandardMaterial color="#0a2630" emissive="#4fbfd3" emissiveIntensity={0.2} roughness={0.5} /></mesh>
    </group>
  )
}
...
function AnchorPresence({ anchor, active, onSelect, walkTarget, mode }: { anchor: ReplayWorldAnchor; active: boolean; onSelect: (anchor: ReplayWorldAnchor) => void; walkTarget: MutableRefObject<THREE.Vector3 | null>; mode: ReplayNavigationMode }) {
  ...
  useFrame(({ clock }) => {
    if (group.current) group.current.rotation.y = anchor.evidenceLevel === 'inferred' ? Math.sin(clock.elapsedTime * 0.34) * 0.1 : 0
  })
