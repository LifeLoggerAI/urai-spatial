...
export default function LoginClient({ mode = 'signin' }: { mode?: AuthEntryMode }) {
  ...
  useEffect(() => {
    ...
    return onAuthStateChanged(auth, (nextUser) => {
      ...
      setMessage(nextUser ? 'Your private world is ready.' : mode === 'signup'
        ? 'Continue through the configured Firebase identity provider. A new URAI account is created only after provider consent.'
  ...
  return (
    <main data-route-owner="canonical-auth-entry" style={{minHeight:'100svh',display:'grid',placeItems:'center',padding:'max(28px,env(safe-area-inset-top)) 20px max(34px,env(safe-area-inset-bottom))',background:'radial-gradient(circle at 50% 24%,#15303a 0,#071119 38%,#020609 78%)',color:'#f6fafc',fontFamily:'var(--font-sans)'}}>
      <section style={{width:'min(520px,100%)',padding:'clamp(26px,6vw,46px)',border:'1px solid rgba(188,239,246,.15)',borderRadius:32,background:'rgba(5,14,20,.72)',boxShadow:'0 30px 100px rgba(0,0,0,.42)',backdropFilter:'blur(22px)'}}>
        ...
        <p style={{margin:'44px 0 0',fontSize:11,letterSpacing:'.22em',textTransform:'uppercase',color:'#88aeb7'}}>{mode === 'signup' ? 'Private account' : 'Private threshold'}</p>
        <h1 style={{margin:'10px 0 12px',fontSize:'clamp(42px,9vw,66px)',lineHeight:.94,letterSpacing:'-.055em'}}>{mode === 'signup' ? 'Create your world.' : 'Enter your world.'}</h1>
        ...
        {state === 'signed-in' && user ? (
          ...
          <div style={{marginTop:24}}><button type="button" disabled={state==='unavailable'||state==='working'} onClick={() => void enter()} style={buttonPrimary}>{state==='working'?'Opening provider...':mode === 'signup' ? 'Continue to create account' : 'Continue securely'}</button></div>
        )}
