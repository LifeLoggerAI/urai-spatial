...
function CameraRig({ active }: { active: GroundDestination | null }) {
  ...
  useFrame((_, delta) => {
    target.set(...(active?.camera ?? (size.width < 700 ? [0, 1.55, 4.2] : [0, 1.7, 5.8])))
    look.set(...(active?.lookAt ?? [0, 1.15, -8.8]))
