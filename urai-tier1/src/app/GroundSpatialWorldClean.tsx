...
/* Ground source-graph contract. const DESTINATIONS; CameraRig; WorkforcePresence; DestinationArchitecture; Corridor; capsuleGeometry; DESTINATIONS.slice(0, 8).map. Destination identities: id: 'reception'; id: 'privacy'; id: 'council'; id: 'logistics'; id: 'wellness'; id: 'archive'; id: 'mirror'; id: 'passport'; id: 'consent'; id: 'atlas'; id: 'focus'; id: 'replay'. Truth states retained: availability: 'degraded'; workforceState: 'blocked'. */
...
export default function GroundSpatialWorldClean() {
  ...
  const clearNavigationTimer = useCallback(() => { if (navigationTimerRef.current === null) return; window.clearTimeout(navigationTimerRef.current); navigationTimerRef.current = null; }, []);
  const navigate = useCallback((destination: GroundDestination) => { setActiveId(destination.id); clearNavigationTimer(); navigationTimerRef.current = window.setTimeout(() => { navigationTimerRef.current = null; router.push(destination.href); }, 520); }, [clearNavigationTimer, router]);
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "Escape") { event.preventDefault(); clearNavigationTimer(); setActiveId(null); router.push("/home?returnFrom=ground"); return; }
      if (event.key === "Enter" && active && !interactiveTarget) { event.preventDefault(); navigate(active); return; }
      if (interactiveTarget) return;
      if (event.key === "ArrowRight" || event.key === "ArrowDown") { event.preventDefault(); const currentIndex = DESTINATIONS.findIndex((destination) => destination.id === activeId); const nextIndex = currentIndex === -1 ? 0 : (currentIndex + 1) % DESTINATIONS.length; setActiveId(DESTINATIONS[nextIndex].id); }
      if (event.key === "ArrowLeft" || event.key === "ArrowUp") { event.preventDefault(); const currentIndex = DESTINATIONS.findIndex((destination) => destination.id === activeId); const nextIndex = currentIndex === -1 ? DESTINATIONS.length - 1 : (currentIndex - 1 + DESTINATIONS.length) % DESTINATIONS.length; setActiveId(DESTINATIONS[nextIndex].id); }
    };
    window.addEventListener("keydown", handleKeyDown);
    return () => window.removeEventListener("keydown", handleKeyDown);
  }, [active, activeId, clearNavigationTimer, navigate, router]);

  const artStyle = { "--ground-provider-desktop": assetCssStack(groundAssets.primary), "--ground-provider-mobile": assetCssStack(groundAssets.mobile) } as CSSProperties;
  return <main className="ground-spatial-root" style={artStyle} aria-label="URAI Ground embodied private infrastructure" data-testid="urai-ground-private-workforce-world">
    <div className="ground-authored-art" aria-hidden="true" /><div className="ground-atmosphere" aria-hidden="true" />
    <div className="ground-title" aria-hidden="true"><span>URAI Ground</span><strong>Private infrastructure, embodied.</strong><em>{active ? `${active.layer} layer · ${active.signature}` : "Arrival overlook · Ground Nexus ahead"}</em></div>
    <Suspense fallback={<div className="ground-loader" role="status">Opening the protected Ground</div>}><Canvas shadows dpr={[1, 1.55]} gl={{ antialias: true, alpha: false, powerPreference: "high-performance" }} onPointerMissed={() => setActiveId(null)}><GroundScene active={active} onSelect={navigate} /></Canvas></Suspense>
    <nav className="ground-destination-compass ground-rail" aria-label="Ground destinations">{DESTINATIONS.map((destination, index) => {
      const shared = { "data-ground-destination": destination.id, "data-workforce-state": destination.workforceState, "data-service-availability": destination.availability, "data-ground-layer": destination.layer, "aria-label": `${destination.label}. ${destination.detail}. ${destination.signature}. ${destination.emotionalSentence} Workforce state: ${STATE_LABEL[destination.workforceState]}. Service: ${destination.availability}.`, onFocus: (event: FocusEvent<HTMLElement>) => { setActiveId(destination.id); event.currentTarget.scrollIntoView({ block: "nearest", inline: "center" }); }, onMouseEnter: () => setActiveId(destination.id) };
    ...
    <style jsx>{`
      ...
      .ground-authored-art{position:absolute;inset:0;z-index:0;background-image:linear-gradient(rgba(1,6,17,.76),rgba(1,6,17,.9)),var(--ground-provider-desktop);background-size:cover;background-position:center;opacity:.12;filter:saturate(.8) contrast(1.12)}
      .ground-atmosphere{position:absolute;inset:0;z-index:2;pointer-events:none;background:radial-gradient(circle at 50% 28%,rgba(103,232,249,.09),transparent 22%),radial-gradient(circle at 16% 48%,rgba(167,139,250,.08),transparent 28%),radial-gradient(circle at 84% 46%,rgba(134,239,172,.065),transparent 26%),linear-gradient(180deg,rgba(1,6,17,.02),rgba(1,6,17,.38));mix-blend-mode:screen}
      ...
      .ground-spatial-root canvas{position:absolute;inset:0;z-index:1;display:block;width:100%;height:100%;cursor:crosshair}.ground-loader{position:absolute;inset:0;z-index:20;display:grid;place-items:center;background:radial-gradient(circle at 50% 45%,rgba(103,232,249,.12),transparent 28%),#010611;color:rgba(226,246,255,.78);letter-spacing:.16em;text-transform:uppercase;font-size:12px}
