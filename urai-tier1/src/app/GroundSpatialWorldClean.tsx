  ...
  return (
    ...
    >
      ...
      <style jsx>{`
        ...
        @media(max-width:760px){.ground-brand{left:14px;top:14px;max-width:62vw}.ground-brand strong{font-size:10px}.ground-home-return{right:12px;top:12px}.ground-directory{left:0;right:0;bottom:max(14px,env(safe-area-inset-bottom));transform:none;max-width:none;padding-inline:max(68px,env(safe-area-inset-left)) max(68px,env(safe-area-inset-right));scroll-padding-inline-start:max(68px,env(safe-area-inset-left));scroll-padding-inline-end:max(68px,env(safe-area-inset-right));mask-image:linear-gradient(90deg,transparent,#000 54px,#000 calc(100% - 54px),transparent)}.ground-directory button{flex:0 0 48px}.ground-directory button:is(:hover,:focus-visible,[aria-current="location"]){flex-basis:auto}.ground-prompt{bottom:78px;max-width:calc(100vw - 28px);overflow:hidden;text-overflow:ellipsis}}
        @media(prefers-reduced-motion:reduce){.ground-directory button,.ground-directory strong{transition:none}}
      `}</style>
    </main>
  );
}
