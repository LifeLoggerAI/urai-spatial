...
function vaultSurface(width: number, rise: number, length: number) {
  ...
  for (let j = 0; j <= bays; j++) for (let i = 0; i <= columns; i++) {
    ...
    positions.push(Math.cos(a) * (width + ripple), Math.sin(a) * rise + .05, -z)
...
export default function GroundVaultArchitecture({ activeId, onSelect }: {
  ...
  useEffect(() => () => { shell.dispose(); floor.dispose(); arches.forEach(g => g.dispose()); maps.forEach(t => t.dispose()) }, [shell, floor, arches, maps])
