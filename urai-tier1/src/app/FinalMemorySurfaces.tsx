...
export function FinalFocusChamber() {
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#020713] text-white" data-testid="urai-final-focus-chamber" data-route-polish="selected-memory-camera-chamber" data-canon="camera-from-life-map-no-avatar-orb">
      ...
      <section className="relative z-10 grid min-h-screen items-center gap-8 px-5 pb-28 pt-20 lg:grid-cols-[minmax(0,1fr)_430px] lg:px-10">
        <div className="max-w-4xl">
          <p className="text-xs font-black uppercase tracking-[0.42em] text-cyan-200">Selected memory camera chamber</p>
          ...
          <p className="mt-6 max-w-2xl text-base font-semibold leading-8 text-slate-100/82 md:text-lg">
            Focus is not a separate avatar room. It is the Life Map camera pushed into one selected memory: image, signal, title, and one clear doorway into Replay.
          </p>
          <div className="mt-7 flex flex-wrap gap-3">
            <Link href="/replay?memoryId=quiet-reset&manifestId=replay-recovery-thread&from=focus-camera" className="rounded-full bg-cyan-100 px-5 py-3 text-sm font-black text-slate-950 no-underline shadow-2xl shadow-cyan-400/20">Camera into Replay</Link>
            <Link href="/life-map?unwind=focus" className="rounded-full border border-white/20 bg-slate-950/55 px-5 py-3 text-sm font-black text-white no-underline backdrop-blur-xl">Unwind to Life Map</Link>
          </div>
        </div>
        ...
        <aside className="rounded-[2rem] border border-cyan-100/15 bg-slate-950/62 p-5 shadow-2xl shadow-cyan-950/30 backdrop-blur-2xl">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.22em] text-cyan-200">Memory readout</p>
          <p className="mt-2 text-sm font-semibold leading-6 text-slate-100/78">One memory is selected from the Life Map. Replay is one camera move away. The avatar and orb remain anchored back on the ground.</p>
...
export function FinalReplayFilm() {
  const beats = ['Pressure', 'Ground anchor', 'Life Map camera', 'Focus memory', 'Replay thread', 'Mirror', 'Passport'] as const
  return (
    <main className="relative min-h-screen overflow-hidden bg-black text-white" data-testid="urai-final-replay-film" data-route-polish="cinematic-memory-camera-film" data-canon="camera-from-focus-no-avatar-orb">
      ...
      <section className="relative z-10 grid min-h-screen items-center gap-8 px-5 pb-28 pt-20 lg:grid-cols-[minmax(0,1fr)_420px] lg:px-10">
        <div className="max-w-5xl">
          <p className="text-xs font-black uppercase tracking-[0.42em] text-cyan-200">Cinematic memory camera film</p>
          <h1 className="mt-5 max-w-[11ch] text-[clamp(4.5rem,11vw,10rem)] font-black leading-[0.78] tracking-[-0.12em]">Replay the thread.</h1>
          <p className="mt-6 max-w-2xl text-base font-semibold leading-8 text-slate-100/82 md:text-lg">
            Replay is the camera moving deeper through the selected memory. The avatar and orb do not appear here; they remain on the ground while the memory thread opens.
          </p>
          <div className="mt-7 flex flex-wrap gap-3">
            ...
            <Link href="/focus?memoryId=quiet-reset&unwind=replay" className="rounded-full border border-white/20 bg-slate-950/55 px-5 py-3 text-sm font-black text-white no-underline backdrop-blur-xl">Unwind to Focus</Link>
          </div>
        </div>
