...
function EmbodiedSelf({ walkTarget, nearby }: Pick<SceneProps, 'walkTarget' | 'nearby'>) {
  ...
  return (
    <group position={AVATAR_POSITION} name="home-embodied-self" data-testid="urai-home-embodied-avatar">
      <mesh position={[0, 2.9, 0]} onClick={activate}><sphereGeometry args={[0.25, 24, 24]} /><meshPhysicalMaterial color="#c9eef2" emissive="#9f91ff" emissiveIntensity={0.38} transparent opacity={0.36} transmission={0.5} depthWrite={false} /></mesh>
      <mesh position={[0, 1.62, 0]} onClick={activate}><capsuleGeometry args={[0.32, 1.55, 10, 20]} /><meshPhysicalMaterial color="#81cbd1" emissive="#7cecf2" emissiveIntensity={0.2} transparent opacity={0.23} transmission={0.62} depthWrite={false} /></mesh>
    </group>
  )
}

function Threshold({ kind, position, walkTarget, nearby, onTravel }: {
  kind: 'ground' | 'life-map'; position: THREE.Vector3
...
function PlayerCamera({ input, yaw, pitch, walkTarget, nearby, reducedMotion, shellRef, onNearbyChange }: Pick<SceneProps, 'input' | 'yaw' | 'pitch' | 'walkTarget' | 'nearby' | 'reducedMotion' | 'shellRef' | 'onNearbyChange'>) {
  ...
  useFrame((_, delta) => {
    const motion = stepEmbodiedMotion({
      position: position.current, velocity: velocity.current, input, target: walkTarget, yaw: yaw.current, delta,
      speed: reducedMotion ? 1.8 : 2.8, acceleration: reducedMotion ? 14 : 8, deceleration: reducedMotion ? 18 : 10,
      bounds: HOME_BOUNDS, obstacles: HOME_OBSTACLES,
    })
...
export default function EmbodiedHomeSpatialCanvas({ onOrbOpen, webglAvailable }: Props) {
  ...
  useEffect(() => { requestAnimationFrame(() => performance.mark('urai:first-home-spatial-frame')) }, [])
  ...
  const reset = useCallback(() => { walkTarget.current = HOME_SPAWN.clone(); yaw.current = 0; pitch.current = -0.04 }, [])
  ...
  const artStyle = { '--home-authored-desktop': assetCssStack(homeAssets.primary), '--home-authored-mobile': assetCssStack(homeAssets.mobile) } as CSSProperties
  const prompt = nearbyState === 'orb' ? 'Open the Orb companion' : nearbyState === 'avatar' ? 'Meet your embodied self' : nearbyState === 'ground' ? 'Descend into Ground' : nearbyState === 'life-map' ? 'Ascend into Life Map' : 'Walk the sanctuary'

  return <div ref={shellRef} className="urai-home-embodied-shell" style={artStyle} data-home-spatial-renderer="webgl" data-home-movement="walk-keyboard-click-touch" data-home-pointer-lock="false" data-home-visible-world="sanctuary-geometry-memory-vignettes" data-home-camera-mode={dragging ? 'look' : 'embodied'} aria-label="Walkable URAI personal sanctuary" {...look}>
    <div className="urai-home-embodied-art" aria-hidden="true" />
    <Canvas className="urai-home-spatial-canvas urai-home-embodied-canvas" style={{ position: 'absolute', inset: 0 }} dpr={[1, 1.5]} camera={{ position: [0, 1.68, 7.4], fov: 56, near: 0.08, far: 150 }} gl={{ antialias: true, alpha: true, powerPreference: 'high-performance' }}>
      <HomeScene input={input} yaw={yaw} pitch={pitch} walkTarget={walkTarget} nearby={nearby} reducedMotion={reducedMotion} shellRef={shellRef} onNearbyChange={setNearbyState} onOrbOpen={onOrbOpen} onTravel={travel} />
    </Canvas>
    ...
    <MovementHelp realm="Home" summary="Walk through a real sanctuary of paths, architecture, memory worlds, the Orb, Ground doorway, and Life Map threshold." controls="WASD or arrows move. Click ground or a memory world to approach. Drag to look. Enter interacts. R resets." />
    <MobileMovementPad input={input} label="Home movement controls" />
    <nav className="urai-home-direct-controls" data-movement-ui="true" aria-label="Direct Home destinations"><button type="button" aria-label="Open Orb directly" onClick={onOrbOpen}>Orb</button><button type="button" aria-label="Open Ground directly" onClick={() => travel('infrastructure-hub')}>Ground</button><button type="button" aria-label="Open Life Map directly" onClick={() => travel('life-map')}>Life Map</button></nav>
    <style jsx>{`
