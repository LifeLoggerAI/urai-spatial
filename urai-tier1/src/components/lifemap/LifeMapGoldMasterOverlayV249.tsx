...
function stoneGeometry(seed: number) {
  ...
  for (let index = 0; index < position.count; index += 1) {
    ...
    const grain = 1
      + .075 * Math.sin(direction.x * 5.7 + direction.z * 4.1 + seed * .013)
      + .035 * Math.sin(direction.y * 11.2 - direction.x * 7.4 + seed * .021)
...
function tube(points: Point3[], radius: number) {
  return new THREE.TubeGeometry(new THREE.CatmullRomCurve3(points.map(point => new THREE.Vector3(...point))), 24, radius, 7, false)
}
...
function semanticFamilyParts(node: LifeMapNode, seed: number, active: boolean): FamilyPart[] {
  ...
  if (node.type === 'memory') {
    return [{ geometry: memoryBody(seed, node.aura, active), position: [0, 0, 0], rotation: [0, -.16, 0], scale: [.82, .95, .72] }]
  }
  if (node.type === 'season') {
    const arc = Array.from({ length: 9 }, (_, index) => {
      const t = index / 8, angle = THREE.MathUtils.lerp(-1.25, 1.05, t)
      return [Math.sin(angle) * .72, -.18 + Math.sin(t * Math.PI) * .42, Math.cos(angle) * .52 - .20] as Point3
    })
    return [
      { geometry: tube(arc, .055), position: [0, 0, 0], rotation: [0, 0, 0], scale: [1, 1, 1] },
      { geometry: stone(), position: [-.46, -.34, .05], rotation: [0, .4, -.08], scale: [.34, .24, .48] },
    ]
  }
  if (node.type === 'ritual') {
    return [
  ...
  if (node.type === 'recovery') {
    const branches = [-1, 0, 1].map((side, branch) => ({
      geometry: tube([[0, -.34, 0], [side * .10, -.06, -.03], [side * (.24 + branch * .025), .20, -.10 - branch * .05], [side * (.34 + branch * .035), .42 + branch * .05, -.16 - branch * .08]], .032 - branch * .004),
  ...
  const spiralPoints = Array.from({ length: 15 }, (_, index) => {
    const t = index / 14, angle = t * Math.PI * 3.15, radius = .12 + t * .48
    return [Math.cos(angle) * radius, -.32 + t * .62, Math.sin(angle) * radius * .58] as Point3
  })
  return [
    { geometry: tube(spiralPoints, .038), position: [0, 0, 0], rotation: [0, 0, 0], scale: [1, 1, 1] },
    { geometry: stone(), position: [0, -.38, .02], rotation: [0, -.2, 0], scale: [.64, .20, .62] },
  ]
}
...
function motesGeometry(seed: number, active: boolean) {
  ...
  for (let index = 0; index < count; index += 1) {
    const angle = index * 2.39996323 + seeded(seed, index) * .5, radius = .24 + Math.sqrt((index + .5) / count) * (active ? .68 : .42)
    positions.set([Math.cos(angle) * radius, -.08 + seeded(seed + index, 17) * (active ? .70 : .42), Math.sin(angle) * radius * .62], index * 3)
  }
...
function authoredTerrainGeometry() {
  ...
  for (let row = 0; row <= rows; row += 1) {
    ...
    for (let column = 0; column <= columns; column += 1) {
      ...
      const age = THREE.MathUtils.clamp((-z + 4) / 45, 0, 1)
      const exposed = THREE.MathUtils.clamp((y + 4.4) / 2.4, 0, 1)
      const stain = .5 + .5 * Math.sin(x * .43 - z * .31) * Math.cos(x * .79 + z * .17)
      const color = shadow.clone().lerp(earth, .34 + exposed * .40).lerp(moss, .06 + stain * .14).lerp(history, age * .10)
  ...
  for (let row = 0; row < rows; row += 1) for (let column = 0; column < columns; column += 1) {
    ...
    if ((row + column) % 2) indices.push(a, c, b, b, c, d)
...
function trailGeometry() {
  ...
  for (let row = 0; row <= rows; row += 1) {
    ...
    for (const side of [-1, 1] as const) {
      const x = center + side * width, y = lifeMapTerrainHeight(x, z) + .07
      positions.push(x, y, z)
      const color = earth.clone().lerp(worn, .42 + .20 * (1 - t)); colors.push(color.r, color.g, color.b)
    }
  }
...
function MemoryPlace({ node, index, active, reducedMotion, onSelect, arrival }: { node: LifeMapNode; index: number; active: boolean; reducedMotion: boolean; onSelect: (node: LifeMapNode) => void; arrival: boolean }) {
  ...
  const parts = useMemo(() => semanticFamilyParts(node, seed, active), [active, node, seed])
  const motes = useMemo(() => motesGeometry(seed, active), [active, seed])
  ...
  return <group position={point} name={`life-map-v255-memory-place-${node.id}`} userData={{ artRevision: 'v255-semantic-memory-families', semanticFamily: node.type, semanticNode: node.id }} onClick={activate}>
    <group ref={root} position={[0, lift, 0]} scale={scale}>
      {parts.map((part, partIndex) => <mesh key={partIndex} geometry={part.geometry} position={part.position} rotation={part.rotation} scale={part.scale} castShadow receiveShadow>
        <meshStandardMaterial vertexColors={node.type === 'memory'} color={node.type === 'memory' ? '#ffffff' : FAMILY_BASE[node.type]} emissive={node.aura} emissiveIntensity={active ? .065 : .028} roughness={.86} metalness={0} transparent={(part.opacity ?? 1) < 1} opacity={part.opacity ?? 1} />
      </mesh>)}
      ...
      <pointLight position={[0, .32, .24]} color={node.aura} intensity={active ? .24 : .08} distance={active ? 3.6 : 2.0} decay={2} />
    </group>
  </group>
}
...
function TerritoryMarker({ node, index }: { node: LifeMapNode; index: number }) {
  ...
  const geometry = useMemo(() => stoneGeometry(nodeSeed(node, index) + 701), [index, node])
  useEffect(() => () => geometry.dispose(), [geometry])
  const variation = .86 + seeded(nodeSeed(node, index), 703) * .32
  return <mesh geometry={geometry} position={[point[0], point[1] - .61, point[2] - .18]} scale={[1.55 * variation, .20, 2.05 * variation]} rotation={[0, seeded(nodeSeed(node, index), 704) * Math.PI, 0]} raycast={() => null} receiveShadow name={`life-map-v255-chapter-landmark-${node.eraId || node.id}`} userData={{ visualOnly: true, interactionOwner: false, chapterLandmark: true }}>
    <meshStandardMaterial color="#3f4d47" emissive={node.aura} emissiveIntensity={.018} roughness={.98} metalness={0} />
  </mesh>
}

function SelectedSanctuary({ node, index, reducedMotion }: { node: LifeMapNode; index: number; reducedMotion: boolean }) {
  const point = useMemo<Point3>(() => lifeMapLocalPoint(node, index), [index, node])
  const branches = useMemo(() => Array.from({ length: 3 }, (_, branch) => {
    const side = branch % 2 ? -1 : 1, offset = Math.floor(branch / 2)
    return tube([
      [side * (1.48 + offset * .16), -.40, -1.12 - offset * .18],
      [side * (1.10 + offset * .10), -.18, -.72],
      [side * (.72 + offset * .06), .12, -.24],
      [side * (.46 + offset * .04), .44, .08],
    ], .010 + (branch % 2) * .0015)
  }), [])
  useEffect(() => () => branches.forEach(geometry => geometry.dispose()), [branches])
  return <group position={[point[0], point[1] - .02, point[2]]} name="life-map-v255-contextual-memory-sanctuary" userData={{ visualOnly: true, interactionOwner: false, scaleMode: 'intimate-with-context', arrivalMeaning: 'inside-history-not-node-zoom' }}>
    ...
    <pointLight position={[-.76, .52, .54]} color={node.aura} intensity={.17} distance={4.0} decay={2} />
    <pointLight position={[.92, .34, -.58]} color="#ddb184" intensity={.07} distance={3.2} decay={2} />
  </group>
}
...
export function LifeMapGoldMasterOverlay({ nodes, selected, phase, reducedMotion, onSelect }: Props) {
  ...
  const territories = useMemo(() => {
    const seen = new Set<string>(), result: { node: LifeMapNode; index: number }[] = []
    nodes.forEach((node, index) => { const key = node.eraId || node.id; if (!seen.has(key)) { seen.add(key); result.push({ node, index }) } })
    return result
  }, [nodes])
  return <><RetireRejectedLifeMapVisuals /><group name="life-map-v249-personal-universe-geography" scale={stage.scale} position={stage.position} userData={{ artRevision: 'v255-authored-personal-universe', visualRepair: 'semantic-families-authored-geology-deliberate-portrait-composition' }}>
    <mesh geometry={terrain} receiveShadow castShadow name="life-map-v255-authored-memory-terrain" userData={{ visualAuthority: 'authored-chapter-geography', topology: 'continuous-explorable-world' }}>
      <meshStandardMaterial vertexColors color="#9aa397" emissive="#14211e" emissiveIntensity={.10} roughness={.97} metalness={0} side={THREE.DoubleSide} />
    </mesh>
    <mesh geometry={trail} receiveShadow name="life-map-v255-worn-lineage-footpath"><meshStandardMaterial vertexColors color="#b6a489" emissive="#4e4033" emissiveIntensity={.045} roughness={.96} /></mesh>
    <group name="life-map-v255-authored-chapter-territories" userData={{ visualOnly: true, interactionOwner: false }}>{territories.map(({ node, index }) => <TerritoryMarker key={node.eraId || node.id} node={node} index={index} />)}</group>
