...
function rootTubes(seed:number,active:boolean){
  return Array.from({length:active?5:4},(_,index)=>{
    const angle=(Math.PI*2*index)/(active?5:4)+seeded(seed,60+index)*.34,length=(active?1.10:.72)+seeded(seed,70+index)*(active?.34:.24)
    ...
    return new THREE.TubeGeometry(new THREE.CatmullRomCurve3(points),24,active?.017:.011,6,false)
  })
}

function motesGeometry(seed:number,active:boolean){
  const count=active?58:28,positions=new Float32Array(count*3)
  for(let index=0;index<count;index++){
    const angle=index*2.39996323+seeded(seed,index)*.5,radius=.38+Math.sqrt((index+.5)/count)*(active?1.08:.78)
    positions.set([Math.cos(angle)*radius,-.18+seeded(seed+index,17)*(active?1.30:.82),Math.sin(angle)*radius*.70],index*3)
  }
...
export function LifeMapGoldMasterOverlay({nodes,selected,phase,reducedMotion,onSelect}:Props){
  ...
  return <><RetireRejectedLifeMapVisuals/><group name="life-map-v249-personal-universe-geography" scale={stage.scale} position={stage.position} userData={{artRevision:'v256-contextual-personal-universe',visualRepair:'selected-memory-remains-readable-without-cage-while-surrounding-geography-stays-visible'}}>
