...
function nodeType(value: unknown): LifeMapNodeType {
  return typeof value === "string" && nodeTypes.includes(value as LifeMapNodeType) ? (value as LifeMapNodeType) : "memory";
}

function sourceType(value: unknown): LifeMapEventSourceType {
...
function eraType(value: unknown): LifeMapEraType {
...
function position3(value: unknown): [number, number, number] | undefined {
...
function normalizeEvent(id: string, data: DocumentData): LifeMapEvent {
  return {
    ...
    position: position3(data.position),
    ...
    tags: Array.isArray(data.tags) ? stringArray(data.tags) : undefined,
  };
}
...
function normalizeEra(id: string, data: DocumentData): LifeMapEra {
  return {
    ...
    nodeIds: stringArray(data.nodeIds),
  };
}
