...
function LivingGalaxyField({ tier, reducedMotion, selected }: { tier: "low" | "medium" | "high"; reducedMotion: boolean; selected: boolean }) {
  ...
  const geometry = useMemo(() => {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      const z = (seeded(i + 173, 21.19) - .5) * (2.6 + radius * .055)
...
function CosmicWorld({ nodes, selected, selectedIndex, phase, reducedMotion, tier, onSelect, onWebGLState, memoryStarReview }: { nodes: LifeMapNode[]; selected: LifeMapNode | null; selectedIndex: number; phase: Phase; reducedMotion: boolean; tier: "low" | "medium" | "high"; onSelect: (node: LifeMapNode) => void; onWebGLState: (state: WebGLState) => void; memoryStarReview?: string }) {
  ...
  if (reviewNode && (memoryStarReview === "isolated" || memoryStarReview === "hover" || memoryStarReview === "near-cluster")) {
    ...
    return <><color attach="background" args={["#020611"]} /><ambientLight intensity={.055} color="#d9edff" /><MemoryStarReviewCamera clustered={memoryStarReview === "near-cluster"} /><WebGLRecovery onState={onWebGLState} /><group name="memory-star-reference-review" userData={{ explicitDemoReview: true, reviewState: memoryStarReview }}>
