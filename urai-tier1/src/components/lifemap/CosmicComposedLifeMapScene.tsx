...
function cosmicPoint(node: LifeMapNode): Point3 {
...
function StellarDepthField({ count, reducedMotion, salt, size, opacity, depthSpan, zOffset = -8 }: { count: number; reducedMotion: boolean; salt: number; size: number; opacity: number; depthSpan: number; zOffset?: number }) {
  ...
  const geometry = useMemo(() => {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      const z = zOffset - Math.pow(seeded(i, 7 + salt), .76) * depthSpan + Math.sin(angle) * radius * .16;
...
function LivingGalaxyField({ tier, reducedMotion, selected }: { tier: "low" | "medium" | "high"; reducedMotion: boolean; selected: boolean }) {
  const root = useRef<THREE.Points>(null);
  const disc = useMemo(() => makeDiscTexture(2.55), []);
  const core = useMemo(() => makeDiscTexture(4.8, true), []);
  const count = tier === "low" ? 3600 : tier === "medium" ? 5600 : 8200;
  const geometry = useMemo(() => {
    const positions = new Float32Array(count * 3);
    const colors = new Float32Array(count * 3);
    const pearl = new THREE.Color("#fff8e9");
    const warm = new THREE.Color("#ffc47f");
    const cool = new THREE.Color("#a8ddff");
    const violet = new THREE.Color("#c8a9ff");
    for (let i = 0; i < count; i += 1) {
      const radius = .28 + Math.pow(seeded(i + 173, 18.11), .72) * 27.5;
      const arm = i % 6;
      const angle = arm * Math.PI / 3 + radius * .205 + (seeded(i + 173, 18.77) - .5) * (.28 + radius * .010);
      const armWidth = .42 + radius * .052;
      const cross = (seeded(i + 173, 19.41) - .5) * armWidth;
      const x = Math.cos(angle) * radius * 1.05 + Math.cos(angle + Math.PI / 2) * cross;
      const y = Math.sin(angle) * radius * .58 + Math.sin(angle + Math.PI / 2) * cross * .72
        + (seeded(i + 173, 20.03) - .5) * (1.1 + radius * .035);
      const z = (seeded(i + 173, 21.19) - .5) * (8.0 + radius * .72)
        + Math.sin(angle * 2.0) * 1.15
        + Math.sin(angle) * radius * .18;
      positions.set([x, y, z], i * 3);
      const coreWeight = Math.exp(-radius / 7.0);
      const armWarmth = .18 + .34 * (.5 + .5 * Math.sin(angle * 1.15));
      const base = i % 31 === 0 ? warm : i % 17 === 0 ? violet : i % 9 === 0 ? pearl : cool;
      const color = base.clone().lerp(warm, coreWeight * .68 + armWarmth * .14).multiplyScalar(.62 + seeded(i + 173, 22.73) * .52);
      colors.set([color.r, color.g, color.b], i * 3);
    }
    const result = new THREE.BufferGeometry();
    result.setAttribute("position", new THREE.BufferAttribute(positions, 3));
  ...
  useEffect(() => () => { geometry.dispose(); disc.dispose(); core.dispose(); }, [core, disc, geometry]);
  useFrame(({ clock }) => {
    if (!root.current || reducedMotion) return;
    root.current.rotation.z = Math.sin(clock.elapsedTime * .007) * .010;
    root.current.rotation.y = Math.sin(clock.elapsedTime * .005) * .010;
  });
  return <group
    name="life-map-v290-living-galaxy-overview"
    position={[0, .7, -31]}
    rotation={[-.08, -.035, -.12]}
    scale={[1.12, 1.08, 1.18]}
    userData={{ visualRole: "recognizable-layered-living-galaxy-volume", antiReference: "no-flat-star-wall" }}
  >
    <sprite position={[0, 0, -.7]} scale={[12.5, 8.1, 1]} raycast={() => null} name="life-map-v290-galactic-core-haze">
      <spriteMaterial map={core} color="#f0ba82" transparent opacity={selected ? .10 : .24} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite position={[0, 0, -.3]} scale={[4.6, 3.4, 1]} raycast={() => null} name="life-map-v290-galactic-core">
      <spriteMaterial map={core} color="#fff0d4" transparent opacity={selected ? .24 : .72} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <points ref={root} name="life-map-v290-spiral-arm-memory-field" geometry={geometry} raycast={() => null} userData={{ visualRole: "face-readable-spiral-memory-galaxy", antiReference: "no-glass-shards-no-flat-star-wall" }}>
      <pointsMaterial map={disc} alphaTest={.008} vertexColors size={tier === "low" ? .25 : .31} transparent opacity={selected ? .42 : .96} depthWrite={false} blending={THREE.AdditiveBlending} sizeAttenuation />
    </points>
  </group>;
}

const NEBULA_VERTEX = `varying vec2 vUv;void main(){vUv=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}`;
// Elliptical falloff alone still has opacity at the horizontal quad edges.
// Fade all four UV boundaries so layered veils cannot expose rectangular seams.
const NEBULA_FRAGMENT = `uniform vec3 colorA;uniform vec3 colorB;uniform float seed;uniform float opacity;varying vec2 vUv;float hash21(vec2 p){p=fract(p*vec2(123.34,345.45));p+=dot(p,p+34.345);return fract(p.x*p.y);}float noise(vec2 p){vec2 i=floor(p),f=fract(p);f=f*f*(3.-2.*f);return mix(mix(hash21(i),hash21(i+vec2(1,0)),f.x),mix(hash21(i+vec2(0,1)),hash21(i+vec2(1)),f.x),f.y);}float fbm(vec2 p){float n=0.,a=.5;for(int i=0;i<5;i++){n+=noise(p)*a;p=p*2.03+vec2(7.1,3.7);a*=.5;}return n;}void main(){vec2 p=(vUv-.5)*2.;float radial=1.-smoothstep(.03,1.12,length(p*vec2(.70,1.05)));float edgeDistance=min(min(vUv.x,1.-vUv.x),min(vUv.y,1.-vUv.y));float edgeFade=smoothstep(0.,.16,edgeDistance);float cloud=fbm(p*1.48+seed)+.68*fbm(p*3.25-seed)+.24*fbm(p*7.2+seed*.7);float filaments=smoothstep(.24,1.08,cloud);float a=radial*edgeFade*filaments*opacity;vec3 c=mix(colorA,colorB,smoothstep(.20,.96,cloud));gl_FragColor=vec4(c,a);}`;
...
function SelectedMemoryFormation({ point, aura, phase, index }: { point: Point3; aura: string; phase: Phase; index: number }) {
  ...
  const scale = phase === "arrival" ? 1.58 : phase === "approach" ? 1.42 : phase === "travel" ? 1.24 : 1.06;
  ...
  return <group name="life-map-v287-selected-memory-stellar-weather" userData={{ visualRole: "selected-memory-star-with-volumetric-weather", visualAcceptance: "v287-selected-memory-star-no-glass-shards", interactionOwner: false, pointWallpaper: false, journeyPhase: phase }} raycast={() => null}>
    <sprite position={point} scale={[scale * 1.48, scale * 1.20, 1]} raycast={() => null} name="life-map-selected-memory-corona">
      <spriteMaterial map={halo} color={aura} transparent opacity={phase === "arrival" ? .15 : .11} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite position={point} scale={[scale * .72, scale * .66, 1]} raycast={() => null} name="life-map-selected-memory-inner-corona">
      <spriteMaterial map={halo} color={palette[1]} transparent opacity={phase === "arrival" ? .22 : .16} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite position={point} scale={[scale * .31, scale * .31, 1]} raycast={() => null} name="life-map-selected-memory-photosphere">
      <spriteMaterial map={photosphere} color="#fff8e9" transparent opacity={.78} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <CelestialTerritoryStars position={point} palette={palette} index={120 + index} spread={scale * 2.05} strength={0.84} />
    <NebulaVeil position={[point[0] - scale * .18, point[1] + scale * .08, point[2] - 1.4]} scale={[scale * 3.4, scale * 2.25]} rotation={-.18} colors={palette} opacity={phase === "arrival" ? .11 : .075} seed={17.13 + index * .01} />
    <NebulaVeil position={[point[0] + scale * .34, point[1] - scale * .12, point[2] - 5.6]} scale={[scale * 2.9, scale * 1.85]} rotation={.27} colors={[palette[1], palette[0]]} opacity={phase === "arrival" ? .085 : .06} seed={21.47 + index * .01} />
    <pointLight position={[point[0] - .8, point[1] + .9, point[2] + 1.2]} color={aura} intensity={phase === "arrival" ? .82 : .46} distance={11} decay={2} />
...
function OverviewRegions({ nodes, phase }: { nodes: LifeMapNode[]; phase: Phase }) {
  ...
  const regions = useMemo(() => {
    ...
    nodes.forEach((node, index) => {
      ...
      existing.points.push(cosmicPoint(node));
  ...
  return <group name="life-map-overview-personal-universe-regions" userData={{ visualRole: "data-derived-personal-universe-geography", portraitAuthority: portrait ? "v285-vertical-territory-composition" : "desktop-v284-archipelago-composition", pointWallpaper: false, graphEdges: false }}>
    {regions.map((region, index) => <group key={region.key} name={`life-map-overview-region-${index}`} userData={{ regionKey: region.key, visualRole: "personal-universe-region" }}>
      <NebulaVeil position={region.center} scale={[region.spread * 1.45, region.spread * .82]} rotation={(index % 2 ? 1 : -1) * (.12 + index * .025)} colors={region.palette} opacity={portrait ? .29 : .27} seed={region.seed} />
      <NebulaVeil position={region.secondary} scale={[region.spread * 1.05, region.spread * .62]} rotation={(index % 2 ? -1 : 1) * (.18 + index * .02)} colors={[region.palette[1], region.palette[0]]} opacity={portrait ? .19 : .17} seed={region.seed + .37} />
      <NebulaVeil position={region.tertiary} scale={[region.spread * .72, region.spread * .44]} rotation={(index % 2 ? 1 : -1) * (.31 + index * .017)} colors={region.palette} opacity={portrait ? .12 : .105} seed={region.seed + .71} />
...
function SelectedTravelWeather({ node, index, phase }: { node: LifeMapNode; index: number; phase: Phase }) {
  ...
  const point = cosmicPoint(node);
  const strength = phase === "departure" ? .34 : phase === "travel" ? .32 : phase === "approach" ? .28 : .22;
...
function MemoryStar({ node, index, active, related, overview, onSelect, positionOverride, forceHover = false }: { node: LifeMapNode; index: number; active: boolean; related: boolean; overview: boolean; onSelect: (node: LifeMapNode) => void; positionOverride?: Point3; forceHover?: boolean }) {
  const point = useMemo(() => positionOverride ?? cosmicPoint(node), [index, node, positionOverride]);
  ...
  const hoverBoost = hoverEmphasis ? 1.16 : 1;
  const outer = (active ? 1.08 : related ? .82 : .74) * overviewBoost * approachBoost * hoverBoost;
  const mid = (active ? .52 : related ? .40 : .36) * overviewBoost * (active && !overview ? 3.3 : 1) * hoverBoost;
  const hot = (active ? .20 : related ? .17 : .16) * (overview ? 1.24 : 1) * (active && !overview ? 2.4 : 1) * hoverBoost;
  return <group name={`life-map-memory-star-${node.id}`} position={point} userData={{ semanticType: node.type, visualAuthority: "stellar-memory-not-node-graph", stellarMorphology: "point-photosphere-layered-corona-no-visible-sphere", overviewEmphasis: overview, hoverEmphasis }} onClick={(event) => { event.stopPropagation(); onSelect(node); }} onPointerOver={(event) => pointer(event, true)} onPointerOut={(event) => pointer(event, false)}>
...
function CameraRig({ selected, selectedIndex, phase, reducedMotion }: { selected: LifeMapNode | null; selectedIndex: number; phase: Phase; reducedMotion: boolean }) { const { camera, pointer, size } = useThree(), look = useRef(new THREE.Vector3()), initialized = useRef(false); const goal = useCallback((): Goal => { const portrait = size.height > size.width; const overview = new THREE.Vector3(0, portrait ? .65 : 1.55, portrait ? 18.5 : 19.5), targetOverview = new THREE.Vector3(0, portrait ? -.45 : .10, portrait ? -26 : -25); if (!selected || phase === "overview") return { position: overview.toArray() as Point3, target: targetOverview.toArray() as Point3, fov: portrait ? 42 : 44 }; const target = new THREE.Vector3(...cosmicPoint(selected)), travelOrigin = new THREE.Vector3(0, portrait ? 1.2 : 2.7, portrait ? 29 : 25.5), dir = travelOrigin.clone().sub(target).normalize(), side = new THREE.Vector3(dir.z, 0, -dir.x).normalize(), sign = Math.sign(target.x) || 1; const distance = phase === "departure" ? 21 : phase === "travel" ? 16.5 : phase === "approach" ? 11.2 : portrait ? 8.6 : 7.2; const position = target.clone().addScaledVector(dir, distance).addScaledVector(side, phase === "travel" ? sign * 3.4 : phase === "approach" ? sign * 1.1 : 0); position.y += phase === "travel" ? 2.2 : phase === "approach" ? .8 : .25; return { position: position.toArray() as Point3, target: target.toArray() as Point3, fov: portrait ? phase === "arrival" ? 50 : 56 : phase === "arrival" ? 40 : 49 }; }, [phase, selected, selectedIndex, size.height, size.width]); useLayoutEffect(() => { if (initialized.current) return; const next = goal(); camera.position.set(...next.position); look.current.set(...next.target); camera.lookAt(look.current); if (camera instanceof THREE.PerspectiveCamera) { camera.fov = next.fov; camera.updateProjectionMatrix(); } initialized.current = true; }, [camera, goal]); useFrame((_, delta) => { const next = goal(), position = new THREE.Vector3(...next.position), target = new THREE.Vector3(...next.target); if (!selected && phase === "overview" && !reducedMotion) { position.x += pointer.x * 2.15; position.y += pointer.y * .82; target.x += pointer.x * .84; target.y += pointer.y * .34; } if (reducedMotion) { camera.position.copy(position); look.current.copy(target); } else { const rate = phase === "overview" ? 2.2 : phase === "arrival" ? 4.4 : 2.65; camera.position.lerp(position, 1 - Math.exp(-rate * delta)); look.current.lerp(target, 1 - Math.exp(-4.2 * delta)); } camera.lookAt(look.current); if (camera instanceof THREE.PerspectiveCamera) { camera.fov = reducedMotion ? next.fov : THREE.MathUtils.damp(camera.fov, next.fov, 4, delta); camera.updateProjectionMatrix(); } }); return null; }

function MemoryStarReviewCamera({ clustered = false }: { clustered?: boolean }) {
  const { camera } = useThree();
  useLayoutEffect(() => {
    camera.position.set(0, clustered ? 1.0 : 0.25, clustered ? 8.5 : 4.8);
    camera.lookAt(0, clustered ? 0.45 : 0, -12);
    if (camera instanceof THREE.PerspectiveCamera) {
      camera.fov = clustered ? 46 : 40;
      camera.updateProjectionMatrix();
    }
  }, [camera, clustered]);
  return null;
...
function CosmicWorld({ nodes, selected, selectedIndex, phase, reducedMotion, tier, onSelect, onWebGLState, memoryStarReview }: { nodes: LifeMapNode[]; selected: LifeMapNode | null; selectedIndex: number; phase: Phase; reducedMotion: boolean; tier: "low" | "medium" | "high"; onSelect: (node: LifeMapNode) => void; onWebGLState: (state: WebGLState) => void; memoryStarReview?: string }) {
  const reviewNode = selected ?? nodes[0] ?? null;
  ...
  const selectedPoint = selected ? cosmicPoint(selected) : null;
  ...
  const overviewWeather = phase === "overview" ? .32 : .055;
  const overview = phase === "overview";
  return <><color attach="background" args={["#020611"]} /><fog attach="fog" args={["#020611", 78, 170]} /><ambientLight intensity={.045} color="#d9edff" /><RenderProof /><WebGLRecovery onState={onWebGLState} /><CameraRig selected={selected} selectedIndex={selectedIndex} phase={phase} reducedMotion={reducedMotion} /><group name="life-map-deep-space"><Stars radius={152} depth={138} count={Math.max(180, Math.round(starCount * .24))} factor={.68} saturation={.30} fade speed={0} /><StellarDepthField count={Math.max(90, Math.round(nearCount * .72))} reducedMotion={reducedMotion} salt={.37} size={.62} opacity={.94} depthSpan={30} zOffset={-2} /><StellarDepthField count={midCount} reducedMotion={reducedMotion} salt={4.13} size={.18} opacity={.62} depthSpan={58} zOffset={-24} /><StellarDepthField count={farCount} reducedMotion={reducedMotion} salt={7.19} size={.065} opacity={.32} depthSpan={88} zOffset={-58} /><LivingGalaxyField tier={tier} reducedMotion={reducedMotion} selected={Boolean(selected)} /><group name="life-map-emotional-weather"><NebulaVeil position={[-12, 5, -20]} scale={[54, 31]} rotation={-.21} colors={["#17384a", "#493c42"]} opacity={.40 * overviewWeather} seed={.09} /><NebulaVeil position={[13, -6, -29]} scale={[56, 33]} rotation={.16} colors={["#123443", "#4b403d"]} opacity={.36 * overviewWeather} seed={.51} /><NebulaVeil position={[-2, 0, -39]} scale={[92, 46]} rotation={-.10} colors={["#122838", "#43363f"]} opacity={.34 * overviewWeather} seed={.19} /><NebulaVeil position={[4, 3, -83]} scale={[120, 66]} rotation={-.04} colors={["#09172a", "#2d2735"]} opacity={.26 * overviewWeather} seed={.31} /><NebulaVeil position={[-25, 10, -49]} scale={[70, 36]} rotation={-.17} colors={["#123444", "#4a3f42"]} opacity={.28 * overviewWeather} seed={.73} /><NebulaVeil position={[28, -10, -58]} scale={[76, 38]} rotation={.20} colors={["#103443", "#574238"]} opacity={.25 * overviewWeather} seed={2.17} /></group><OverviewRegions nodes={nodes} phase={phase} /><PortraitOverviewDepth phase={phase} /></group><Constellations /><group name="life-map-memory-stars">{nodes.map((node, index) => { const active = selected?.id === node.id, related = Boolean(selected && (selected.connectedTo.includes(node.id) || node.connectedTo.includes(selected.id))); return <MemoryStar key={node.id} node={node} index={index} active={active} related={related} overview={overview} onSelect={onSelect} />; })}</group>{selectedPoint ? <group name="life-map-selected-local-depth" position={selectedPoint} scale={[1.28, 1.28, 1.28]}><StellarDepthField count={selectedLocalCount} reducedMotion={reducedMotion} salt={12.31} size={.22} opacity={.48} depthSpan={54} /></group> : null}{selected ? <SelectedTravelWeather node={selected} index={selectedIndex} phase={phase} /> : null}{selected ? <SelectedMemoryDust node={selected} index={selectedIndex} /> : null}</>;
}

export default function ComposedLifeMapScene() {
  const router = useRouter(), params = useSearchParams(), profile = useAdaptiveSpatialQuality(); const explicitDemo = params.get("demo") === "1", overviewRequested = params.get("overview") === "1"; const memoryStarReview = explicitDemo ? token(params.get("memoryStarReview")) : ""; const { nodes, loading, sourceMode } = useLifeMapEvents(explicitDemo ? "demo-user" : undefined); const queryNode = token(params.get("node") || params.get("memoryId")), manifestId = token(params.get("manifestId"), DEFAULT_MANIFEST_ID); const [selectedId, setSelectedId] = useState<string | null>(overviewRequested ? null : queryNode || null); const [phase, setPhase] = useState<Phase>(() => !overviewRequested && queryNode ? "arrival" : "overview"); const [webglState, setWebglState] = useState<WebGLState>("ready"), [software, setSoftware] = useState<boolean | null>(null); const journey = useRef(0);
  const selected = useMemo(() => nodes.find((node) => node.id === selectedId) || null, [nodes, selectedId]);
  const selectedIndex = Math.max(0, nodes.findIndex((node) => node.id === selected?.id)); const withIdentity = useCallback((next: URLSearchParams) => { lifeMapIdentitySearch(params).forEach((value, key) => next.set(key, value)); return next; }, [params]);
