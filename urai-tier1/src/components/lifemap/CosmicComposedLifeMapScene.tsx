...
// V281 literal-pixel polish: preserve V280's data-derived overview contract and
// V279 selected journey while strengthening semantic geography after exact-head review.
...
function PortraitOverviewDepth({ phase }: { phase: Phase }) {
  ...
  return <group name="life-map-portrait-overview-depth" userData={{ visualRole: "portrait-only-asymmetric-volumetric-depth", pointWallpaper: false }}>
    <NebulaVeil position={[-8.5, 10, -15]} scale={[74, 43]} rotation={-.29} colors={["#1c5268", "#65545f"]} opacity={.28} seed={8.17} />
    <NebulaVeil position={[12, 2, -24]} scale={[82, 50]} rotation={.24} colors={["#174c60", "#6d5a4e"]} opacity={.25} seed={8.83} />
    <NebulaVeil position={[-4, -11, -34]} scale={[92, 58]} rotation={-.12} colors={["#1a465d", "#574d66"]} opacity={.22} seed={9.41} />
    <NebulaVeil position={[10, 15, -48]} scale={[106, 64]} rotation={.09} colors={["#123d55", "#5d525b"]} opacity={.19} seed={10.07} />
    <NebulaVeil position={[-13, -5, -63]} scale={[118, 72]} rotation={-.18} colors={["#12364f", "#4d5362"]} opacity={.16} seed={10.91} />
  </group>;
}
...
function OverviewRegions({ nodes, phase }: { nodes: LifeMapNode[]; phase: Phase }) {
  const regions = useMemo(() => {
    ...
    return Array.from(buckets.values())
      ...
      .map((bucket, index) => {
        ...
        const side = (seeded(regionSeed, 2.9) - .5) * 9;
        const lift = (seeded(regionSeed, 5.1) - .5) * 6.4;
        const depth = 5 + seeded(regionSeed, 7.3) * 7;
        ...
        const tertiary: Point3 = [center[0] - side * .48, center[1] - lift * .36, center[2] + 2.4];
        const beacon: Point3 = [center[0], center[1], center[2] + 1.4];
  ...
  return <group name="life-map-overview-personal-universe-regions" userData={{ visualRole: "data-derived-personal-universe-geography", pointWallpaper: false, graphEdges: false }}>
    {regions.map((region, index) => <group key={region.key} name={`life-map-overview-region-${index}`} userData={{ regionKey: region.key, visualRole: "personal-universe-region" }}>
      <NebulaVeil position={region.center} scale={[region.spread, region.spread * .58]} rotation={(index % 2 ? 1 : -1) * (.12 + index * .025)} colors={region.palette} opacity={.36} seed={region.seed} />
      <NebulaVeil position={region.secondary} scale={[region.spread * .78, region.spread * .46]} rotation={(index % 2 ? -1 : 1) * (.18 + index * .02)} colors={[region.palette[1], region.palette[0]]} opacity={.22} seed={region.seed + .37} />
      <NebulaVeil position={region.tertiary} scale={[region.spread * .54, region.spread * .32]} rotation={(index % 2 ? 1 : -1) * (.31 + index * .017)} colors={region.palette} opacity={.14} seed={region.seed + .71} />
