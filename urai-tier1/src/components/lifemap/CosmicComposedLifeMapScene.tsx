...
function SelectedTravelWeather({ node, index, phase }: { node: LifeMapNode; index: number; phase: Phase }) {
  ...
  const strength = phase === "departure" ? .44 : phase === "travel" ? .52 : phase === "approach" ? .42 : .30;
  return <group name="life-map-selected-travel-weather"><NebulaVeil position={[point[0] - 6.6, point[1] + 3.4, point[2] - 8]} scale={[33, 19]} rotation={-.23} colors={["#123b54", "#50375c"]} opacity={strength} seed={3.11} /><NebulaVeil position={[point[0] + 7.8, point[1] - 4.1, point[2] - 12]} scale={[29, 17]} rotation={.18} colors={["#173653", "#27566a"]} opacity={strength * .88} seed={4.37} /><NebulaVeil position={[point[0] - 1.6, point[1] + .6, point[2] - 19]} scale={[46, 24]} rotation={-.07} colors={["#0a2742", "#49385b"]} opacity={strength * .80} seed={5.73} /></group>;
}
...
function CameraRig({ selected, selectedIndex, phase, reducedMotion }: { selected: LifeMapNode | null; selectedIndex: number; phase: Phase; reducedMotion: boolean }) { const { camera, pointer, size } = useThree(), look = useRef(new THREE.Vector3()), initialized = useRef(false); const goal = useCallback((): Goal => { const portrait = size.height > size.width; const overview = new THREE.Vector3(0, portrait ? 1.0 : 2.4, portrait ? 30.5 : 27), targetOverview = new THREE.Vector3(0, .1, -25); if (!selected || phase === "overview") return { position: overview.toArray() as Point3, target: targetOverview.toArray() as Point3, fov: portrait ? 59 : 54 }; const target = new THREE.Vector3(...cosmicPoint(selected, selectedIndex)), dir = overview.clone().sub(target).normalize(), side = new THREE.Vector3(dir.z, 0, -dir.x).normalize(), sign = Math.sign(target.x) || 1; const distance = phase === "departure" ? 22.5 : phase === "travel" ? 17.4 : phase === "approach" ? 12.8 : portrait ? 10.8 : 9.2; const position = target.clone().addScaledVector(dir, distance).addScaledVector(side, phase === "travel" ? sign * 3.4 : phase === "approach" ? sign * 1.1 : 0); position.y += phase === "travel" ? 2.2 : phase === "approach" ? .8 : .25; return { position: position.toArray() as Point3, target: target.toArray() as Point3, fov: portrait ? 56 : phase === "arrival" ? 46 : 50 }; }, [phase, selected, selectedIndex, size.height, size.width]); useLayoutEffect(() => { if (initialized.current) return; const next = goal(); camera.position.set(...next.position); look.current.set(...next.target); camera.lookAt(look.current); if (camera instanceof THREE.PerspectiveCamera) { camera.fov = next.fov; camera.updateProjectionMatrix(); } initialized.current = true; }, [camera, goal]); useFrame((_, delta) => { const next = goal(), position = new THREE.Vector3(...next.position), target = new THREE.Vector3(...next.target); if (!selected && phase === "overview" && !reducedMotion) { position.x += pointer.x * 1.25; position.y += pointer.y * .52; target.x += pointer.x * .58; target.y += pointer.y * .22; } if (reducedMotion) { camera.position.copy(position); look.current.copy(target); } else { const rate = phase === "overview" ? 2.2 : phase === "arrival" ? 4.4 : 2.65; camera.position.lerp(position, 1 - Math.exp(-rate * delta)); look.current.lerp(target, 1 - Math.exp(-4.2 * delta)); } camera.lookAt(look.current); if (camera instanceof THREE.PerspectiveCamera) { camera.fov = reducedMotion ? next.fov : THREE.MathUtils.damp(camera.fov, next.fov, 4, delta); camera.updateProjectionMatrix(); } }); return null; }
...
function CosmicWorld({ nodes, selected, selectedIndex, phase, reducedMotion, tier, onSelect, onWebGLState }: { nodes: LifeMapNode[]; selected: LifeMapNode | null; selectedIndex: number; phase: Phase; reducedMotion: boolean; tier: "low" | "medium" | "high"; onSelect: (node: LifeMapNode) => void; onWebGLState: (state: WebGLState) => void }) {
  const starCount = tier === "low" ? 760 : tier === "medium" ? 1280 : 1920;
  return <><color attach="background" args={["#020611"]} /><fog attach="fog" args={["#020611", 78, 178]} /><ambientLight intensity={.045} color="#d9edff" /><RenderProof /><WebGLRecovery onState={onWebGLState} /><CameraRig selected={selected} selectedIndex={selectedIndex} phase={phase} reducedMotion={reducedMotion} /><group name="life-map-deep-space"><Stars radius={132} depth={105} count={starCount} factor={1.45} saturation={.38} fade speed={0} /><StellarDepthField count={starCount} reducedMotion={reducedMotion} salt={.37} size={.14} opacity={.92} depthSpan={106} /><StellarDepthField count={Math.round(starCount * .58)} reducedMotion={reducedMotion} salt={7.19} size={.095} opacity={.72} depthSpan={62} /><group name="life-map-emotional-weather"><NebulaVeil position={[-12, 5, -20]} scale={[55, 30]} rotation={-.21} colors={["#163a55", "#503a58"]} opacity={.64} seed={.09} /><NebulaVeil position={[13, -6, -29]} scale={[58, 33]} rotation={.16} colors={["#0e3b50", "#303f66"]} opacity={.58} seed={.51} /><NebulaVeil position={[-2, 0, -39]} scale={[98, 44]} rotation={-.10} colors={["#102844", "#433254"]} opacity={.62} seed={.19} /><NebulaVeil position={[4, 3, -83]} scale={[134, 76]} rotation={-.04} colors={["#081a35", "#2e2149"]} opacity={.50} seed={.31} /><NebulaVeil position={[-25, 10, -49]} scale={[74, 35]} rotation={-.17} colors={["#0c3453", "#33416e"]} opacity={.44} seed={.73} /><NebulaVeil position={[28, -10, -58]} scale={[82, 38]} rotation={.20} colors={["#0a364e", "#403f68"]} opacity={.40} seed={2.17} /></group></group><Constellations /><group name="life-map-memory-stars">{nodes.map((node, index) => { const active = selected?.id === node.id, related = Boolean(selected && (selected.connectedTo.includes(node.id) || node.connectedTo.includes(selected.id))); return <MemoryStar key={node.id} node={node} index={index} active={active} related={related} onSelect={onSelect} />; })}</group>{selected ? <SelectedTravelWeather node={selected} index={selectedIndex} phase={phase} /> : null}{selected ? <SelectedMemoryDust node={selected} index={selectedIndex} /> : null}</>;
}
