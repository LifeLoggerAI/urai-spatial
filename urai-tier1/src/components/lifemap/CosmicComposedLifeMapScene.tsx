...
function LivingGalaxyField({ tier, reducedMotion, selected }: { tier: "low" | "medium" | "high"; reducedMotion: boolean; selected: boolean }) {
  ...
  const count = tier === "low" ? 2600 : tier === "medium" ? 4200 : 6200;
  const geometry = useMemo(() => {
  ...
  return <points ref={root} name="life-map-v286-layered-living-galaxy-core" geometry={geometry} raycast={() => null} userData={{ visualRole: "layered-living-galaxy", antiReference: "no-glass-shards-no-flat-star-wall" }}>
    <pointsMaterial map={disc} alphaTest={.01} vertexColors size={tier === "low" ? .14 : .17} transparent opacity={selected ? .46 : .80} depthWrite={false} blending={THREE.AdditiveBlending} sizeAttenuation />
  </points>;
}
...
function CosmicWorld({ nodes, selected, selectedIndex, phase, reducedMotion, tier, onSelect, onWebGLState }: { nodes: LifeMapNode[]; selected: LifeMapNode | null; selectedIndex: number; phase: Phase; reducedMotion: boolean; tier: "low" | "medium" | "high"; onSelect: (node: LifeMapNode) => void; onWebGLState: (state: WebGLState) => void }) {
  const starCount = tier === "low" ? 1000 : tier === "medium" ? 1700 : 2600;
