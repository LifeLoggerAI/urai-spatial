...
// V280 overview-authorship repair: preserve V279's accepted departure bridge and
// selected journey while giving overview real data-derived personal-universe regions.
...
function PortraitOverviewDepth({ phase }: { phase: Phase }) {
  ...
  return <group name="life-map-portrait-overview-depth" userData={{ visualRole: "portrait-only-asymmetric-volumetric-depth", pointWallpaper: false }}>
    <NebulaVeil position={[-8.5, 10, -15]} scale={[74, 43]} rotation={-.29} colors={["#1c5268", "#65545f"]} opacity={.42} seed={8.17} />
    <NebulaVeil position={[12, 2, -24]} scale={[82, 50]} rotation={.24} colors={["#174c60", "#6d5a4e"]} opacity={.39} seed={8.83} />
    <NebulaVeil position={[-4, -11, -34]} scale={[92, 58]} rotation={-.12} colors={["#1a465d", "#574d66"]} opacity={.36} seed={9.41} />
    <NebulaVeil position={[10, 15, -48]} scale={[106, 64]} rotation={.09} colors={["#123d55", "#5d525b"]} opacity={.33} seed={10.07} />
    <NebulaVeil position={[-13, -5, -63]} scale={[118, 72]} rotation={-.18} colors={["#12364f", "#4d5362"]} opacity={.30} seed={10.91} />
  </group>;
}
...
function OverviewRegions({ nodes, phase }: { nodes: LifeMapNode[]; phase: Phase }) {
  const regions = useMemo(() => {
    ...
    return Array.from(buckets.values())
      ...
      .map((bucket, index) => {
        ...
        const center: Point3 = [
          bucket.points.reduce((sum, point) => sum + point[0], 0) / divisor,
          bucket.points.reduce((sum, point) => sum + point[1], 0) / divisor,
          bucket.points.reduce((sum, point) => sum + point[2], 0) / divisor,
        ];
        const regionSeed = hash(bucket.key);
        const side = (seeded(regionSeed, 2.9) - .5) * 7.5;
        const lift = (seeded(regionSeed, 5.1) - .5) * 5.5;
        const depth = 4.5 + seeded(regionSeed, 7.3) * 8;
        const spread = 16 + Math.min(9, bucket.points.length * 1.8);
        const secondary: Point3 = [center[0] + side, center[1] + lift, center[2] - depth];
        return { key: bucket.key, center, secondary, spread, palette: palettes[index % palettes.length], seed: 14.2 + index * .83 };
      });
  }, [nodes]);
  ...
  return <group name="life-map-overview-personal-universe-regions" userData={{ visualRole: "data-derived-personal-universe-geography", pointWallpaper: false, graphEdges: false }}>
    {regions.map((region, index) => <group key={region.key} name={`life-map-overview-region-${index}`} userData={{ regionKey: region.key, visualRole: "personal-universe-region" }}>
      <NebulaVeil position={region.center} scale={[region.spread, region.spread * .56]} rotation={(index % 2 ? 1 : -1) * (.12 + index * .025)} colors={region.palette} opacity={.28} seed={region.seed} />
      <NebulaVeil position={region.secondary} scale={[region.spread * .72, region.spread * .42]} rotation={(index % 2 ? -1 : 1) * (.18 + index * .02)} colors={[region.palette[1], region.palette[0]]} opacity={.17} seed={region.seed + .37} />
    </group>)}
  </group>;
}
...
function MemoryStar({ node, index, active, related, onSelect }: { node: LifeMapNode; index: number; active: boolean; related: boolean; onSelect: (node: LifeMapNode) => void }) {
  ...
  const outer = active ? 1.08 : related ? .82 : .74;
  const mid = active ? .52 : related ? .40 : .36;
  const hot = active ? .20 : related ? .17 : .16;
  return <group name={`life-map-memory-star-${node.id}`} position={point} userData={{ semanticType: node.type, visualAuthority: "stellar-memory-not-node-graph", stellarMorphology: "point-photosphere-layered-corona-no-visible-sphere" }} onClick={(event) => { event.stopPropagation(); onSelect(node); }} onPointerOver={(event) => pointer(event, true)} onPointerOut={(event) => pointer(event, false)}>
    <sprite scale={[outer * 1.24, outer, 1]}><spriteMaterial map={halo} color={node.aura} transparent opacity={active ? .20 : related ? .19 : .17} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} /></sprite>
    <sprite scale={[mid * 1.10, mid, 1]}><spriteMaterial map={halo} color={core} transparent opacity={active ? .50 : related ? .44 : .40} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} /></sprite>
    <sprite scale={[hot, hot, 1]}><spriteMaterial map={photosphere} color="#fff8e9" transparent opacity={active ? .94 : related ? .91 : .90} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} /></sprite>
    <pointLight color={node.aura} intensity={active ? .24 : related ? .07 : .024} distance={active ? 3.4 : 2.2} decay={2} />
...
function CosmicWorld({ nodes, selected, selectedIndex, phase, reducedMotion, tier, onSelect, onWebGLState }: { nodes: LifeMapNode[]; selected: LifeMapNode | null; selectedIndex: number; phase: Phase; reducedMotion: boolean; tier: "low" | "medium" | "high"; onSelect: (node: LifeMapNode) => void; onWebGLState: (state: WebGLState) => void }) {
  ...
  const overviewWeather = phase === "overview" ? .56 : 1;
  return <><color attach="background" args={["#020611"]} /><fog attach="fog" args={["#020611", 78, 170]} /><ambientLight intensity={.045} color="#d9edff" /><RenderProof /><WebGLRecovery onState={onWebGLState} /><CameraRig selected={selected} selectedIndex={selectedIndex} phase={phase} reducedMotion={reducedMotion} /><group name="life-map-deep-space"><Stars radius={132} depth={105} count={starCount} factor={1.05} saturation={.30} fade speed={0} /><StellarDepthField count={nearCount} reducedMotion={reducedMotion} salt={.37} size={.25} opacity={.96} depthSpan={42} /><StellarDepthField count={midCount} reducedMotion={reducedMotion} salt={4.13} size={.17} opacity={.88} depthSpan={66} /><StellarDepthField count={farCount} reducedMotion={reducedMotion} salt={7.19} size={.11} opacity={.72} depthSpan={96} /><group name="life-map-emotional-weather"><NebulaVeil position={[-12, 5, -20]} scale={[60, 36]} rotation={-.21} colors={["#163b4d", "#3a3a42"]} opacity={.68 * overviewWeather} seed={.09} /><NebulaVeil position={[13, -6, -29]} scale={[62, 38]} rotation={.16} colors={["#113844", "#34414a"]} opacity={.62 * overviewWeather} seed={.51} /><NebulaVeil position={[-2, 0, -39]} scale={[105, 54]} rotation={-.10} colors={["#142b3a", "#3e3a43"]} opacity={.68 * overviewWeather} seed={.19} /><NebulaVeil position={[4, 3, -83]} scale={[134, 76]} rotation={-.04} colors={["#09182a", "#272838"]} opacity={.48 * overviewWeather} seed={.31} /><NebulaVeil position={[-25, 10, -49]} scale={[78, 41]} rotation={-.17} colors={["#10394a", "#3a4148"]} opacity={.48 * overviewWeather} seed={.73} /><NebulaVeil position={[28, -10, -58]} scale={[86, 43]} rotation={.20} colors={["#103746", "#4a4037"]} opacity={.44 * overviewWeather} seed={2.17} /></group><OverviewRegions nodes={nodes} phase={phase} /><PortraitOverviewDepth phase={phase} /></group><Constellations /><group name="life-map-memory-stars">{nodes.map((node, index) => { const active = selected?.id === node.id, related = Boolean(selected && (selected.connectedTo.includes(node.id) || node.connectedTo.includes(selected.id))); return <MemoryStar key={node.id} node={node} index={index} active={active} related={related} onSelect={onSelect} />; })}</group>{selectedPoint ? <group name="life-map-selected-local-depth" position={selectedPoint} scale={[.68, .68, .68]}><StellarDepthField count={selectedLocalCount} reducedMotion={reducedMotion} salt={12.31} size={.22} opacity={.90} depthSpan={66} /></group> : null}{selected ? <SelectedTravelWeather node={selected} index={selectedIndex} phase={phase} /> : null}{selected ? <SelectedMemoryDust node={selected} index={selectedIndex} /> : null}</>;
}
