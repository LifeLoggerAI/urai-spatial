...
function SelectedTravelWeather({ node, index, phase }: { node: LifeMapNode; index: number; phase: Phase }) {
  ...
  const strength = phase === "departure" ? .54 : phase === "travel" ? .64 : phase === "approach" ? .50 : .42;
  return <group name="life-map-selected-travel-weather"><NebulaVeil position={[point[0] - 6.6, point[1] + 3.4, point[2] - 8]} scale={[35, 21]} rotation={-.23} colors={["#143d4d", "#474044"]} opacity={strength} seed={3.11} /><NebulaVeil position={[point[0] + 7.8, point[1] - 4.1, point[2] - 12]} scale={[32, 19]} rotation={.18} colors={["#173a46", "#334650"]} opacity={strength * .88} seed={4.37} /><NebulaVeil position={[point[0] - 1.6, point[1] + .6, point[2] - 19]} scale={[50, 27]} rotation={-.07} colors={["#102b3a", "#403a42"]} opacity={strength * .80} seed={5.73} /><NebulaVeil position={[point[0] + 1.2, point[1] + 7.2, point[2] - 5]} scale={[42, 24]} rotation={.08} colors={["#184653", "#51473d"]} opacity={strength * .52} seed={6.41} /></group>;
}
...
function MemoryStar({ node, index, active, related, onSelect }: { node: LifeMapNode; index: number; active: boolean; related: boolean; onSelect: (node: LifeMapNode) => void }) {
  ...
  const outer = active ? 1.08 : related ? .68 : .48;
  const mid = active ? .52 : related ? .31 : .25;
  const hot = active ? .20 : related ? .145 : .12;
  return <group name={`life-map-memory-star-${node.id}`} position={point} userData={{ semanticType: node.type, visualAuthority: "stellar-memory-not-node-graph", stellarMorphology: "point-photosphere-layered-corona-no-visible-sphere" }} onClick={(event) => { event.stopPropagation(); onSelect(node); }} onPointerOver={(event) => pointer(event, true)} onPointerOut={(event) => pointer(event, false)}>
    <sprite scale={[outer * 1.24, outer, 1]}><spriteMaterial map={halo} color={node.aura} transparent opacity={active ? .20 : related ? .13 : .09} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} /></sprite>
    <sprite scale={[mid * 1.10, mid, 1]}><spriteMaterial map={halo} color={core} transparent opacity={active ? .50 : related ? .34 : .26} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} /></sprite>
    <sprite scale={[hot, hot, 1]}><spriteMaterial map={photosphere} color="#fff8e9" transparent opacity={active ? .94 : related ? .86 : .80} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} /></sprite>
