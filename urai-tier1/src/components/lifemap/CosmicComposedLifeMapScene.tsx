...
function SelectedMemoryFormation({ point, aura, phase, index }: { point: Point3; aura: string; phase: Phase; index: number }) {
  ...
  const scale = phase === "arrival" ? 4.5 : phase === "approach" ? 3.9 : phase === "travel" ? 3.0 : 2.3;
  return <group name="life-map-v287-selected-memory-stellar-weather" userData={{ visualRole: "selected-memory-star-with-volumetric-weather", visualAcceptance: "v287-selected-memory-star-no-glass-shards", interactionOwner: false, pointWallpaper: false, journeyPhase: phase }} raycast={() => null}>
    ...
    <NebulaVeil position={[point[0] - scale * .18, point[1] + scale * .08, point[2] - 1.4]} scale={[scale * 6.5, scale * 4.1]} rotation={-.18} colors={palette} opacity={phase === "arrival" ? .38 : .25} seed={17.13 + index * .01} />
    <NebulaVeil position={[point[0] + scale * .34, point[1] - scale * .12, point[2] - 5.6]} scale={[scale * 5.4, scale * 3.4]} rotation={.27} colors={[palette[1], palette[0]]} opacity={phase === "arrival" ? .28 : .19} seed={21.47 + index * .01} />
    <pointLight position={[point[0] - .8, point[1] + .9, point[2] + 1.2]} color={aura} intensity={phase === "arrival" ? 1.65 : .88} distance={16} decay={2} />
...
function MemoryStar({ node, index, active, related, overview, onSelect }: { node: LifeMapNode; index: number; active: boolean; related: boolean; overview: boolean; onSelect: (node: LifeMapNode) => void }) {
  ...
  const approachBoost = active && !overview ? 2.72 : 1;
  const outer = (active ? 1.08 : related ? .82 : .74) * overviewBoost * approachBoost;
  const mid = (active ? .52 : related ? .40 : .36) * overviewBoost * (active && !overview ? 2.18 : 1);
  const hot = (active ? .20 : related ? .17 : .16) * (overview ? 1.24 : 1) * (active && !overview ? 1.92 : 1);
