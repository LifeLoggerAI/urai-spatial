...
function LivingGalaxyField({ tier, reducedMotion, selected }: { tier: "low" | "medium" | "high"; reducedMotion: boolean; selected: boolean }) {
  ...
  const count = tier === "low" ? 3200 : tier === "medium" ? 5200 : 7600;
  const geometry = useMemo(() => {
    const positions = new Float32Array(count * 3);
    const colors = new Float32Array(count * 3);
    const pearl = new THREE.Color("#fff8e9");
    const warm = new THREE.Color("#ffd19b");
    const cool = new THREE.Color("#b9ebff");
    const violet = new THREE.Color("#c9b6ff");
    for (let i = 0; i < count; i += 1) {
      const radius = .45 + Math.pow(seeded(i + 173, 18.11), .66) * 31.5;
      const arm = i % 6;
      const angle = arm * Math.PI / 3 + radius * .155 + (seeded(i + 173, 18.77) - .5) * .94;
      const cross = (seeded(i + 173, 19.41) - .5) * (1.05 + radius * .115);
      const x = Math.cos(angle) * radius * 1.16 + Math.cos(angle + Math.PI / 2) * cross;
      const y = (seeded(i + 173, 20.03) - .5) * (4.8 + radius * .25) + Math.sin(angle * 2.08) * 1.05;
      const z = -17.0 - radius * .68 + (seeded(i + 173, 21.19) - .5) * (5.0 + radius * .12);
      positions.set([x, y, z], i * 3);
      const core = Math.exp(-radius / 8.5);
      const base = i % 29 === 0 ? warm : i % 17 === 0 ? violet : i % 11 === 0 ? pearl : cool;
      const color = base.clone().lerp(warm, core * .56).multiplyScalar(.52 + seeded(i + 173, 22.73) * .58);
      colors.set([color.r, color.g, color.b], i * 3);
    }
    const result = new THREE.BufferGeometry();
    result.setAttribute("position", new THREE.BufferAttribute(positions, 3));
  ...
  useFrame(({ clock }) => {
    ...
    root.current.rotation.z = Math.sin(clock.elapsedTime * .009) * .006;
    root.current.rotation.y = Math.sin(clock.elapsedTime * .006) * .008;
  });
  return <group name="life-map-v289-tilted-living-galaxy-volume" rotation={[-.34, -.06, -.08]} scale={[1, 1.12, 1]} userData={{ visualRole: "layered-living-galaxy-volume", antiReference: "no-flat-star-wall" }}>
    <points ref={root} name="life-map-v286-layered-living-galaxy-core" geometry={geometry} raycast={() => null} userData={{ visualRole: "layered-living-galaxy", antiReference: "no-glass-shards-no-flat-star-wall" }}>
      <pointsMaterial map={disc} alphaTest={.008} vertexColors size={tier === "low" ? .24 : .30} transparent opacity={selected ? .62 : .98} depthWrite={false} blending={THREE.AdditiveBlending} sizeAttenuation />
    </points>
  </group>;
}
