...
function RenderProof({ minObjects = 20, minAnchors = 8 }: { minObjects?: number; minAnchors?: number } = {}) {
  const { gl, scene } = useThree();
  const publish = useCallback(() => { const root = gl.domElement.closest<HTMLElement>('[data-testid="urai-true-3d-life-map"]'); if (!root) return; let objects = 0, anchors = 0; scene.traverse((object) => { if (!object.visible) return; objects += 1; if (object.name.startsWith("life-map-")) anchors += 1; }); const ready = gl.info.render.calls > 0 && objects > minObjects && anchors >= minAnchors; root.dataset.lifeMapRenderReady = ready ? "true" : "false"; root.dataset.lifeMapVisibleObjects = String(objects); root.dataset.lifeMapVisibleAnchors = String(anchors); root.dataset.lifeMapRenderCalls = String(gl.info.render.calls); root.dataset.lifeMapRenderTriangles = String(gl.info.render.triangles); }, [gl, minAnchors, minObjects, scene]);
...
function CosmicWorld({ nodes, selected, selectedIndex, phase, reducedMotion, tier, onSelect, onWebGLState, memoryStarReview }: { nodes: LifeMapNode[]; selected: LifeMapNode | null; selectedIndex: number; phase: Phase; reducedMotion: boolean; tier: "low" | "medium" | "high"; onSelect: (node: LifeMapNode) => void; onWebGLState: (state: WebGLState) => void; memoryStarReview?: string }) {
  ...
  if (reviewNode && (memoryStarReview === "isolated" || memoryStarReview === "hover" || memoryStarReview === "near-cluster")) {
    ...
    return <><color attach="background" args={["#020611"]} /><ambientLight intensity={.055} color="#d9edff" /><RenderProof minObjects={7} minAnchors={1} /><MemoryStarReviewCamera clustered={memoryStarReview === "near-cluster"} /><WebGLRecovery onState={onWebGLState} /><group name="memory-star-reference-review" userData={{ explicitDemoReview: true, reviewState: memoryStarReview }}>
