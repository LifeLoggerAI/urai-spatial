...
function SelectedMemoryFormation({ point, aura, phase, index }: { point: Point3; aura: string; phase: Phase; index: number }) {
  ...
  return <group name="life-map-v287-selected-memory-stellar-weather" userData={{ visualRole: "selected-memory-star-with-volumetric-weather", visualAcceptance: "v287-selected-memory-star-no-glass-shards", interactionOwner: false, pointWallpaper: false, journeyPhase: phase }} raycast={() => null}>
    <sprite position={point} scale={[scale * 1.70, scale * 1.28, 1]} raycast={() => null} name="life-map-selected-memory-corona">
      <spriteMaterial map={halo} color={aura} transparent opacity={phase === "arrival" ? .30 : .22} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
    <sprite position={point} scale={[scale * .88, scale * .76, 1]} raycast={() => null} name="life-map-selected-memory-inner-corona">
      <spriteMaterial map={halo} color={palette[1]} transparent opacity={phase === "arrival" ? .42 : .30} depthWrite={false} blending={THREE.AdditiveBlending} toneMapped={false} />
    </sprite>
