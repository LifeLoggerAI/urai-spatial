...
// V282 retained-pixel repair: preserve V280 data-derived overview authority and
// V279 selected journey while replacing smooth cosmic wallpaper with authored celestial landmarks.
...
function buildCelestialCanopyGeometry(seed: number, span: number, rise: number) {
  const columns = 18, rows = 5, positions: number[] = [], colors: number[] = [], indices: number[] = [];
  const cool = new THREE.Color("#4f95a5"), warm = new THREE.Color("#b28a72"), pearl = new THREE.Color("#c9d9d0");
  for (let row = 0; row <= rows; row += 1) {
    const v = row / rows;
    for (let column = 0; column <= columns; column += 1) {
      const u = column / columns, x = (u - .5) * span;
      const bow = Math.sin(u * Math.PI), asymmetry = Math.sin(u * 5.4 + seed) * (.18 + v * .16);
      const y = (v - .5) * rise + bow * (1.25 + .55 * Math.sin(seed * .41)) + asymmetry;
      const z = -Math.abs(u - .5) * span * .16 + Math.sin(u * Math.PI * 2.2 + v * 2.6 + seed) * (.32 + v * .18) + (v - .5) * .55;
      positions.push(x, y, z);
      const color = cool.clone().lerp(warm, .18 + v * .38).lerp(pearl, bow * .17);
...
function CelestialRegionCanopy({ position, palette, index, spread, rotation = 0, opacity = .56 }: { position: Point3; palette: [string, string]; index: number; spread: number; rotation?: number; opacity?: number }) {
  const geometry = useMemo(() => buildCelestialCanopyGeometry(2.8 + index * 1.37, spread, spread * .34), [index, spread]);
  useEffect(() => () => geometry.dispose(), [geometry]);
  return <mesh name={`life-map-v282-celestial-canopy-${index}`} geometry={geometry} position={position} rotation={[.12 * Math.sin(index), rotation, (index % 2 ? 1 : -1) * .11]} raycast={() => null} userData={{ visualRole: "data-derived-celestial-landmark", pointWallpaper: false, graphEdges: false, interactionOwner: false }}>
    <meshStandardMaterial vertexColors color={palette[0]} emissive={palette[1]} emissiveIntensity={.34} transparent opacity={opacity} depthWrite={false} roughness={.72} metalness={0} side={THREE.DoubleSide} blending={THREE.AdditiveBlending} />
  </mesh>;
}
...
function SelectedMemoryFormation({ point, aura, phase, index }: { point: Point3; aura: string; phase: Phase; index: number }) {
  ...
  const scale = phase === "arrival" ? 9.4 : phase === "approach" ? 8.0 : phase === "travel" ? 6.8 : 5.8;
  return <group name="life-map-v282-selected-memory-celestial-formation" userData={{ visualRole: "selected-memory-authored-volumetric-formation", interactionOwner: false, pointWallpaper: false, journeyPhase: phase }} raycast={() => null}>
    <CelestialRegionCanopy position={[point[0] - scale * .24, point[1] + scale * .08, point[2] - .8]} palette={palette} index={30 + index} spread={scale} rotation={-.32} opacity={.68} />
    <CelestialRegionCanopy position={[point[0] + scale * .22, point[1] - scale * .10, point[2] - 2.4]} palette={[palette[1], palette[0]]} index={60 + index} spread={scale * .82} rotation={.38} opacity={.52} />
    <CelestialRegionCanopy position={[point[0] - scale * .05, point[1] + scale * .24, point[2] - 4.6]} palette={palette} index={90 + index} spread={scale * .62} rotation={.06} opacity={.40} />
    <pointLight position={[point[0] - .8, point[1] + .9, point[2] + 1.2]} color={aura} intensity={phase === "arrival" ? 1.4 : .86} distance={14} decay={2} />
    <pointLight position={[point[0] + 1.6, point[1] - .4, point[2] - 3.2]} color="#e2c49b" intensity={.44} distance={10} decay={2} />
  </group>;
}
...
function OverviewRegions({ nodes, phase }: { nodes: LifeMapNode[]; phase: Phase }) {
  const regions = useMemo(() => {
    ...
    return Array.from(buckets.values())
      ...
      .map((bucket, index) => {
        ...
        const center: Point3 = [rawCenter[0] * 1.24, rawCenter[1] * 1.18, THREE.MathUtils.clamp(rawCenter[2] + 6, -42, -22)];
        const side = (seeded(regionSeed, 2.9) - .5) * 9, lift = (seeded(regionSeed, 5.1) - .5) * 6.4, depth = 5 + seeded(regionSeed, 7.3) * 7;
        const spread = 10.5 + Math.min(7.5, Math.sqrt(bucket.points.length) * 3.2);
        const secondary: Point3 = [center[0] + side, center[1] + lift, center[2] - depth], tertiary: Point3 = [center[0] - side * .48, center[1] - lift * .36, center[2] + 2.4], beacon: Point3 = [center[0], center[1], center[2] + 1.4];
        const strength = 1.08 + Math.min(.58, bucket.points.length * .07);
  ...
  return <group name="life-map-overview-personal-universe-regions" userData={{ visualRole: "data-derived-personal-universe-geography", pointWallpaper: false, graphEdges: false }}>
    {regions.map((region, index) => <group key={region.key} name={`life-map-overview-region-${index}`} userData={{ regionKey: region.key, visualRole: "personal-universe-region" }}>
      <NebulaVeil position={region.center} scale={[region.spread, region.spread * .58]} rotation={(index % 2 ? 1 : -1) * (.12 + index * .025)} colors={region.palette} opacity={.24} seed={region.seed} />
      <NebulaVeil position={region.secondary} scale={[region.spread * .78, region.spread * .46]} rotation={(index % 2 ? -1 : 1) * (.18 + index * .02)} colors={[region.palette[1], region.palette[0]]} opacity={.15} seed={region.seed + .37} />
      <NebulaVeil position={region.tertiary} scale={[region.spread * .54, region.spread * .32]} rotation={(index % 2 ? 1 : -1) * (.31 + index * .017)} colors={region.palette} opacity={.09} seed={region.seed + .71} />
      <CelestialRegionCanopy position={region.center} palette={region.palette} index={index} spread={region.spread * .72} rotation={(index % 2 ? 1 : -1) * .28} opacity={.62} />
      <CelestialRegionCanopy position={region.secondary} palette={[region.palette[1], region.palette[0]]} index={index + 10} spread={region.spread * .48} rotation={(index % 2 ? -1 : 1) * .43} opacity={.44} />
