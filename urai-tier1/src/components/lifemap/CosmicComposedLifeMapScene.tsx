...
function LivingGalaxyField({ tier, reducedMotion, selected }: { tier: "low" | "medium" | "high"; reducedMotion: boolean; selected: boolean }) {
  ...
  const geometry = useMemo(() => {
    ...
    const pearl = new THREE.Color("#f4efe3");
    const warm = new THREE.Color("#e9c78f");
    const cool = new THREE.Color("#9fd7e6");
    const violet = new THREE.Color("#b6a4d5");
    for (let i = 0; i < count; i += 1) {
      ...
      const y = (seeded(i + 173, 20.03) - .5) * (2.6 + radius * .17) + Math.sin(angle * 2.08) * .72;
      const z = -15.5 - radius * .86 + (seeded(i + 173, 21.19) - .5) * (4.4 + radius * .10);
      positions.set([x, y, z], i * 3);
  ...
  return <points ref={root} name="life-map-v286-layered-living-galaxy-core" geometry={geometry} raycast={() => null} userData={{ visualRole: "layered-living-galaxy", antiReference: "no-glass-shards-no-flat-star-wall" }}>
    <pointsMaterial map={disc} alphaTest={.01} vertexColors size={tier === "low" ? .18 : .22} transparent opacity={selected ? .50 : .94} depthWrite={false} blending={THREE.AdditiveBlending} sizeAttenuation />
  </points>;
}
...
function OverviewRegions({ nodes, phase }: { nodes: LifeMapNode[]; phase: Phase }) {
  ...
  return <group name="life-map-overview-personal-universe-regions" userData={{ visualRole: "data-derived-personal-universe-geography", portraitAuthority: portrait ? "v285-vertical-territory-composition" : "desktop-v284-archipelago-composition", pointWallpaper: false, graphEdges: false }}>
    {regions.map((region, index) => <group key={region.key} name={`life-map-overview-region-${index}`} userData={{ regionKey: region.key, visualRole: "personal-universe-region" }}>
      <NebulaVeil position={region.center} scale={[region.spread * 1.45, region.spread * .82]} rotation={(index % 2 ? 1 : -1) * (.12 + index * .025)} colors={region.palette} opacity={portrait ? .20 : .18} seed={region.seed} />
      <NebulaVeil position={region.secondary} scale={[region.spread * 1.05, region.spread * .62]} rotation={(index % 2 ? -1 : 1) * (.18 + index * .02)} colors={[region.palette[1], region.palette[0]]} opacity={portrait ? .12 : .10} seed={region.seed + .37} />
      <NebulaVeil position={region.tertiary} scale={[region.spread * .72, region.spread * .44]} rotation={(index % 2 ? 1 : -1) * (.31 + index * .017)} colors={region.palette} opacity={portrait ? .08 : .065} seed={region.seed + .71} />
