...
export default function LifeMapSemanticNavigator() {
  ...
  const destinationHref = useCallback((route: "focus" | "replay" | "timeline" | "location-map") => {
  ...
  return <>
    <nav className="life-map-journey-rail" aria-label="Life Map journey controls">
    ...
    {selected ? <aside className="life-map-semantic-inspector" aria-label="Selected life object details">
      ...
      <nav aria-label="Selected memory destinations">
        <button type="button" onClick={() => router.push(destinationHref("focus"))}>Focus</button>
        ...
        <button type="button" onClick={() => router.push(destinationHref("timeline"))}>Timeline</button>
        <button type="button" onClick={() => router.push(destinationHref("location-map"))}>Place</button>
      </nav>
    </aside> : null}

    <style jsx>{`
      .life-map-journey-rail{position:fixed;z-index:40;left:50%;bottom:max(18px,env(safe-area-inset-bottom));transform:translateX(-50%);display:flex;gap:7px;padding:7px;border:1px solid rgba(195,240,255,.18);border-radius:999px;background:rgba(2,7,18,.78);backdrop-filter:blur(18px)}button{font:inherit}.life-map-journey-rail button,.life-map-navigator button,.life-map-semantic-inspector button{min-height:48px;border:1px solid rgba(220,248,255,.2);border-radius:999px;background:rgba(10,25,40,.9);color:#f8fbff;padding:0 16px;font-weight:800;cursor:pointer}.life-map-navigator{position:fixed;z-index:42;right:max(18px,env(safe-area-inset-right));bottom:max(18px,env(safe-area-inset-bottom));width:min(420px,calc(100vw - 36px));border:1px solid rgba(195,240,255,.2);border-radius:22px;background:rgba(2,7,18,.9);backdrop-filter:blur(24px);color:#f8fbff}.life-map-navigator summary{padding:15px 18px;font-weight:800;cursor:pointer}.life-map-navigator section{display:grid;gap:12px;max-height:70vh;padding:0 16px 16px}.life-map-navigator label{font-size:12px;color:rgba(230,245,255,.75)}.life-map-navigator input{min-height:48px;border:1px solid rgba(205,244,255,.2);border-radius:16px;background:rgba(6,17,29,.95);color:#fff;padding:0 14px;font:inherit}.filter-row{display:flex;gap:7px;overflow:auto}.filter-row button{min-height:38px;white-space:nowrap;padding:0 12px;font-size:11px}.filter-row button[data-active='true']{border-color:rgba(221,250,255,.8);background:rgba(24,67,88,.95)}.semantic-results{display:grid;gap:8px;overflow:auto}.semantic-results>button{height:auto;display:grid;gap:4px;text-align:left;padding:12px 14px;border-radius:16px}.semantic-results>button[data-selected='true']{border-color:rgba(221,250,255,.85);background:rgba(18,58,78,.95)}.semantic-results span,.semantic-results small{color:rgba(225,243,255,.68)}.semantic-results small{line-height:1.35}.privacy-truth{margin:0;font-size:10px;letter-spacing:.13em;text-transform:uppercase;color:rgba(194,244,255,.7)}.life-map-semantic-inspector{position:fixed;z-index:41;left:max(20px,env(safe-area-inset-left));bottom:max(88px,calc(env(safe-area-inset-bottom) + 76px));width:min(470px,calc(100vw - 40px));display:grid;gap:12px;padding:20px;border:1px solid rgba(195,240,255,.2);border-radius:24px;background:rgba(4,12,23,.9);backdrop-filter:blur(22px);color:#f8fbff}.life-map-semantic-inspector span,.life-map-semantic-inspector small{font-size:11px;color:rgba(214,242,255,.7);text-transform:uppercase;letter-spacing:.12em}.life-map-semantic-inspector h2{margin:0;font-size:clamp(24px,4vw,42px);line-height:.95}.life-map-semantic-inspector p{margin:0;color:rgba(235,246,255,.78);line-height:1.45}.life-map-semantic-inspector nav,.related-paths{display:flex;flex-wrap:wrap;gap:7px}.life-map-semantic-inspector nav button,.related-paths button{min-height:42px;padding:0 13px;font-size:12px}.related-paths{align-items:center}.related-paths strong{font-size:11px;letter-spacing:.12em;text-transform:uppercase;color:rgba(214,242,255,.7)}@media(max-width:760px){.life-map-journey-rail{bottom:max(10px,env(safe-area-inset-bottom));width:calc(100vw - 24px);justify-content:center}.life-map-journey-rail button{flex:1}.life-map-navigator{right:12px;bottom:max(72px,calc(env(safe-area-inset-bottom) + 62px));width:calc(100vw - 24px)}.life-map-navigator section{max-height:62vh}.life-map-semantic-inspector{left:12px;bottom:max(72px,calc(env(safe-area-inset-bottom) + 62px));width:calc(100vw - 24px);max-height:38vh;overflow:auto;padding:15px}.life-map-semantic-inspector nav button{flex:1}.related-paths{display:none}}@media(prefers-reduced-motion:reduce){*{scroll-behavior:auto!important;transition:none!important;animation:none!important}}
    `}</style>
  </>;
}
