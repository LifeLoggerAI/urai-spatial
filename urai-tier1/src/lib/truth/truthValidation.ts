...
export function validateTruthRecord(record: TruthRecord): readonly string[] {
  ...
  if (isFactualTruthKind(record.kind) && record.sourceRefs.some((source) => source.provenance === 'generated-context')) {
    errors.push('FACTUAL_TRUTH_CANNOT_DEPEND_ON_GENERATED_CONTEXT')
  }
...
export function canPromoteTruth(from: TruthKind, to: TruthKind) {
  if (from === 'scenario' || from === 'generated-context' || from === 'forecast' || from === 'hypothesis' || from === 'interpretation') {
    return !isFactualTruthKind(to)
  }
  return true
}
