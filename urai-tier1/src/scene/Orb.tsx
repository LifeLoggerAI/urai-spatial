...
const orbPalette: Record<OrbState, { aura: string; halo: string; light: string; emissive: string; intensity: number }> = {
  idle: { aura: '#9b7cff', halo: '#8edcff', light: '#cbb6ff', emissive: '#d7c5ff', intensity: 1.8 },
  listening: { aura: '#6ee7ff', halo: '#d9f7ff', light: '#8be9ff', emissive: '#bff7ff', intensity: 2.35 },
  memoryBloom: { aura: '#f0abfc', halo: '#fef3c7', light: '#f5d0fe', emissive: '#f0abfc', intensity: 2.55 },
  ritual: { aura: '#fbbf24', halo: '#fed7aa', light: '#fde68a', emissive: '#facc15', intensity: 2.75 },
  recovery: { aura: '#86efac', halo: '#bbf7d0', light: '#a7f3d0', emissive: '#bbf7d0', intensity: 2.45 },
}
...
export default function Orb({ state = 'idle' }: { state?: OrbState }) {
  ...
  useEffect(() => subscribeNarratorSpeaking(setNarratorSpeakingState), [])

  useFrame(({ clock }) => {
    const t = clock.elapsedTime
    const activeState = narratorSpeaking && state === 'idle' ? 'listening' : state
    const palette = orbPalette[activeState]
    const stateBoost = activeState === 'ritual' ? 0.09 : activeState === 'memoryBloom' ? 0.075 : activeState === 'listening' ? 0.06 : 0.035
    ...
    if (orbRef.current) {
      ...
      if (material instanceof THREE.MeshStandardMaterial) {
        ...
        material.emissiveIntensity += (0.72 * speechPulse - material.emissiveIntensity) * 0.08
      }
    }
    ...
    if (lightRef.current) {
      ...
      lightRef.current.intensity += (palette.intensity * speechPulse - lightRef.current.intensity) * 0.08
    }
  })

  return (
    <group position={[0, -0.18, -1.2]}>
      <pointLight ref={lightRef} position={[0, 0.15, 0.1]} intensity={orbPalette[state].intensity} color={orbPalette[state].light} distance={5.8} />

      <mesh ref={haloRef} rotation={[0, 0, 0]} position={[0, 0, -0.08]}>
        <circleGeometry args={[0.72, 64]} />
        <meshBasicMaterial color={orbPalette[state].halo} transparent opacity={0.08} depthWrite={false} blending={THREE.AdditiveBlending} />
      </mesh>

      <mesh ref={auraRef} position={[0, 0, 0]}>
        <sphereGeometry args={[0.54, 64, 64]} />
        <meshBasicMaterial color={orbPalette[state].aura} transparent opacity={0.18} depthWrite={false} blending={THREE.AdditiveBlending} />
      </mesh>
