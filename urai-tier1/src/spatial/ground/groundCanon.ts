...
export const GROUND_DESKTOP_SPEED_MPS = 2.55
export const GROUND_MOBILE_SPEED_MPS = 2.4
export const GROUND_VR_SPEED_MPS = 1.6
export const GROUND_PRECISION_SPEED_MPS = 1.15
export const GROUND_ACCELERATION_MPS2 = 8.5
export const GROUND_DECELERATION_MPS2 = 10.2
...
export function buildGroundObstacleField(profile: 'temperate' | 'urban' | 'woodland' | 'arid' | 'coastal'): MovementObstacle[] {
  ...
  if (profile === 'arid') return items.slice(0, 12).map((item) => ({ x: item.x, z: item.z, radius: collisionRadiusForScale(2.2 * item.scale, 'rock') }))
  if (profile === 'coastal') return items.slice(0, 10).map((item) => ({ x: item.x, z: item.z, radius: collisionRadiusForScale(1.25 * item.scale, 'rock') }))

  const woodland = profile === 'woodland'
  const treeObstacles = items.slice(0, woodland ? 18 : 12).map((item) => ({ x: item.x, z: item.z, radius: collisionRadiusForScale(1 + item.scale * 0.38, 'tree') }))
  const rockObstacles = items.slice(0, 8).map((item) => ({ x: item.x * 0.55, z: item.z + 2.2, radius: collisionRadiusForScale(0.82 * item.scale, 'rock') }))
