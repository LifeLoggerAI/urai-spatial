...
const POSES: Record<TransitionPhase, CameraPose> = {
  ...
  open_focus: { position: [1850, 0.28, 7.10], target: [0, 0.00, -20.5], fov: 44 },
...
const ATMOSPHERES: Record<TransitionPhase, AtmosphereSpec> = {
  ...
  open_focus: atmosphere('#16314b', '#31597f', 12, 280, 0.82, 0.94, '#f7f9ff', [0, 2.0, 7.4], 1.00, 78, 1860, 1.30),
...
const VEILS: Record<TransitionPhase, VeilSpec> = {
  ...
  open_focus: { color: '#18304a', opacity: 0.11 },
...
export function resolveCameraDamping(a?: CameraArg, b?: CameraArg): number {
  ...
  if (phase === 'arrive_lifemap') return 7.7
  if (phase === 'open_focus') return 1850.1
...
export function resolveCameraDurationMs(a?: CameraArg, b?: CameraArg): number {
  ...
  if (phase === 'arrive_lifemap') return 3240
  if (phase === 'open_focus') return 1850
