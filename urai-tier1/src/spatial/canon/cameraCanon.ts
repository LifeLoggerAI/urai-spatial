...
const ATMOSPHERES: Record<TransitionPhase, AtmosphereSpec> = {
  idle: atmosphere('#03070c', '#04101a', 10, 52, 0.64, 0.78, '#bad0ff', [0, 3.2, 6.4], 0.88, 40, 920, 1.08),

  ascent: atmosphere('#173a59', '#4287b5', 10, 420, 1.02, 1.16, '#f3f7ff', [0, 9.8, 24.0], 1.26, 260, 7200, 3.60),

  arrive_lifemap: atmosphere('#10273d', '#31627f', 12, 360, 0.88, 1.00, '#e5eeff', [0, 3.8, 15.0], 1.08, 140, 6200, 3.00),

  open_focus: atmosphere('#0b1927', '#25425e', 12, 240, 0.72, 0.86, '#eef3ff', [0, 2.0, 7.2], 0.92, 70, 1800, 1.30),

  open_replay: atmosphere('#0a141d', '#20364c', 12, 120, 0.50, 0.60, '#dbe5f7', [0, 0.72, 3.6], 0.62, 54, 1100, 1.20),

  close_replay: atmosphere('#0d1d2c', '#294861', 12, 220, 0.70, 0.82, '#e8efff', [0, 1.9, 7.2], 0.88, 80, 1800, 1.38),

  close_focus: atmosphere('#10273d', '#31627f', 12, 360, 0.88, 1.00, '#e5eeff', [0, 3.8, 15.0], 1.08, 140, 6200, 3.00),

  go_home: atmosphere('#03070c', '#04101a', 10, 52, 0.64, 0.78, '#bad0ff', [0, 3.2, 6.4], 0.88, 40, 920, 1.08),
}
...
function atmosphereLerp(from: AtmosphereSpec, to: AtmosphereSpec, t: number): AtmosphereSpec {
  return {
...
export function resolvePose(a?: CameraArg, b?: CameraArg): CameraPose {
  ...
  const hasNumericProgress = typeof a === 'number' || typeof b === 'number'
  if (phase === 'idle' || !hasNumericProgress) return POSES[phase]
...
export function resolveVeil(a?: CameraArg, b?: CameraArg): VeilSpec {
  const { phase, t } = parseArgs(a, b)
  const hasNumericProgress = typeof a === 'number' || typeof b === 'number'
  if (phase === 'idle' || !hasNumericProgress) return VEILS[phase]
  const [fromPhase, toPhase] = resolveBlendPair(phase)
...
export function resolveAtmosphere(a?: CameraArg, b?: CameraArg): AtmosphereSpec {
  const { phase, t } = parseArgs(a, b)
  const hasNumericProgress = typeof a === 'number' || typeof b === 'number'
  if (phase === 'idle' || !hasNumericProgress) return ATMOSPHERES[phase]
  const [fromPhase, toPhase] = resolveBlendPair(phase)
  return atmosphereLerp(ATMOSPHERES[fromPhase], ATMOSPHERES[toPhase], t)
}
