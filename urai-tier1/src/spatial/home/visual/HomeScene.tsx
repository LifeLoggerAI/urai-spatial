...
export default function HomeScene({ homeWorldState, state = "home", opening, enterLifeMap, onReplay, onUnwind, onFocus, narratorText, className }: HomeSceneProps) {
  ...
  return (
    ...
    >
      ...
      <section className="urai-horizon-system" data-testid="urai-home-horizon" aria-hidden="true">
        <div className="horizon-glow" /><div className="horizon-threshold" /><div className="terrain terrain-far" /><div className="mist mist-high" /><div className="terrain terrain-mid" /><div className="mist mist-mid" /><div className="terrain terrain-near" /><div className="mist mist-low" />
      </section>

      <section className="urai-ground" data-testid="urai-home-ground" aria-hidden="true">
        <div className="ground-plane ground-back" /><div className="ground-plane ground-mid" /><div className="root-network" data-testid="home-layer-root-network" /><div className="bloom-field" data-testid="home-layer-bloom-field" /><div className="ground-plane ground-front" /><div className="light-flecks" />
      </section>

      <section className="urai-hero" aria-label="Your symbolic avatar and orb companion">
        <div className="avatar-shadow" aria-hidden="true" /><div className="avatar-aura" aria-hidden="true" />
        <div className="avatar" data-testid="urai-home-avatar" role="img" aria-label="A calm symbolic avatar standing beneath the living orb"><div className="avatar-head" /><div className="avatar-neck" /><div className="avatar-shoulders" /><div className="avatar-body" /><div className="avatar-chest-glow" /><div className="avatar-leg leg-left" /><div className="avatar-leg leg-right" /></div>
        <div className="orb-reflection" aria-hidden="true" />
        <button type="button" className="orb-companion" onClick={enterLifeMap} disabled={mode === "enteringLifeMap"} aria-disabled={mode === "enteringLifeMap"} aria-label="Enter Life Map" data-testid="urai-orb-button"><span className="orb-halo" aria-hidden="true" /><span className="orb-core" aria-hidden="true" /><span className="orb-shine" aria-hidden="true" /><span className="orb-ring ring-a" aria-hidden="true" /><span className="orb-ring ring-b" aria-hidden="true" /><span className="orb-particles" aria-hidden="true">{Array.from({ length: 12 }, (_, index) => <i key={index} style={{ "--i": index } as React.CSSProperties} />)}</span></button>
      </section>
      ...
      <nav className="command-ribbon" data-testid="urai-command-ribbon" aria-label="Home World controls">
        <button type="button" onClick={enterLifeMap} disabled={mode === "enteringLifeMap"} aria-disabled={mode === "enteringLifeMap"} data-testid="home-control-lifemap">LifeMap</button>
        <button type="button" onClick={onReplay} disabled={mode === "enteringLifeMap"} aria-disabled={mode === "enteringLifeMap"} data-testid="home-control-replay">Replay</button>
        <button type="button" onClick={onUnwind} disabled={mode === "enteringLifeMap"} aria-disabled={mode === "enteringLifeMap"} data-testid="home-control-unwind">Mirror</button>
        {onFocus ? <button type="button" onClick={onFocus} disabled={mode === "enteringLifeMap"} aria-disabled={mode === "enteringLifeMap"} data-testid="home-control-focus">Focus</button> : null}
      </nav>
