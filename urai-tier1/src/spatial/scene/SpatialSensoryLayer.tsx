...
function ReadySpatialSensoryLayer({ materialPath, particlePath, loadingPath }: ReadySensoryPaths) {
  ...
  useFrame(({ clock }) => {
    ...
    if (loadingRing.current) {
      if (loadingStartedAt.current === null) loadingStartedAt.current = clock.elapsedTime
      const elapsedSinceMountMs = Math.max(0, (clock.elapsedTime - loadingStartedAt.current) * 1000)
      const progress = Math.min(1, elapsedSinceMountMs / loadingDurationMs)
