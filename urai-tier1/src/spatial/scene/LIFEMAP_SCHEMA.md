...
- Pulse speed = unresolved or recent emotional charge
- Aura radius = emotional impact
- Line thickness = edge `strength`
- Glow color = `emotionalTone` and `auraColor`
...
- Timeline, constellation, emotional weather, recovery, shadow, dream, relationship, chapter, and Mirror modes
- Memory detail card
...
The following functions are present in `lifeMapModel.ts` and currently return demo fallback data. Replace these bodies with Firestore calls when the production Firebase config is ready:
...
Paste this into Firebase Studio after pulling the branch:
