  ...
  switch (action.type) {
    ...
    case 'SET_GLOWING_STARS':
      return {
        ...
        stars: state.stars.map((star) => {
          ...
          if (action.ids.includes(star.id)) return { ...star, state: 'glowing', lastActivatedAt: Date.now() };
    ...
    case 'PUSH_PATTERN_MESSAGES': {
      ...
      const messages = unseen.reduce(
        (acc, insight) =>
          pushMessage(acc, createMessage('pattern', insight.message, 24000, ['visual', 'voice', 'haptic'], { voicePitch: 0.98 })),
        state.messages
      );
...
export default function LifeMapScene() {
  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      dispatch({
        type: 'PUSH_MESSAGE',
        msg: createMessage('glow', GLOW_LINES[Math.floor(rngRef.current() * GLOW_LINES.length)], 12000, ['visual', 'haptic']),
      });
      ...
      if (insights.length) {
        ...
        insights.forEach((insight) => {
          ...
          emitNarratorEvent({
            ...
            pattern: { id: insight.id, type: insight.type, strength: insight.strength },
          });
        });
      }
      ...
      emitTimelineSync({ phase: 'living', activeStarId: currentActiveStarId, activeChapterId: currentActiveChapterId });
    ...
    <main className="life-map-shell" aria-label="URAI Spatial Life Map scene">
            ...
            {state.stars
              ...
              .map(([a, b]) => {
                ...
                return (
                  <line
                    key={`${a}-${b}`}
                    className={`connection-line is-flowing ${isActive ? 'is-active' : activeStar ? 'is-dimmed' : 'is-glowing'}`}
                    x1={s1.x}
                    y1={s1.y}
                    x2={s2.x}
                    y2={s2.y}
      ...
      <aside className="panel companion-panel" aria-label="Narrator controls">
        ...
        <div className="toggle-row">
          <button type="button" className={state.voiceEnabled ? 'is-on' : ''} onClick={() => dispatch({ type: 'SET_VOICE_ENABLED', value: !state.voiceEnabled })}>
            Voice {state.voiceEnabled ? 'On' : 'Off'}
          </button>
          <button type="button" className={state.hapticsEnabled ? 'is-on' : ''} onClick={() => dispatch({ type: 'SET_HAPTICS_ENABLED', value: !state.hapticsEnabled })}>
