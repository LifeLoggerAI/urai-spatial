...
function makeStars(): MemoryStar[] {
  return Array.from({ length: 92 }, (_, index) => {
    ...
    return {
      ...
      x: 4 + randA * 92,
      y: 3 + randB * 50,
      size: 1.2 + ((index % 7) / 7) * 3.8,
      opacity: 0.25 + ((index % 11) / 11) * 0.55,
    ...
    <main className={`urai-spatial-shell phase-${phase}`}>
      ...
      {isMap ? (
        ...
        <section className="home-layer" aria-label="URAI home world">
          <button className="sky-hit" onClick={() => goto("lifemap")} aria-label="Enter the sky">
            ENTER THE SKY
          </button>
      ...
      {card ? (
        <section className="focus-card" aria-label={card.title}>
          ...
          <span>{card.copy}</span>
          <div>
            <button onClick={() => goto("replay")}>Replay</button>
            <button onClick={() => goto("lifemap")}>Unwind</button>
          </div>
        </section>
      ) : null}

      <nav className="bottom-dock" aria-label="Spatial actions">
        <button onClick={() => goto("lifemap")}>✦ LifeMap</button>
        <button onClick={() => goto("replay")}>⟳ Replay</button>
        <button onClick={() => goto("home")}>↺ Unwind</button>
      </nav>
      ...
      <style jsx>{`
        ...
        .cosmic-bg {
          background:
            radial-gradient(circle at 50% 20%, rgba(126, 181, 219, 0.52), transparent 30%),
