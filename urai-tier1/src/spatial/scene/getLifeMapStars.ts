...
export function getLifeMapStars(): { stars: LifeMapStar[]; source: "seed" | "fallback" } {
  ...
  for (let i = 0; i < 10; i += 1) {
    ...
    stars.push({ id: `major-${i}`, x: Math.cos(a) * (16 + (i % 3) * 3), y: 16 + Math.sin(a * 1.4) * 7, z: -165 - i * 11, r: 0.9, color: ["#9fd3ff", "#c6a7ff", "#ffd18e", "#7ce2ff", "#ff9fb8"][i % 5], tone: TONES[i % TONES.length], soft: 1, sort: i, title: `Memory ${i + 1}`, era: `Cycle ${i + 1}`, narrator: "A signal from your emotional field.", major: true })
  }
  for (let i = 0; i < 70; i += 1) {
    stars.push({ id: `bg-${i}`, x: ((i * 37) % 96) - 48, y: 4 + ((i * 23) % 38), z: -120 - ((i * 41) % 190), r: 0.06 + ((i * 13) % 8) * 0.01, color: "#8ea3c7", tone: "neutral", soft: 0.7, sort: 100 + i, title: "Background", era: "", narrator: "", major: false })
  }
