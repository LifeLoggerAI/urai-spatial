...
export function getLifeMapStars(): { stars: LifeMapStar[]; source: "seed" | "fallback" } {
  ...
  for (let i = 0; i < 10; i += 1) {
    ...
    stars.push({ id: `major-${i}`, x: Math.cos(a) * (12 + (i % 3) * 3), y: 13 + Math.sin(a * 1.4) * 5, z: -38 - i * 3.8, r: 1.35, color: ["#9fd3ff", "#c6a7ff", "#ffd18e", "#7ce2ff", "#ff9fb8"][i % 5], tone: TONES[i % TONES.length], soft: 1, sort: i, title: `Memory ${i + 1}`, era: `Cycle ${i + 1}`, narrator: "A signal from your emotional field.", major: true })
  }
  for (let i = 0; i < 70; i += 1) {
    stars.push({ id: `bg-${i}`, x: ((i * 37) % 48) - 24, y: 4 + ((i * 23) % 24), z: -30 - ((i * 41) % 58), r: 0.12 + ((i * 13) % 8) * 0.02, color: "#8ea3c7", tone: "neutral", soft: 0.7, sort: 100 + i, title: "Background", era: "", narrator: "", major: false })
  }
