...
function StarMesh({ star, sceneOpacity, dimmed, selected, interactive, onSelectStar }: {
  star: StarPoint;
  sceneOpacity: number;
  dimmed: boolean;
  selected: boolean;
  ...
  return (
    ...
    >
      <mesh ref={halo}>
        ...
        <meshBasicMaterial color={selected ? "#c9ddff" : "#8faeff"} transparent opacity={star.alpha * 0.25 * sceneOpacity} depthWrite={false} />
      </mesh>
      <mesh ref={core}>
        <sphereGeometry args={[star.radius, 14, 14]} />
        <meshBasicMaterial color={selected ? "#eef5ff" : "#dce7ff"} transparent opacity={star.alpha * sceneOpacity} depthWrite={false} />
      </mesh>
...
export default function LifeMap({ phase, progress, opacity, selectedStar, onSelectStar }: Props) {
  ...
  const continuityProgress = phase === "enter_ascent" || phase === "enter_separation" || phase === "enter_arrival" ? Math.max(0, Math.min(1, progress)) : 1;
  const continuityEase = continuityProgress * continuityProgress * (3 - 2 * continuityProgress);
  const lifeMapContinuityY = phase === "enter_ascent" ? 3.8 * (1 - continuityEase) : phase === "enter_separation" ? 1.35 * (1 - continuityEase) : 0;
  const lifeMapContinuityZ = phase === "enter_ascent" ? -10.5 * (1 - continuityEase) : phase === "enter_separation" ? -3.2 * (1 - continuityEase) : 0;
  const lifeMapContinuityScale = phase === "enter_ascent" ? 0.86 + 0.14 * continuityEase : phase === "enter_separation" ? 0.96 + 0.04 * continuityEase : 1;
  ...
  return (
    <group visible={sceneOpacity > 0.001} position={[0, lifeMapContinuityY, lifeMapContinuityZ]} scale={lifeMapContinuityScale}>
      {stars.map((star) => {
        const dimmed = !!selectedStar && selectedStar.id !== star.id && ["focus_lock", "focus_travel", "focus_arrive", "REPLAY", "return_from_replay", "return_to_lifemap"].includes(phase);
        return <StarMesh key={star.id} star={star} sceneOpacity={sceneOpacity} dimmed={dimmed} selected={selectedStar?.id === star.id} interactive={interactive} onSelectStar={onSelectStar} />;
      })}
    </group>
  );
}
