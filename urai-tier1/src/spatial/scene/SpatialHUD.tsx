...
export default function SpatialHUD({
  ...
  const hideDiagnosticLabels = publicDemoMode || recordingMode;

  return (
    <div style={root}>
      {!hideDiagnosticLabels && (
      ...
      <div style={panelBottom}>
        <button aria-label={primaryAriaLabel} style={{ ...btn, opacity: isBusy ? 0.6 : 1 }} disabled={isBusy} onClick={onOpen}>
          {primaryLabel}
        </button>

        {canBack ? <button aria-label="Go Back" style={btn} onClick={onBack}>Back / Escape</button> : null}
        {canReplay ? <button aria-label="Enter Replay" style={btn} onClick={onReplay}>Enter Replay</button> : null}
        {!hideDiagnosticLabels && <span style={hintText}>{helperText}</span>}
      </div>
    </div>
  );
}

const root: CSSProperties = { position: "absolute", inset: 0, pointerEvents: "none", zIndex: 20 };
const glass: CSSProperties = { backdropFilter: "blur(18px)", WebkitBackdropFilter: "blur(18px)", background: "rgba(8,12,28,.52)", border: "1px solid rgba(170,190,255,.22)", boxShadow: "0 18px 60px rgba(0,0,0,.36)", borderRadius: 20, pointerEvents: "auto" };
...
const eyebrow: CSSProperties = { fontSize: 11, letterSpacing: ".14em", textTransform: "uppercase", color: "#b9e7ff" };
const phaseText: CSSProperties = { fontSize: 13, color: "#eaf2ff", marginTop: 4 };
const statusText: CSSProperties = { fontSize: 12 };
const hintText: CSSProperties = { fontSize: 11, opacity: 0.82 };
