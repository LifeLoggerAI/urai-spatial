...
type Props = { phaseLabel: string; starCount: number; memoryTitle: string; source: string; canReplay: boolean; canBack: boolean; isBusy: boolean; onOpen:()=>void; onBack:()=>void; onReplay:()=>void };

export default function SpatialHUD({ phaseLabel, starCount, memoryTitle, source, canReplay, canBack, isBusy, onOpen, onBack, onReplay }: Props) {
  ...
  return <div style={{position:"absolute", inset:0, pointerEvents:"none", zIndex:20}}>
    ...
    <div style={panelTopRight}><div style={{fontSize:12}}>{starCount} memory stars</div><div style={{fontSize:12}}>Memory: {memoryTitle}</div><div style={{fontSize:12}}>{source === "seed" ? "Demo constellation" : source}</div></div>
    <div style={panelBottom}><button aria-label="Open LifeMap" style={{...btn, opacity: isBusy ? .6 : 1}} disabled={isBusy} onClick={onOpen}>{primaryLabel}</button>{canBack ? <button aria-label="Go Back" style={btn} onClick={onBack}>Back / Escape</button> : null}{canReplay ? <button aria-label="Enter Replay" style={btn} onClick={onReplay}>Enter Replay</button> : null}{canBack ? <span style={{fontSize:11,opacity:.82}}>ESC to return</span> : <span style={{fontSize:11,opacity:.82}}>Tap a star to open a memory</span>}</div>
  </div>
}
