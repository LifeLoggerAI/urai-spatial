...
export function buildSpatialCuratedDeckDiff(input: {
  ...
  const sceneModeShiftCount = countSceneModeShifts(baseDeck.cards, targetDeck.cards);
  ...
  const parts = [
    ...
    `card delta ${cardCountDelta}`,
    `scene mode shifts ${sceneModeShiftCount}`,
    `selected star shifts ${selectedStarShiftCount}`,
  ];
