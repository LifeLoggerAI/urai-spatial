...
function toOperatorText(input: {
  ...
  return `Run ${input.purgePosture} purge posture · delete ${input.deletionEligibility} · cold-safe ${input.coldStorageSafe ? "yes" : "no"}.`;
}
...
export function buildSpatialCuratedDeckPurgeReadiness(input: {
  ...
  const parts = [
    `purge ${purgeScore}`,
    `posture ${purgePosture}`,
    `delete ${deletionEligibility}`,
    `cold-safe ${coldStorageSafe ? "yes" : "no"}`,
    `expiry ${expiry.expiryPosture}`,
    `horizon ${horizon.storageHorizon}`,
    `reopen ${reopenWatch.reopenState}`,
