...
export default function ReplayScene({
  ...
  useFrame((state, delta) => {
    ...
    if (rootRef.current) {
      ...
      rootRef.current.position.z = THREE.MathUtils.damp(rootRef.current.position.z, -0.5 - gate * 1.1, 3.0, delta);
    ...
    if (coreRef.current) {
      ...
      coreRef.current.position.z = THREE.MathUtils.damp(coreRef.current.position.z, -1.6 - gate * 0.9, 4.2, delta);
    ...
    if (hazeRef.current) {
      ...
      hazeRef.current.position.z = THREE.MathUtils.damp(hazeRef.current.position.z, -1.2 - gate * 0.5, 3.0, delta);
  ...
  return (
    <group {...props}>
      <fog attach="fog" args={["#020710", 2.5, 16]} />
<ambientLight intensity={0.45 + opacity * 0.35} />
      ...
      <mesh ref={shellRef} position={[0, 0, -2.2]}>
        ...
        <meshBasicMaterial color="#6b4cff" transparent opacity={0.075} side={THREE.BackSide} depthWrite={false}  />
      </mesh>
      ...
      <mesh position={[0, -1.1, -3.8]} rotation={[-Math.PI / 2, 0, 0]}>
        ...
        <meshBasicMaterial color="#090611" transparent opacity={0.075} + opacity * 0.18} depthWrite={false}  />
      </mesh>
    </group>
  );
}
