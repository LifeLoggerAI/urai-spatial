...
export default function Starfield(props: {
  ...
  return <group position={[0, props.yOffset ?? 0, props.zOffset ?? 0]} scale={props.worldScale ?? 1}>
    {links.map(([a, b], i) => <line key={`link-${i}`}>
      ...
      <lineBasicMaterial color="#8eb3ff" transparent opacity={0.34 * (props.opacity ?? 1)} depthWrite={false} toneMapped={false} />
    </line>)}
    ...
    {allStars.map((star, idx) => {
      ...
      return <group key={star.id}>
        <mesh
          ...
          <meshBasicMaterial color={isSelected ? "#ffe27a" : isHovered ? "#d7eeff" : star.color} transparent opacity={(props.opacity ?? 1) * dim * (star.major ? 0.95 : 0.42)} depthWrite={false} toneMapped={false} />
        </mesh>
        {star.major ? <mesh position={position} renderOrder={2}>
          <sphereGeometry args={[rad * 1.9, 14, 14]} />
          <meshBasicMaterial color={isSelected ? "#fff5bf" : star.color} transparent opacity={isSelected ? 0.62 : isHovered ? 0.44 : 0.32} depthWrite={false} toneMapped={false} blending={THREE.AdditiveBlending} />
        </mesh> : null}
        {star.major && (isSelected || isHovered) ? <mesh position={[position[0], position[1] + 1.3, position[2]]} renderOrder={4}>
          <planeGeometry args={[2.8, 0.48]} />
          <meshBasicMaterial color={isSelected ? "#fff4c4" : "#b9d7ff"} transparent opacity={0.88} toneMapped={false} depthWrite={false} />
        </mesh> : null}
      </group>
    })}
  </group>
