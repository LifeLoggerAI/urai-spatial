...
export default function Starfield(props: {
  ...
  return <group position={[0, props.yOffset ?? 0, props.zOffset ?? 0]} scale={props.worldScale ?? 1}>
    {links.map(([a, b], i) => <line key={`link-${i}`}>
      ...
      <lineBasicMaterial color="#6f8bc6" transparent opacity={0.28 * (props.opacity ?? 1)} />
    </line>)}
    ...
    {allStars.map((star, idx) => {
      ...
      const dim = selected && !isSelected ? 0.25 : 1
      ...
      return <group key={star.id}>
        <mesh
          ...
          <meshBasicMaterial color={isSelected ? "#ffe27a" : star.color} transparent opacity={(props.opacity ?? 1) * dim * (star.major ? 0.95 : 0.52)} />
        </mesh>
