...
export function HomeAtmosphericSky({ reducedMotion }: { reducedMotion: boolean }) {
  ...
  const material = useMemo(() => new THREE.ShaderMaterial({
    ...
    uniforms: {
      uTime: { value: 0 },
      uZenith: { value: new THREE.Color('#163842') },
      uHorizon: { value: new THREE.Color('#9a9d83') },
      uCloud: { value: new THREE.Color('#729995') },
    },
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        vec3 sky=mix(uHorizon,uZenith,smoothstep(-.03,.36,direction.y));
        vec3 p=direction*vec3(6.,24.,6.)+vec3(uTime*.002,0.,0.);
        float cloud=noise(p)+.45*noise(p*2.03+2.1)+.18*noise(p*4.1-3.);
        float wisps=smoothstep(.72,1.15,cloud)*smoothstep(.025,.12,elevation)*(1.-smoothstep(.55,.94,elevation));
        sky=mix(sky,uCloud,wisps*.48);
        float warmth=pow(max(0.,dot(direction,normalize(vec3(-.65,.16,-.74)))),8.);
        sky+=vec3(.12,.052,.015)*warmth;
