...
import type { SpatialCuratedDeckRetentionLifecycleSignal } from "@/spatial/curation/spatialCuratedDeckRetentionLifecycleTypes";
...
export default function SpatialCuratedDeckRetentionLifecyclePanel() {
  ...
  const retention = useMemo(
    () => buildSpatialCuratedDeckRetentionLifecycle({ entries: vaultEntries, activeEntryId }),
    [entries, activeEntryId],
  );

  const activeSignals = useMemo(
    () => retention.signals.filter((signal) => signal.active).slice(0, 4),
    [retention.signals],
  );
  ...
  return (
    ...
    >
      ...
      >
        Curated Deck Retention Lifecycle
      </div>
      ...
      >
        {retention.summaryText}
      </div>

      <div style={chipRowStyle}>
        <MetricChip label="cadence" value={retention.reviewCadence} />
        <MetricChip label="path" value={retention.retentionPath} />
      </div>

      <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
        {activeSignals.length === 0 ? (
          <div style={{ fontSize: 12, lineHeight: 1.45, opacity: 0.82 }}>
            No active retention-lifecycle signals for the selected curated deck vault entry.
          </div>
        ) : (
          activeSignals.map((signal) => (
            <RetentionLifecycleSignalRow key={signal.id} signal={signal} />
          ))
        )}
      </div>
    </div>
...
function RetentionLifecycleSignalRow(input: {
  signal: SpatialCuratedDeckRetentionLifecycleSignal;
}) {
  return (
    <div
      style={{
    ...
    >
      ...
      >
        ...
        <span style={{ opacity: 0.72 }}>{input.signal.weight}</span>
      </div>
