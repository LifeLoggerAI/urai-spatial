...
export default function SpatialCuratedDeckDivergencePanel() {
  ...
  const divergence = useMemo(
    () => buildSpatialCuratedDeckDivergence({ entries: vaultEntries, activeEntryId }),
    [vaultEntries, activeEntryId],
  );
  const bandLabel =
    divergence.divergenceBand === "aligned"
      ? "Aligned"
      : divergence.divergenceBand === "offset"
        ? "Offset"
        : "Outlier";
  ...
  return (
    ...
    >
      ...
      >
        {divergence.summaryText}
      </div>

      <div style={chipRowStyle}>
        <MetricChip label="cohort" value={String(divergence.cohortSize)} />
        <MetricChip label="avg cards" value={String(divergence.averageCardCount)} />
        <MetricChip label="Δ cards" value={String(divergence.cardCountDeltaFromAverage)} />
      </div>

      <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
        <SignalRow
          label="dominant source"
          value={divergence.dominantSource ?? "n/a"}
          aligned={divergence.activeMatchesDominantSource}
        />
        <SignalRow
          label="common first card"
          value={divergence.commonFirstCardEntryId ?? "n/a"}
          aligned={divergence.activeMatchesCommonFirstCard}
        />
        <SignalRow
          label="dominant first scene"
          value={divergence.dominantFirstSceneMode ?? "n/a"}
          aligned={divergence.activeMatchesDominantFirstSceneMode}
        />
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ opacity: 0.72 }}>{input.aligned ? "aligned" : "offset"}</span>
      </div>
