...
import type { SpatialCuratedDeckActionItem } from "@/spatial/curation/spatialCuratedDeckActionTypes";
...
export default function SpatialCuratedDeckActionPanel() {
  ...
  const action = useMemo(
    () => buildSpatialCuratedDeckAction({ entries: vaultEntries, activeEntryId }),
    [vaultEntries, activeEntryId],
  );
  const activeItems = useMemo(
    () => action.items.filter((item) => item.active).slice(0, 4),
    [action.items],
  );
  ...
  return (
    ...
    >
      ...
      >
        next step: {action.recommendedAction}
      </div>

      <div style={chipRowStyle}>
        <MetricChip label="items" value={String(action.activeItemCount)} />
        <MetricChip label="entries" value={String(action.totalEntries)} />
      </div>

      <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
        {activeItems.length === 0 ? (
          <div style={{ fontSize: 12, lineHeight: 1.45, opacity: 0.82 }}>
            No active operator action items for the selected curated deck vault entry.
          </div>
        ) : (
          activeItems.map((item) => <ActionRow key={item.id} item={item} />)
        )}
      </div>
    </div>
  );
...
function ActionRow(input: { item: SpatialCuratedDeckActionItem }) {
  return (
    <div
      style={{
        borderRadius: 12,
    ...
    >
      ...
      >
        ...
        <span style={{ opacity: 0.72 }}>{input.item.priority}</span>
      </div>
