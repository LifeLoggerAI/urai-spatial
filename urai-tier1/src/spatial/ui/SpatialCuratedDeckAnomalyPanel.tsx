...
import type { SpatialCuratedDeckAnomalyFlag } from "@/spatial/curation/spatialCuratedDeckAnomalyTypes";
...
export default function SpatialCuratedDeckAnomalyPanel() {
  ...
  const anomaly = useMemo(
    () => buildSpatialCuratedDeckAnomaly({ entries: vaultEntries, activeEntryId }),
    [vaultEntries, activeEntryId],
  );
  const bandLabel =
    anomaly.anomalyBand === "clear"
      ? "Clear"
      : anomaly.anomalyBand === "watch"
        ? "Watch"
        : "Anomalous";

  const activeFlags = useMemo(
    () => anomaly.flags.filter((flag) => flag.active),
    [anomaly.flags],
  );
  ...
  return (
    ...
    >
      ...
      <div style={chipRowStyle}>
        <MetricChip label="flags" value={String(anomaly.flagCount)} />
        <MetricChip label="total" value={String(anomaly.totalEntries)} />
      </div>

      <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
        {activeFlags.length === 0 ? (
          <div style={{ fontSize: 12, lineHeight: 1.45, opacity: 0.82 }}>
            No active anomaly flags for the selected curated deck vault entry.
          </div>
        ) : (
          activeFlags.map((flag) => <FlagRow key={flag.id} flag={flag} />)
        )}
      </div>
    </div>
  );
...
function FlagRow(input: { flag: SpatialCuratedDeckAnomalyFlag }) {
  return (
    <div
      style={{
        borderRadius: 12,
    ...
    >
      ...
      >
        ...
        <span style={{ opacity: 0.72 }}>{input.flag.severity}</span>
      </div>
