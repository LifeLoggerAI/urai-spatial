  ...
  return (
    ...
    >
      ...
      <div style={chipRowStyle}>
        <MetricChip label="state" value={gate.recoveryState} />
        <MetricChip label="conditions" value={String(gate.activeConditionCount)} />
      </div>

      <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
        {activeConditions.length === 0 ? (
          <div style={{ fontSize: 12, lineHeight: 1.45, opacity: 0.82 }}>
            No active recovery-gate conditions for the selected curated deck vault entry.
          </div>
        ) : (
          activeConditions.map((condition) => (
            <RecoveryGateConditionRow key={condition.id} condition={condition} />
          ))
        )}
      </div>
    </div>
...
function RecoveryGateConditionRow(input: {
  condition: SpatialCuratedDeckRecoveryGateCondition;
}) {
  return (
    <div
      style={{
    ...
    >
      ...
      >
        ...
        <span style={{ opacity: 0.72 }}>{input.condition.status}</span>
      </div>
