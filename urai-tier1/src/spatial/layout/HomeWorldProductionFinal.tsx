...
function SanctuaryCourt({ target }: { target: MutableRefObject<THREE.Vector3 | null> }) {
  ...
  const pack = useStonePack(0.88, 1.12)
  ...
  return <group name="home-authored-terrain" userData={{ treatment: 'built-photographic-obsidian-court-v19', source: HOME_PHOTOGRAPHIC_PBR_V19 }}>
    ...
    <mesh name="home-obsidian-walkable-terrain" position={[0,0.002,-1.45]} rotation={[-Math.PI / 2,0,0]} receiveShadow><planeGeometry args={[16.45,19.1,1,1]} /><StoneTopMaterial pack={pack} color="#2c3331" relief={0.009} /></mesh>
    <mesh position={[0,0.016,-2.15]} rotation={[-Math.PI / 2,0,0]} receiveShadow><circleGeometry args={[1.78,128]} /><meshPhysicalMaterial color="#101615" roughness={0.42} metalness={0.12} clearcoat={0.42} clearcoatRoughness={0.32} envMapIntensity={1.05} /></mesh>
    <mesh position={[0,0.022,-2.15]} rotation={[-Math.PI / 2,0,0]}><ringGeometry args={[1.18,1.194,160]} /><meshStandardMaterial color="#83785f" emissive="#3e3526" emissiveIntensity={0.028} metalness={0.86} roughness={0.3} /></mesh>
    <mesh position={[0,0.024,-2.15]} rotation={[-Math.PI / 2,0,0]}><ringGeometry args={[1.54,1.553,160]} /><meshStandardMaterial color="#557779" emissive="#27494b" emissiveIntensity={0.035} metalness={0.76} roughness={0.32} /></mesh>
...
function PhysicalEnvironment(){return <Environment files={HOME_HDR} background={false} environmentIntensity={0.62} />}
