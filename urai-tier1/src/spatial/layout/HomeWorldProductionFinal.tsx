  ...
  return (
    <div aria-hidden="true" style={{ position: "absolute", inset: 0, zIndex: 0, overflow: "hidden", background: "#061015" }}>
      <picture>
        ...
        <img
          src={CINEMATIC_HOME_ART}
          alt=""
          draggable={false}
          style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", objectPosition: "center 48%", filter: "brightness(.76) saturate(.9) contrast(1.06)", transform: "scale(1.025)" }}
        />
      </picture>
...
function Ridge({ z, width, height, opacity }: { z: number; width: number; height: number; opacity: number }) {
  ...
  return (
    <mesh geometry={geometry} position={[0, 0, z]}>
      <meshStandardMaterial color="#07191d" transparent opacity={opacity} roughness={1} side={THREE.DoubleSide} />
    </mesh>
  );
}
...
function NaturalHorizon() {
  return (
    <group name="home-mountain-horizon" userData={{ role: "layered-atmospheric-depth" }}>
      <Ridge z={-34} width={90} height={10} opacity={0.32} />
      <Ridge z={-52} width={128} height={16} opacity={0.22} />
    </group>
  );
}
...
function LivingGround({ reducedMotion }: { reducedMotion: boolean }) {
  ...
  useFrame(({ clock }) => {
    ...
    ref.current.position.y = Math.sin(clock.elapsedTime * 0.26) * 0.035;
  });
  return (
    <group name="home-living-vegetation" userData={{ role: "subtle-living-ground" }}>
      <points ref={ref} geometry={geometry}>
        <pointsMaterial color="#d3f4c7" size={0.045} transparent opacity={0.45} depthWrite={false} toneMapped={false} />
      </points>
    </group>
  );
}
...
function ReflectingWater() {
  return (
    <group name="home-reflecting-water" position={[4.9, terrainHeight(4.9, -7.2) + 0.02, -7.2]}>
      <mesh rotation={[-Math.PI / 2, 0, 0]} scale={[1.5, 1, 1]}>
        <circleGeometry args={[2.5, 128]} />
        <meshPhysicalMaterial color="#0e3a46" roughness={0.06} clearcoat={1} clearcoatRoughness={0.08} transparent opacity={0.54} />
      </mesh>
    </group>
  );
}
...
function OrbDust({ reducedMotion }: { reducedMotion: boolean }) {
  ...
  return (
    <points ref={ref} geometry={geometry}>
      <pointsMaterial color="#c9fff1" size={0.028} transparent opacity={0.48} depthWrite={false} toneMapped={false} />
    </points>
  );
}
...
function OrbSanctuary({ onOpen, reducedMotion }: { onOpen: () => void; reducedMotion: boolean }) {
  ...
  return (
    <group name="home-orb-sanctuary" ref={root} position={ORB} onClick={(event) => { event.stopPropagation(); if (!useSceneStore.getState().inputLocked) onOpen(); }}>
      <mesh scale={0.78}>
        <sphereGeometry args={[1, 64, 64]} />
        <meshPhysicalMaterial color="#d5fff4" emissive="#67c7af" emissiveIntensity={0.5} roughness={0.06} transmission={0.42} clearcoat={1} transparent opacity={0.42} />
      </mesh>
      <mesh ref={core} scale={0.38}>
        <sphereGeometry args={[1, 56, 56]} />
        <meshStandardMaterial color="#ffe2a6" emissive="#efaa50" emissiveIntensity={3.2} roughness={0.12} toneMapped={false} />
      </mesh>
...
function LifeMapSkyLookout({ onEnter }: { onEnter: () => void }) {
  return (
    <group name="home-life-map-sky-lookout" position={LIFE_MAP_LOOKOUT} userData={{ transition: "sky-ascent" }}>
      <mesh position={[0, 0.7, 0]} onClick={(event) => { event.stopPropagation(); onEnter(); }}>
        <boxGeometry args={[3.8, 2.4, 3.8]} />
        <meshBasicMaterial transparent opacity={0} depthWrite={false} colorWrite={false} />
      </mesh>
...
function EmbodiedPresence({ root }: { root: MutableRefObject<THREE.Group | null> }) {
  return (
    <group ref={root} name="home-authored-embodied-self" position={SPAWN}>
      <mesh position={[0, 0.01, 0.25]} rotation={[-Math.PI / 2, 0, 0]} scale={[0.42, 1, 1]}>
        <circleGeometry args={[0.38, 40]} />
        <meshBasicMaterial color="#000806" transparent opacity={0.08} depthWrite={false} />
      </mesh>
    </group>
  );
}
...
function PlayerRig({ input, yaw, pitch, target, avatar, onNearby, groundDescent, onGroundComplete, reducedMotion }: {
  ...
  useFrame(({ clock }, delta) => {
    ...
    const candidates: readonly [Nearby, THREE.Vector3, number][] = [
      ["orb", ORB, 2.2],
...
function HomeScene(props: {
  ...
  return (
    <>
      ...
      <ReflectingWater />
      <OrbSanctuary onOpen={props.onOrbOpen} reducedMotion={props.reducedMotion} />
...
export function HomeWorldProductionFinal({ onOrbOpen = requestUraiWorldOrbOpen, webglAvailable = true }: Props) {
  ...
  return (
    <main
      ...
      data-home-runtime-assets="launch-key-art.webp local-three-dimensional-terrain living-orb reflecting-water"
      ...
      data-home-orb-clip={ORB_CLIPS[orbState]}
      ...
      style={{ position: "relative", overflow: "hidden", background: "#061015" }}
      {...look}
    >
      <CinematicBackdrop />
      <div style={{ position: "absolute", inset: 0, zIndex: 1 }}>
        <Canvas
          ...
          dpr={[1, 1.3]}
          shadows
          camera={{ position: [0, 1.7, 8], fov: 50, near: 0.05, far: 300 }}
