...
const HOME_GROUND_MODEL = "/assets/urai/generated/models/ground-world-terrain-v1.glb";
...
function prepared(source: THREE.Object3D, role: "sanctuary" | "ground" | "orb") {
  const clone = source.clone(true);
  clone.traverse((object) => {
    if (!(object instanceof THREE.Mesh)) return;
    const name = object.name.toLowerCase();
    if (role !== "orb" && /portal|ring|debug|marker|label|mannequin|avatar|beam|monolith|threshold/.test(name)) object.visible = false;
    if (!object.visible) return;
    object.castShadow = true;
    object.receiveShadow = true;
    if (role === "sanctuary") object.material = new THREE.MeshStandardMaterial({ color: /stone|path|ground|basin/.test(name) ? "#59655d" : "#45584d", roughness: 0.94, metalness: 0 });
    if (role === "ground") object.material = new THREE.MeshStandardMaterial({ color: /stone|rock/.test(name) ? "#586158" : "#36513c", roughness: 0.98, metalness: 0 });
  });
  return clone;
}
...
function WorldGeometry({ target }: { target: MutableRefObject<THREE.Vector3 | null> }) {
  ...
  const groundSource = useGLTF(HOME_GROUND_MODEL);
  const sanctuary = useMemo(() => prepared(sanctuarySource.scene, "sanctuary"), [sanctuarySource.scene]);
  const ground = useMemo(() => {
    const object = prepared(groundSource.scene, "ground");
    const box = new THREE.Box3().setFromObject(object);
    if (!box.isEmpty()) {
      const size = box.getSize(new THREE.Vector3());
      const center = box.getCenter(new THREE.Vector3());
      const scale = Math.min(1.1, 23 / Math.max(size.x, size.z, 1));
      object.scale.setScalar(scale);
      object.position.set(-center.x * scale, -box.min.y * scale - 0.35, -4.4 - center.z * scale);
    }
    return object;
  }, [groundSource.scene]);
  ...
  return <group name="home-authored-terrain"><primitive object={ground} /><primitive object={sanctuary} /><mesh name="home-natural-terrain" geometry={TERRAIN} receiveShadow onClick={walk}><meshStandardMaterial color="#3d6047" roughness={0.99} /></mesh><mesh name="home-walkable-navigation-surface" rotation={[-Math.PI/2,0,0]} position={[0,0.55,-2]} onClick={walk}><planeGeometry args={[25,28]} /><meshBasicMaterial transparent opacity={0} depthWrite={false} colorWrite={false} /></mesh></group>;
}

const FERNS = [[-7,-5,.85,-.3],[-6,-8,1.05,.7],[-5,-11,.9,1.8],[-3.8,-4,.74,-1.4],[-3,-9,1.12,2.3],[-2,-12,.9,.2],[2.4,-4,.8,.9],[3.3,-9,1.08,-.6],[4.2,-11,.9,1.4],[5.7,-5,.8,2.6],[6.5,-8,1.03,-1.8],[7.2,-11,.9,.4],[-8,1,.8,1.1],[-6,2.5,.75,-2.2],[6,2,.78,2],[8,.2,.86,-.8]] as const;
function Nature() {
...
function AuthoredOrb({ onOpen, reducedMotion }: { onOpen: () => void; reducedMotion: boolean }) {
  ...
  const object = useMemo(() => { const o=prepared(source.scene,"orb"); o.scale.setScalar(.46); return o; }, [source.scene]);
  const root = useRef<THREE.Group>(null);
  const light = useRef<THREE.PointLight>(null);
  useFrame(({clock}) => { if(reducedMotion) return; const breathe=1+Math.sin(clock.elapsedTime*.82)*.012; root.current?.scale.setScalar(breathe); if(light.current) light.current.intensity=.7+Math.sin(clock.elapsedTime*.82)*.08; });
  return <group ref={root} name="home-orb-sanctuary" position={ORB} onClick={e=>{e.stopPropagation();onOpen();}} userData={{geometryOwner:"urai-orb-avatar-v1.glb",primitiveSphere:false}}><primitive object={object}/><pointLight ref={light} color="#9ad1cb" intensity={.7} distance={5.5} decay={2}/></group>;
}
...
function Scene({ input,yaw,pitch,target,avatar,onNearby,onOrb,onGround,onGroundComplete,onLifeMap,onReady,groundDescent,reducedMotion }: any) {
  ...
  return <><color attach="background" args={[cosmic?"#01050b":"#183033"]}/><Stars radius={190} depth={90} count={cosmic?2500:180} factor={cosmic?2.8:.48} saturation={.12} fade speed={reducedMotion?0:.018}/><fogExp2 attach="fog" args={[cosmic?"#050b14":"#183033",cosmic?.0017:.011]}/><ambientLight intensity={cosmic?.12:.58} color="#d2ded6"/><hemisphereLight args={["#bfd7d2","#17251a",cosmic?.2:.88]}/><directionalLight position={[8,16,7]} intensity={cosmic?.32:2} color="#eef3e8" castShadow/><WorldGeometry target={target}/><Nature/><AuthoredOrb onOpen={onOrb} reducedMotion={reducedMotion}/><Presence root={avatar}/><Landmarks onGround={onGround} onLifeMap={onLifeMap}/><Rig input={input} yaw={yaw} pitch={pitch} target={target} avatar={avatar} onNearby={onNearby} groundDescent={groundDescent} onGroundComplete={onGroundComplete} reducedMotion={reducedMotion}/>{!cosmic?<ContactShadows position={[0,-.08,-2.5]} opacity={.18} scale={28} blur={5} far={14} resolution={512} frames={1}/>:null}</>;
}
