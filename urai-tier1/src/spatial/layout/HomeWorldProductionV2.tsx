...
const ORB = new THREE.Vector3(0, 1.18, -3.35);
...
function terrainHeight(x: number, z: number) {
  const rolling = Math.sin(x * .072) * .34 + Math.cos(z * .061) * .29 + Math.sin((x + z) * .038) * .18;
  const detail = Math.sin(x * .31 + z * .17) * .045 + Math.cos(z * .27 - x * .13) * .034;
  const clearing = -Math.exp(-((x / 8.8) ** 2 + ((z + 1.2) / 10.7) ** 2)) * .16;
  const distance = THREE.MathUtils.clamp((-z - 8) / 54, 0, 1.55);
  const ridgeProfile = 7.4 + Math.sin(x * .052) * 2.1 + Math.cos(x * .103) * 1.05 + Math.sin(x * .021 + 1.4) * 1.25;
  const ridge = Math.pow(distance, 1.45) * ridgeProfile;
  const sideLift = Math.pow(Math.min(1, Math.abs(x) / 42), 1.8) * 1.7;
  return rolling + detail + clearing + ridge + sideLift - .16;
}

function makeTerrain() {
  const geometry = new THREE.PlaneGeometry(190, 190, 260, 260);
  geometry.rotateX(-Math.PI / 2);
  const position = geometry.attributes.position as THREE.BufferAttribute;
  const colors = new Float32Array(position.count * 3);
  const low = new THREE.Color("#203a2e");
  const high = new THREE.Color("#6a8467");
  const color = new THREE.Color();
  for (let i = 0; i < position.count; i += 1) {
    const x = position.getX(i), z = position.getZ(i), y = terrainHeight(x, z);
    position.setY(i, y);
    const variation = THREE.MathUtils.clamp(.43 + Math.sin(x * .17) * .09 + Math.cos(z * .14) * .09 + y * .038, 0, 1);
...
function prepareAuthoredSanctuary(source: THREE.Object3D) {
  ...
  const rejectedStageForms = /sanctuary-terrain|sanctuary-inner-earth|sanctuary-foreground-landing|mirror-basin|orb-pedestal|sanctuary-heart-light/i;
  ...
  world.userData.suppressedBasinStage = true;
...
const FERN_PATCHES = [
  [-9.4,5.2,.68,-.8],[-8.1,3.3,.92,.6],[-6.7,4.1,.74,1.9],[-5.4,2.5,.82,-2.3],[5.6,3.5,.76,.8],[7.2,4.8,.88,-1.5],[8.6,2.7,.72,2.5],[9.5,5.4,.66,-.2],
  [-9.2,.5,.78,1.3],[-7.8,-1.8,.96,-.5],[-6.6,-4.8,.82,2.2],[-5.8,-7.5,1.08,.3],[-4.8,-10.6,.86,-1.7],[-3.6,-12.4,.72,2.8],
  [9.1,.2,.8,-1.1],[7.7,-2.2,.94,1.7],[6.5,-5.1,.84,-2.4],[5.7,-7.8,1.06,.5],[4.7,-10.8,.88,2.1],[3.5,-12.7,.74,-.9],
  [-4.5,-2.5,.66,.2],[-3.7,-5.7,.82,1.5],[-2.8,-9.5,.72,-2.7],[2.9,-2.9,.68,2.8],[3.8,-6.2,.84,-1.2],[2.6,-10.4,.76,.7],
  [-10.6,-4.2,.72,-2.1],[-10.2,-8.8,.9,.9],[-8.9,-12.8,.78,2.6],[10.7,-4.7,.74,1.2],[10.1,-9.1,.92,-2.4],[8.8,-13.1,.8,.4],
] as const;
...
function ReflectingWater() {
  return <group name="home-reflecting-water" position={[4.9, terrainHeight(4.9, -7.2) + .018, -7.2]}><mesh rotation={[-Math.PI / 2, 0, 0]}><circleGeometry args={[2.35, 64]} /><meshPhysicalMaterial color="#173f42" roughness={.1} metalness={0} clearcoat={1} clearcoatRoughness={.12} transparent opacity={.62} /></mesh></group>;
}
...
function AuthoredOrb({ onOpen, reducedMotion, state }: { onOpen: () => void; reducedMotion: boolean; state: OrbState }) {
  ...
  useFrame(({ clock }, delta) => {
    ...
    if (light.current) light.current.intensity = .82 + Math.sin(clock.elapsedTime * .82) * .08;
  });
  return <group ref={root} name="home-orb-sanctuary" position={ORB} onClick={(event) => { event.stopPropagation(); onOpen(); }} userData={{ geometryOwner: "urai-orb-avatar-v1.glb", primitiveSphere: false }}><primitive object={object} /><pointLight ref={light} color="#a8ded5" intensity={.82} distance={5.2} decay={2} /></group>;
}
...
function Rig({ input, yaw, pitch, target, avatar, onNearby, groundDescent, onGroundComplete, reducedMotion }: { input: MovementInput; yaw: MutableRefObject<number>; pitch: MutableRefObject<number>; target: MutableRefObject<THREE.Vector3 | null>; avatar: MutableRefObject<THREE.Group | null>; onNearby: (nearby: Nearby) => void; groundDescent: boolean; onGroundComplete: () => void; reducedMotion: boolean }) {
  ...
  const place = useCallback(() => {
    ...
    camera.lookAt(look.x, 1.02 + pitch.current, look.z);
...
function Scene({ input, yaw, pitch, target, avatar, onNearby, onOrb, onGround, onGroundComplete, onLifeMap, onReady, groundDescent, reducedMotion, orbState }: any) {
  ...
  return <><color attach="background" args={[cosmic ? "#01050b" : "#2b4641"]} />{cosmic ? <Stars radius={190} depth={90} count={2500} factor={2.8} saturation={.12} fade speed={reducedMotion ? 0 : .018} /> : null}<fogExp2 attach="fog" args={[cosmic ? "#050b14" : "#2b4641", cosmic ? .0017 : .014]} /><ambientLight intensity={cosmic ? .12 : .78} color="#e4ece2" /><hemisphereLight args={["#e4ece2", "#213326", cosmic ? .2 : 1.2]} /><directionalLight position={[8, 16, 7]} intensity={cosmic ? .32 : 2.7} color="#fbf4dc" castShadow /><WorldGeometry target={target} /><Nature /><ReflectingWater /><AuthoredOrb onOpen={onOrb} reducedMotion={reducedMotion} state={orbState} /><Presence root={avatar} /><Landmarks onGround={onGround} onLifeMap={onLifeMap} /><Rig input={input} yaw={yaw} pitch={pitch} target={target} avatar={avatar} onNearby={onNearby} groundDescent={groundDescent} onGroundComplete={onGroundComplete} reducedMotion={reducedMotion} />{!cosmic ? <ContactShadows position={[0, -.08, -2.5]} opacity={.12} scale={30} blur={7} far={16} resolution={512} frames={1} /> : null}</>;
}
