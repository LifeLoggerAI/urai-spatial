...
function OrbMachine({ state, reducedMotion, onOpen }: { state: OrbState; reducedMotion: boolean; onOpen: () => void }) {
  ...
  useFrame(({ clock }) => { if (root.current) root.current.rotation.y = reducedMotion ? 0 : Math.sin(clock.elapsedTime * 0.18) * 0.010 })
  const sensory = resolveOrbSensoryOutput(state, reducedMotion, true)
  return <group name="home-orb-engineered-cradle" userData={{ treatment: 'v70-recessed-wall-integrated-reliquary', governedIdentity: GOVERNED_ORB }}>
...
function PortalFrame({ destination, position, onActivate }: { destination: 'ground' | 'life-map'; position: THREE.Vector3; onActivate: () => void }) {
  const tone = destination === 'ground' ? '#778d84' : '#7f8498'; const name = destination === 'ground' ? 'home-ground-environmental-threshold' : 'home-life-map-sky-lookout'; const facing = destination === 'ground' ? 0.28 : -0.28
  return <group name={name} position={position} rotation={[0, facing, 0]} userData={{ treatment: 'v70-recessed-stone-industrial-threshold', destination, governedPortalIdentity: GOVERNED_PORTAL }}>
    <StoneMass position={[-1.13, 1.52, -0.38]} scale={[0.52, 3.34, 0.88]} /><StoneMass position={[1.13, 1.52, -0.38]} scale={[0.52, 3.34, 0.88]} /><StoneMass position={[0, 3.08, -0.38]} scale={[2.78, 0.44, 0.88]} />
    <mesh name={destination === 'life-map' ? 'home-life-map-physical-portal' : 'home-ground-physical-threshold'} position={[0, 1.48, -0.30]} receiveShadow><planeGeometry args={[1.55, 2.38]} /><meshStandardMaterial color="#050706" emissive={tone} emissiveIntensity={0.03} roughness={0.96} metalness={0.01} /></mesh>
    ...
    <pointLight position={[0, 2.0, 0.08]} color={tone} intensity={0.17} distance={2.8} decay={2} />
  </group>
}
...
function Sanctuary({ target, reducedMotion, orbState, onOrb, onGround, onLifeMap }: { target: MutableRefObject<THREE.Vector3 | null>; reducedMotion: boolean; orbState: OrbState; onOrb: () => void; onGround: () => void; onLifeMap: () => void }) {
  ...
  return <>
    ...
    <group name="home-sanctuary-pavilion" userData={{ visualOwner: 'v70-deep-photogrammetry-industrial-sanctuary', construction: 'deep-scanned-rock-apse-with-integrated-service-machine', governedHomeIdentity: GOVERNED_HOME }}>
      ...
      <StoneMass position={[-3.78, 0.42, -8.0]} scale={[2.25, 0.70, 5.30]} /><StoneMass position={[3.78, 0.42, -8.0]} scale={[2.25, 0.70, 5.30]} />
      <Beam from={[-4.65, 4.48, -10.55]} to={[-2.10, 4.98, -9.55]} width={0.17} depth={0.15} /><Beam from={[4.65, 4.48, -10.55]} to={[2.10, 4.98, -9.55]} width={0.17} depth={0.15} /><Beam from={[-2.10, 4.98, -9.55]} to={[2.10, 4.98, -9.55]} width={0.16} depth={0.14} />
...
function Scene(props: { input: MovementInput; yaw: MutableRefObject<number>; pitch: MutableRefObject<number>; target: MutableRefObject<THREE.Vector3 | null>; nearby: (value: Nearby) => void; transition: Transition; reducedMotion: boolean; orbState: OrbState; onOrb: () => void; onGround: () => void; onLifeMap: () => void; onReady: () => void }) {
  return <><color attach="background" args={['#080b0b']} /><fogExp2 attach="fog" args={['#101514', 0.013]} /><Environment files={HOME_HDR} background={false} environmentIntensity={0.62} /><ambientLight intensity={0.32} color="#c6d0ca" /><hemisphereLight args={['#8f9f98', '#090b0a', 0.42]} /><directionalLight position={[-6, 9, 4]} intensity={0.96} color="#d7c49e" castShadow shadow-mapSize-width={768} shadow-mapSize-height={768} /><directionalLight position={[6, 6, -9]} intensity={0.30} color="#79958e" /><Sanctuary target={props.target} reducedMotion={props.reducedMotion} orbState={props.orbState} onOrb={props.onOrb} onGround={props.onGround} onLifeMap={props.onLifeMap} /><PlayerRig input={props.input} yaw={props.yaw} pitch={props.pitch} target={props.target} onNearby={props.nearby} transition={props.transition} /><ReadySignal onReady={props.onReady} /></>
}
...
export function HomeWorldProductionV70({ onOrbOpen = requestUraiWorldOrbOpen, webglAvailable = true }: Props) {
  ...
  const yaw = useRef(0); const pitch = useRef(0.06); const target = useRef<THREE.Vector3 | null>(null); const markSceneReady = useCallback(() => setSceneReady(true), [])
