...
function SculptedCanyonGround({ onWalk }: { onWalk: (event: ThreeEvent<MouseEvent>) => void }) {
  ...
  const geometry = useMemo(() => {
    ...
    const shadow = new THREE.Color('#12241d'); const moss = new THREE.Color('#9d9a78'); const warmStrata = new THREE.Color('#b18a5d')
    ...
    for (let zi=0; zi<=zSegments; zi += 1) {
      ...
      for (let xi=0; xi<=xSegments; xi += 1) {
        ...
        const ridgeMask = THREE.MathUtils.clamp((tz-0.20)*0.82 + lateral*0.48 + asymmetricRidges*0.20,0,0.68)
        ...
        const shade = THREE.MathUtils.clamp(0.10+y*0.145+(1-tz)*0.10+Math.abs(fracture)*0.65+asymmetricRidges*0.09,0,1)
  ...
  return <mesh name="home-v125-sculpted-canyon-ground" geometry={geometry} position={[0,0.035,0]} receiveShadow onClick={onWalk} userData={{ v165Refinement: 'smoothed-open-basin-clear-camera-corridors-no-road-groove', v167Refinement: 'single-authority-deep-canyon-basin-raised-horizon-no-overlap-islands', v172Refinement: 'stratified-high-rim-canyon-with-recessed-destination-basins-no-dead-midground', v174Refinement: 'asymmetric-continuous-ridge-geology-destination-amphitheaters-textured-floor-no-dead-midground', v175Refinement: 'terraced-erosion-canyon-visible-strata-raised-far-rim-detailed-foreground-no-smooth-bowl' }}><meshPhysicalMaterial color="#788778" map={stone.color} normalMap={stone.normal} normalScale={new THREE.Vector2(1.34,1.34)} roughnessMap={stone.arm} roughness={0.86} metalness={0.001} envMapIntensity={0.76} vertexColors /></mesh>
}
...
function LivingOrb({state,reducedMotion,onOrb}:{state:OrbState;reducedMotion:boolean;onOrb:()=>void}){
  ...
  return <group ref={group} name="home-v126-apse-integrated-orb" position={[ORB.x,ORB.y,ORB.z]} scale={[1.50,1.50,1.50]} onClick={(event) => { event.stopPropagation(); onOrb() }} userData={{v165Refinement:'contained-memory-mote-heart-primary-presence-no-capsule-no-aura-no-pedestal',v167Refinement:'filled-irregular-memory-swarm-small-seed-no-shell-silhouette',v172Refinement:'larger-filled-memory-swarm-near-invisible-seed-open-air-beneath-no-aura-no-pedestal',v174Refinement:'wide-contained-memory-swarm-dense-living-heart-open-air-beneath-no-aura-no-pedestal',v175Refinement:'dense-horizontal-living-memory-cloud-with-compact-multi-depth-heart-no-fountain-no-ball'}}>
    ...
    <mesh name="home-v133-orb-memory-seed" geometry={memoryVolume} scale={[0.012,0.016,0.011]}><meshPhysicalMaterial color="#426d5b" emissive={palette.accent} emissiveIntensity={0.012} roughness={0.62} metalness={0.001} clearcoat={0.01} transparent opacity={0.10}/></mesh>
