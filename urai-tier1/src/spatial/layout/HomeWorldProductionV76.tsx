...
function MemoryConstellation({ reducedMotion }: { reducedMotion: boolean }) {
  ...
  return <group ref={group} name="home-v177-recollection-currents" userData={{treatment:'memory-mote-currents-bind-destination-weather-to-living-orb-no-wireframe-no-rings-no-overlay'}}>
    <points geometry={nodes}><pointsMaterial color="#f0e3c5" size={0.066} transparent opacity={0.54} depthWrite={false} sizeAttenuation toneMapped={false}/></points>
  </group>
}
...
function MemorySediment({ reducedMotion }: { reducedMotion: boolean }) {
  const geometry=useMemo(()=>{const positions:number[]=[];const colors:number[]=[];const amber=new THREE.Color('#efc88c'),teal=new THREE.Color('#7be0c1'),lilac=new THREE.Color('#b6a8eb');for(let trace=0;trace<9;trace+=1){const originX=-6.2+trace*1.55,originZ=4.5-(trace%3)*2.4;for(let index=0;index<72;index+=1){const t=index/71,x=originX+Math.sin(t*Math.PI*2.2+trace)*(.34+t*.42),z=originZ-t*(3.0+(trace%4)*.55),y=-.02+Math.sin(x*1.8+z)*.055+((index*17)%13)/260;positions.push(x,y,z);const base=trace<3?teal:trace>6?lilac:amber,c=base.clone().lerp(amber,t*.18);colors.push(c.r,c.g,c.b)}}const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.Float32BufferAttribute(positions,3));g.setAttribute('color',new THREE.Float32BufferAttribute(colors,3));return g},[])
  const points=useRef<THREE.Points>(null);useFrame(({clock})=>{if(points.current&&!reducedMotion)(points.current.material as THREE.PointsMaterial).opacity=.38+Math.sin(clock.elapsedTime*.34)*.06})
  return <points ref={points} name="home-v178-embedded-memory-sediment" geometry={geometry} userData={{treatment:'nine-organic-signal-deposits-in-near-ground-no-road-no-interface-glyphs'}}><pointsMaterial vertexColors size={0.038} transparent opacity={0.42} depthWrite={false} sizeAttenuation toneMapped={false}/></points>
}
...
function LivingOrb({state,reducedMotion,onOrb}:{state:OrbState;reducedMotion:boolean;onOrb:()=>void}){
  ...
  const shards=useMemo(()=>Array.from({length:17},(_,index)=>{const angle=index*2.3999632297,radius=0.08+((index*7)%7)/42;return {position:[Math.cos(angle)*radius,Math.sin(angle*1.37)*0.20,Math.sin(angle)*radius*0.72+0.10] as Vec3,rotation:[angle*0.10,angle*0.18,angle*0.14] as Vec3,scale:0.024+((index*3)%5)*0.004}}),[])
  ...
  return <group ref={group} name="home-v126-apse-integrated-orb" position={[ORB.x,ORB.y,ORB.z]} scale={[1.50,1.50,1.50]} onClick={(event) => { event.stopPropagation(); onOrb() }} userData={{v165Refinement:'contained-memory-mote-heart-primary-presence-no-capsule-no-aura-no-pedestal',v167Refinement:'filled-irregular-memory-swarm-small-seed-no-shell-silhouette',v172Refinement:'larger-filled-memory-swarm-near-invisible-seed-open-air-beneath-no-aura-no-pedestal',v174Refinement:'wide-contained-memory-swarm-dense-living-heart-open-air-beneath-no-aura-no-pedestal',v175Refinement:'dense-horizontal-living-memory-cloud-with-compact-multi-depth-heart-no-fountain-no-ball'}}>
    ...
    {shards.map((shard,index)=><mesh key={index} position={shard.position} rotation={shard.rotation} scale={shard.scale}><tetrahedronGeometry args={[1,0]}/><meshStandardMaterial color={index%3===0?'#fff1c8':palette.core} emissive={palette.accent} emissiveIntensity={0.86} roughness={0.28} metalness={0.06} transparent opacity={0.92} toneMapped={false}/></mesh>)}
