...
function LivingOrb({state,reducedMotion,onOrb}:{state:OrbState;reducedMotion:boolean;onOrb:()=>void}){
  ...
  const shards=useMemo(()=>Array.from({length:11},(_,index)=>{const angle=index*2.3999632297,radius=0.055+((index*7)%6)/54;return {position:[Math.cos(angle)*radius,Math.sin(angle*1.19)*0.105,Math.sin(angle)*radius*0.58+0.04] as Vec3,rotation:[angle*0.17,angle*0.23,angle*0.19] as Vec3,scale:0.046+((index*3)%5)*0.006}}),[])
