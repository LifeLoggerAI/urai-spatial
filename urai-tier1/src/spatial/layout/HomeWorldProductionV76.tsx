...
function useSanctuaryStone() {
  ...
  return useMemo(() => {
    const prepare = (source: THREE.Texture, color = false) => {
      ...
      texture.repeat.set(3.6,4.4); texture.colorSpace = color ? THREE.SRGBColorSpace : THREE.NoColorSpace; texture.anisotropy = 6; texture.needsUpdate = true
      return texture
    }
...
function SculptedCanyonGround({ onWalk }: { onWalk: (event: ThreeEvent<MouseEvent>) => void }) {
  ...
  const geometry = useMemo(() => {
    ...
    const shadow = new THREE.Color('#17231e'); const moss = new THREE.Color('#b5aa86')
    const groundTint = new THREE.Color('#3d805a'); const lifeTint = new THREE.Color('#735fa1')
    for (let zi=0; zi<=zSegments; zi += 1) {
      ...
      for (let xi=0; xi<=xSegments; xi += 1) {
        ...
        const walkingChannel = Math.exp(-Math.pow(x/3.10,4)); const depthEnvelope = 0.50 + tz*0.65
        const sideRise = Math.pow(lateral,2.15)*depthEnvelope*(1.55 + Math.sin(z*0.24+x*0.11)*0.28)
        const fracture = (Math.sin(x*1.02+z*0.52)*0.22 + Math.cos(x*0.44-z*0.88)*0.16 + Math.sin((x+z)*0.28)*0.12)*(0.28+lateral*0.72)
        const channelRelief = walkingChannel*(Math.sin(z*0.32)*0.090 + Math.cos(z*0.15)*0.055)
        const basinLayer = Math.sin(tz*Math.PI*5.8+x*0.12)*0.22*Math.pow(tz,1.15)*(1-walkingChannel*0.45)
        const groundWeight = Math.exp(-(Math.pow((x+4.92)/2.15,2)+Math.pow((z+9.72)/3.10,2)))
        const lifeWeight = Math.exp(-(Math.pow((x-4.92)/2.10,2)+Math.pow((z+9.80)/3.00,2)))
        const destinationCarve = groundWeight*1.35 + lifeWeight*1.20
        const destinationShoulder = groundWeight*(1-groundWeight)*1.35 + lifeWeight*(1-lifeWeight)*1.25
        const portNearRidge = Math.exp(-(Math.pow((x+5.40)/2.40,2)+Math.pow((z+3.40)/4.80,2)))*0.95
        const starboardNearRidge = Math.exp(-(Math.pow((x-5.90)/2.20,2)+Math.pow((z+5.60)/4.20,2)))*1.08
        const portMidRidge = Math.exp(-(Math.pow((x+4.10)/2.60,2)+Math.pow((z+11.70)/3.20,2)))*1.25
        const starboardMidRidge = Math.exp(-(Math.pow((x-3.60)/2.70,2)+Math.pow((z+12.80)/3.00,2)))*1.10
        const asymmetricRidges = portNearRidge + starboardNearRidge + portMidRidge + starboardMidRidge
        const floorRelief = (Math.sin(x*1.45+z*0.72)*0.08 + Math.cos(x*0.82-z*1.31)*0.06)*(0.35+0.65*tz)
        const strata = (Math.sin(z*1.30+x*0.55)*0.09 + Math.cos(z*0.76-x*0.39)*0.06)*(0.18+lateral*0.82)
        const descent = tz*0.16
        const farBasinLift = Math.pow(tz,3.10)*(3.85 + Math.sin(x*0.38+0.80)*0.85 + Math.sin(x*0.87-1.30)*0.42)
        const y = -0.40 + descent + farBasinLift + sideRise + fracture*(1-walkingChannel*0.88) + channelRelief + basinLayer + destinationShoulder - destinationCarve + asymmetricRidges + floorRelief + strata
        positions.push(x,y,z)
        const shade = THREE.MathUtils.clamp(0.18+y*0.17+(1-tz)*0.08+Math.abs(strata)*1.15+asymmetricRidges*0.08-walkingChannel*0.02,0,1)
        const c = shadow.clone().lerp(moss,shade)
        c.lerp(groundTint,groundWeight*0.62); c.lerp(lifeTint,lifeWeight*0.58)
  ...
  return <mesh name="home-v125-sculpted-canyon-ground" geometry={geometry} position={[0,0.035,0]} receiveShadow onClick={onWalk} userData={{ v165Refinement: 'smoothed-open-basin-clear-camera-corridors-no-road-groove', v167Refinement: 'single-authority-deep-canyon-basin-raised-horizon-no-overlap-islands', v172Refinement: 'stratified-high-rim-canyon-with-recessed-destination-basins-no-dead-midground', v174Refinement: 'asymmetric-continuous-ridge-geology-destination-amphitheaters-textured-floor-no-dead-midground' }}><meshPhysicalMaterial color="#c3c1a8" map={stone.color} normalMap={stone.normal} normalScale={new THREE.Vector2(1.18,1.18)} roughnessMap={stone.arm} roughness={0.72} metalness={0.002} envMapIntensity={1.08} vertexColors /></mesh>
}
...
function FramedFissure({side,onActivate}:{side:'ground'|'life-map';onActivate:()=>void}){
  ...
  return <group name={`home-v126-${side}-framed-fissure`} userData={{v165Refinement:'terrain-flush-readable-destination-cut-clear-camera-corridor-no-door-no-ring',v167Refinement:'destination-cut-owned-by-single-basin-no-overlap',v172Refinement:'recessed-basin-scar-raised-to-live-terrain-surface-local-color-language-no-gate',v174Refinement:'recessed-basin-scar-seated-in-live-terrain-amphitheater-local-color-language-no-gate'}} position={[x,isGround?0.96:0.92,isGround?-9.72:-9.80]} rotation={[-1.47,isGround?0.13:-0.13,isGround?-0.08:0.08]} scale={isGround ? [1.05, 1.34, 0.50] : [1.02, 1.30, 0.50]}>
    <mesh name={`home-v151-${side}-retained-stone-provenance`} geometry={outer} castShadow receiveShadow><meshPhysicalMaterial color={isGround?'#356949':'#514d76'} map={stone.color} normalMap={stone.normal} normalScale={new THREE.Vector2(0.62,0.62)} roughnessMap={stone.arm} roughness={0.80} metalness={0.001} envMapIntensity={0.96}/></mesh>
    <mesh name={`home-v153-${side}-retired-threshold-panel`} geometry={field} position={[0,0,0.025]}><meshStandardMaterial color={isGround?'#07170f':'#100d19'} emissive={color} emissiveIntensity={0.46} roughness={1} side={THREE.DoubleSide}/></mesh>
    <points name={`home-v149-${side}-threshold-signal-field`} geometry={seamMotes} position={[0,0,0.10]}><pointsMaterial color={color} size={0.030} transparent opacity={0.90} depthWrite={false} sizeAttenuation toneMapped={false}/></points>
    ...
    <pointLight position={[0,1.08,0.34]} color={color} intensity={2.65} distance={9.0} decay={2}/>
  </group>
}
...
function LivingOrb({state,reducedMotion,onOrb}:{state:OrbState;reducedMotion:boolean;onOrb:()=>void}){
  ...
  const moteGeometry=useMemo(()=>{const positions:number[]=[];for(let index=0;index<1080;index+=1){const verticalSample=((((index*613)%1087)/1086)*2)-1;const angle=index*2.3999632297+Math.sin(index*0.31)*0.14;const radialSample=((index*431)%1091)/1090;const radius=0.05+Math.pow(radialSample,1.35)*0.68;const latitude=Math.sqrt(Math.max(0,1-verticalSample*verticalSample));const irregular=0.86+Math.sin(index*0.19)*0.12+Math.cos(index*0.073)*0.07;positions.push(Math.cos(angle)*latitude*radius*1.12*irregular,verticalSample*radius*0.72+Math.sin(index*0.11)*0.018,Math.sin(angle)*latitude*radius*0.92*irregular)}const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.Float32BufferAttribute(positions,3));return g},[])
  ...
  const shards=useMemo(()=>Array.from({length:6},(_,index)=>{const angle=index*2.3999632297,radius=0.10+((index*7)%5)/52;return {position:[Math.cos(angle)*radius,Math.sin(angle*1.37)*0.18,Math.sin(angle)*radius*0.72] as Vec3,rotation:[angle*0.10,angle*0.18,angle*0.14] as Vec3,scale:0.022+((index*3)%5)*0.003}}),[])
  useFrame(({clock})=>{if(!group.current||reducedMotion)return;const t=clock.getElapsedTime();group.current.position.y=ORB.y+Math.sin(t*(state==='speaking'?1.30:0.62))*0.022;group.current.rotation.y=Math.sin(t*0.14)*0.028})
  return <group ref={group} name="home-v126-apse-integrated-orb" position={[ORB.x,ORB.y,ORB.z]} scale={[1.52,1.52,1.52]} onClick={(event) => { event.stopPropagation(); onOrb() }} userData={{v165Refinement:'contained-memory-mote-heart-primary-presence-no-capsule-no-aura-no-pedestal',v167Refinement:'filled-irregular-memory-swarm-small-seed-no-shell-silhouette',v172Refinement:'larger-filled-memory-swarm-near-invisible-seed-open-air-beneath-no-aura-no-pedestal',v174Refinement:'wide-contained-memory-swarm-dense-living-heart-open-air-beneath-no-aura-no-pedestal'}}>
    <mesh name="home-v132-orb-memory-volume" geometry={memoryVolume} castShadow scale={[0.25,0.22,0.24]}><meshPhysicalMaterial color="#416f5c" emissive={palette.accent} emissiveIntensity={0.018} roughness={0.66} metalness={0.004} transmission={0.01} thickness={0.10} transparent opacity={0.006} depthWrite={false}/></mesh>
    <primitive object={orb} visible={false}/>
    <points name="home-v126-orb-memory-motes" geometry={moteGeometry} scale={[1.32,1.02,1.18]}><pointsMaterial color={palette.core} size={palette.moteSize*0.90} transparent opacity={0.86} depthWrite={false} sizeAttenuation toneMapped={false}/></points>
    <points name="home-v154-orb-memory-depth-motes" geometry={moteGeometry} scale={[1.52,1.10,1.34]}><pointsMaterial color={palette.accent} size={palette.moteSize*0.34} transparent opacity={0.26} depthWrite={false} sizeAttenuation toneMapped={false}/></points>
    <points name="home-v174-orb-memory-nucleus-motes" geometry={moteGeometry} scale={[0.62,0.50,0.58]}><pointsMaterial color={palette.core} size={palette.moteSize*0.62} transparent opacity={0.62} depthWrite={false} sizeAttenuation toneMapped={false}/></points>
    <mesh name="home-v133-orb-memory-seed" geometry={memoryVolume} scale={[0.018,0.024,0.016]}><meshPhysicalMaterial color="#426d5b" emissive={palette.accent} emissiveIntensity={0.018} roughness={0.58} metalness={0.002} clearcoat={0.02} transparent opacity={0.14}/></mesh>
    {shards.map((shard,index)=><mesh key={index} position={shard.position} rotation={shard.rotation} scale={shard.scale}><tetrahedronGeometry args={[1,0]}/><meshStandardMaterial color={palette.core} emissive={palette.accent} emissiveIntensity={0.06} roughness={0.58} transparent opacity={0.34}/></mesh>)}
...
export function HomeV76Sanctuary({reducedMotion,orbState,onOrb,onGround,onLifeMap,onWalk}:Props){
  return <group name="home-v126-ground-owned-open-sanctuary" userData={{activeArtRevision:'v154-visible-canyon-fissures-memory-swarm-no-pedestal',visualIteration:'v158-ground-scar-thresholds-hairline-path-sunken-geology',currentVisualRefinement:'v174-asymmetric-layered-canyon-destination-amphitheaters-wide-memory-swarm-no-runway',v165PixelRepair:'remove-v164-jagged-connected-shelves-clear-camera-corridors-contain-orb-shell',v167PixelRepair:'remove-overlapping-ground-islands-raise-canyon-horizon-rebuild-orb-as-filled-swarm',v171PixelRepair:'raise-continuous-basin-rim-add-geologic-relief-strengthen-destination-light-minimize-orb-seed',v172PixelRepair:'carve-destination-basins-raise-live-scars-strengthen-strata-enlarge-swarm-reduce-dead-sky',v174PixelRepair:'break-symmetric-bowl-with-continuous-ridge-overlap-seat-destinations-compress-orb-fountain-into-wide-swarm',compatibilityMarkers:LEGACY_CONTRACT_MARKERS,legacySourceAssets:LEGACY_SOURCE_ASSETS,historicalV76ContractOnly:true}}>
    ...
    <ambientLight intensity={0.74} color="#e0e3d5"/><hemisphereLight args={['#e7e3cf','#17261f',1.12]}/><directionalLight position={[-5,9,4]} intensity={2.35} color="#f3c98e" castShadow/><directionalLight position={[6,6,-8]} intensity={1.04} color="#70b894"/><directionalLight position={[-7,4,-10]} intensity={0.76} color="#9d8ed8"/><spotLight position={[0,8.4,-1.8]} target-position={[ORB.x,ORB.y,ORB.z]} angle={0.58} penumbra={0.86} intensity={2.45} color="#e7f6ee" distance={24}/><pointLight position={[0,1.8,2.8]} intensity={0.58} color="#f0cfa3" distance={10} decay={2}/><pointLight position={[-4.9,1.70,-9.7]} intensity={2.40} color="#58df95" distance={8.6} decay={2}/><pointLight position={[4.9,1.62,-9.8]} intensity={2.22} color="#a38cf0" distance={8.6} decay={2}/>
    <group name="home-authored-terrain" userData={{v174Refinement:'single-visible-asymmetric-stratified-ground-authority-with-recessed-destination-amphitheaters'}}/><group name="home-sanctuary-pavilion" userData={{v174Refinement:'continuous-ridge-overlap-and-far-rim-own-silhouette-no-detached-pile-family'}}/><group name="home-v49-scanned-detail-layer" userData={{v167Refinement:'edge-provenance-outside-primary-frustum-no-card-wall'}}/><group name="home-v49-authored-practicals" userData={{v174Refinement:'ground-green-life-map-violet-localized-basin-light-readable-without-gates'}}/><group name="home-authored-embodied-self" userData={{presentation:'privacy-preserving-first-person-presence-v126'}}/><group name="home-mountain-horizon" userData={{v174Refinement:'asymmetric-continuous-far-rim-and-overlapping-ridges-reduce-dead-sky-without-repeated-mountains'}}/><group name="home-living-vegetation" userData={{treatment:'reserved-beyond-clear-navigation-channel-v126'}}/>
  </group>
}
