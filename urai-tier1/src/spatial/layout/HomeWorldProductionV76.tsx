...
function SculptedCanyonGround({ onWalk }: { onWalk: (event: ThreeEvent<MouseEvent>) => void }) {
  ...
  const geometry = useMemo(() => {
    ...
    const shadow = new THREE.Color('#041b1b'); const moss = new THREE.Color('#568277'); const warmStrata = new THREE.Color('#91765e')
...
function MemorySky() {
  const material=useMemo(()=>new THREE.ShaderMaterial({side:THREE.BackSide,depthWrite:false,uniforms:{zenith:{value:new THREE.Color('#02090b')},upper:{value:new THREE.Color('#07191b')},horizon:{value:new THREE.Color('#173f3c')},memory:{value:new THREE.Color('#352d4b')}},vertexShader:'varying vec3 vDirection; void main(){vDirection=normalize(position);gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}',fragmentShader:'uniform vec3 zenith;uniform vec3 upper;uniform vec3 horizon;uniform vec3 memory;varying vec3 vDirection;void main(){float h=clamp(vDirection.y*.5+.5,0.,1.);vec3 c=mix(horizon,upper,smoothstep(.34,.62,h));c=mix(c,zenith,smoothstep(.58,.96,h));float band=exp(-pow((h-.38)*7.0,2.0));c+=memory*band*.16;gl_FragColor=vec4(c,1.);}' }),[])
  return <mesh name="home-v183-world-space-memory-sky" material={material} frustumCulled={false}><sphereGeometry args={[70,32,16]}/></mesh>
}

function AncestralMemoryVeils({reducedMotion}:{reducedMotion:boolean}) {
  const geometry=useMemo(()=>{const positions:number[]=[];const colors:number[]=[];const indices:number[]=[];const low=new THREE.Color('#296f69'),high=new THREE.Color('#75689a');const columns=28;for(let row=0;row<2;row+=1){for(let index=0;index<=columns;index+=1){const t=index/columns,x=(t-.5)*7.8,y=row===0?0:6.8+Math.sin(t*Math.PI*2.0)*.8,z=Math.sin(t*Math.PI*2.3)*.42;positions.push(x,y,z);const c=low.clone().lerp(high,row*.72+t*.16);colors.push(c.r,c.g,c.b)}}for(let index=0;index<columns;index+=1){const a=index,b=index+1,c=columns+1+index,d=c+1;indices.push(a,b,c,b,d,c)}const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.Float32BufferAttribute(positions,3));g.setAttribute('color',new THREE.Float32BufferAttribute(colors,3));g.setIndex(indices);g.computeVertexNormals();return g},[])
  const group=useRef<THREE.Group>(null);useFrame(({clock})=>{if(group.current&&!reducedMotion)group.current.rotation.y=Math.sin(clock.elapsedTime*.055)*.018})
  const material=<meshBasicMaterial vertexColors transparent opacity={0.105} depthWrite={false} side={THREE.DoubleSide} blending={THREE.AdditiveBlending}/>
  return <group ref={group} name="home-v183-ancestral-memory-weather-veils" userData={{treatment:'three-distant-world-space-memory-weather-curtains-beyond-basin-rim'}}>
    <mesh geometry={geometry} position={[-6.8,2.1,-21.0]} rotation={[0,.28,-.10]}>{material}</mesh>
    <mesh geometry={geometry} position={[.2,2.8,-23.5]} rotation={[0,-.08,.04]} scale={[1.20,1.08,1]}>{material}</mesh>
    <mesh geometry={geometry} position={[7.0,1.9,-20.5]} rotation={[0,-.31,.12]}>{material}</mesh>
  </group>
}
...
function LivingOrb({state,reducedMotion,onOrb}:{state:OrbState;reducedMotion:boolean;onOrb:()=>void}){
  ...
  const heartGeometry=useMemo(()=>{const geometry=new THREE.IcosahedronGeometry(0.14,1);const positions=geometry.getAttribute('position') as THREE.BufferAttribute;for(let index=0;index<positions.count;index+=1){const x=positions.getX(index),y=positions.getY(index),z=positions.getZ(index),latitude=y/.14,weathering=0.84+Math.sin(index*2.17)*0.13+Math.cos(index*.73)*0.06;positions.setXYZ(index,x*weathering*(0.78-latitude*.20),y*weathering*1.10,z*weathering*.62)}positions.needsUpdate=true;geometry.computeVertexNormals();return geometry},[])
  ...
  return <group ref={group} name="home-v126-apse-integrated-orb" position={[ORB.x,ORB.y,ORB.z]} scale={[1.50,1.50,1.50]} onClick={(event) => { event.stopPropagation(); onOrb() }} userData={{v165Refinement:'contained-memory-mote-heart-primary-presence-no-capsule-no-aura-no-pedestal',v167Refinement:'filled-irregular-memory-swarm-small-seed-no-shell-silhouette',v172Refinement:'larger-filled-memory-swarm-near-invisible-seed-open-air-beneath-no-aura-no-pedestal',v174Refinement:'wide-contained-memory-swarm-dense-living-heart-open-air-beneath-no-aura-no-pedestal',v175Refinement:'dense-horizontal-living-memory-cloud-with-compact-multi-depth-heart-no-fountain-no-ball'}}>
    ...
    <mesh name="home-v182-orb-faceted-mineral-seed" geometry={heartGeometry} rotation={[0.18,-0.34,0.10]} scale={[1.00,1.08,0.92]} castShadow><meshStandardMaterial color="#326c5a" emissive={palette.accent} emissiveIntensity={0.045} roughness={0.56} metalness={0.025} flatShading/></mesh>
