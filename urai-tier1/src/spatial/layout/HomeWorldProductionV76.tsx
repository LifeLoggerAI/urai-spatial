...
function SculptedCanyonGround({ onWalk }: { onWalk: (event: ThreeEvent<MouseEvent>) => void }) {
  ...
  const geometry = useMemo(() => {
    ...
    const shadow = new THREE.Color('#1f392f'); const moss = new THREE.Color('#91a68f')
    for (let zi=0; zi<=zSegments; zi += 1) {
      ...
      for (let xi=0; xi<=xSegments; xi += 1) {
        ...
        const walkingChannel = Math.exp(-Math.pow(x/3.10,4)); const depthEnvelope = 0.62 + tz*0.88
        const sideRise = Math.pow(lateral,1.66)*depthEnvelope*(2.15 + Math.sin(z*0.24+x*0.11)*0.34)
        const fracture = (Math.sin(x*1.02+z*0.52)*0.15 + Math.cos(x*0.44-z*0.88)*0.11 + Math.sin((x+z)*0.28)*0.09)*(0.28+lateral*0.72)
        const channelRelief = walkingChannel*(Math.sin(z*0.32)*0.045 + Math.cos(z*0.15)*0.032)
        const basinLayer = Math.sin(tz*Math.PI*5.0 + x*0.12)*0.13*Math.pow(tz,1.25)*(1-walkingChannel*0.58)
        const descent = tz*0.34; const farBasinLift = Math.pow(tz,2.15)*2.45
        const y = -0.36 + descent + farBasinLift + sideRise + fracture*(1-walkingChannel*0.92) + channelRelief + basinLayer
        positions.push(x,y,z)
        const shade = THREE.MathUtils.clamp(0.19+y*0.22+(1-tz)*0.07-walkingChannel*0.04,0,1)
  ...
  return <mesh name="home-v125-sculpted-canyon-ground" geometry={geometry} position={[0,0.035,0]} receiveShadow onClick={onWalk} userData={{ v165Refinement: 'smoothed-open-basin-clear-camera-corridors-no-road-groove', v167Refinement: 'single-authority-deep-canyon-basin-raised-horizon-no-overlap-islands', v171Refinement: 'high-relief-continuous-canyon-rim-layered-stone-no-dead-sky' }}><meshPhysicalMaterial color="#4e6d5e" map={stone.color} normalMap={stone.normal} normalScale={new THREE.Vector2(0.68,0.68)} roughnessMap={stone.arm} roughness={0.84} metalness={0.002} envMapIntensity={0.92} vertexColors /></mesh>
}
...
function FramedFissure({side,onActivate}:{side:'ground'|'life-map';onActivate:()=>void}){
  const stone=useSanctuaryStone();const isGround=side==='ground';const x=isGround?-4.92:4.92;const color=isGround?'#78d7a7':'#b3a4ef'
  ...
  return <group name={`home-v126-${side}-framed-fissure`} userData={{v165Refinement:'terrain-flush-readable-destination-cut-clear-camera-corridor-no-door-no-ring',v167Refinement:'destination-cut-owned-by-single-basin-no-overlap',v171Refinement:'bright-localized-rift-language-without-gate-silhouette'}} position={[x,isGround?0.05:0.06,isGround?-9.72:-9.80]} rotation={[-1.47,isGround?0.13:-0.13,isGround?-0.08:0.08]} scale={isGround ? [0.38, 0.58, 0.34] : [0.37, 0.59, 0.34]}>
    <mesh name={`home-v151-${side}-retained-stone-provenance`} geometry={outer} castShadow receiveShadow><meshPhysicalMaterial color={isGround?'#3f6652':'#55506f'} map={stone.color} normalMap={stone.normal} normalScale={new THREE.Vector2(0.36,0.36)} roughnessMap={stone.arm} roughness={0.91} metalness={0.001} envMapIntensity={0.70}/></mesh>
    <mesh name={`home-v153-${side}-retired-threshold-panel`} geometry={field} position={[0,0,0.025]}><meshStandardMaterial color={isGround?'#06100b':'#0a0910'} emissive={color} emissiveIntensity={0.11} roughness={1} side={THREE.DoubleSide}/></mesh>
    <points name={`home-v149-${side}-threshold-signal-field`} geometry={seamMotes} position={[0,0,0.10]}><pointsMaterial color={color} size={0.016} transparent opacity={0.62} depthWrite={false} sizeAttenuation toneMapped={false}/></points>
    ...
    <pointLight position={[0,1.08,0.34]} color={color} intensity={0.90} distance={6.2} decay={2}/>
  </group>
}
...
function LivingOrb({state,reducedMotion,onOrb}:{state:OrbState;reducedMotion:boolean;onOrb:()=>void}){
  ...
  return <group ref={group} name="home-v126-apse-integrated-orb" position={[ORB.x,ORB.y,ORB.z]} scale={[1.42,1.42,1.42]} onClick={(event) => { event.stopPropagation(); onOrb() }} userData={{v165Refinement:'contained-memory-mote-heart-primary-presence-no-capsule-no-aura-no-pedestal',v167Refinement:'filled-irregular-memory-swarm-small-seed-no-shell-silhouette',v171Refinement:'swarm-dominant-memory-presence-minimal-seed-no-glowing-ball-read'}}>
    <mesh name="home-v132-orb-memory-volume" geometry={memoryVolume} castShadow scale={[0.36,0.40,0.34]}><meshPhysicalMaterial color="#4d8a72" emissive={palette.accent} emissiveIntensity={0.035} roughness={0.62} metalness={0.004} transmission={0.01} thickness={0.14} transparent opacity={0.020} depthWrite={false}/></mesh>
    <primitive object={orb} visible={false}/>
    <points name="home-v126-orb-memory-motes" geometry={moteGeometry} scale={[1.20,1.24,1.14]}><pointsMaterial color={palette.core} size={palette.moteSize*0.82} transparent opacity={0.72} depthWrite={false} sizeAttenuation toneMapped={false}/></points>
    <points name="home-v154-orb-memory-depth-motes" geometry={moteGeometry} scale={[1.38,1.42,1.28]}><pointsMaterial color={palette.accent} size={palette.moteSize*0.34} transparent opacity={0.22} depthWrite={false} sizeAttenuation toneMapped={false}/></points>
    <mesh name="home-v133-orb-memory-seed" geometry={memoryVolume} scale={[0.10,0.14,0.09]}><meshPhysicalMaterial color="#7ac0a2" emissive={palette.accent} emissiveIntensity={0.12} roughness={0.46} metalness={0.004} clearcoat={0.05}/></mesh>
...
export function HomeV76Sanctuary({reducedMotion,orbState,onOrb,onGround,onLifeMap,onWalk}:Props){
  return <group name="home-v126-ground-owned-open-sanctuary" userData={{activeArtRevision:'v154-visible-canyon-fissures-memory-swarm-no-pedestal',visualIteration:'v158-ground-scar-thresholds-hairline-path-sunken-geology',currentVisualRefinement:'v171-high-relief-canyon-swarm-dominant-orb-no-runway',v165PixelRepair:'remove-v164-jagged-connected-shelves-clear-camera-corridors-contain-orb-shell',v167PixelRepair:'remove-overlapping-ground-islands-raise-canyon-horizon-rebuild-orb-as-filled-swarm',v171PixelRepair:'raise-continuous-basin-rim-add-geologic-relief-strengthen-destination-light-minimize-orb-seed',compatibilityMarkers:LEGACY_CONTRACT_MARKERS,legacySourceAssets:LEGACY_SOURCE_ASSETS,historicalV76ContractOnly:true}}>
    ...
    <ambientLight intensity={0.72} color="#dce9e1"/><hemisphereLight args={['#dcece3','#22362d',1.08]}/><directionalLight position={[-5,9,4]} intensity={2.30} color="#f0d4a5" castShadow/><directionalLight position={[6,6,-8]} intensity={0.95} color="#79b6a2"/><spotLight position={[0,8.4,-1.8]} target-position={[ORB.x,ORB.y,ORB.z]} angle={0.58} penumbra={0.86} intensity={2.20} color="#dff2e9" distance={22}/><pointLight position={[0,1.8,2.8]} intensity={0.44} color="#e8d4b6" distance={9} decay={2}/><pointLight position={[-4.9,1.15,-9.7]} intensity={1.05} color="#67cfa0" distance={6.5} decay={2}/><pointLight position={[4.9,1.15,-9.8]} intensity={0.95} color="#9a8bd8" distance={6.5} decay={2}/>
    <group name="home-authored-terrain" userData={{v171Refinement:'single-visible-high-relief-ground-authority-layered-depth-no-overlap-islands'}}/><group name="home-sanctuary-pavilion" userData={{v171Refinement:'continuous-canyon-rim-owns-silhouette-no-detached-pile-family'}}/><group name="home-v49-scanned-detail-layer" userData={{v167Refinement:'edge-provenance-outside-primary-frustum-no-card-wall'}}/><group name="home-v49-authored-practicals" userData={{v171Refinement:'localized-green-violet-destination-light-subordinate-to-swarm'}}/><group name="home-authored-embodied-self" userData={{presentation:'privacy-preserving-first-person-presence-v126'}}/><group name="home-mountain-horizon" userData={{v171Refinement:'raised-continuous-far-basin-rim-reduces-dead-sky'}}/><group name="home-living-vegetation" userData={{treatment:'reserved-beyond-clear-navigation-channel-v126'}}/>
  </group>
}
