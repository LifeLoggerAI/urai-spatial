...
function MemorySky() {
  const material=useMemo(()=>new THREE.ShaderMaterial({side:THREE.BackSide,depthWrite:false,uniforms:{zenith:{value:new THREE.Color('#01080a')},upper:{value:new THREE.Color('#08262a')},horizon:{value:new THREE.Color('#1b4b46')},memory:{value:new THREE.Color('#413757')}},vertexShader:'varying vec3 vDirection; void main(){vDirection=normalize(position);gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}',fragmentShader:'uniform vec3 zenith;uniform vec3 upper;uniform vec3 horizon;uniform vec3 memory;varying vec3 vDirection;void main(){float h=clamp(vDirection.y*.5+.5,0.,1.);vec3 c=mix(horizon,upper,smoothstep(.34,.64,h));c=mix(c,zenith,smoothstep(.62,.98,h));float band=exp(-pow((h-.40)*6.2,2.0));c+=memory*band*.13;gl_FragColor=vec4(c,1.);}' }),[])
