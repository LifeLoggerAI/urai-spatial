...
function SculptedCanyonGround({ onWalk }: { onWalk: (event: ThreeEvent<MouseEvent>) => void }) {
  ...
  const geometry = useMemo(() => {
    ...
    const shadow = new THREE.Color('#10231e'); const moss = new THREE.Color('#a8ad91'); const warmStrata = new THREE.Color('#c19667')
...
function MemoryConstellation({ reducedMotion }: { reducedMotion: boolean }) {
  const branches = useMemo(() => {
    ...
    const anchors:Vec3[]=[[-4.8,0.9,-11.8],[-3.7,3.2,-12.6],[-2.1,4.7,-13.0],[0,5.5,-13.4],[2.2,4.5,-13.0],[3.8,3.0,-12.5],[4.9,0.9,-11.8]]
    const heart:Vec3=[ORB.x,ORB.y,-10.9]
    for(let index=0;index<anchors.length;index+=1){const anchor=anchors[index];positions.push(...heart,...anchor);if(index>0)positions.push(...anchors[index-1],...anchor)}
  ...
  const nodes=useMemo(()=>{const values:number[]=[];for(let index=0;index<92;index+=1){const t=index/91,side=index%2===0?-1:1;values.push(side*(0.3+4.4*t)+Math.sin(index*1.7)*0.16,1.0+Math.sin(t*Math.PI)*4.25+Math.cos(index*.8)*.12,-11.85-t*1.45)}const geometry=new THREE.BufferGeometry();geometry.setAttribute('position',new THREE.Float32BufferAttribute(values,3));return geometry},[])
  const group=useRef<THREE.Group>(null)
  useFrame(({clock})=>{if(group.current&&!reducedMotion)group.current.rotation.y=Math.sin(clock.elapsedTime*.12)*.012})
  return <group ref={group} name="home-v176-recollection-constellation" userData={{treatment:'asymmetric-memory-branches-bind-destination-scars-to-living-orb-no-rings-no-overlay'}}>
    <lineSegments geometry={branches}><lineBasicMaterial color="#b8dfcd" transparent opacity={0.22} toneMapped={false}/></lineSegments>
    <points geometry={nodes}><pointsMaterial color="#f0e3c5" size={0.058} transparent opacity={0.68} depthWrite={false} sizeAttenuation toneMapped={false}/></points>
  </group>
}
...
function LivingOrb({state,reducedMotion,onOrb}:{state:OrbState;reducedMotion:boolean;onOrb:()=>void}){
  ...
  const shards=useMemo(()=>Array.from({length:11},(_,index)=>{const angle=index*2.3999632297,radius=0.13+((index*7)%5)/30;return {position:[Math.cos(angle)*radius,Math.sin(angle*1.37)*0.28,Math.sin(angle)*radius*0.72+0.10] as Vec3,rotation:[angle*0.10,angle*0.18,angle*0.14] as Vec3,scale:0.046+((index*3)%5)*0.006}}),[])
  ...
  return <group ref={group} name="home-v126-apse-integrated-orb" position={[ORB.x,ORB.y,ORB.z]} scale={[1.50,1.50,1.50]} onClick={(event) => { event.stopPropagation(); onOrb() }} userData={{v165Refinement:'contained-memory-mote-heart-primary-presence-no-capsule-no-aura-no-pedestal',v167Refinement:'filled-irregular-memory-swarm-small-seed-no-shell-silhouette',v172Refinement:'larger-filled-memory-swarm-near-invisible-seed-open-air-beneath-no-aura-no-pedestal',v174Refinement:'wide-contained-memory-swarm-dense-living-heart-open-air-beneath-no-aura-no-pedestal',v175Refinement:'dense-horizontal-living-memory-cloud-with-compact-multi-depth-heart-no-fountain-no-ball'}}>
    ...
    <points name="home-v126-orb-memory-motes" geometry={moteGeometry} scale={[1.30,0.96,1.18]}><pointsMaterial color={palette.core} size={palette.moteSize*0.88} transparent opacity={0.84} depthWrite={false} sizeAttenuation toneMapped={false}/></points>
    <points name="home-v154-orb-memory-depth-motes" geometry={moteGeometry} scale={[1.50,1.04,1.34]}><pointsMaterial color={palette.accent} size={palette.moteSize*0.32} transparent opacity={0.25} depthWrite={false} sizeAttenuation toneMapped={false}/></points>
    <points name="home-v174-orb-memory-nucleus-motes" geometry={moteGeometry} scale={[0.58,0.44,0.54]}><pointsMaterial color={palette.core} size={palette.moteSize*0.60} transparent opacity={0.60} depthWrite={false} sizeAttenuation toneMapped={false}/></points>
    <points name="home-v175-orb-memory-heart-motes" geometry={moteGeometry} scale={[0.30,0.24,0.28]}><pointsMaterial color={palette.accent} size={palette.moteSize*0.48} transparent opacity={0.58} depthWrite={false} sizeAttenuation toneMapped={false}/></points>
    <mesh name="home-v176-orb-faceted-heart" geometry={memoryVolume} scale={[0.17,0.22,0.15]}><meshPhysicalMaterial color={palette.core} emissive={palette.accent} emissiveIntensity={0.92} roughness={0.24} metalness={0.08} clearcoat={0.42} clearcoatRoughness={0.2} transparent opacity={0.92} toneMapped={false}/></mesh>
    ...
    {shards.map((shard,index)=><mesh key={index} position={shard.position} rotation={shard.rotation} scale={shard.scale}><tetrahedronGeometry args={[1,0]}/><meshStandardMaterial color={palette.core} emissive={palette.accent} emissiveIntensity={0.05} roughness={0.62} transparent opacity={0.30}/></mesh>)}
    ...
    <pointLight color={palette.core} intensity={palette.intensity*0.96} distance={6.0} decay={2}/><pointLight position={[0.42,-0.08,0.46]} color={palette.accent} intensity={palette.intensity*0.34} distance={4.0} decay={2}/>
