...
function SculptedCanyonGround({ onWalk }: { onWalk: (event: ThreeEvent<MouseEvent>) => void }) {
  ...
  const geometry = useMemo(() => {
    ...
    for (let zi=0; zi<=zSegments; zi += 1) {
      ...
      for (let xi=0; xi<=xSegments; xi += 1) {
        ...
        const chippedRelief=(Math.sin(x*5.7+z*3.1)*0.050+Math.cos(x*3.9-z*6.2)*0.038+Math.sin(x*8.3+z*1.9)*0.024)*(0.34+lateral*0.66)
  ...
  return <mesh name="home-v125-sculpted-canyon-ground" geometry={geometry} position={[0,0.035,0]} receiveShadow onClick={onWalk} userData={{ v175Refinement: 'terraced-erosion-canyon-visible-strata-raised-far-rim-detailed-foreground-no-smooth-bowl', v185Refinement: 'continuous-weathered-canyon-camera-safe-destination-basins-soft-strata-no-contour-staircase', v188Refinement:'fine-scale-chipped-relief-warped-strata-and-human-scale-stone-texels-break-smooth-heightfield-read' }}><meshPhysicalMaterial color="#77938a" map={stone.color} normalMap={stone.normal} normalScale={new THREE.Vector2(1.18,1.18)} roughnessMap={stone.arm} roughness={0.88} metalness={0.001} envMapIntensity={0.72} vertexColors /></mesh>
}
...
function FramedFissure({side,onActivate}:{side:'ground'|'life-map';onActivate:()=>void}){
  ...
  return <group name={`home-v126-${side}-framed-fissure`} userData={{v165Refinement:'terrain-flush-readable-destination-cut-clear-camera-corridor-no-door-no-ring',v167Refinement:'destination-cut-owned-by-single-basin-no-overlap',v172Refinement:'recessed-basin-scar-raised-to-live-terrain-surface-local-color-language-no-gate',v174Refinement:'recessed-basin-scar-seated-in-live-terrain-amphitheater-local-color-language-no-gate',v175Refinement:'basin-wide-branching-signal-field-no-slab-no-door-no-ring',v185Refinement:'camera-safe-basin-wide-ground-level-signal-place-no-upright-gate'}} position={[x,isGround?0.70:0.64,isGround?-8.72:-8.78]} rotation={[0,isGround?0.10:-0.10,0]} scale={[1,1,1]}>
    ...
    <mesh name={`home-v188-${side}-terrain-seated-memory-stone`} geometry={memoryStone} position={[0,0.02,0]} rotation={[0.06,isGround?0.34:-0.28,isGround?-0.08:0.07]} scale={[1.18,0.26,0.78]} castShadow receiveShadow><meshStandardMaterial color={isGround?'#244b37':'#3f365f'} emissive={color} emissiveIntensity={0.12} roughness={0.82} metalness={0.008} flatShading/></mesh>
    <points name={`home-v149-${side}-threshold-signal-field`} geometry={seamMotes} position={[0,0.12,0]} scale={[0.72,0.34,0.70]}><pointsMaterial color={color} size={0.022} transparent opacity={0.46} depthWrite={false} sizeAttenuation toneMapped={false}/></points>
...
function LivingOrb({state,reducedMotion,onOrb}:{state:OrbState;reducedMotion:boolean;onOrb:()=>void}){
  ...
  return <group ref={group} name="home-v126-apse-integrated-orb" position={[ORB.x,ORB.y,ORB.z]} scale={[1.72,1.72,1.72]} onClick={(event) => { event.stopPropagation(); onOrb() }} userData={{v165Refinement:'contained-memory-mote-heart-primary-presence-no-capsule-no-aura-no-pedestal',v167Refinement:'filled-irregular-memory-swarm-small-seed-no-shell-silhouette',v172Refinement:'larger-filled-memory-swarm-near-invisible-seed-open-air-beneath-no-aura-no-pedestal',v174Refinement:'wide-contained-memory-swarm-dense-living-heart-open-air-beneath-no-aura-no-pedestal',v175Refinement:'dense-horizontal-living-memory-cloud-with-compact-multi-depth-heart-no-fountain-no-ball',v185Refinement:'large-contained-point-memory-presence-dense-dark-heart-no-solid-ball-no-fountain',v186Refinement:'bounded-fine-grain-memory-heart-readable-near-and-far-no-particle-wall'}}>
    ...
    <mesh name="home-v188-orb-heart-port-lobe" geometry={heartGeometry} position={[-0.12,0.03,0.01]} rotation={[0.18,-0.34,0.22]} scale={[1.18,1.42,0.92]} castShadow><meshStandardMaterial color="#244d42" emissive={palette.core} emissiveIntensity={0.34} roughness={0.64} metalness={0.012} flatShading/></mesh>
    <mesh name="home-v188-orb-heart-starboard-lobe" geometry={heartGeometry} position={[0.11,-0.025,0.035]} rotation={[-0.14,0.42,-0.31]} scale={[0.96,1.12,0.78]} castShadow><meshStandardMaterial color="#315848" emissive={palette.accent} emissiveIntensity={0.26} roughness={0.68} metalness={0.008} flatShading/></mesh>
    <mesh name="home-v188-orb-heart-crown-lobe" geometry={heartGeometry} position={[-0.015,0.16,-0.025]} rotation={[0.34,0.12,0.46]} scale={[0.72,0.92,0.64]} castShadow><meshStandardMaterial color="#294f43" emissive={palette.core} emissiveIntensity={0.30} roughness={0.66} metalness={0.006} flatShading/></mesh>
    <points name="home-v126-orb-memory-motes" geometry={moteGeometry} scale={[0.62,0.48,0.56]}><pointsMaterial color={palette.core} size={palette.moteSize*0.22} transparent opacity={0.34} depthWrite={false} sizeAttenuation toneMapped={false}/></points>
    ...
    <pointLight color={palette.core} intensity={palette.intensity*0.70} distance={6.2} decay={2}/><pointLight position={[0.42,-0.08,0.46]} color={palette.accent} intensity={palette.intensity*0.22} distance={4.1} decay={2}/>
