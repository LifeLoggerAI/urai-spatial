...
function Orb({ onOpen, reducedMotion }: { onOpen: () => void; reducedMotion: boolean }) {
  const root = useRef<THREE.Group>(null)
  const light = useRef<THREE.PointLight>(null)
  useFrame(({ clock }) => {
    if (!root.current || reducedMotion) return
    root.current.position.y = ORB.y + Math.sin(clock.elapsedTime * .7) * .08
    root.current.rotation.y = clock.elapsedTime * .12
    const breathe = 1 + Math.sin(clock.elapsedTime * .95) * .035
    root.current.scale.setScalar(breathe)
    if (light.current) light.current.intensity = 2.1 + Math.sin(clock.elapsedTime * .95) * .22
  })
  return <group ref={root} name="home-orb-sanctuary" position={ORB} onClick={(event) => { event.stopPropagation(); onOpen() }}>
    <mesh scale={.52}><sphereGeometry args={[1, 64, 64]} /><meshPhysicalMaterial color="#d3f4ef" emissive="#5eaaa3" emissiveIntensity={.82} roughness={.16} metalness={.05} clearcoat={1} clearcoatRoughness={.12} /></mesh>
    <mesh scale={.66}><sphereGeometry args={[1, 48, 48]} /><meshBasicMaterial color="#8de1d4" transparent opacity={.055} depthWrite={false} /></mesh>
    <pointLight ref={light} color="#a7f0e5" intensity={2.1} distance={9} decay={2} />
  </group>
}
...
function Scene(props: { input: MovementInput; yaw: MutableRefObject<number>; pitch: MutableRefObject<number>; target: MutableRefObject<THREE.Vector3 | null>; avatar: MutableRefObject<THREE.Group | null>; onNearby: (value: Nearby) => void; onOrbOpen: () => void; onGround: () => void; onGroundComplete: () => void; onLifeMap: () => void; onReady: () => void; groundDescent: boolean; reducedMotion: boolean }) {
  ...
  return <>
    ...
    <Water />
    <Orb onOpen={props.onOrbOpen} reducedMotion={props.reducedMotion} />
