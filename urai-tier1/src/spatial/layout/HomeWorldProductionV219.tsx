...
const SPAWN=new THREE.Vector3(0,.04,4.6), ORB=new THREE.Vector3(-.18,1.7,-6.9), GROUND=new THREE.Vector3(-4.85,0,-8.25), LIFE_MAP=new THREE.Vector3(4.85,0,-8.25)
const BOUNDS={minX:-7.4,maxX:7.4,minZ:-14,maxZ:6.7}
...
function useStoneMaps():StoneMaps{
  ...
  return useMemo(()=>{
    const prep=(s:THREE.Texture,srgb=false)=>{const t=s.clone();t.wrapS=t.wrapT=THREE.RepeatWrapping;t.repeat.set(8,11);t.anisotropy=8;t.colorSpace=srgb?THREE.SRGBColorSpace:THREE.NoColorSpace;t.needsUpdate=true;return t}
    return{color:prep(c,true),normal:prep(n),arm:prep(a)}
  },[a,c,n])
}

function height(x:number,z:number){
  const d=THREE.MathUtils.clamp((5.8-z)/23,0,1),l=Math.abs(x)/9.2
  return-.52+Math.pow(l,2.25)*(.25+d*2.5)+Math.pow(d,3.1)*(2.25+.42*Math.sin(x*.34))+(Math.sin(x*.62+z*.37)*.30+Math.sin(x*1.43-z*.81)*.15)*(0.2+l*.6)+Math.exp(-(Math.pow((x+6)/2.2,2)+Math.pow((z+3)/5,2)))*1.45+Math.exp(-(Math.pow((x-6.2)/2.1,2)+Math.pow((z+6)/4.8,2)))*1.6
}

function Terrain({onWalk}:{onWalk:(e:ThreeEvent<MouseEvent>)=>void}){
  const maps=useStoneMaps()
  const g=useMemo(()=>{
    const xs=92,zs=118,p:number[]=[],uv:number[]=[],idx:number[]=[]
    for(let zi=0;zi<=zs;zi++){const tz=zi/zs,z=6-tz*24;for(let xi=0;xi<=xs;xi++){const tx=xi/xs,x=-9.4+tx*18.8;p.push(x,height(x,z),z);uv.push(tx*9,tz*12)}}
  ...
  return <mesh name="home-v219-continuous-weathered-sanctuary" geometry={g} receiveShadow onClick={onWalk}><meshStandardMaterial color="#456454" map={maps.color} normalMap={maps.normal} normalScale={new THREE.Vector2(.55,.55)} roughnessMap={maps.arm} roughness={.95} metalness={.01}/></mesh>
}

function rockGeometry(seed:number){
  const g=new THREE.IcosahedronGeometry(1,3),a=g.attributes.position as THREE.BufferAttribute,v=new THREE.Vector3()
  for(let i=0;i<a.count;i++){v.fromBufferAttribute(a,i);const m=1+Math.sin(v.x*5.1+seed)*.09+Math.cos(v.y*7.3-seed*.4)*.055+Math.sin(v.z*9.2+seed)*.04;v.multiplyScalar(m);a.setXYZ(i,v.x,v.y,v.z)}
...
function Rock({name,position,scale,seed,tint,rotation=[0,0,0]}:{name:string;position:[number,number,number];scale:[number,number,number];seed:number;tint:string;rotation?:[number,number,number]}){
  const maps=useStoneMaps(),g=useMemo(()=>rockGeometry(seed),[seed])
  return <mesh name={name} geometry={g} position={position} scale={scale} rotation={rotation} castShadow receiveShadow><meshStandardMaterial color={tint} map={maps.color} normalMap={maps.normal} normalScale={new THREE.Vector2(.38,.38)} roughnessMap={maps.arm} roughness={.92} metalness={.01}/></mesh>
}

function EnvironmentMasses(){
  return <><Rock name="home-v219-port-near-ridge" position={[-8,.7,-4]} scale={[3.3,2.1,3.5]} seed={21} tint="#385246"/><Rock name="home-v219-port-far-ridge" position={[-7.6,2,-14]} scale={[4.4,3.6,4.2]} seed={23} tint="#2e473d" rotation={[0,-.18,.1]}/><Rock name="home-v219-starboard-near-ridge" position={[8,.8,-5.8]} scale={[3.5,2.3,3.7]} seed={25} tint="#3b584a"/><Rock name="home-v219-starboard-far-ridge" position={[7.2,2.2,-15]} scale={[4.6,3.8,4.4]} seed={27} tint="#30483f" rotation={[0,.16,-.1]}/></>
}

function GroundPlace({onOpen}:{onOpen:()=>void}){
  const y=height(-4.85,-8.25)
  return <group name="home-v219-ground-place" position={[-4.85,y+.15,-8.25]} onClick={e=>{e.stopPropagation();onOpen()}}><Rock name="ground-v219-rooted-shelf" position={[0,.15,0]} scale={[2.25,.52,1.9]} seed={41} tint="#4a6556"/><Rock name="ground-v219-eroded-shelter" position={[-.3,1.05,-1.2]} scale={[2.1,1.2,.7]} seed={43} tint="#354f43" rotation={[0,.12,-.08]}/><Rock name="ground-v219-port-shoulder" position={[-1.5,.48,.3]} scale={[1,.48,.95]} seed={45} tint="#3d594b"/><Rock name="ground-v219-starboard-shoulder" position={[1.42,.43,-.1]} scale={[.95,.45,1.02]} seed={47} tint="#405c4e"/><pointLight position={[-.25,.52,-.25]} color="#c79a66" intensity={1.25} distance={4.2} decay={2}/></group>
}

function LifeMapPlace({onOpen}:{onOpen:()=>void}){
  const y=height(4.85,-8.25)
  const a=useMemo(()=>new THREE.CatmullRomCurve3([new THREE.Vector3(0,0,0),new THREE.Vector3(-.35,.7,-.05),new THREE.Vector3(-.52,1.4,-.2),new THREE.Vector3(-.28,2.2,-.4)]),[])
  const b=useMemo(()=>new THREE.CatmullRomCurve3([new THREE.Vector3(.05,.4,0),new THREE.Vector3(.52,1,-.05),new THREE.Vector3(.82,1.7,-.24),new THREE.Vector3(.64,2.4,-.46)]),[])
  return <group name="home-v219-life-map-place" position={[4.85,y+.08,-8.25]} onClick={e=>{e.stopPropagation();onOpen()}}><Rock name="lifemap-v219-rooted-foundation" position={[0,.15,.05]} scale={[2.1,.46,1.8]} seed={61} tint="#42495a"/><mesh castShadow><tubeGeometry args={[a,44,.23,9,false]}/><meshStandardMaterial color="#565a70" roughness={.84}/></mesh><mesh castShadow><tubeGeometry args={[b,44,.21,9,false]}/><meshStandardMaterial color="#655f7c" roughness={.82}/></mesh><Rock name="lifemap-v219-crown" position={[.1,2.35,-.42]} scale={[1.12,.5,.8]} seed={67} tint="#69627f" rotation={[0,.12,.18]}/><pointLight position={[.1,1.6,.2]} color="#aaa7db" intensity={1.2} distance={4.8} decay={2}/></group>
}

function orbGeometry(){
  const nu=56,nv=32,p:number[]=[],c:number[]=[],idx:number[]=[],dark=new THREE.Color('#245545'),light=new THREE.Color('#78b79b'),warm=new THREE.Color('#c58e59')
  for(let j=0;j<=nv;j++){const v=j/nv,phi=Math.PI*v;for(let i=0;i<nu;i++){const theta=Math.PI*2*i/nu,l=.16*Math.sin(3*theta+phi*.7)+.10*Math.sin(5*theta-phi*1.3)+.07*Math.cos(2*theta+phi*2.1),r=.74*(1+l+.14*Math.cos(theta-.6)*Math.sin(phi)**2);p.push(r*Math.sin(phi)*Math.cos(theta)*(.9+.1*Math.cos(phi))+.1*(1-v)*Math.sin(phi),1.02*Math.cos(phi)+.16*Math.sin(2*phi+theta*.8),r*Math.sin(phi)*Math.sin(theta)*(.74+.1*Math.sin(theta+.5)));const q=dark.clone().lerp(light,.3+(.5+.5*Math.sin(phi*9+theta*2.4))*.35).lerp(warm,Math.max(0,Math.cos(theta-.8))*Math.sin(phi)**3*.18);c.push(q.r,q.g,q.b)}}
...
const POSTURE:Record<OrbState,{s:[number,number,number];r:[number,number,number];speed:number}>={
  dormant:{s:[.86,.78,.82],r:[.14,-.12,-.12],speed:.12},idle:{s:[1,1,1],r:[-.06,.16,-.04],speed:.42},attention:{s:[1.08,1.12,.9],r:[-.14,.3,.12],speed:.7},listening:{s:[.9,1.22,.88],r:[.18,-.24,-.1],speed:.3},thinking:{s:[1.12,.92,1.08],r:[-.24,.48,.18],speed:.24},speaking:{s:[1.18,1.06,.84],r:[.08,.16,-.2],speed:1},guiding:{s:[.96,1.28,.88],r:[-.28,-.1,.16],speed:.52},reflecting:{s:[.92,1.02,1.16],r:[.22,.54,-.16],speed:.18},calming:{s:[1.1,.88,1.05],r:[-.04,-.2,.06],speed:.16},privacy:{s:[.82,.92,.78],r:[.3,.62,.22],speed:.1},warning:{s:[1.2,1.16,.78],r:[-.3,-.34,-.22],speed:1.28},transition:{s:[.88,1.34,.82],r:[-.36,.28,.24],speed:.82}
}

function OrbPresence({state,reducedMotion,onOpen}:{state:OrbState;reducedMotion:boolean;onOpen:()=>void}){
  const ref=useRef<THREE.Group>(null),g=useMemo(orbGeometry,[]),p=POSTURE[state]
  useFrame(({clock})=>{if(!ref.current)return;const t=clock.elapsedTime*p.speed,b=reducedMotion?1:1+Math.sin(t*.8)*.025;ref.current.scale.set(p.s[0]*b,p.s[1]*b,p.s[2]*b);ref.current.rotation.set(p.r[0],p.r[1]+(reducedMotion?0:Math.sin(t)*.15),p.r[2])})
  return <group ref={ref} name="home-v219-connected-asymmetric-living-memory-presence" position={ORB} onClick={e=>{e.stopPropagation();onOpen()}}><mesh geometry={g} castShadow receiveShadow><meshPhysicalMaterial vertexColors roughness={.57} metalness={.03} clearcoat={.18} clearcoatRoughness={.62} emissive={state==='warning'?'#6d291d':'#173d32'} emissiveIntensity={state==='warning'?.45:.17}/></mesh><mesh geometry={g} scale={[.76,.76,.76]}><meshPhysicalMaterial color="#153c32" emissive="#2a8067" emissiveIntensity={.18} side={THREE.BackSide} roughness={.45}/></mesh><pointLight color={state==='warning'?'#d07052':'#6cc2a0'} intensity={1.05} distance={4.8} decay={2}/></group>
}

function Cadence({active}:{active:boolean}){
  const{invalidate,setFrameloop}=useThree()
  useEffect(()=>{if(!active){setFrameloop('always');return}setFrameloop('demand');const id=window.setInterval(invalidate,280);invalidate();return()=>window.clearInterval(id)},[active,invalidate,setFrameloop])
  return null
}
...
function Rig({input,yaw,pitch,target,onNearby,transition,owner}:{input:MovementInput;yaw:MutableRefObject<number>;pitch:MutableRefObject<number>;target:MutableRefObject<THREE.Vector3|null>;onNearby:(n:Nearby)=>void;transition:Transition;owner:MutableRefObject<HTMLElement|null>}){
  ...
  useEffect(()=>{camera.position.set(0,1.58,4.6);camera.lookAt(0,1.2,-8.6);camera.near=.1;camera.far=120;camera.updateProjectionMatrix()},[camera])
  useFrame((_,delta)=>{
    ...
    if(shell){shell.dataset.homePlayerX=pos.current.x.toFixed(3);shell.dataset.homePlayerZ=pos.current.z.toFixed(3);shell.dataset.homeDistance=pos.current.distanceTo(SPAWN).toFixed(3);shell.dataset.homeDistanceOrb=Math.hypot(pos.current.x-ORB.x,pos.current.z-ORB.z).toFixed(3);shell.dataset.homeDistanceGround=Math.hypot(pos.current.x-GROUND.x,pos.current.z-GROUND.z).toFixed(3);shell.dataset.homeDistanceLifeMap=Math.hypot(pos.current.x-LIFE_MAP.x,pos.current.z-LIFE_MAP.z).toFixed(3);shell.dataset.homeMoving=vel.current.lengthSq()>.0004?'true':'false';shell.dataset.homeRenderedFrames=String(frames.current)}
    let near:Nearby=null,best=1e9
    for(const[n,q,r]of[['orb',ORB,2.35],['ground',GROUND,2.65],['life-map',LIFE_MAP,2.65]] as const){const d=Math.hypot(pos.current.x-q.x,pos.current.z-q.z);if(d<r&&d<best){near=n;best=d}}
    if(camera instanceof THREE.PerspectiveCamera){const f=size.height>size.width?50:40;if(camera.fov!==f){camera.fov=f;camera.updateProjectionMatrix()}}
    camera.position.lerp(pos.current.clone().add(new THREE.Vector3(Math.sin(yaw.current)*(near?1.5:.08),1.56,Math.cos(yaw.current)*(near?1.5:.08))),1-Math.pow(.0008,delta))
    camera.lookAt(pos.current.clone().add(new THREE.Vector3(-Math.sin(yaw.current)*10,1.25+pitch.current*.38,-Math.cos(yaw.current)*10)))
...
function Scene(props:{input:MovementInput;yaw:MutableRefObject<number>;pitch:MutableRefObject<number>;target:MutableRefObject<THREE.Vector3|null>;onNearby:(n:Nearby)=>void;transition:Transition;reducedMotion:boolean;orbState:OrbState;onOrb:()=>void;onGround:()=>void;onLifeMap:()=>void;onReady:()=>void;owner:MutableRefObject<HTMLElement|null>}){
  ...
  return <><Cadence active={props.reducedMotion}/><color attach="background" args={['#061315']}/><fogExp2 attach="fog" args={['#12322f',.022]}/><ambientLight intensity={.46} color="#c9d8cf"/><hemisphereLight args={['#c3d8cf','#352a22',.68]}/><directionalLight position={[-6,10,4]} intensity={1.7} color="#e7cda2" castShadow shadow-mapSize-width={1024} shadow-mapSize-height={1024}/><directionalLight position={[7,6,-10]} intensity={.5} color="#7ea99b"/><Terrain onWalk={walk}/><EnvironmentMasses/><GroundPlace onOpen={props.onGround}/><LifeMapPlace onOpen={props.onLifeMap}/><OrbPresence state={props.orbState} reducedMotion={props.reducedMotion} onOpen={props.onOrb}/><Rig input={props.input} yaw={props.yaw} pitch={props.pitch} target={props.target} onNearby={props.onNearby} transition={props.transition} owner={props.owner}/></>
}
