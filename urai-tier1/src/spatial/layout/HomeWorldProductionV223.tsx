...
function CameraRig({
  ...
  stableMode: StableHomeMode
  homeTransition: string | null
  movement: MovementInput
  firstPersonPosition: MutableRefObject<THREE.Vector3>
  firstPersonVelocity: MutableRefObject<THREE.Vector3>
  firstPersonTarget: MutableRefObject<THREE.Vector3 | null>
  runtimeCameraPosition: MutableRefObject<THREE.Vector3>
  ...
  useFrame((_, delta) => {
    ...
    if (camera instanceof THREE.PerspectiveCamera) {
      const desiredFov = transition === 'life-map'
        ? (portrait ? 64 : 50)
        : firstPerson || semanticEmbodiment
          ? (portrait ? 66 : 58)
          : transition === 'ground'
            ? (portrait ? 60 : 48)
            : (portrait ? 58 : 52)
    ...
    if (firstPerson) {
      stepEmbodiedMotion({
        position: firstPersonPosition.current,
        ...
        acceleration: 6.5,
        deceleration: 8,
        bounds: HOME_FIRST_PERSON_BOUNDS,
        arrivalRadius: .32,
      })
...
function Scene({
  ...
  return <>
    ...
    <group name="home-visible-user-avatar">
      <HomeEmbodiedAvatar
        position={[AVATAR_POSITION.x, avatarGroundY, AVATAR_POSITION.z]}
        rotationY={Math.PI}
        scale={.72}
        state={avatarState}
    ...
    <CameraRig
      ...
      movement={movement}
      firstPersonPosition={firstPersonPosition}
      firstPersonVelocity={firstPersonVelocity}
      firstPersonTarget={firstPersonTarget}
      runtimeCameraPosition={runtimeCameraPosition}
...
export function HomeWorldProductionV223({ onOrbOpen = requestUraiWorldOrbOpen, webglAvailable = true }: Props) {
  ...
  const readRuntimeSnapshot = useCallback(() => ({
    ...
    cameraPosition: runtimeCameraPosition.current.clone(),
    yaw: yaw.current,
    pitch: pitch.current,
    orbState,
  }), [orbState])
  ...
  const { state: homeExperience, api: homeApi } = useHomeExperienceController({
    reducedMotion,
    readRuntimeSnapshot,
    onDestinationCommit: (destination) => commitDestination(destination),
    onStableRestore: restoreStableOrigin,
  })

  const stableMode: StableHomeMode = homeExperience.stableState === 'HOME_PRESENTATION'
    ? 'HOME_PRESENTATION'
    : homeExperience.stableState === 'AVATAR_HOME_FIRST_PERSON' || homeExperience.stableState === 'AVATAR_SELF_VIEW'
      ? 'AVATAR_HOME_FIRST_PERSON'
      : homeExperience.origin.stableState
  stableModeRef.current = stableMode
  const firstPerson = stableMode === 'AVATAR_HOME_FIRST_PERSON'
  const movement = useMovementInput({
    enabled: firstPerson && homeExperience.stableState === 'AVATAR_HOME_FIRST_PERSON' && !homeExperience.inputLocked && transition === 'none',
  })

  const avatarState: HomeAvatarPresentationState = firstPerson
    ? 'hidden-first-person'
    : homeExperience.transition === 'AVATAR_EMBODIMENT_TRANSITION'
      ? 'embodying'
      : homeExperience.transition === 'EMBODIMENT_UNWIND'
        ? 'returning'
        : avatarTargeted
          ? 'targeted'
          : 'rest'
  ...
  const openAvatar = useCallback(() => {
    if (transition !== 'none' || homeExperience.inputLocked || homeExperience.stableState !== 'HOME_PRESENTATION') return
    homeApi.activateAvatar()
  }, [homeApi, homeExperience.inputLocked, homeExperience.stableState, transition])
  const openOrb = useCallback(() => {
    if (transition !== 'none' || homeExperience.inputLocked) return
    setOrbState('attention')
    onOrbOpen()
  }, [homeExperience.inputLocked, onOrbOpen, transition])
  ...
  }, [homeApi, homeExperience.inputLocked, transition])
  ...
  const look = useDragLook({
    yaw,
    pitch,
    enabled: transition === 'none' && !homeExperience.inputLocked && homeExperience.stableState !== 'AVATAR_SELF_VIEW',
    sensitivity: firstPerson ? .0032 : .0022,
    minPitch: firstPerson ? -.62 : -.28,
    maxPitch: firstPerson ? .58 : .25,
      ...
      : homeExperience.transition === 'AVATAR_EMBODIMENT_TRANSITION'
        ...
        : homeExperience.transition === 'EMBODIMENT_UNWIND'
          ...
          : firstPerson
            ? 'AVATAR_HOME_FIRST_PERSON'
            : 'HOME_PRESENTATION'
  const cameraMode = transition !== 'none'
    ? transition
    : firstPerson
      ? dragging ? 'avatar-home-first-person-look' : 'avatar-home-first-person'
      : dragging ? 'cinematic-third-person-look' : 'cinematic-third-person'
  ...
  return <main
    ...
    data-home-embodied-self={firstPerson ? 'camera-only-avatar-embodied-first-person' : 'visible-cinematic-avatar'}
    data-home-presence-presentation={firstPerson ? 'avatar-hidden-camera-first-person' : 'visible-avatar-third-person'}
    data-home-movement={firstPerson ? 'persistent-first-person-keyboard-mobile-camera-look' : 'camera-look-world-surface-selection'}
    data-home-pointer-lock="false"
    data-home-first-person-body="none-non-xr"
    data-home-stable-state={homeExperience.stableState}
    data-home-semantic-transition={homeExperience.transition ?? 'none'}
    data-home-assets-ready={ready ? 'true' : 'false'}
    data-home-ready={ready ? 'true' : 'warming'}
    data-home-input-ready={ready && !homeExperience.inputLocked ? 'true' : 'false'}
    data-home-interaction-ready={ready && !homeExperience.inputLocked ? 'true' : 'false'}
    ...
    data-home-camera-mode={cameraMode}
    data-home-scene-phase={phase}
    data-home-transition-sequence={transition === 'none' ? homeExperience.transition ?? 'idle' : `${transition}:traversal`}
    data-home-portal-sequence="idle"
    data-home-input-locked={transition !== 'none' || homeExperience.inputLocked ? 'true' : 'false'}
  ...
  >
    ...
    >
      <Scene
        ...
        owner={worldRef}
        stableMode={stableMode}
        homeTransition={homeExperience.transition}
        movement={movement}
        firstPersonPosition={firstPersonPosition}
        firstPersonVelocity={firstPersonVelocity}
        firstPersonTarget={firstPersonTarget}
        ...
        onRestoreComplete={homeApi.completeRestore}
        onComplete={completeTransition}
      />
    </Canvas>

    {homeExperience.stableState === 'AVATAR_HOME_FIRST_PERSON' ? <>
      <MovementHelp realm="Home" summary="Move through your Home in first person without a synthetic body overlay." controls="WASD or arrow keys move · drag to look · Escape returns to Home presentation." />
      <MobileMovementPad input={movement} label="Move through Home" />
      <button
        type="button"
        data-movement-ui="true"
        onClick={homeApi.openSelfView}
        style={{ position: 'absolute', right: 'max(16px, env(safe-area-inset-right))', bottom: 'max(18px, calc(env(safe-area-inset-bottom) + 8px))', zIndex: 35, minWidth: 48, minHeight: 48, borderRadius: 999, border: '1px solid rgba(225,242,235,.24)', background: 'rgba(4,14,17,.68)', color: '#f4faf7', padding: '0 16px', font: '600 12px/1 system-ui', backdropFilter: 'blur(14px)' }}
      >Self view</button>
    </> : null}
    ...
    <span className="sr-only" role="status" aria-live="polite">
      {transition === 'ground'
        ? 'Entering your physical Ground world.'
        : transition === 'life-map'
          ? 'Ascending into your Life Map.'
          : homeExperience.transition === 'AVATAR_EMBODIMENT_TRANSITION'
            ? 'Entering first-person Home.'
            : homeExperience.stableState === 'AVATAR_HOME_FIRST_PERSON'
              ? 'First-person Home active. Your Avatar body and synthetic hands are not shown.'
              : ''}
    </span>
