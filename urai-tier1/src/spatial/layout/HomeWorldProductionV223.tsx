...
function Rig({input,yaw,pitch,target,onNearby,transition,owner}:{input:MovementInput;yaw:MutableRefObject<number>;pitch:MutableRefObject<number>;target:MutableRefObject<THREE.Vector3|null>;onNearby:(n:Nearby)=>void;transition:Transition;owner:MutableRefObject<HTMLElement|null>}){
  ...
  useFrame((_,delta)=>{
    ...
    if(camera instanceof THREE.PerspectiveCamera){
      const f=size.height>size.width?(near==='orb'?54:50):(near==='orb'?48:42)
    ...
    let desired=pos.current.clone().add(new THREE.Vector3(Math.sin(yaw.current)*(near?1.32:.08),1.56,Math.cos(yaw.current)*(near?1.32:.08)))
    let look=pos.current.clone().add(forward)
    if(near==='orb'){
      desired=pos.current.clone().add(new THREE.Vector3(.58,1.62,2.42))
      look=destinationFocus.current.copy(ORB).add(new THREE.Vector3(0,.28,0))
    }else if(near==='ground'){
      desired=pos.current.clone().add(new THREE.Vector3(.78,1.58,1.72))
      look=destinationFocus.current.copy(GROUND).add(new THREE.Vector3(0,.52,0))
    }else if(near==='life-map'){
      desired=pos.current.clone().add(new THREE.Vector3(-.78,1.62,1.72))
      look=destinationFocus.current.copy(LIFE_MAP).add(new THREE.Vector3(0,.92,0))
    }
