...
function prepareModel(source: THREE.Object3D, targetSpan: number, materialMode: 'stone' | 'metal' | 'portal' | 'orb') {
  ...
  root.traverse((object) => {
    ...
    const materials = sourceMaterials.map((material) => {
      ...
      if (clone instanceof THREE.MeshStandardMaterial) {
        ...
        clone.envMapIntensity = materialMode === 'portal' ? 0.82 : 0.72
...
function GovernedModel({ url, position, rotation = [0, 0, 0], scale = [1, 1, 1], span, mode, name }: { url: string; position: Vec3; rotation?: Vec3; scale?: Vec3; span: number; mode: 'stone' | 'metal' | 'portal' | 'orb'; name: string }) {
  const gltf = useGLTF(url)
  const model = useMemo(() => prepareModel(gltf.scene, span, mode), [gltf.scene, mode, span])
  return <group name={name} position={position as [number, number, number]} rotation={rotation as [number, number, number]} scale={scale as [number, number, number]} userData={{ runtimeAsset: url, provenance: 'urai-governed-authored-binary', exactRuntimeOwner: true }}><primitive object={model} /></group>
}
...
function LoadBeam({ from, to, radius = 0.12, color = '#404944' }: { from: Vec3; to: Vec3; radius?: number; color?: string }) {
  ...
  return <mesh position={midpoint} quaternion={quaternion} castShadow receiveShadow><cylinderGeometry args={[radius * 0.86, radius, length, 10, 1, false]} /><meshStandardMaterial color={color} roughness={0.48} metalness={0.58} envMapIntensity={0.72} /></mesh>
}
...
function OrbReliquary({ state, reducedMotion, onOpen }: { state: OrbState; reducedMotion: boolean; onOpen: () => void }) {
  ...
  return <group name="home-orb-engineered-cradle" userData={{ treatment: 'v67-load-bearing-wall-integrated-reliquary', source: GOVERNED_ORB }}>
    ...
    <group ref={root} name="home-orb-sanctuary" position={ORB} onClick={(event: ThreeEvent<MouseEvent>) => { event.stopPropagation(); onOpen() }} userData={{ orbState: state, animation: sensory.animation, treatment: 'v67-six-armored-fragment-machine-with-contained-core', governedOrb: GOVERNED_ORB }}>
      ...
      <pointLight color="#85aa9f" intensity={0.30} distance={3.8} decay={2} />
...
function Threshold({ destination, position, onActivate }: { destination: 'ground' | 'life-map'; position: THREE.Vector3; onActivate: () => void }) {
  ...
  return <group name={destination === 'ground' ? 'home-ground-environmental-threshold' : 'home-life-map-sky-lookout'} position={position} userData={{ treatment: 'v67-recessed-structural-threshold', destination, governedPortal: GOVERNED_PORTAL }}>
    ...
    <mesh name={destination === 'life-map' ? 'home-life-map-physical-portal' : undefined} position={[0, 1.48, -0.24]} receiveShadow><planeGeometry args={[1.58, 2.40]} /><meshStandardMaterial color="#07100f" emissive={tone} emissiveIntensity={0.12} roughness={0.70} metalness={0.18} /></mesh>
    <pointLight position={[0, 1.62, 0.18]} color={tone} intensity={0.24} distance={3.8} decay={2} />
...
function Sanctuary({ target, reducedMotion, orbState, onOrb, onGround, onLifeMap }: { target: MutableRefObject<THREE.Vector3 | null>; reducedMotion: boolean; orbState: OrbState; onOrb: () => void; onGround: () => void; onLifeMap: () => void }) {
  ...
  return <>
    ...
    <group name="home-sanctuary-pavilion" userData={{ visualOwner: 'v67-governed-authored-chamber', construction: 'governed-home-entry-chamber-with-scanned-stone-integration', source: GOVERNED_HOME }}>
      <GovernedModel url={GOVERNED_HOME} name="home-v67-governed-entry-chamber" position={[0, 2.80, -2.35]} rotation={[0, Math.PI, 0]} scale={[1.00, 0.92, 1.00]} span={18.4} mode="stone" />
      ...
      <ProductionAsset url={ROCK_RELIEF} name="home-v67-scanned-right-relief" position={[5.52, 1.55, -5.38]} rotation={[-0.03, -1.26, -0.02]} scale={[0.72, 0.98, 0.44]} span={2.80} />
      ...
      <pointLight position={[-4.30, 2.30, -4.72]} color="#d1a66f" intensity={0.42} distance={4.8} decay={2} />
      <pointLight position={[4.30, 2.24, -5.30]} color="#7eaaa1" intensity={0.38} distance={4.8} decay={2} />
      <group name="home-v47-reliquary-cavity" /><group name="home-v47-side-gallery" /><group name="home-v47-reliquary-apse" />
    </group>
    <group name="home-v49-scanned-detail-layer" userData={{ treatment: 'v67-single-source-bounded-photogrammetry-relief-pair' }} />
    ...
    <pointLight position={[0, 3.20, -6.90]} color="#b6cfc4" intensity={0.46} distance={7.0} decay={2} />
  </>
}
...
function PlayerRig({ input, yaw, pitch, target, onNearby, transition }: { input: MovementInput; yaw: MutableRefObject<number>; pitch: MutableRefObject<number>; target: MutableRefObject<THREE.Vector3 | null>; onNearby: (value: Nearby) => void; transition: Transition }) {
  ...
  useEffect(() => { camera.near = 0.1; camera.far = 110; camera.position.set(4.45, 1.65, 3.15); camera.lookAt(0, 2.2, -7.2); camera.updateProjectionMatrix() }, [camera])
  useFrame((_, delta) => {
...
function Scene({ input, yaw, pitch, target, nearby, transition, reducedMotion, orbState, onOrb, onGround, onLifeMap, onReady }: { input: MovementInput; yaw: MutableRefObject<number>; pitch: MutableRefObject<number>; target: MutableRefObject<THREE.Vector3 | null>; nearby: (value: Nearby) => void; transition: Transition; reducedMotion: boolean; orbState: OrbState; onOrb: () => void; onGround: () => void; onLifeMap: () => void; onReady: () => void }) {
  return <><color attach="background" args={['#080d0e']} /><fogExp2 attach="fog" args={['#0e1514', 0.016]} /><Environment files={HOME_HDR} background={false} environmentIntensity={0.52} /><ambientLight intensity={0.28} color="#cedbd4" /><hemisphereLight args={['#879b96', '#0a0f0d', 0.38]} /><directionalLight position={[-7, 10, 5]} intensity={0.90} color="#ddc49a" castShadow shadow-mapSize-width={768} shadow-mapSize-height={768} /><directionalLight position={[7, 6, -8]} intensity={0.36} color="#769b96" /><Sanctuary target={target} reducedMotion={reducedMotion} orbState={orbState} onOrb={onOrb} onGround={onGround} onLifeMap={onLifeMap} /><PlayerRig input={input} yaw={yaw} pitch={pitch} target={target} onNearby={nearby} transition={transition} /><ReadySignal onReady={onReady} /></>
}
...
export function HomeWorldProductionV67({ onOrbOpen = requestUraiWorldOrbOpen, webglAvailable = true }: Props) {
  ...
  useEffect(() => {
    ...
    const traversalTimer = window.setTimeout(() => setPortalSequence(traversal), reducedMotion ? 80 : 220); const closingTimer = window.setTimeout(() => setPortalSequence(closing), reducedMotion ? 420 : 1500)
    const navigationTimer = window.setTimeout(() => { if (transition === 'ground') requestUraiWorldTravel({ destination: 'infrastructure-hub', href: '/ground/', entryPortal: 'home-ground', cameraCheckpoint: 'home-ground-descent' }); else requestUraiWorldTravel({ destination: 'life-map', href: '/life-map/?from=home-sky', entryPortal: 'home-sky', cameraCheckpoint: 'home-sky-ascent-complete' }) }, reducedMotion ? 720 : 2350)
  ...
  return <main className={`${styles.world} urai-asset-home-world`} data-urai-home-production data-urai-true-3d="true" data-home-primary-owner="asset-driven" data-home-visible-world="v67-asymmetric-scanned-stone-relic-sanctuary" data-home-world-character="production-cinematic-sacred-tech" data-home-physical-base="canted-scanned-stone-machine-reliquary" data-home-visual-ownership="three-dimensional-geometry" data-home-desktop-mobile-world="same-scene" data-home-embodied-self="privacy-preserving-first-person" data-home-movement="walk-keyboard-click-touch" data-home-visual-grade="cinematic-pbr-v67-armored-reliquary" data-home-final-art-revision="v67-armored-sanctuary-replacement" data-home-art-certification="v67-retained-pixel-candidate-not-certified" data-home-scanned-composition="single-bounded-photogrammetry-relief-v67" data-home-pbr-environment="local-cc0-hdri-studio-small-08" data-home-assets-ready={ready ? 'true' : 'false'} data-home-runtime-assets="urai-final-glb-home-entry-chamber urai-final-glb-portal-ring urai-final-glb-orb-avatar polyhaven-rock-face-01 polyhaven-industrial-caged-sconce polyhaven-rock-tile-floor-pbr studio-small-08-hdri" data-home-scenery-assets="urai-final-glb-home-entry-chamber portal-ring-master urai-orb-avatar polyhaven-rock-face-01 industrial-caged-sconce rock-tile-floor-pbr studio-small-08-hdri" data-home-authored-regions="home-authored-terrain home-mountain-horizon home-living-vegetation home-sanctuary-pavilion home-life-map-physical-portal" data-home-nearby={nearby ?? 'none'} data-home-camera-mode={transition !== 'none' ? transition : dragging ? 'look' : 'embodied-third-person'} data-home-scene-phase={transition === 'none' ? 'HOME' : transition.toUpperCase()} data-home-portal-sequence={portalSequence} data-home-portal-lifecycle="environmental-approach-traversal-arrival" data-home-animation-owner="enclosed-reliquary-v66-six-fragment-orb" data-home-input-locked={transition !== 'none' ? 'true' : 'false'} data-home-orb-state={orbState} data-home-orb-clip={resolveOrbSensoryOutput(orbState, reducedMotion, true).animation} data-home-orb-model-clip={reducedMotion ? 'stopped-reduced-motion' : ORB_CLIPS[orbState]} data-testid="home-visible-navigable-sanctuary-world" style={{ position: 'relative', overflow: 'hidden', background: '#080d0e' }} {...look}>
    ...
    <span className="sr-only" data-testid="urai-home-webgl-orb">The engineered six-fragment Orb relic-machine is physically integrated into the Home sanctuary.</span><span className="sr-only" data-testid="urai-home-embodied-avatar">Your privacy-preserving embodied Home presence remains active.</span>
  </main>
}
