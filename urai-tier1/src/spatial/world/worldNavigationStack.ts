...
export function writeWorldNavigationStack(storage: Pick<Storage, 'setItem'>, stack: readonly UraiWorldNavigationCheckpoint[]) {
...
export function popWorldNavigationCheckpoint(storage: Pick<Storage, 'getItem' | 'setItem'>): UraiWorldNavigationCheckpoint | undefined {
  const stack = readWorldNavigationStack(storage)
