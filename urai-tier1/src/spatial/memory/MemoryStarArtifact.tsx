...
function styleFor(morphology: MemoryMorphology, replayProgress = 0) {
  return {
    ...
    '--memory-replay-progress': Math.max(0, Math.min(1, replayProgress)),
  } as CSSProperties
}

export default function MemoryStarArtifact({
  morphology,
  replay = false,
  replayProgress = 0,
  replaySegment = 'memory',
}: {
  morphology: MemoryMorphology
  replay?: boolean
  replayProgress?: number
  replaySegment?: string
}) {
  return (
    <div
      ...
      data-replay-segment={replaySegment}
      data-replay-progress={Math.round(Math.max(0, Math.min(1, replayProgress)) * 100)}
      style={styleFor(morphology, replayProgress)}
      aria-hidden="true"
    >
      ...
      {replay ? <div className="memory-star-artifact__replay-phase"><span>{replaySegment}</span></div> : null}
