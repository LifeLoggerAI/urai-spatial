...
export function LocationMapScene({ places }: { places: MemoryPlace[] }) {
  ...
  const applyUrl = useCallback(() => {
    ...
    const point = points.find(item => item.place.id === params.get('placeId'))
    if (point) { setSelectedId(point.place.id); setCamera(focusCamera(point)); return }
    setSelectedId(null); setCamera(OVERVIEW)
  }, [focusCamera, points])
  ...
  useEffect(() => {
    ...
    } catch { setAccess(explicitDemo ? 'demo' : 'threshold') }
    applyUrl()
  ...
  if (access === 'checking') return <main className="locationAtlas locationAtlas--empty" data-location-map-owner="canonical-route"><section><p>URAI · Private emotional geography</p><h1>Opening the atlas.</h1><span>Checking the private threshold without mounting personal location history.</span></section></main>
  ...
  return <main className="locationAtlas" style={style} data-launch-surface="premium-emotional-weather-atlas" data-location-map-owner="canonical-route" data-location-map-renderer="layered-spatial-atlas" data-location-map-source={demoData || access === 'demo' ? 'disclosed-demo' : 'private-repository'} data-privacy-mode={privacyMode} data-entry-portal={entryPortal} data-camera-checkpoint={checkpoint} data-reduced-motion={reducedMotion ? 'true' : 'false'}>
    ...
    <header className="locationAtlasHeader" data-atlas-panel><div><span>URAI · Emotional geography</span><strong>Private atlas</strong></div><div className="locationAtlasStatus"><span>Private view</span><span>{demoData || access === 'demo' ? 'Disclosed sample places' : 'Permissioned places'}</span>{offline ? <span>Offline · local view retained</span> : null}</div><nav><Link href="/life-map">Life Map</Link><Link href="/home">Home</Link></nav></header>
    ...
    <div className="locationAtlasTruth" data-atlas-panel><span>{demoData || access === 'demo' ? 'Sample atlas' : 'Private atlas'}</span><small>Symbolic positions protect precise history. No live location requested.</small></div>
