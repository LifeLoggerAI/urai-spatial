...
function makePoints(places: MemoryPlace[]): AtlasPoint[] {
...
function currentParams() { return typeof window === 'undefined' ? new URLSearchParams() : new URLSearchParams(window.location.search) }
...
export function LocationMapScene({ places }: { places: MemoryPlace[] }) {
  ...
  const points = useMemo(() => makePoints(places), [places])
  ...
  const demo = places.length > 0 && places.every(place => place.privacyLevel === 'demo')

  const applyUrl = useCallback(() => {
    const params = currentParams()
    const requested = params.get('placeId')
    const point = points.find(item => item.place.id === requested)
    setPrivacyMode(params.get('privacyMode') || 'private')
    setEntryPortal(params.get('entryPortal') || 'location-beacon')
    setCheckpoint(params.get('cameraCheckpoint') || 'atlas-world-view')
    if (!point) { setSelectedId(null); setCamera(OVERVIEW); return }
    setSelectedId(point.place.id)
    const width = window.innerWidth
    const height = window.innerHeight
    setCamera({ x: ((50 - point.x) / 100) * width * .86, y: ((50 - point.y) / 100) * height * .68, zoom: width < 760 ? 1.34 : 1.54 })
  }, [points])
  ...
  }, [entryPortal, privacyMode, router])
  ...
  const focus = useCallback((point: AtlasPoint, index: number, push = true) => {
    const width = window.innerWidth
    const height = window.innerHeight
    setSelectedId(point.place.id); setActiveIndex(index); setCheckpoint('place-focus')
    setCamera({ x: ((50 - point.x) / 100) * width * .86, y: ((50 - point.y) / 100) * height * .68, zoom: width < 760 ? 1.34 : 1.54 })
  ...
  const moveMarker = (amount: number) => {
    ...
    setActiveIndex(next); markerRefs.current[next]?.focus(); setAnnouncement(points[next].place.title)
  }
  ...
  const style = {
    ...baseStyle,
    '--atlas-x': `${camera.x}px`, '--atlas-y': `${camera.y}px`, '--atlas-zoom': camera.zoom,
    '--weather-color': selected ? tone(selected.place) : '#8eeaff', '--weather-strength': selected?.place.emotionalOverlay.intensity ?? .34,
  } as CSSProperties
  ...
  return <main className="locationAtlas" style={style} data-launch-surface="premium-emotional-weather-atlas" data-location-map-owner="canonical-route" data-location-map-renderer="layered-spatial-atlas" data-location-map-source={demo ? 'disclosed-demo' : 'private-repository'} data-privacy-mode={privacyMode} data-entry-portal={entryPortal} data-camera-checkpoint={checkpoint} data-reduced-motion={reducedMotion ? 'true' : 'false'}>
    ...
    <header className="locationAtlasHeader" data-atlas-panel><div><span>URAI · Emotional geography</span><strong>Private atlas</strong></div><div className="locationAtlasStatus"><span>Private view</span><span>{demo ? 'Disclosed sample places' : 'Permissioned places'}</span>{offline ? <span>Offline · local view retained</span> : null}</div><nav><Link href="/life-map">Life Map</Link><Link href="/home">Home</Link></nav></header>
    ...
    <div className="locationAtlasTruth" data-atlas-panel><span>{demo ? 'Sample atlas' : 'Private atlas'}</span><small>Symbolic positions protect precise history. No live location requested.</small></div>
