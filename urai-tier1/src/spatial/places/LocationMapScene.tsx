...
export function LocationMapScene({ places }: { places: MemoryPlace[] }) {
  return (
    <main className="relative h-[100svh] max-h-[100svh] min-h-[100svh] overflow-hidden bg-[#020617] text-white" data-launch-surface="premium-emotional-weather-atlas">
      ...
      <section className="relative z-10 mx-auto flex h-full max-h-[100svh] max-w-7xl flex-col overflow-hidden px-4 py-4 md:px-10 md:py-8">
        <header className="shrink-0 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
        ...
        <section className="mt-4 grid shrink-0 grid-cols-2 gap-2 md:mt-8 md:grid-cols-4 md:gap-3" aria-label="Global emotional weather legend">
          {['Calm field', 'Threshold storm', 'Recovery glow', 'Memory fog'].map((label) => (
            <article key={label} className="rounded-2xl border border-white/10 bg-black/30 p-3 backdrop-blur-xl md:p-4">
        ...
        <div className="relative mt-4 grid min-h-0 flex-1 gap-3 overflow-y-auto pr-1 md:mt-8 md:grid-cols-2 md:gap-4 lg:grid-cols-3" aria-label="Private place memory doors">
          {places.map((place, index) => (
            <Link
              ...
              className="group relative min-h-[11.5rem] overflow-hidden rounded-[1.6rem] border border-white/10 bg-white/[0.06] p-4 shadow-[0_26px_90px_rgba(0,0,0,.34)] backdrop-blur-xl transition hover:-translate-y-1 hover:bg-white/[0.1] md:rounded-[2rem] md:p-5"
            >
              <div
              ...
              <div className="relative">
                ...
                <div className="mt-4 rounded-2xl border border-white/10 bg-black/25 p-3 text-xs leading-5 text-slate-300 md:mt-5">
