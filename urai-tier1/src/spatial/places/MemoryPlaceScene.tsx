...
function PlaceObjectMesh({ object }: { object: PlaceObject }) {
  const scale = Math.max(0.2, object.scale);
  ...
  return (
    <group position={object.position} rotation={object.rotation ?? [0, 0, 0]}>
      {isPortal ? (
        <mesh scale={[scale * 0.9, scale * 1.45, 0.08]}>
          <boxGeometry args={[1, 1, 1]} />
          <meshStandardMaterial emissive="#7dd3fc" color="#1f2937" metalness={0.2} roughness={0.45} />
        </mesh>
      ) : (
        <mesh scale={[scale, scale, scale]}>
          <sphereGeometry args={[0.38, 32, 32]} />
          <meshStandardMaterial emissive="#a78bfa" color="#f8fafc" metalness={0.1} roughness={0.35} />
        </mesh>
      )}
    </group>
  );
...
function PlaceWorld({ place, objects }: { place: MemoryPlace; objects: PlaceObject[] }) {
  ...
  return (
    <>
      ...
      {objects.map((object) => (
        <PlaceObjectMesh key={object.id} object={object} />
      ))}
    </>
  );
}
    ...
    <main className="min-h-screen bg-slate-950 text-white">
      <section className="relative h-screen overflow-hidden">
        <Canvas className="absolute inset-0">
          <PlaceWorld place={place} objects={objects} />
        </Canvas>
        ...
        <div className="absolute bottom-0 left-0 right-0 z-10 flex flex-col gap-3 bg-gradient-to-t from-slate-950 via-slate-950/90 to-transparent p-6 md:flex-row md:items-end md:justify-between">
          <div className="rounded-3xl border border-white/10 bg-white/10 p-4 backdrop-blur">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {objects.map((object) => (
                <span key={object.id} className="rounded-full border border-white/15 px-3 py-1 text-xs text-slate-100">
                  {object.label}
                </span>
              ))}
            </div>
