...
function buildReplaySteps(selectedStarId: SelectedStar): string[] {
  const title =
    str(selectedStarId?.title) ??
    str(selectedStarId?.label) ??
    str(selectedStarId?.name) ??
    "Memory Replay";
...
function getReplayGlow(selectedStarId: SelectedStar): string {
  return (
    str(selectedStarId?.glow) ??
    str(selectedStarId?.color) ??
    str(selectedStarId?.auraColor) ??
    "rgba(255,255,255,0.35)"
  );
}

function getReplayMeta(selectedStarId: SelectedStar): { title: string; subtitle: string } {
  return {
    title:
      str(selectedStarId?.title) ??
      str(selectedStarId?.label) ??
      ...
      "Memory Replay",
    subtitle:
      str(selectedStarId?.subtitle) ??
      str(selectedStarId?.description) ??
      "Replaying the selected memory thread.",
  };
}

export default function ReplayOverlay() {
  const selectedStarRaw = useSceneStore((s: any) => s.selectedStarId ?? null);
  ...
  const selectedStarId = useMemo<SelectedStar>(() => {
    return asRecord(selectedStarRaw);
  }, [selectedStarRaw]);

  const steps = useMemo(() => buildReplaySteps(selectedStarId), [selectedStarId]);
