...
export function rollbackReplayOperation(
  ...
  message: string,
  baseline: ReplayOperationState = emptyState(),
): ReplayOperationState {
  const retainedAudit = state.audit.filter((entry) => entry.id !== operation.id)
  const baselineIds = new Set(baseline.audit.map((entry) => entry.id))
  const concurrent = retainedAudit.filter((entry) => !baselineIds.has(entry.id))
  const replayed = concurrent.reduce<ReplayOperationState>((current, entry) => {
    const applied = applyReplayOperation({ ...current, pending: [] }, entry)
    return { ...applied, pending: [] }
  }, { ...baseline, pending: [], audit: baseline.audit.slice(-MAX_AUDIT_ENTRIES), error: undefined })
  return {
    ...replayed,
...
export async function executeReplayOperation(options: {
  ...
  const optimisticStored = writeReplayOperationState(storage, operation.ownerId, operation.memoryId, optimistic)
  ...
  try {
    ...
    const latest = optimisticStored
      ...
      : optimistic
    const represented = latest.audit.some((entry) => entry.id === operation.id)
      ? latest
      : applyReplayOperation(latest, operation)
    const settled = settleReplayOperation(represented, operation.id)
...
export async function flushReplayOperationQueue(options: {
  ...
  for (const operation of [...initial.pending]) {
    try {
      ...
      const source = latest.audit.some((entry) => entry.id === operation.id) ? latest : state
      state = settleReplayOperation(source, operation.id)
      writeReplayOperationState(storage, ownerId, memoryId, state)
    } catch {
      return readReplayOperationState(storage, ownerId, memoryId)
    }
  }
  const stored = readReplayOperationState(storage, ownerId, memoryId)
  return stored.audit.length || stored.pending.length || stored.saved || stored.hidden || stored.correction ? stored : state
}
