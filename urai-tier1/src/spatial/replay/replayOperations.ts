...
export type ReplayOperationTransport = {
  persist(operation: ReplayOperation): Promise<void>
}
...
function storageKey(ownerId: string, memoryId: string) {
  return `${STORAGE_PREFIX}${ownerId}:${memoryId}`
}

function emptyState(): ReplayOperationState {
  return { saved: false, hidden: false, pending: [], audit: [] }
}
...
export function readReplayOperationState(
  storage: Pick<ReplayOperationStorage, 'getItem'>,
  ownerId: string,
  memoryId: string,
): ReplayOperationState {
  try {
...
export function writeReplayOperationState(
  storage: Pick<ReplayOperationStorage, 'setItem'>,
  ownerId: string,
  memoryId: string,
  state: ReplayOperationState,
) {
  storage.setItem(storageKey(ownerId, memoryId), JSON.stringify({
    ...state,
...
export function applyReplayOperation(
  ...
  const next: ReplayOperationState = {
    ...state,
    error: undefined,
    pending: [...state.pending, operation],
    audit: [...state.audit, operation].slice(-MAX_AUDIT_ENTRIES),
  }
...
export function rollbackReplayOperation(
  state: ReplayOperationState,
  operation: ReplayOperation,
  message: string,
): ReplayOperationState {
  const previousAudit = state.audit.filter((entry) => entry.id !== operation.id)
  const replayed = previousAudit.reduce<ReplayOperationState>(
    (current, entry) => {
...
export function settleReplayOperation(
  state: ReplayOperationState,
  operationId: string,
): ReplayOperationState {
  return {
    ...state,
    pending: state.pending.filter((entry) => entry.id !== operationId),
    error: undefined,
  }
}
...
export async function executeReplayOperation(options: {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Replay operation could not be saved.'
    const rolledBack = rollbackReplayOperation(optimistic, operation, message)
