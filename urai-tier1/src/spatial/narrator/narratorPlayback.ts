...
class NarratorPlaybackController {
  ...
  async playLine(line: NarratorLine) {
    ...
    this.delayTimer = setTimeout(async () => {
      ...
      const blob = await requestNarratorAudio(line, this.aborter.signal, this.externalVoiceConsent);
      if (!blob) {
      ...
      audio.onended = () => { cleanup(); this.emit(null, false); };
      audio.onerror = () => { cleanup(); this.fallbackSpeech(line); };
      this.cutoffTimer = setTimeout(() => this.stopLine("duration-cutoff"), line.durationMs);
      try { await audio.play(); } catch { this.fallbackSpeech(line); }
    }, line.delayMs + jitter);
  }
