...
export function resolveEraCompareById(
  ...
  const similarities = unique([
    memory.chapter && (str(compareRow?.chapter) ?? str(compareRow?.arc)) === memory.chapter ? `shared chapter · ${memory.chapter}` : undefined,
    memory.timeband && (str(compareRow?.timeband) ?? str(compareRow?.season)) === memory.timeband ? `shared timeband · ${memory.timeband}` : undefined,
    memory.emotion && (str(compareRow?.emotion) ?? str(compareRow?.primaryEmotion)) === memory.emotion ? `shared emotion · ${memory.emotion}` : undefined,
    storyArc?.arcStage ? `current arc stage · ${storyArc.arcStage}` : undefined,
  ]).slice(0, 6);

  const differences = unique([
    memory.chapter && compareRow && (str(compareRow.chapter) ?? str(compareRow.arc)) !== memory.chapter ? `chapter shift · ${memory.chapter} → ${str(compareRow.chapter) ?? str(compareRow.arc)}` : undefined,
    memory.timeband && compareRow && (str(compareRow.timeband) ?? str(compareRow.season)) !== memory.timeband ? `timeband shift · ${memory.timeband} → ${str(compareRow.timeband) ?? str(compareRow.season)}` : undefined,
    memory.emotion && compareRow && (str(compareRow.emotion) ?? str(compareRow.primaryEmotion)) !== memory.emotion ? `emotion shift · ${memory.emotion} → ${str(compareRow.emotion) ?? str(compareRow.primaryEmotion)}` : undefined,
  ]).slice(0, 6);
  ...
  const summary =
    compareTitle
      ? `This memory can now be compared against a nearby era match to expose continuity and change across time, tone, and chapter.`
      : `Era compare is active, but the current dataset does not yet provide a strong secondary era target.`;

  return {
    id,
    title: `${memory.title} Era Compare`,
