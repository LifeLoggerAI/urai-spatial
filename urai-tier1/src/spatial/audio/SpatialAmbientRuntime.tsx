...
export function SpatialAmbientRuntime(){
  ...
  useEffect(()=>{
    ...
    const handleCue=(event:Event)=>{const cue=(event as CustomEvent<{cue?:SpatialAudioCue}>).detail?.cue;if(!cue||!(cue in CUE_CAPTIONS))return;setLiveCaption(CUE_CAPTIONS[cue]);if(consented&&!muted)audio.playCue(cue);if('vibrate' in navigator){if(cue==='error')navigator.vibrate([18,35,18]);else if(cue==='permission')navigator.vibrate(12);else if(cue==='confirm'||cue==='orb-confirm')navigator.vibrate(8)}};
