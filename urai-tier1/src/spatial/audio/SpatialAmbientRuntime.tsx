...
export function SpatialAmbientRuntime(){
  ...
  useEffect(()=>{
    ...
    const handleMute=(event:Event)=>{const nextMuted=Boolean((event as CustomEvent<{muted?:boolean}>).detail?.muted);setMuted(nextMuted);try{sessionStorage.setItem(MUTE_KEY,nextMuted?'true':'false')}catch{} if(nextMuted){audio.stopAllAudio();return} if(consented&&spatialPhase)audio.setAmbientPhase(spatialPhase)};
    const handleCue=(event:Event)=>{const cue=(event as CustomEvent<{cue?:SpatialAudioCue}>).detail?.cue;if(!cue||!(cue in CUE_CAPTIONS))return;setLiveCaption(CUE_CAPTIONS[cue]);if(consented&&!muted&&(cue==='confirm'||cue==='permission'))audio.playCue(cue);if(cue==='error')requestHapticCue('action-error','audio-cue');else if(cue==='permission')requestHapticCue('permission-acknowledged','audio-cue');else if(cue==='confirm'||cue==='orb-confirm')requestHapticCue('action-confirmed','audio-cue')};
