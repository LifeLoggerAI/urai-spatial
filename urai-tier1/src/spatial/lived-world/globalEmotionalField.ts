...
export type GlobalEmotionalFieldCell = {
  id: string
  regionKey: string
  precision: GlobalFieldPrecision
  timeBucket: string
  cohortSize: number
  requiredMinimumCohort: number
  sensitiveThresholdApproved: boolean
  locationSensitive: boolean
  sensitiveInferenceDerived: boolean
  signal: Partial<Record<GlobalEmotionalSignal, number>>
  confidence: number
  policyVersion: string
  riskAssessmentId: string
}

export type GlobalEmotionalFieldDecision = {
  state: GlobalFieldState
  reason: string
  cell: GlobalEmotionalFieldCell | null
}

export type GlobalEmotionalFieldConsent = ConsentDecisionSnapshot & {
  purpose: typeof GLOBAL_EMOTIONAL_FIELD_PURPOSE
  tier: 'C8'
}
...
/**
 * Public-good field evaluation is intentionally stricter than ordinary display.
 * The privacy repository's current anonymization standard establishes 100 users
 * as the default floor and requires higher minimums for location/sensitive data.
 * This function therefore refuses sensitive/location cells unless the cell also
 * carries an approved higher threshold (>100), rather than guessing that value.
 */
export function evaluateGlobalEmotionalFieldCell(args: {
  cell: GlobalEmotionalFieldCell | null | undefined
  consent: GlobalEmotionalFieldConsent | null | undefined
}): GlobalEmotionalFieldDecision {
  const { cell, consent } = args
  if (!consent || consent.status !== 'granted') return { state: 'suppressed', reason: 'DEDICATED_PUBLIC_GOOD_CONSENT_NOT_GRANTED', cell: null }
  ...
  if ((cell.locationSensitive || cell.sensitiveInferenceDerived) && (!cell.sensitiveThresholdApproved || cell.requiredMinimumCohort <= GLOBAL_EMOTIONAL_FIELD_DEFAULT_MINIMUM_COHORT)) {
    return { state: 'suppressed', reason: 'HIGHER_SENSITIVE_COHORT_THRESHOLD_REQUIRED', cell: null }
  }
...
export type PersonalEmotionalWeather = {
  state: 'off' | 'private-pattern'
...
export function evaluatePersonalEmotionalWeather(args: {
  consent: ConsentDecisionSnapshot | null | undefined
  weather: PersonalEmotionalWeather | null | undefined
}): PersonalEmotionalWeather | null {
