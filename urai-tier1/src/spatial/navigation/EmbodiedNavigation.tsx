...
export function MobileMovementPad({ input, label }: { input: MovementInput; label: string }) {
  ...
  return (
    <div className="urai-mobile-movement" data-movement-ui="true" role="group" aria-label={label}>
      ...
      <style jsx>{`
        .urai-mobile-movement{display:none;position:absolute;left:max(12px,env(safe-area-inset-left));bottom:max(82px,calc(env(safe-area-inset-bottom) + 72px));z-index:28;grid-template-columns:repeat(3,48px);grid-template-rows:repeat(2,48px);gap:5px;touch-action:none;pointer-events:none}
        button{pointer-events:auto;width:48px;height:48px;border:1px solid rgba(207,250,254,.25);border-radius:16px;background:rgba(2,12,26,.68);backdrop-filter:blur(14px);color:#fff;font:800 20px/1 system-ui;box-shadow:0 10px 30px rgba(0,0,0,.28)}button:first-child{grid-column:2}.urai-mobile-movement button:nth-child(2){grid-column:1;grid-row:2}.urai-mobile-movement button:nth-child(3){grid-column:2;grid-row:2}.urai-mobile-movement button:nth-child(4){grid-column:3;grid-row:2}button[data-active="true"],button:focus-visible{background:rgba(35,103,130,.9);outline:3px solid #fff;outline-offset:2px}
