...
function toOperatorText(input: {
  ...
  return `Run ${input.watchBand} breach watch · likelihood ${input.breachLikelihood}.`;
}
...
export function buildSpatialCuratedDeckBreachWatch(input: {
  ...
  const parts = [
    `breach ${breachWatchScore}`,
    `band ${watchBand}`,
    `likelihood ${breachLikelihood}`,
    `triggers ${activeTriggers.length}`,
    `mitigations ${requiredMitigations.length}`,
    `sla ${sla.breachRisk}`,
