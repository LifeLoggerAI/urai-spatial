...
export function buildSpatialCuratedDeckDrift(input: {
  ...
  for (let i = 1; i < windowEntries.length; i += 1) {
    ...
    steps.push({
      id: `${fromEntry.id}__${toEntry.id}`,
      fromEntryId: fromEntry.id,
      toEntryId: toEntry.id,
      label: `${fromEntry.label} → ${toEntry.label}`,
  ...
  const parts = [
    `account window ${windowEntries.length}`,
    `steps ${steps.length}`,
    `drift ${driftScore}`,
    `band ${driftBand}`,
    `source changed ${sourceChangedCount}`,
    `scene shifts ${sceneModeShiftTotal}`,
    `star shifts ${selectedStarShiftTotal}`,
  ];
