...
test('ecosystem registry is exact-stack bound, fail-closed, and names every governed identity', () => {
  ...
  assert.equal(registry.stackedBaseCommit, 'f8b624251514ee979d230efc7675e8936f6e14de')
  ...
  assert.equal(registry.dependencies.trustedPreview.mergeCommit, '36c31888d097dfacaebf162e785c2cbb0ff012fb')
  assert.equal(registry.dependencies.trustedPreview.mergedIntoMain, true)
  assert.deepEqual(registry.dependencies.trustedPreview.followUpRepairPullRequests, [1021, 1023])
  assert.equal(registry.dependencies.trustedPreview.currentMainAfterRepairs, '3e88a085e137a727aa65b4cc7e6aa107e27e2b9d')
  assert.equal(registry.dependencies.trustedPreview.requiredBeforeEcosystemMerge, false)
  assert.equal(registry.dependencies.consentGatedGeography.pullRequest, 1024)
  assert.equal(registry.dependencies.consentGatedGeography.exactHead, 'f8b624251514ee979d230efc7675e8936f6e14de')
  assert.equal(registry.dependencies.consentGatedGeography.exactHeadWorkflowCount, 24)
...
test('RuAi access defaults deny private data, exact location, and medical automation', () => {
  const registry = JSON.parse(fs.readFileSync(governancePath, 'utf8'))
  ...
  assert.equal(registry.ruaiMinimumControls.exactLocationDefaultDenied, true)
...
test('receipts describe the current dependency stack without transferring stale authority', () => {
  ...
  for (const source of [convergence, ledger]) {
    assert.match(source, /PR #1024/)
    assert.match(source, /f8b624251514ee979d230efc7675e8936f6e14de/)
    assert.match(source, /3e88a085e137a727aa65b4cc7e6aa107e27e2b9d/)
    assert.match(source, /24\/24 exact-head workflows green/i)
    assert.match(source, /issue `?#999`?/i)
  }
  assert.match(convergence, /merged externally/)
  assert.match(convergence, /No workflow or artifact from a stale stack transfers authority/)
  assert.match(ledger, /Former PRs #1013, #1019, #1020, and #1022 are superseded/)
})
