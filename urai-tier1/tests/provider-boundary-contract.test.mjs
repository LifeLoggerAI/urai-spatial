...
const narratorCopy = fs.readFileSync(new URL('../src/spatial/narrator/narratorCopy.ts', import.meta.url), 'utf8')
...
test('ElevenLabs is rights-bound, duration-bounded, single-attempt and session-consented', () => {
  ...
  assert.match(providerFunctions, /redirect: 'error'/)
  assert.match(providerFunctions, /ELEVENLABS_MAX_RESPONSE_BYTES/)
  assert.match(providerFunctions, /ELEVENLABS_RESPONSE_TOO_LARGE/)
  assert.match(providerFunctions, /OPENAI_RESPONSE_TOO_LARGE/)
  ...
  assert.match(providerRuntimeEnv, /ELEVENLABS_MAX_RESPONSE_BYTES=8388608/)
  assert.doesNotMatch(providerRuntimeEnv, /pNInz6obpgDQGcFmaJgB/)
  assert.doesNotMatch(rootEnvExample, /pNInz6obpgDQGcFmaJgB/)
  assert.doesNotMatch(narratorCopy, /pNInz6obpgDQGcFmaJgB/)
  assert.match(narratorCopy, /NEXT_PUBLIC_URAI_ELEVENLABS_DEFAULT_VOICE_ID \|\| ""/)
