...
test('Focus focal authority is the selected memory star and cannot regress to V321 terrain cavities', () => {
  assert.match(focus, /Locked product authority; V341 is the current literal-pixel implementation/)
  ...
  assert.match(focus, /selected-memory-star-resolving-through-memory-v341/)
  assert.match(focus, /radial-gradient\(circle at 50% 50%,#000 0 34%/)
  assert.match(focus, /opacity=\{memory \? 0\.74 : 0\.18\}/)
