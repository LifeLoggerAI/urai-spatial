...
test('Focus focal authority is the selected memory star and cannot regress to V321 terrain cavities', () => {
  ...
  assert.match(focus, /scale=\{\[4\.62, 4\.10, 1\]\}/)
  assert.match(focus, /opacity=\{memory \? \.46 : \.03\}/)
  assert.match(focus, /distanceFactor=\{1\.46\}/)
  ...
  assert.match(focus, /width:168px;height:108px/)
  assert.match(focus, /opacity:\.72;filter:saturate\(1\.04\)/)
  ...
  assert.match(focus, /opacity=\{memory \? 0\.26 : 0\.16\}/)
