...
test('Focus focal authority is the selected memory star and cannot regress to V321 terrain cavities', () => {
  ...
  assert.match(focus, /selected-memory-star-resolving-through-memory-v367/)
  ...
  assert.match(focus, /opacity=\{memory \? \.48 : \.04\}/)
  assert.match(focus, /distanceFactor=\{1\.72\}/)
  ...
  assert.match(focus, /opacity=\{memory \? \.52 : \.03\}/)
  assert.match(focus, /opacity=\{memory \? \.36 : \.02\}/)
  ...
  assert.doesNotMatch(focus, /mask-image:radial-gradient\(ellipse at 49% 51%,transparent 0 27%/)
  assert.match(focus, /mask-image:radial-gradient\(circle at 50% 50%,#000 0 44%/)
  assert.doesNotMatch(focus, /repeating-conic-gradient\(/)
  assert.match(focus, /\.focusStarMemoryButton::before\{[^}]*border-radius:50%/)
