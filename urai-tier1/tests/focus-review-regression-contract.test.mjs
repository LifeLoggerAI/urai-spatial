...
test('Focus focal authority is the selected memory star and cannot regress to V321 terrain cavities', () => {
  assert.match(focus, /Locked product authority; V350 is the current literal-pixel implementation/)
  ...
  assert.match(focus, /selected-memory-star-resolving-through-memory-v350/)
  ...
  assert.match(focus, /new THREE\.IcosahedronGeometry\(1, 5\)/)
  assert.doesNotMatch(focus, /makeFocusCoronaShellGeometry|geometry=\{coronaShellA\}|geometry=\{coronaShellB\}/)
  assert.match(focus, /width:168px;height:112px/)
  assert.match(focus, /opacity:\.60;filter:saturate\(1\.02\)/)
  ...
  assert.match(focus, /opacity=\{memory \? 0\.58 : 0\.16\}/)
  assert.match(focus, /function makeFocusSphereTexture\(\)/)
  assert.match(focus, /<meshBasicMaterial[\s\S]*map=\{sphereTexture\}[\s\S]*blending=\{THREE\.AdditiveBlending\}/)
