...
test('Focus focal authority is the selected memory star and cannot regress to V321 terrain cavities', () => {
  assert.match(focus, /Locked product authority; V383 is the current literal-pixel implementation/)
  ...
  assert.match(focus, /selected-memory-star-resolving-through-memory-v383/)
  ...
  assert.match(focus, /scale=\{\[4\.65, 3\.85, 1\]\}/)
  assert.match(focus, /opacity=\{memory \? \.82 : \.10\}/)
  assert.match(focus, /scale=\{\[6\.35, 4\.85, 1\]\}/)
  assert.match(focus, /opacity=\{memory \? \.76 : \.06\}/)
  assert.match(focus, /scale=\{\[2\.18, 1\.82, 1\]\}/)
  assert.match(focus, /opacity=\{memory \? \.74 : \.05\}/)
  ...
  assert.match(focus, /focusMemoryVisual\{[^}]*width:70%;height:48%/)
  ...
  assert.match(focus, /opacity:\.80;filter:saturate\(\.88\)/)
