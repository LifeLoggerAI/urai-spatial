...
test('Focus focal authority is the selected memory star and cannot regress to V321 terrain cavities', () => {
  assert.match(focus, /Locked product authority; V351 is the current literal-pixel implementation/)
  ...
  assert.match(focus, /selected-memory-star-resolving-through-memory-v351/)
  ...
  assert.match(focus, /width:116px;height:76px/)
  assert.match(focus, /opacity:\.34;filter:saturate\(\.96\)/)
  ...
  assert.match(focus, /opacity=\{memory \? 0\.54 : 0\.16\}/)
