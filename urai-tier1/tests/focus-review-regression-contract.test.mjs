...
test('Focus focal authority is the selected memory star and cannot regress to V321 terrain cavities', () => {
  assert.match(focus, /Locked product authority; V368 is the current literal-pixel implementation/)
  ...
  assert.match(focus, /selected-memory-star-resolving-through-memory-v368/)
  ...
  assert.match(focus, /distanceFactor=\{1\.82\}/)
  ...
  assert.match(focus, /opacity=\{memory \? \.68 : \.03\}/)
  assert.match(focus, /opacity=\{memory \? \.46 : \.02\}/)
  ...
  assert.match(focus, /border-radius:34% 66% 43% 57%\/58% 38% 62% 42%/)
  ...
  assert.match(focus, /\.focusStarMemoryButton::before\{[^}]*border-radius:41% 59% 46% 54%\/57% 43% 61% 39%/)
  ...
  assert.match(focus, /<meshBasicMaterial[\s\S]*map=\{sphereTexture\}[\s\S]*blending=\{THREE\.NormalBlending\}[\s\S]*depthWrite=\{false\}/)
