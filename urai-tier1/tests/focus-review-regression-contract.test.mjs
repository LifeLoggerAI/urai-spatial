...
test('Focus focal authority is the selected memory star and cannot regress to V321 terrain cavities', () => {
  ...
  assert.match(focus, /selected-memory-star-resolving-through-memory-v374/)
  ...
  assert.match(focus, /scale=\{\[5\.80, 4\.40, 1\]\}/)
  assert.match(focus, /opacity=\{memory \? \.58 : \.04\}/)
  assert.match(focus, /distanceFactor=\{1\.92\}/)
  assert.match(focus, /function makeFocusCoronaFilamentGeometry\(count: number, innerRadius: number, outerRadius: number, phase: number\)/)
  ...
  assert.match(focus, /opacity=\{memory \? \.92 : \.03\}/)
  assert.match(focus, /opacity=\{memory \? \.86 : \.02\}/)
