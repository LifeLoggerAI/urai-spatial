...
test('Focus focal authority is the selected memory star and cannot regress to V321 terrain cavities', () => {
  ...
  assert.match(focus, /selected-memory-star-resolving-through-memory-v366/)
  ...
  assert.match(focus, /const boundary = THREE\.MathUtils\.clamp\(\.90/)
  assert.match(focus, /const rayBoundary = rays/)
  assert.match(focus, /scale=\{\[3\.06, 2\.58, 1\]\}/)
  assert.match(focus, /opacity=\{memory \? \.42 : \.04\}/)
  assert.match(focus, /distanceFactor=\{1\.62\}/)
  ...
  assert.match(focus, /opacity=\{memory \? \.38 : \.03\}/)
  assert.match(focus, /opacity=\{memory \? \.26 : \.02\}/)
  assert.match(focus, /box-shadow:0 0 24px rgba\(255,247,211,\.92\),0 0 42px rgba\(255,174,67,\.28\)/)
