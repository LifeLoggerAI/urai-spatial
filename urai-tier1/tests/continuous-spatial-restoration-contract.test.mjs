...
const naturalProof = read('../scripts/run-continuous-spatial-proof-v22-natural.mjs')
...
test('V288 biomorphic reliquary is restored as current visible authority while V291 retains interaction semantics', () => {
  has(visualAuthority, 'Current Home Orb visual authority shim')
  has(visualAuthority, 'visible morphology is restored to')
  assert.match(visualAuthority, /<HomeOrbGroundedV288\s*\/>/)
  ...
  has(renderer, "visualAuthority: 'v288-grounded-biomorphic-reliquary'")
  has(renderer, "interactionAuthority: 'v291-current-home-orb-state-and-speech-runtime'")
...
test('v22 natural proof follows current Home controller states without stale traversal contracts', () => {
  for (const marker of [
    'getBoundingClientRect()',
    'urai:onboarding:v3:setup-complete',
