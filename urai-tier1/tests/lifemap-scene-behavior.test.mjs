...
test('Production artifacts are differentiated by meaning rather than generic bubbles', () => {
  ...
  assert.match(world, /scale={active \? 1\.04 : emphasis \? 1\.16 \+ importance \* 0\.30 : 1\.08 \+ importance \* 0\.30}/)
  assert.match(world, /position=\{\[memoryX, memoryY - \.28, memoryZ - \.6\]\}/)
...
test('Relationships use curved semantic path classes, living pulses, and privacy-aware rendering', () => {
  ...
  assert.match(world, /function LineageRibbon/)
  assert.match(world, /name="life-map-lineage-ribbon"/)
  assert.match(world, /active \? \.18 : \.022/)
  assert.match(world, /resolvePathKind\(source, target\)/)
  assert.match(world, /kind === "protected"/)
  assert.match(world, /dashed={kind === "inferred" \|\| kind === "corrected"}/)
...
test('Reduced motion portrait adaptive quality and high contrast retain equivalent journeys', () => {
  ...
  assert.match(source, /lifeMapOverviewCamera\(nodes, size\.height > size\.width, size\.width \/ Math\.max\(size\.height, 1\)\)/)
  assert.match(source, /: overviewGoal/)
  assert.doesNotMatch(source, /positionGoal\.current\.set\(0, 3\.0, 10\.2\)/)
