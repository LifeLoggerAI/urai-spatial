...
test('Life Map uses deterministic sequential travel with selected-memory stand-off', () => {
  ...
  assert.match(cosmic, /setPhase\(profile\.reducedMotion \? "arrival" : "departure"\)/)
  assert.match(cosmic, /phase === "departure"\) setPhase\("travel"\)/)
  assert.match(cosmic, /phase === "travel"\) setPhase\("approach"\)/)
  assert.match(cosmic, /phase === "approach"\) setPhase\("arrival"\)/)
  assert.match(cosmic, /if \(profile\.reducedMotion\) \{ journey\.current \+= 1; setPhase\("arrival"\); return; \}/)
