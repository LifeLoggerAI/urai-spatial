...
test('TierOneExperience maps routed modes to the canonical HomeScene shell', () => {
  ...
  assert.match(source, /export type TierOneExperienceMode = "home" \| "ascent" \| "life-map" \| "demo" \| "replay" \| "focus" \| "unwind" \| "mirror"/)
  assert.match(source, /if \(mode === "replay"\) return "replay" as const/)
  assert.match(source, /if \(mode === "focus" \|\| mode === "mirror" \|\| mode === "unwind"\) return "detail" as const/)
...
test('HomeScene keeps Home, Ascent, and Life Map visual authority separate', () => {
  ...
  assert.match(source, /type SceneMode = 'home' \| 'ascent' \| 'life-map' \| 'demo' \| 'replay' \| 'focus' \| 'unwind' \| 'mirror'/)
  ...
  assert.match(source, /const showOrb = sceneMode === 'focus' \|\| sceneMode === 'replay' \|\| sceneMode === 'unwind' \|\| sceneMode === 'mirror'/)
...
test('HomeScene locks silent home to ascent to lifemap routing', () => {
  const source = flat(homeScene)
  ...
  assert.match(source, /onClick=\{isHomeMode \? enterLifeMap : undefined\}/)
