...
const files = {
  ...
  constellationLayerRaw: read(['src/spatial/constellation/ConstellationLayer.tsx']),
  manifestTypesRaw: read(['src/spatial/assets/manifestTypes.ts']),
  narratorHudRaw: read(['src/spatial/narrator/NarratorHud.tsx']),
}
...
const constellationLayer = flat(files.constellationLayerRaw)
const manifestTypes = flat(files.manifestTypesRaw)
const narratorHud = flat(files.narratorHudRaw)
...
test('HomeScene routes Home to Ascent to Life Map', () => {
...
test('HomeScene locks focus, replay, mirror, and unwind behavior', () => {
  ...
  assert.match(scene, /router\.push\(manifestMirrorHref\(activeManifestId\)\)/)
  ...
  assert.match(scene, /if \(sceneMode === 'focus' \|\| sceneMode === 'mirror'\)/)
...
test('focus, replay, and mirror use demo fallback instead of unavailable error copy', () => {
  ...
  assert.match(scene, /const modeNeedsManifest = sceneMode === 'focus' \|\| sceneMode === 'replay' \|\| sceneMode === 'mirror'/)
...
test('Life Map AAA HUD exposes filters, status, narrator fallback, replay shell, and mirror summary', () => {
  assert.match(scene, /LIFE_MAP_FILTERS/)
  assert.match(scene, /Timeline/)
  assert.match(scene, /Seasons/)
  assert.match(scene, /People/)
  assert.match(scene, /Places/)
  assert.match(scene, /Rituals/)
  assert.match(scene, /Recovery/)
  assert.match(scene, /Dreams/)
  assert.match(scene, /Mirror/)
  assert.match(scene, /data-testid="urai-life-map-controls"/)
  assert.match(scene, /data-testid="urai-life-map-status"/)
  assert.match(scene, /data-testid="urai-replay-shell"/)
  assert.match(scene, /data-testid="urai-mirror-reflection-grid"/)
...
test('manifest schema includes AAA Life Map meaning fields and seed fallback normalizes them', () => {
  ...
  assert.match(manifests, /function normalizeManifest/)
  assert.match(manifests, /title: 'Soft Memory Bloom'/)
  assert.match(manifests, /title: 'Recovery Arc'/)
  assert.match(manifests, /title: 'Threshold Storm'/)
  assert.match(manifests, /title: 'Mirror Focus'/)
  assert.match(manifests, /reflectionSummary/)
})
