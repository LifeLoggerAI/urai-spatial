...
test('canonical Life Map has a 3D cosmic constellation and travel controls', () => {
  ...
  assert.match(constellation, /function LifeMapMythicMotifs/)
  assert.match(constellation, /function LifeMapAnchorStars/)
  assert.match(constellation, /function LifeMapEdgeConstellations/)
  ...
  assert.match(constellation, /data-testid="lifemap-mythic-constellation-motifs"/)
  assert.match(constellation, /data-testid="lifemap-anchor-stars"/)
  assert.match(constellation, /data-testid="lifemap-edge-constellations"/)
  assert.match(constellation, /data-testid=\{`lifemap-motif-\$\{motif\.id\}`\}/)
...
test('Life Map starfield is deterministic and reduced-motion safe', () => {
  ...
  assert.match(source, /const lifeMapMotifs: LifeMapMotif\[\]/)
  assert.match(source, /const nebulaClouds: NebulaCloud\[\]/)
  assert.match(source, /const anchorStars: AnchorStar\[\]/)
  assert.match(source, /const edgeConstellations: EdgeConstellation\[\]/)
  assert.match(source, /id: 'inner-profile'/)
  assert.match(source, /id: 'guardian-wolf'/)
  assert.match(source, /id: 'blue-center'/)
  assert.match(source, /id: 'ember-left'/)
  assert.match(source, /speed=\{reducedMotion \? 0 : 0\.34\}/)
  assert.match(source, /speed=\{reducedMotion \? 0 : 0\.16\}/)
  assert.match(source, /if \(!fieldRef\.current \|\| reducedMotion\) return/)
  assert.match(source, /if \(!ref\.current \|\| reducedMotion\) return/)
  assert.match(source, /if \(nodes\.length < 2\) return \[\]/)
})
