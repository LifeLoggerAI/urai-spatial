...
test('shared world runtime owns explicit consent, mute, route ambience and accessible equivalents', () => {
  ...
  assert.match(runtime, /destination\s*===\s*'replay'[\s\S]*return\s*null/)
  assert.match(runtime, /replay: 'Replay is source-audio-first\. No generic ambient loop plays when an authorized memory source has no audio\.'/)
