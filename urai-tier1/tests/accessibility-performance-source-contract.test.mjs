...
test('accessibility and performance implementation contracts are present', () => {
  ...
  requireText(embodiedHome, 'const ORB_POSITION = new THREE.Vector3(0, 1.36, -1.15)')
  requireText(embodiedHome, '<meshBasicMaterial transparent opacity={0} colorWrite={false} depthWrite={false} />')
  assert.doesNotMatch(embodiedHome, /name="home-only-companion"|emissiveIntensity=\{hovered|<pointLight color="#7cecf2"/, 'Embodied Home must not paint a second DOM-only Orb over the authored sanctuary Orb')
