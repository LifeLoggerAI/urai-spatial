...
test('accessibility and performance implementation contracts are present on two-mode Home and first-person Ground', () => {
  ...
  for (const marker of [
    "open ? 'Close Orb travel controls' : 'Open Orb travel controls'", 'aria-expanded={open}', 'aria-controls="urai-world-companion-menu"', 'inert={!open ? true : undefined}', 'firstControl?.focus()', 'orbRef.current?.focus()', "event.key !== 'Escape'", 'onClick={toggleCompanion}',
  ...
  for (const marker of ['min-height: 48px;', 'min-width: 48px;', 'env(safe-area-inset-bottom)', '@media (prefers-reduced-motion: reduce)']) requireText(companionCss, marker)
  ...
  for (const marker of ['/assets/urai/generated/human-makehuman-v4/home-human-makehuman-v4.glb', 'const idle = actions.idle_breath', 'if (!idle || reducedMotion || !visible)']) requireText(homeAvatar, marker)
  for (const marker of ["event.key !== 'Escape'", 'persistHomeReturnFrame(frame)']) requireText(homeController, marker)
  ...
  assert.doesNotMatch(currentHome, /first-person-hand|floating-hand|player-hand/, 'Non-XR Home first person must not synthesize hands')

  for (const marker of [
    'data-ground-exploration="first-person"', 'data-ground-runtime-owner="first-person-lived-world"', 'data-ground-camera="eye-level-terrain-following"', 'data-ground-collision="visible-terrain-heightfield"', 'data-ground-place-layer="consent-aware-empty-by-default"', 'data-ground-private-location-mounted="false"', 'MobileMovementPad', 'aria-label="Return Home"', 'aria-label="Ground place and privacy tools"', 'min-width:48px', 'min-height:48px', 'env(safe-area-inset-right)', 'env(safe-area-inset-left)', '@media(prefers-reduced-motion:reduce)',
  ...
  for (const marker of ['min-height:48px', 'env(safe-area-inset-left)', 'env(safe-area-inset-right)', 'env(safe-area-inset-bottom)', '@media(prefers-reduced-motion:reduce)']) requireText(focus, marker)
