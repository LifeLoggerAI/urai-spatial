...
test('accessibility and performance implementation contracts are present', () => {
  ...
  requireText(companion, 'const request = takePendingUraiWorldOrbOpen() ?? event.detail')
  requireText(companion, 'externalActivatorRef.current = request.returnFocusTo ?? null')
  ...
  requireText(worldEvents, 'pendingOrbOpenDetail = { returnFocusTo }')
  ...
  requireText(onboarding, 'data-route={pathname}')
  requireText(onboardingCss, '.uraiV2OnboardingCard[data-route="/home"]')
  requireText(onboardingCss, 'grid-template-columns: 92px 1fr;')
  requireText(onboardingCss, 'width: min(304px, calc(100vw - 36px));')
  ...
  requireText(embodiedEvidence, "home.getAttribute('data-home-player-z')")
  requireText(embodiedEvidence, 'Math.abs(Number(await')
  ...
  for (const marker of ['[data-urai-audit-action="orb-controls"]', "name: 'Open URAI Orb companion', exact: true", "toHaveAttribute('aria-hidden', 'false'", 'hasScrollableAncestor', 'scrollableGroundRail']) requireText(accessibilityEvidence, marker)
})
