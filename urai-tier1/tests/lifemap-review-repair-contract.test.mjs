...
test('Focus final composition gives one connected V272 living-memory fold pixel authority and rejects stale sphere/ring/lamella/shard dominance', () => {
  ...
  assert.match(focusGeology, /v280-lean-leading-tip-deep-waist-3d-s-gesture-tucked-counter-end-local-asymmetric-knot/)
  assert.match(focusGeology, /V280 removes that head-like mass/)
  assert.match(focusGeology, /const widthProfile = 1 - \.36 \* middleWaist/)
  assert.match(focusGeology, /- \.255 \* counterTuck/)
  assert.match(focusGeology, /const ridgeLift = \.092 \* endTaper \* ridge/)
