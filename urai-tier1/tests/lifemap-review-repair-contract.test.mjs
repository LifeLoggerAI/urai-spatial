...
test('Focus final composition preserves selected-star identity and rejects terrain or orb ownership', () => {
  ...
  assert.match(focusSource, /sourceMedia && memory && <FocusMemoryMedia/)
  assert.match(focusSource, /media=\{sourceMedia\} onState=\{setMediaState\}/)
  assert.match(focusSource, /<MemoryVisualContent memory=\{memory\} compact \/>/)
