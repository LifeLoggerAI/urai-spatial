...
test('portrait overview compacts the full celestial memory volume without dead-sky collapse', () => {
  ...
  assert.match(layoutSource, /\? \{ scale: \[\.38, \.72, \.92\], position: \[0, -\.10, 1\.25\] \}/)
  assert.match(layoutSource, /\+ \(portrait \? 2\.2 : 4\.5\)/)
  assert.match(layoutSource, /\+ \(portrait \? 2\.8 : 3\.6\)/)
  assert.match(layoutSource, /const widthDistance = halfWidth \/ Math\.max\(horizontalTan \* \(portrait \? 1\.02 : \.88\), \.08\)/)
  assert.match(layoutSource, /const heightDistance = halfHeight \/ Math\.max\(verticalTan \* \(portrait \? \.94 : \.86\), \.08\)/)
  assert.match(layoutSource, /const distance = Math\.max\(24, widthDistance, heightDistance\)/)
  assert.match(layoutSource, /target\[2\] - \(portrait \? 2\.0 : 4\.0\)/)
...
test('Focus final composition gives live WebGL luminous memory lamellae pixel authority and rejects stale static sphere/ring dominance', () => {
  ...
  assert.match(focusSource, /v269-living-luminous-memory-lamella/)
  assert.match(focusSource, /materialAuthority: 'vertex-energy-not-mineral-terrain-map'/)
  assert.match(focusSource, /data-focus-visual-revision="v269-luminous-memory-material-separation"/)
