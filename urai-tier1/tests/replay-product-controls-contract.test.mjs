...
test('Replay ignores stale authenticated reads and retries after a memory switch', () => {
  assert.match(client, /if \(!memory \|\| !mutable\)/)
  assert.ok((client.match(/let activeRequest = true/g) ?? []).length >= 2)
...
test('Replay persistence requires authenticated ownership and uses an existing protected collection', () => {
  ...
  assert.match(transport, /encodeURIComponent\(value\)/)
