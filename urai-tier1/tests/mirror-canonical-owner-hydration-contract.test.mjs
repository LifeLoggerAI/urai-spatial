...
// Final mobile-clearance exact-head trigger; behavior assertions remain fail-closed.
...
test('mobile Mirror reserves a clear touch zone and scroll ownership for inspection', () => {
  assert.match(mirrorClient, /\.mirrorOrb\{position:absolute;z-index:22;/)
  assert.match(mirrorClient, /data-movement-ui="true"/)
  assert.match(mirrorClient, /\.mirrorInspection\{touch-action:pan-y;overscroll-behavior:contain;/)
