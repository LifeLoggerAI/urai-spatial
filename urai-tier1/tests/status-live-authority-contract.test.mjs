...
test('Launch truth certifies web scope and preserves separate blocks', () => {
  ...
  assert.match(launchTruth, /path: '\/spatial\/ar-vr'[\s\S]*state: 'blocked'/)
  assert.match(launchTruth, /XR web entry is gated and is not part of the certified public launch surface/)
  assert.doesNotMatch(launchTruth, /Live XR browser preview/)
