...
test('persistent world navigation preserves canonical ownership and separate hit targets', () => {
  ...
  assert.match(controller, /currentWorld\.destination === 'location-map'/)
  assert.match(controller, /Life Map and Location Map own their realm-specific Escape contracts/)
  ...
  assert.doesNotMatch(companion, /PRIMARY_DESTINATIONS|SECONDARY_DESTINATIONS|destinationButtons|Public constellation/)
  ...
  assert.match(embodiedNavigation, /\.urai-mobile-movement\{[^}]*pointer-events:none/)
  assert.match(embodiedNavigation, /button\{pointer-events:auto;width:48px;height:48px/)
  ...
  assert.match(lifeMap, /next\.set\("returnNode", selected\.id\)/)
  assert.match(lifeMap, /router\.push\(destinationHref\("focus"\)\)/)
  assert.match(lifeMap, /router\.push\(destinationHref\("replay"\)\)/)
  ...
  assert.match(visualAudit, /Open UrAi Orb companion/)
