...
test('persistent world navigation preserves canonical ownership and separate hit targets', () => {
  ...
  const directDoorways = read('src/app/HomeDirectDoorways.tsx')
  ...
  assert.match(finalCss, /\.urai-final-home-doorways \{[\s\S]*display: none !important/)
  assert.match(finalCss, /\.urai-home-direct-doorways \{[\s\S]*position: fixed;[\s\S]*touch-action: manipulation !important/)
  assert.match(finalCss, /\.urai-home-direct-doorways a \{[\s\S]*min-height: 48px;[\s\S]*touch-action: manipulation !important/)
  assert.match(finalCss, /bottom: max\(96px, calc\(env\(safe-area-inset-bottom\) \+ 96px\)\)/)
  assert.match(finalCss, /@media \(max-width: 760px\)[\s\S]*\.urai-home-direct-doorways[\s\S]*bottom: max\(112px/)
