...
test('Home is one believable authored 3D real-world environment with atmospheric imagery only', () => {
  ...
  assert.match(homeCss, /\.world::before\s*\{[\s\S]*z-index:\s*3[\s\S]*opacity:\s*\.07[\s\S]*mix-blend-mode:\s*soft-light/)
  ...
  assert.match(homeProduction, /gl\.setClearColor\(0x9ba99a, 1\)/)
