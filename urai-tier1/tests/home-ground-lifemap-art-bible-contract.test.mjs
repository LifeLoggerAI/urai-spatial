...
test('Ground remains a bodyless first-person lived world with privacy-safe empty-by-default place authority', () => {
  ...
  assert.match(groundOwner, /GROUND_BROADLEAF_CANOPY = "\/assets\/urai\/ground-production\/cc0\/polyhaven-jacaranda-web-v1\.glb"/)
  assert.match(groundOwner, /class GroundCanopyBoundary extends Component/)
  assert.match(groundOwner, /scanned-understory-remains-if-canopy-load-fails/)
  assert.match(groundOwner, /cc0-scanned-broadleaf-edge-canopy-v35/)
  assert.match(groundOwner, /cc0-photoreal-broadleaf-canopy-v35/)
  assert.match(groundOwner, /web-optimized-decimated-meshopt-webp/)
