...
test('Ground is one embodied cinematic infrastructure world', () => {
  ...
  assert.match(groundOwner, /<color attach="background" args=\{\["#01040a"\]\} \/>/)
  assert.match(groundOwner, /gl\.setClearColor\(0x01040a, 1\)/)
  assert.match(groundOwner, /\.ground-spatial-root\{[^}]*background:#01040a/)
