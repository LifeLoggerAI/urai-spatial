...
test('Home keeps governed Orb states, reduced motion, and real traversal semantics', () => {
  ...
  assert.match(currentHomeVisual, /home-v229-textured-inhabited-valley-and-distant-ridge/)
  assert.match(currentHomeVisual, /map=\{albedo\} normalMap=\{maps\[1\]\} roughnessMap=\{maps\[2\]\}/)
  assert.match(currentHomeVisual, /const albedo = useSanctuarySoilTexture\(\)/)
