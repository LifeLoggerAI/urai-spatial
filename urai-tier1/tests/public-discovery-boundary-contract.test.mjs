...
test('entity registry distinguishes canonical product names from current and legacy technical aliases', () => {
  ...
  assert.equal(entity.entities.leadEngineer.name, 'Chris Herrin')
  assert.equal(entity.entities.leadEngineer.role, 'Lead Engineer')
  assert.equal(entity.entities.leadEngineer.canonicalUrl, 'https://urai.app/about/labs/#chris-herrin')
  assert.deepEqual(entity.entities.leadEngineer.externalProfiles, [])
  assert.doesNotMatch(JSON.stringify(entity.entities.leadEngineer), /linkedin\.com|sameAs/i)
...
test('Labs and llms publish Chris Herrin as Lead Engineer without ambiguous external identity links', () => {
  const labs = read('src/app/about/labs/page.tsx')
  assert.match(labs, /Chris Herrin/)
  assert.match(labs, /Lead Engineer/)
  assert.match(labs, /id="chris-herrin"/)
  assert.match(labs, /application\/ld\+json/)
  assert.ok(llms.includes('Lead Engineer: Chris Herrin'))
  assert.doesNotMatch(labs, /Founding Engineer|founding engineering contributor/)
