...
test('Home preserves the locked bodyless first-person + Orb + Ground + Sky authority', () => {
  assert.match(currentHome, /data-home-presence-presentation=\{firstPerson \? 'bodyless-first-person-home' : 'camera-only-transition'\}/)
...
test('Home Life Map ascent belongs to the broad sky rather than a localized portal', () => {
  assert.match(currentHome, /<HomeAtmosphericSky reducedMotion=\{reducedMotion\} active=\{transition === 'life-map'\} weatherState=\{personalWeatherState\} onLifeMap=\{onLifeMap\} \/>/)
  assert.match(currentHome, /data-home-distance-life-map="sky-threshold"/)
  assert.match(currentHome, /data-home-life-map-entry="visible-sky-broad-interaction"/)
  assert.match(currentHome, /data-home-scanned-composition="direct-bodyless-first-person-authored-living-memory-orb-sculpted-sanctuary-and-broad-sky-threshold"/)
  assert.doesNotMatch(currentHome, /PORTAL_MODEL/)
  assert.doesNotMatch(currentHome, /home-life-map-physical-portal/)
  assert.doesNotMatch(currentHome, /LifeMapPortal/)
})
