...
test('browser clients are same-origin and static App Router provider owners are absent', () => {
  ...
  for (const routePath of staticRoutePaths) {
    assert.equal(fs.existsSync(routePath), false, `${routePath.pathname} must remain absent from static export`)
  }
})
