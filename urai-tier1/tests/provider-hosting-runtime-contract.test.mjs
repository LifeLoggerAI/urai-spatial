...
const intelligenceEvalProvider = fs.readFileSync(new URL('../../apps/functions/src/intelligenceEvalProvider.ts', import.meta.url), 'utf8')
...
test('static Hosting rewrites every live provider URL to secret-bound Firebase Functions', () => {
  ...
  assert.match(functionsIndex, /openAiIntelligenceEvalProvider/)
...
test('synthetic intelligence evaluation adapter is secret-bound, non-storing and user-data isolated', () => {
  assert.match(intelligenceEvalProvider, /defineSecret\('OPENAI_API_KEY'\)/)
  assert.match(intelligenceEvalProvider, /defineSecret\('URAI_INTELLIGENCE_EVAL_TOKEN'\)/)
  assert.match(intelligenceEvalProvider, /secrets: \[OPENAI_API_KEY, URAI_INTELLIGENCE_EVAL_TOKEN\]/)
  assert.match(intelligenceEvalProvider, /timingSafeEqual/)
  assert.match(intelligenceEvalProvider, /omni-moderation-latest/)
  assert.match(intelligenceEvalProvider, /api\.openai\.com\/v1\/responses/)
  assert.match(intelligenceEvalProvider, /store: false/)
  assert.match(intelligenceEvalProvider, /syntheticContext/)
  assert.match(intelligenceEvalProvider, /responseText/)
  assert.match(intelligenceEvalProvider, /claims/)
  assert.doesNotMatch(intelligenceEvalProvider, /firebase-admin|firestore|privacyPolicy|users\//)
  assert.doesNotMatch(intelligenceEvalProvider, /console\.(log|info|warn|error)\([^)]*(userPrompt|syntheticContext|responseText)/)
})
