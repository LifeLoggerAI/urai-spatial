...
test('route boundary repairs direct-entry selection with a bounded and paced canonical retry and requires a healthy authored world', () => {
  ...
  assert.match(routeBoundary, /frame = window\.requestAnimationFrame\(verifyDirectRouteInvariant\)/)
  assert.doesNotMatch(routeBoundary, /let repaired = false|repaired = true/)
