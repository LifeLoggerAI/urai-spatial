...
test('independent launch-critical auditor uses receipt authority for production checks', () => {
  assert.match(auditor, /const receiptReleaseState = String\(receipt\.releaseState \|\| ''\)/)
  assert.match(auditor, /const productionReady = receiptReleaseState === 'production-ready'/)
  ...
  assert.doesNotMatch(auditor, /const productionReady = asset\.releaseState === 'production-ready'/)
})
