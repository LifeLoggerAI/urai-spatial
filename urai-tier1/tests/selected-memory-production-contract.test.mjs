...
test('Focus is an explorable authored selected-memory world rather than a static composited page', () => {
  ...
  assert.match(focus, /meshBasicMaterial vertexColors color=\{hovered \? '#f4f1e8' : '#ffffff'\} toneMapped=\{false\}/)
  ...
  assert.match(focus, /data-focus-selected-framing=\{memory \? 'v322-selected-memory-approach' : 'neutral-observatory-default'\}/)
  ...
  assert.match(cameraRig, /SELECTED_MEMORY_WIDE_CAMERA/)
  assert.match(cameraRig, /SELECTED_MEMORY_WIDE_TARGET/)
  assert.match(cameraRig, /SELECTED_MEMORY_PORTRAIT_TARGET/)
  assert.match(cameraRig, /selectedMemoryActive/)
  assert.match(cameraRig, /wideSelectedFraming/)
  assert.match(cameraRig, /camera\.position\.set\(\.\.\.framingCamera\)/)
  assert.match(cameraRig, /controls\.current\?\.target\.set\(\.\.\.framingTarget\)/)
  assert.match(cameraRig, /v322-wide-selected-memory-approach/)
  assert.match(cameraRig, /v322-portrait-selected-memory-lowered-target/)
  assert.match(cameraRig, /recenterSignal/)

  assert.match(focus, /data-focus-camera-x="0\.000"/)
  assert.match(focus, /data-focus-camera-y="1\.180"/)
  assert.match(focus, /data-focus-camera-z="5\.100"/)
