...
const focusSource = read('src/app/focus/FocusSourceMedia.tsx')
...
test('Focus resolves the selected Life Map star into a memory-containing close-up rather than terrain', () => {
  ...
  assert.match(focus, /focus-memory-star-corona-glow/)
  assert.match(focus, /focus-memory-star-organic-streamer-field/)
  assert.match(focus, /focus-memory-star-photosphere-surface/)
  assert.match(focus, /focus-memory-star-photosphere-core/)
  ...
  assert.match(focus, /scale=\{\[3\.72, 3\.22, 1\]\}/)
  assert.match(focus, /opacity=\{memory \? \.34 : \.10\}/)
  assert.match(focus, /scale=\{\[4\.36, 3\.58, 1\]\}/)
  assert.match(focus, /opacity=\{memory \? \.18 : \.06\}/)
  assert.match(focus, /scale=\{\[1\.86, 1\.72, 1\]\}/)
  assert.match(focus, /opacity=\{memory \? \.90 : \.05\}/)
  assert.match(focus, /geometry=\{photosphereGeometry\} scale=\{\[\.26, \.245, \.23\]\}/)
  assert.match(focus, /opacity=\{memory \? 0\.16 : 0\.05\}/)
...
test('Focus moment expands into video-first Replay and preserves the same memory into gated AR VR XR', () => {
  ...
  assert.match(focus, /import \{ FocusSourceMedia \} from '\.\/FocusSourceMedia'/)
  assert.match(focus, /<FocusSourceMedia media=\{\{ kind: media\.kind, url: media\.url, caption: media\.caption \}\}/)
  assert.match(focusSource, /data-focus-source-motion="still"/)
  assert.match(focusSource, /onLoadedData=/)
  assert.match(focusSource, /videoWidth > 0 && event\.currentTarget\.readyState >= 2 \? 'ready' : 'failed'/)
  assert.match(focusSource, /visibility: state === 'ready' \? 'visible' : 'hidden'/)
  assert.match(focus, /data-focus-replay-transition="moment-expands-into-replay"/)
  assert.doesNotMatch(focus, /<video[^>]*autoPlay/)
  assert.doesNotMatch(focusSource, /<video[^>]*autoPlay/)
