...
test('Focus resolves the selected Life Map star into a memory-containing close-up rather than terrain', () => {
  ...
  assert.match(focus, /data-focus-visual-revision="v370-irregular-corona-first-read-stellar-memory-field"/)
  ...
  assert.match(focus, /data-focus-stellar-treatment="selected-irregular-corona-first-read-stellar-field-v370"/)
  ...
  assert.match(focus, /geometry=\{photosphereGeometry\} scale=\{\[0\.34, 0\.28, 0\.22\]\}/)
  assert.match(focus, /opacity=\{memory \? 0\.08 : 0\.16\}/)
  ...
  assert.match(focus, /clip-path:polygon\(6% 37%,15% 14%,39% 3%,66% 9%,90% 27%,97% 53%,84% 79%,59% 96%,29% 91%,8% 68%\)/)
  assert.doesNotMatch(focus, /focusMemoryVisual\{[^}]*border-radius:50%/)
  assert.doesNotMatch(focus, /focusMemoryVisual\{[^}]*mask-image:radial-gradient\(circle/)
  assert.match(focus, /filter:blur\(\.9px\);opacity:\.68/)
  assert.match(focus, /width:136px;height:112px/)
  assert.match(focus, /focusStarMemoryButton\{width:122px;height:100px\}/)
  assert.match(focus, /focusStarMemoryButton\{width:112px;height:90px\}/)
  assert.match(focus, /opacity:\.64;filter:saturate\(\.84\)/)
  assert.match(focus, /focusMemoryVisual::before/)
  assert.match(focus, /mix-blend-mode:screen;opacity:\.18;z-index:4/)
  assert.match(focus, /opacity:\.16;z-index:5/)
  assert.match(focus, /focusMemoryVisual::after/)
  assert.match(focus, /box-shadow:none;clip-path:polygon\(6% 37%,15% 14%,39% 3%,66% 9%,90% 27%,97% 53%,84% 79%,59% 96%,29% 91%,8% 68%\)/)
