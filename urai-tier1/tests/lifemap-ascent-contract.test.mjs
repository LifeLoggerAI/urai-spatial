...
const sceneSource = fs.readFileSync(new URL('../src/components/lifemap/CosmicComposedLifeMapScene.tsx', import.meta.url), 'utf8')
...
test('destination becomes interactive through canonical Life Map-owned controls', () => {
  ...
  assert.match(sceneSource, /function MemoryStar/)
  assert.match(sceneSource, /visualAuthority: "stellar-memory-not-node-graph"/)
  assert.match(sceneSource, /stellarMorphology: "point-photosphere-layered-corona-no-visible-sphere"/)
  assert.doesNotMatch(sceneSource, /MEMORY_STAR_MODEL|data-semantic-label|data-artifact-family/)
  assert.match(sceneSource, /onClick=\{\(event\) => \{ event\.stopPropagation\(\); onSelect\(node\); \}\}/)
  assert.match(sceneSource, /showThresholds = Boolean\(selected && phase === "arrival"\)/)
  ...
  assert.match(sceneSource, /const returnHome = useCallback/)
  assert.match(sceneSource, /router\.push\(explicitDemo \? "\/home\?demo=1" : "\/home"\)/)
  assert.match(sceneSource, /onClick=\{returnHome\}/)
