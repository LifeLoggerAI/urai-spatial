...
test('current runtime keeps retired V215/V237 geology out while stellar Memory Stars own visible memory presentation', () => {
  ...
  assert.doesNotMatch(world, /function memoryHeartGeometry\(/)
  assert.doesNotMatch(world, /function memoryFilamentGeometry\(/)
...
test('Focus resolves the selected Life Map star while Replay preserves readable organic-edged media', () => {
  assert.match(focus, /Locked product authority; V343 is the current literal-pixel implementation/)
  ...
  assert.doesNotMatch(focus, /focusMemoryGeology|focusCurrentVisualAuthority|focusSelectedMemoryCavityDepth|createFocusGroundIncision/)
