...
import { applyReplayOperation, executeReplayOperation, flushReplayOperationQueue, readReplayOperationState } from '../src/spatial/replay/replayOperations.ts'
...
const baseOperation = { id: 'op-1', memoryId: 'memory-1', manifestId: 'manifest-1', ownerId: 'owner-1', kind: 'save', createdAt: '2026-07-17T13:00:00.000Z' }
...
test('rolls back a failed optimistic operation without erasing prior accepted history', async () => {
  ...
  const result = await executeReplayOperation({ storage, operation: hide, transport: { async persist() { throw new Error('offline') } } })
  assert.equal(result.saved, true)
...
test('retains pending operations and flushes them in order when transport recovers', async () => {
  const storage = memoryStorage()
  const pending = applyReplayOperation({ saved: false, hidden: false, pending: [], audit: [] }, { ...baseOperation, id: 'op-3', kind: 'hide' })
  storage.setItem('urai-replay-operations-v1:owner-1:memory-1', JSON.stringify(pending))
  const persisted = []
  const result = await flushReplayOperationQueue({ storage, ownerId: 'owner-1', memoryId: 'memory-1', transport: { async persist(operation) { persisted.push(operation.id) } } })
