...
test('semantic navigator is opt-in, semantically controlled, and keyboard accessible without a permanent rail', () => {
  ...
  assert.match(navigator, /useLayoutEffect\(\(\) => \{\s*if \(open\) searchRef\.current\?\.focus\(\)\s*\}, \[open\]\)/)
  assert.doesNotMatch(navigator, /window\.setTimeout\(\(\) => searchRef\.current\?\.focus\(\), 0\)/)
