...
test('portrait travel retains visual density without taking interaction ownership', () => {
  ...
  assert.match(density, /data-life-map-phase='departure'\] canvas[\s\S]*transform: scale\(1\.12\)/)
  assert.match(density, /data-life-map-phase='travel'\] canvas[\s\S]*transform: scale\(1\.45\)/)
  assert.match(density, /transform-origin: 50% 58%/)
