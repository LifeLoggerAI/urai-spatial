...
const production = read('src/spatial/layout/HomeWorldProduction.tsx')
...
test('Home keeps one live camera authority and canonical ascent', () => {
  ...
  assert.match(production, /HomeWorldProduction/)
  assert.match(production, /if \(scene\.phase === "ASCENT"\)/)
  assert.match(production, /scene\.setProgress\(linear\)/)
