...
test('obsolete Ground copy is rejected rather than required by the source owner', () => {
  ...
  assert.match(canonicalGround, /WorkforcePresence/)
  assert.match(canonicalGround, /DestinationArchitecture/)
  assert.match(canonicalGround, /Corridor/)
  ...
  assert.match(canonicalGround, /scrollIntoView\(\{\s*block:\s*'nearest',\s*inline:\s*'center',?\s*\}\)/)
