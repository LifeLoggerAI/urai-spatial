...
test('fine-pointer desktop omits mobile and permanent proof controls', () => {
  ...
  assert.match(css, /> \.home-movement-prompt,[\s\S]*\.urai-mobile-movement,[\s\S]*\.home-discreet-controls \{[\s\S]*display: none !important;/)
})
...
test('desktop provenance controls and unavailable ambience are absent', () => {
  const css = appFile('home-provider-preview-composition.css')
  assert.match(css, /\.home-discreet-controls \{[\s\S]*display: none !important;/)
