...
test('Ground renders the current atmospheric lived world instead of the rejected root-vault substrate', () => {
  ...
  assert.match(ground, /data-ground-art-revision="ground-v29-natural-microrelief-leaf-card-canopy-atmospheric-depth"/)
  ...
  assert.match(ground, /visibleAuthority: "runtime-authored-canopy-v29-leaf-card-foliage"/)
  assert.match(ground, /supersedesVisibleCandidate: "ground-v28-ellipsoid-leaflets-flat-procedural-soil"/)
  assert.match(ground, /literalPixelRepair: "v29-organic-canopy-layout-microrelief-soil-no-paving"/)
  assert.match(ground, /supplementalPixelRepair: "v29-alpha-tested-leaf-cards-dense-understory-non-laned-distribution"/)
  assert.match(ground, /alphaTest=\{0\.32\}/)
  assert.match(ground, /map=\{GROUND_LEAF_CARD_TEXTURE\}/)
  assert.match(ground, /slice\(0, woodland \? 34 : 30\)/)
