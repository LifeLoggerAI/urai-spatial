...
test('Ground renders a lived physical world instead of the retired chamber hub', () => {
  ...
  assert.match(ground, /data-ground-art-revision="ground-v14-deeper-living-substrate-root-geology-continuity"/)
  assert.match(ground, /data-ground-visual-owner="deeper-living-substrate-of-home"/)
  assert.match(ground, /name="ground-deeper-living-substrate-v14"/)
  assert.match(ground, /visualAuthority: "terrain-to-root-to-geology-to-lived-ground"/)
  assert.match(ground, /continuity: "same-home-deeper-substrate"/)
  assert.match(ground, /fantasyCave: false/)
  assert.match(ground, /exteriorForestOwner: false/)
  assert.match(ground, /name="ground-canonical-root-network-v14"/)
  assert.match(ground, /<GroundRootNetwork profile=\{profile\} \/>/)
  assert.match(ground, /<ScannedRock/)
  assert.match(ground, /<FernPatch/)
  assert.match(ground, /name="ground-moisture-channel"/)
  assert.match(ground, /<GroundSubstrateWorld profile=\{profile\} \/>/)
  assert.doesNotMatch(ground, /<AtmosphericGroundSky profile=\{profile\} \/>/)
  assert.doesNotMatch(ground, /<NaturalScatter profile=\{profile\} \/>/)
  assert.doesNotMatch(ground, /<DistantGroundContinuation profile=\{profile\} \/>/)
  assert.match(ground, /<color attach="background" args=\{\["#0b0e0d"\]\} \/>/)
  assert.match(ground, /<fogExp2 attach="fog" args=\{\["#18201d", 0\.028/)
