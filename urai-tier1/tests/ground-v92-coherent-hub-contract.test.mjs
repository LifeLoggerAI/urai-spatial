...
test('Ground renders a lived physical world instead of the retired chamber hub', () => {
  ...
  assert.match(ground, /data-ground-art-revision="ground-natural-surface-v13-varied-branch-flat-leaf-canopy-v13-atmosphere-v4-muted-ridge-v4"/)
  ...
  assert.match(ground, /new THREE\.CircleGeometry\(1, 8\)/)
  assert.match(ground, /crownWidth/)
  assert.match(ground, /transformedBranchDefs/)
  assert.match(ground, /horizonAuthority: "authored-irregular-ridge-v4-muted-fog-blended"/)
  assert.match(ground, /foliageAnchors/)
  assert.match(ground, /seed-varied-branch-architecture-fine-flat-leaf-canopy-v13/)
