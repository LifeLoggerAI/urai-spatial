...
test('Ground renders a lived physical world instead of the retired chamber hub', () => {
  ...
  assert.match(ground, /data-ground-art-revision="ground-natural-surface-v9-instanced-organic-canopy-v9-atmosphere-v2-ridge-v3"/)
  assert.match(ground, /name="ground-authored-natural-canopy-v9"/)
  assert.match(ground, /normalMap=\{normal\}/)
  assert.match(ground, /aoMap=\{arm\}/)
  assert.match(ground, /roughnessMap=\{arm\}/)
  assert.match(ground, /<Sky distance=\{450000\} sunPosition=\{\[-24, 7, -36\]\} turbidity=\{4\.2\} rayleigh=\{2\.6\} mieCoefficient=\{0\.0018\} mieDirectionalG=\{0\.76\} \/>/)
  assert.match(ground, /gl\.toneMappingExposure = 0\.78/)
  assert.match(ground, /CanopyLeafInstances/)
  assert.match(ground, /<instancedMesh/)
  assert.match(ground, /foliageAnchors/)
  assert.match(ground, /branch-tip-instanced-organic-foliage-canopy-v9/)
