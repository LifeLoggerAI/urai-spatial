...
test('living-memory palette stays weathered and restrained rather than cyan crystal or paper-white', () => {
  ...
  assert.match(geology, /focusLiteralPixelRefinement = 'v277-sculptural-tactile-weathered-fold-no-boulder-no-cloth-no-mouth'/)
  assert.match(geology, /color\.r = Math\.min\(\.88, color\.r\)/)
  assert.match(geology, /color\.g = Math\.min\(\.78, color\.g\)/)
  assert.match(geology, /color\.b = Math\.min\(\.58, color\.b\)/)
