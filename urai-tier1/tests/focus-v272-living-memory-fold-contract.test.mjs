...
test('V320 preserves the buried V295 watertight body while readable separated radial basins localize near-black bottoms and ground-blend the subordinate connector', () => {
  ...
  assert.match(geology, /const burialDepth = \.145/)
  assert.doesNotMatch(geology, /const burialDepth = \.060/)
  ...
  assert.match(geology, /\{ t: -\.48, radius: \.44, depth: \.084 \}/)
  assert.match(geology, /\{ t: -\.02, radius: \.48, depth: \.094 \}/)
  assert.match(geology, /\{ t: \.42, radius: \.42, depth: \.080 \}/)
  assert.match(geology, /const connectorDepth = \.0014 \* connectorFalloff/)
  assert.match(geology, /return -Math\.min\(\.100, basinDepth \+ connectorDepth\)/)
  assert.match(geology, /const connectorFalloff = Math\.exp\(-Math\.pow\(nearestDistance \/ \.020, 2\)\)/)
  assert.doesNotMatch(geology, /return -Math\.min\(\.070, connectorDepth \+ pocketDepth\)/)
  assert.match(geology, /const burialDepth = \.145/)
  assert.match(geology, /const hairlineHalfWidth = \.0027 \+ \.00055 \* Math\.sin/)
  assert.doesNotMatch(geology, /const hairlineHalfWidth = \.008 \+ \.0015 \* Math\.sin/)
  assert.match(geology, /const pocketHalfWidth = \.225 \* nearPocket \+ \.255 \* centerPocket \+ \.215 \* farPocket/)
  assert.doesNotMatch(geology, /const pocketHalfWidth = \.135 \* nearPocket \+ \.165 \* centerPocket \+ \.125 \* farPocket/)
  assert.doesNotMatch(geology, /const pocketHalfWidth = \.235 \* nearPocket \+ \.285 \* centerPocket \+ \.220 \* farPocket/)
  assert.match(geology, /focusIncisionAuthority = 'v320-readable-localized-near-black-cavity-bottoms-ground-blended-subordinate-hairline-connector'/)
  ...
  assert.match(geology, /focusCurrentVisualAuthority = 'v320-readable-three-radial-terrain-cavities-localized-bottoms-ground-blended-hairline-connector-buried-closed-body'/)
