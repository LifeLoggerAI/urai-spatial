...
test('Home keeps one V223 Canvas owner with cinematic Avatar presentation and persistent camera-only first-person Home', () => {
  ...
  has(activeHomeRuntime3d, "'visible-cinematic-avatar'")
  has(activeHomeRuntime3d, "'camera-only-avatar-embodied-first-person'")
  has(activeHomeRuntime3d, "'visible-avatar-third-person'")
  has(activeHomeRuntime3d, "'avatar-hidden-camera-first-person'")
  has(activeHomeRuntime3d, 'data-home-first-person-body="none-non-xr"')
  ...
  has(homeAvatar, '/assets/urai/generated/human-makehuman-v4/home-human-makehuman-v4.glb')
  has(activeHomeRuntime3d, 'THREE.LoopOnce')
  has(activeHomeRuntime3d, 'clampWhenFinished = true')
  has(homeAvatar, 'const idle = actions.idle_breath')
  has(activeHomeRuntime3d, 'stepEmbodiedMotion({')
  has(activeHomeRuntime3d, 'useMovementInput({')
  has(activeHomeRuntime3d, 'homeApi.activateAvatar()')
  has(activeHomeRuntime3d, 'homeApi.completeEmbodiment')
  has(activeHomeRuntime3d, 'AvatarSelfView')
  has(homeController, 'persistHomeReturnFrame(frame)')
  ...
  assert.doesNotMatch(activeHomeRuntime3d, /first-person-hand|floating-hand|player-hand/)
