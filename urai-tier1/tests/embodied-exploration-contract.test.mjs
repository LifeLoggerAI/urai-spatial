...
test('Home is the live single-Canvas V76 relic sanctuary with governed identities fail-closed', () => {
  ...
  for (const marker of [
    ...
    'data-home-art-certification="v122-retained-pixels-pending-not-certified"',
...
test('Ground remains walkable infrastructure with paths, boundaries and semantic exits', () => {
  ...
  assert.match(ground, /<primitive object=\{world\} visible \/>/)
  assert.match(ground, /nexus-\(\?:orbit\|spoke\|pedestal\)/)
  has(ground, 'data-ground-visual-revision="v122-authored-depth-no-flat-backdrop-authority"')
  assert.match(ground, /opacity:\.16/)
