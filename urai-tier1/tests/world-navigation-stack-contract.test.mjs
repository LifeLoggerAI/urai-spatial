...
import { clearWorldNavigationStack, popWorldNavigationCheckpoint, pushWorldNavigationCheckpoint, readWorldNavigationStack } from '../src/spatial/world/worldNavigationStack.ts'
...
test('navigation checkpoints unwind in strict last-in first-out order', () => {
  ...
  pushWorldNavigationCheckpoint(storage, { destination: 'home', href: '/home', entryPortal: 'home-threshold', cameraCheckpoint: 'home-threshold', savedAt: 1 })
  pushWorldNavigationCheckpoint(storage, { destination: 'life-map', href: '/life-map?node=quiet-reset', entryPortal: 'sky-ascent', cameraCheckpoint: 'selected-memory', savedAt: 2 })
  pushWorldNavigationCheckpoint(storage, { destination: 'focus', href: '/focus?memoryId=quiet-reset', entryPortal: 'selected-memory', cameraCheckpoint: 'focus-chamber', savedAt: 3 })
...
test('duplicate consecutive checkpoints do not corrupt unwind history', () => {
  const storage = createStorage()
  const checkpoint = { destination: 'focus', href: '/focus?memoryId=quiet-reset', savedAt: 1 }
...
test('malformed storage fails closed and clear removes history', () => {
  ...
  pushWorldNavigationCheckpoint(storage, { destination: 'replay', href: '/replay?memoryId=quiet-reset', savedAt: 1 })
