...
const runtimeLayer = fs.readFileSync(new URL('../src/app/HomeSpatialRuntimeLayer.tsx', import.meta.url), 'utf8')
...
assert.match(runtimeLayer, /role="status" aria-label="Your private world is forming"/)
assert.match(proof, /node\.closest\('\.sr-only'\)/)
assert.match(proof, /homeLoadingHold=1/)
assert.match(proof, /getByText\('Your private world is forming', \{ exact: true \}\)\.filter\(\{ visible: true \}\)\.first\(\)/)
...
assert.match(verifier, /for \(const forbidden of \[/)
assert.match(verifier, /data-home-animation-owner=\"gltf-authored-clips\"/)
assert.match(verifier, /retired procedural or rejected visual marker remains/)
assert.match(verifier, /HomeSanctuaryWorld/)
