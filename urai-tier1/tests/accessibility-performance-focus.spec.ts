...
test.describe('Focus exact-head accessibility and movement evidence', () => {
  test('reduced motion preserves explicit keyboard travel and truthful camera telemetry', async ({ page }) => {
    ...
    await expect(focus).toHaveAttribute('data-focus-spatial', 'selected-memory-star')
    await expect(focus).toHaveAttribute('data-focus-terrain-owner', 'false')
    await expect(focus).toHaveAttribute('data-focus-memory-visual', 'generated-memory-visualization')
