...
assert.ok(source.includes('const reach = 0.08 + t *'), 'memory filaments must branch outward rather than return to an atom-like center');
...
assert.ok(source.includes('Array.from({ length: 9 }'), 'the arrival place must preserve its offset nine-pillar braid');
