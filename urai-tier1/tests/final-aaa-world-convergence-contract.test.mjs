...
test('Orb ownership follows destination canon without visual duplication', () => {
  ...
  assert.match(homeProduction, /v67-six-armored-fragment-machine-with-contained-core/)
  ...
  assert.match(homeProduction, /destination === 'life-map'[\s\S]*name="home-life-map-physical-portal"/)
  assert.match(homeProduction, /<GovernedModel url=\{GOVERNED_PORTAL\}/)
