...
const activeHomeAvatar = read('src/spatial/home/HomeEmbodiedAvatar.tsx')
...
test('Orb and Home ownership preserve predecessor truth while the current candidate uses governed Avatar presentation to bodyless camera-only first-person embodiment', () => {
  ...
  assert.match(activeHomeProduction, /<HomeEmbodiedAvatar/)
  assert.match(activeHomeAvatar, /HOME_AVATAR_MODEL/)
  assert.match(activeHomeAvatar, /urai-home-user-avatar/)
  assert.match(activeHomeAvatar, /state !== 'hidden-first-person'/)
  ...
  assert.match(activeHomeProduction, /data-home-embodied-self=\{firstPerson \? 'camera-only-first-person-home' : 'visible-avatar-home-presentation'\}/)
  assert.match(activeHomeProduction, /data-home-presence-presentation=\{homeState\.transition === 'AVATAR_EMBODIMENT_TRANSITION'/)
  assert.match(activeHomeProduction, /'visible-avatar-presentation-activation-gate'/)
  assert.match(activeHomeProduction, /'bodyless-first-person-home'/)
  assert.match(activeHomeProduction, /data-home-camera-mode=\{homeState\.transition \?\? \(firstPerson \? \(dragging \? 'home-first-person-look' : 'home-first-person'\)/)
  assert.match(activeHomeProduction, /'home-avatar-presentation'/)
  ...
  assert.match(activeHomeProduction, /data-home-non-xr-body-policy="presentation-avatar-then-first-person-camera-only-no-hands-body-rig"/)
  assert.match(activeHomeProduction, /data-home-avatar-activation-gate="required-before-first-person-home"/)
