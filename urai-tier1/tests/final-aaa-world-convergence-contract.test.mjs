...
test('Orb ownership preserves certified V288 predecessor truth while the current candidate converges on the governed living-memory Orb', () => {
  ...
  assert.match(activeHomeProduction, /function\s+VisibleHomeAvatar\s*\(\{ reducedMotion \}/)
  assert.match(activeHomeProduction, /<VisibleHomeAvatar reducedMotion=\{reducedMotion\} \/>/)
  assert.match(activeHomeProduction, /const idle = actions\.idle_breath/)
  ...
  assert.doesNotMatch(activeHomeProduction, /data-home-movement="walk-keyboard-click-touch"/)
