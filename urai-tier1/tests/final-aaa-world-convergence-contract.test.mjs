...
test('Orb and Home ownership preserve predecessor truth while the current candidate uses the governed third-person to camera-only first-person embodiment', () => {
  ...
  assert.match(activeHomeProduction, /data-home-embodied-self=\{firstPerson \? 'camera-only-first-person-home' : 'visible-cinematic-avatar'\}/)
  assert.match(activeHomeProduction, /data-home-presence-presentation=\{avatarVisible \? 'visible-avatar-third-person' : firstPerson \? 'hidden-exterior-avatar-first-person' : 'transitioning'\}/)
  assert.match(activeHomeProduction, /data-home-camera-mode=\{homeState\.transition \?\? \(firstPerson \? 'avatar-home-first-person'/)
