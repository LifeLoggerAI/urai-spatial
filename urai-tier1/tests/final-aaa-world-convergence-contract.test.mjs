...
test('Life Map renders synchronous luminous lenses with dominant selected mode', () => {
  ...
  assert.match(lifeMapSelectedCinematic, /transform: translateY\(clamp\(-250px, -23vh, -150px\)\) scale\(\.72\)/)
  assert.match(lifeMapSelectedCinematic, /@media \(max-width: 760px\)[\s\S]*scale\(\.56\)/)
  assert.match(lifeMapSelectedCinematic, /min-height: 80px/)
