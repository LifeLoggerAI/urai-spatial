...
import type { SpatialCuratedDeckScheduleCheckpoint } from "@/spatial/curation/spatialCuratedDeckScheduleTypes";
...
export default function SpatialCuratedDeckSchedulePanel() {
  ...
  const schedule = useMemo(
    () => buildSpatialCuratedDeckSchedule({ entries: vaultEntries, activeEntryId }),
    [vaultEntries, activeEntryId],
  );
  const requiredCheckpoints = useMemo(
    () => schedule.checkpoints.filter((checkpoint) => checkpoint.required).slice(0, 4),
  ...
  return (
    ...
    >
      ...
      <div style={chipRowStyle}>
        <MetricChip label="eta" value={schedule.etaWindow} />
        <MetricChip label="cycle" value={schedule.reviewCycle} />
      </div>

      <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
        {requiredCheckpoints.length === 0 ? (
          <div style={{ fontSize: 12, lineHeight: 1.45, opacity: 0.82 }}>
            No required schedule checkpoints for the selected curated deck vault entry.
          </div>
        ) : (
          requiredCheckpoints.map((checkpoint) => (
            <ScheduleCheckpointRow key={checkpoint.id} checkpoint={checkpoint} />
          ))
        )}
      </div>
    </div>
...
function ScheduleCheckpointRow(input: {
  checkpoint: SpatialCuratedDeckScheduleCheckpoint;
}) {
  return (
    <div
      style={{
    ...
    >
      ...
      >
        ...
        <span style={{ opacity: 0.72 }}>{input.checkpoint.timing}</span>
      </div>
