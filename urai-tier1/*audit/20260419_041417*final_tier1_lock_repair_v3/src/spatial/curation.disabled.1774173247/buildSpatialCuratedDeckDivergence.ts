...
export function buildSpatialCuratedDeckDivergence(input: {
  ...
  if (!consensus.activeEntryId) {
    return {
      ...
      dominantFirstMode: consensus.dominantFirstMode,
      activeMatchesDominantFirstMode: consensus.activeMatchesDominantFirstMode,
  ...
  if (!consensus.activeMatchesDominantFirstMode) {
    score += 20;
  }
  ...
  const parts = [
    ...
    consensus.activeMatchesDominantFirstMode ? "scene aligned" : "scene offset",
  ];
