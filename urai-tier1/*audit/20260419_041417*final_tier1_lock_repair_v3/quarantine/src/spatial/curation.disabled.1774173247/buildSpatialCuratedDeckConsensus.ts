...
export function buildSpatialCuratedDeckConsensus(input: {
  ...
  if (input.entries.length === 0) {
    return {
      ...
      dominantFirstMode: null,
      activeMatchesDominantFirstMode: false,
  ...
  const dominantFirstMode = getMode(
    cohort
  ...
  const activeFirstMode = activeEntry.deck.cards[0]?.sceneMode ?? null;
  ...
  const activeMatchesDominantFirstMode =
    !!dominantFirstMode && activeFirstMode === dominantFirstMode;
  ...
  return {
    ...
    dominantFirstMode,
    activeMatchesDominantFirstMode,
