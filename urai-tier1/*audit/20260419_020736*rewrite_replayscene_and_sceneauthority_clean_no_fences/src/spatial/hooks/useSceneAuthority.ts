...
if (prev.phase !== "FOCUS") {
illegal(`openLifeMap blocked outside FOCUS/LIFEMAP { phase: ${prev.phase} }`);
}
...
if (!starId) {
illegal("openFocus missing starId");
}
setState((prevRaw) => {
const prev = normalizeRuntimeState(prevRaw);
...
if (!nextStarId) {
illegal("openReplay missing selectedStarId");
}
return {
...prev,
mode: "REPLAY",
...
```
  if (prev.phase === "FOCUS" || prev.phase === "LIFEMAP" || prev.phase === "HOME") {
    return prev;
  }
