...
function depthFactor(z: number) {
  const near = 1.7;
  const far = 0.55;
  const normalized = Math.max(0, Math.min(1, (z + 18) / 36));
  return near - normalized * (near - far);
}

function siblingOpacity(hovered: string | null, id: string) {
  if (!hovered) return 1;
  return hovered === id ? 1 : 0.68;
}
    ...
    <group visible={mode === "lifemap" || mode === "focusStar" || mode === "replay"}>
      {grouped.map((star) => (
        <StarMesh
          ...
          scale={(() => {
            const depth = depthFactor(star.position[2]);
            const focusBoost = hovered === star.id ? 1.22 : 1;
            return depth * focusBoost;
          })()}
