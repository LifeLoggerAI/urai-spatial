...
export default function StarMesh({
  ...
  useFrame((state, delta) => {
    ...
    if (core.current) {
      const nextScale = core.current.scale.x + (targetCore - core.current.scale.x) * (1 - Math.exp(-delta * 10));
      core.current.scale.setScalar(nextScale);
    }

    if (halo.current) {
      const nextScale = halo.current.scale.x + (targetHalo - halo.current.scale.x) * (1 - Math.exp(-delta * 7));
      halo.current.scale.setScalar(nextScale);
    }

    if (ring.current) {
      const nextScale = ring.current.scale.x + (targetRing - ring.current.scale.x) * (1 - Math.exp(-delta * 9));
      ring.current.scale.setScalar(nextScale);
      ring.current.rotation.z += delta * 0.22;
    }
  });

  const haloOpacity = dimmed ? 0.16 : selected ? 0.22 : hovered ? 0.16 : 0.09;
