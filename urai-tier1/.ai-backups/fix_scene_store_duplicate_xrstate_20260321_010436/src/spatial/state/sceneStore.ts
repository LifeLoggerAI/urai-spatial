...
export const useSceneStore = create<SceneState>((set, get) => ({
  ...
  selectObject: (id: string | null) => set({ mode: id ? "object" : "ground", phase: id ? "object-focus" : "ground", selectedObject: id }),
  ...
  setPresenting: (presenting: boolean) =>
    set((state) => ({ xrState: { ...state.xrState, presenting } })),
  appendRollbackPoint: (point: RollbackPoint) =>
    set((state) => ({ rollback: state.rollback.concat(point).slice(-50) })),
