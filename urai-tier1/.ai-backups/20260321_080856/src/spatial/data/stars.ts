...
export type { SpatialStar } from "../types";
...
export function createStars(count = 64, seed = 14414): SpatialStar[] {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    stars.push({
      ...
      title: "Memory " + String(i + 1),
      position: [x, y, z] as Vec3,
      color,
      size,
      glow,
      chapter: band === 0 ? "Origin" : band === 1 ? "Threshold" : band === 2 ? "Ascent" : "Return",
      timeband: band === 0 ? "Early" : band === 1 ? "Middle" : band === 2 ? "Late" : "After",
      signature: band === 0 ? "Seed" : band === 1 ? "Shift" : band === 2 ? "Bloom" : band === 3 ? "Echo" : null,
      tags: ["memory", "spatial"],
    });
  }
