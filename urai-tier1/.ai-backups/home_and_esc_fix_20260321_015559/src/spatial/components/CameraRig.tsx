...
export default function CameraRig() {
  ...
  const homePos = useMemo(() => new Vector3(1.55, 2.92, 12.0), []);
  const homeLook = useMemo(() => new Vector3(-0.42, 1.38, -0.05), []);
  const ascentPos = useMemo(() => new Vector3(0.12, 5.15, 7.25), []);
  const ascentLook = useMemo(() => new Vector3(0.22, 3.6, -10.5), []);
  const lifePos = useMemo(() => new Vector3(0, 4.0, 8.0), []);
  const lifeLook = useMemo(() => new Vector3(0, 2.7, -16.6), []);
  const groundPos = useMemo(() => new Vector3(-0.22, 2.0, 8.7), []);
  const groundLook = useMemo(() => new Vector3(0.1, 1.0, -0.9), []);

  useFrame((_, delta) => {
    const t = clock.getElapsedTime();
    const idleX = Math.sin(t * 0.2) * 0.07;
    const idleY = Math.cos(t * 0.25) * 0.045;

    if (mode === "home") {
      target.copy(homePos).add(temp.set(idleX * 0.38, idleY * 0.25, 0));
      look.copy(homeLook).add(temp.set(idleX * 0.06, idleY * 0.05, 0));
    } else if (phase === "to-lifemap") {
      target.copy(ascentPos).add(temp.set(idleX * 0.05, idleY * 0.04, -1.45));
      look.copy(ascentLook);
    } else if (mode === "lifemap" && !selectedStar) {
      target.copy(lifePos).add(temp.set(idleX * 0.16, idleY * 0.14, 0));
    ...
    camera.fov = MathUtils.lerp(
      camera.fov,
      mode === "replay" ? 35 : mode === "object" ? 39 : phase === "to-lifemap" ? 43 : mode === "lifemap" ? 46 : 47,
