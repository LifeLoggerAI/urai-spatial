...
function SceneContent() {
  ...
  return (
    <>
      ...
      <ambientLight intensity={mode === "replay" ? 0.07 : 0.14} />
      <directionalLight
        position={[4.5, 7.5, 6.5]}
        intensity={mode === "ground" || mode === "object" ? 1.05 : 1.32}
        color="#9abfff"
        castShadow
        shadow-mapSize-width={1024}
        shadow-mapSize-height={1024}
      />
      <directionalLight position={[-6.5, 4.8, -10]} intensity={0.24} color="#214cff" />
      <pointLight position={[-0.55, 1.75, 1.45]} intensity={mode === "home" || phase === "to-home" || phase === "to-lifemap" ? 0.95 : 0} distance={8.5} color="#7aa7ff" />
      <spotLight
        position={[0, 7.2, 4.6]}
        angle={0.44}
        penumbra={1}
        intensity={mode === "replay" ? 1.05 : 0.42}
        color="#ebf3ff"
      />
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#020a26" }}>
      <Canvas
        ...
        dpr={[1, 1.3]}
        performance={{ min: 0.8 }}
        camera={{ position: [1.75, 3.0, 11.6], fov: 48, near: 0.1, far: 240 }}
        gl={{
          antialias: true,
