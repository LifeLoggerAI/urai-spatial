...
import { toSpatialCompareSetContract } from "@/spatial/compare/spatialCompareTypes";
export function buildSpatialNarratorExport(input: {
  accountId: string;
  accountLabel: string | null;
  activeLens: SpatialTimelineLens | null;
  toSpatialCompareSetContract(activeCompareSet): SpatialCompareSet | null;
  ...
  const lines = [
    ...
    input.activeCompareSet
      ? `Compare set: ${input.toSpatialCompareSetContract(activeCompareSet).label}`
      : "Compare set: none",
    input.activeCompareSet
      ? `Scene mode changed: ${input.toSpatialCompareSetContract(activeCompareSet).summary.sceneModeChanged ? "yes" : "no"}`
      : "Scene mode changed: n/a",
    input.activeCompareSet
      ? `Selected star changed: ${input.toSpatialCompareSetContract(activeCompareSet).summary.selectedStarChanged ? "yes" : "no"}`
      : "Selected star changed: n/a",
    input.activeCompareSet
      ? `Locomotion distance: ${input.toSpatialCompareSetContract(activeCompareSet).summary.locomotionDistance}`
    ...
    input.activeCompareSet
      ? `Compared to ${input.toSpatialCompareSetContract(activeCompareSet).baseline.label}, the system now maps toward ${input.toSpatialCompareSetContract(activeCompareSet).target.label}.`
      : "No compare set is currently attached to this narrator export.",
    input.toSpatialCompareSetContract(activeCompareSet) && input.toSpatialCompareSetContract(activeCompareSet).summary.locomotionDistance > 0
      ? `Movement drift has accumulated to ${input.toSpatialCompareSetContract(activeCompareSet).summary.locomotionDistance} units across the compared eras.`
  ...
  return {
    ...
    compareSetLabel: input.toSpatialCompareSetContract(activeCompareSet)?.label ?? null,
    sceneMode: input.snapshot.sceneMode,
    selectedStarId: input.snapshot.selectedStarId,
    title,
    scriptText: lines.join("\n"),
    metadata: {
      locomotionDistance: input.toSpatialCompareSetContract(activeCompareSet)?.summary.locomotionDistance ?? null,
