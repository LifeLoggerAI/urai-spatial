...
function toOperatorText(input: {
  ...
  return `Run ${input.authorization} destruction authorization · path ${input.destructionPath} · evidence ${input.evidenceState}.`;
}
...
export function buildSpatialCuratedDeckDestructionAuthorization(input: {
  ...
  const parts = [
    `authorize ${authorizationScore}`,
    `decision ${authorization}`,
    `path ${destructionPath}`,
    `evidence ${evidenceState}`,
    `purge ${purge.purgePosture}`,
    `expiry ${expiry.expiryPosture}`,
    `archive ${archiveDisposition.disposition}`,
