...
function toOperatorText(input: {
  ...
  return `Run ${input.storageHorizon} retention horizon · expiry ${input.expiryBand} · window ${input.retentionWindow}.`;
}
...
export function buildSpatialCuratedDeckRetentionHorizon(input: {
  ...
  const parts = [
    ...
    `storage ${storageHorizon}`,
    `expiry ${expiryBand}`,
    `window ${retentionWindow}`,
    `lifecycle ${lifecycle.retentionState}`,
