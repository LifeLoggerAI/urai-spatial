...
Fresh LIVE provider readback on 2026-09-19 confirms an active/default LIVE Billing Portal configuration (`bpc_1UHSMz5ab9DrqB5jTX8B3yiJ`) for the LIVE account. That LIVE configuration is not TEST authority and must not be copied into a TEST deployment. A TEST Billing Portal configuration remains unverified/unbound in current evidence; exercise the TEST portal route only after an exact TEST-account configuration is positively read back and bound through approved non-production configuration.
...
The retained TEST catalog is historical provider evidence until the current TEST account/context is freshly read back. No verified non-production HTTPS Stripe webhook deployment with exact deployed SHA and current-account TEST signing-secret binding has been established in the available evidence. No webhook endpoint was created by guessing a staging URL. No TEST event is routed to production `urai.app`, and live billing remains unauthorized.


## 2026-09-19 current production boundary

The most recent positively verified public production runtime remains separate from this TEST runbook. At that readback, the canonical production routes required for Stripe activation returned 404:

- `/api/stripe/webhook`
- `/api/stripe/create-checkout-session`
- `/api/stripe/create-portal-session`

LIVE account inventory at the same launch-closeout stage showed the approved recurring provider prices and an active/default LIVE Billing Portal configuration, but **zero LIVE webhook endpoints**. LIVE billing remains off.

Do not create any LIVE webhook or activate charging until the exact reviewed billing source is deployed to the canonical HTTPS origin, the deployed source SHA is positively read back, the routes are verified, protected secret binding is authorized, and release governance is satisfied.
