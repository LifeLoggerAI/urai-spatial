...
Status: BASELINE / CONTENT SOURCE RECONCILIATION STILL REQUIRED
...
| Shadow | consent gate, intensity controls, exit, no-forced-entry language | MISSING/PARTIAL |
...
| Rituals | invitation, steps, completion/reflection, skip | MISSING/PARTIAL |
| Emotional Weather | evidence explanation, uncertainty, manual correction | MISSING/PARTIAL |
| Mood Forecast / Futures | scenario-not-prediction disclosure | MISSING/PARTIAL |
| Passive signals | source, why collected, permission, pause/disable, retention/deletion | PARTIAL |
| Empty state | meaningful starter world without fake personalization | MISSING |
| Offline/resync | local state, queued changes, conflicts, retry | MISSING |
| Permission denied | why blocked, what remains usable, settings path | MISSING/PARTIAL |
| Subscription | plan/entitlement/current provider truth | PARTIAL / CURRENT SOURCE REQUIRED |
...
Final copy authority must connect this manifest to `urai-content` content packs and current route source. Historical marketing text is not automatically product UI authority.
