...
Tier-1 through Tier-5 are not considered lockable until the scripted checks, frame audit, and manual screenshot review are green. Visual correctness is not enough. The lock must prove route behavior, camera movement, return behavior, ESC unwind, race-condition safety, loading-state stability, and back-stack/deep-link behavior.

## Required screen states

- Home: sky click target and Begin Ascent must both enter Ascent without duplicate timers or blank loading flashes.
- Ascent: auto-transition must resolve to Life Map inside the lock timing window; ESC during Ascent must return Home or otherwise follow the documented contract.
...
- Replay: ESC and Unwind must return exactly one layer to Focus.
- Mirror: route must expose return/ESC affordance, data-sync copy, loading/empty/error/success posture, and deterministic ESC/back behavior.
...
- ESC unwind Replay -> Focus -> Life Map -> Home.
- Browser back/forward after stack unwind.
- Ascent timing window.
- Reset View stability in Focus.
- Rapid Begin Ascent and ESC race coverage.
- ESC spam from Replay.
- Focus/Replay missing-manifest fallback behavior.
- Mirror route evidence and Mirror ESC contract.
