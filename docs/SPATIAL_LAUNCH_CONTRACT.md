...
URAI Spatial is currently a fallback/demo spatial shell. It is designed to preview the immersive URAI interface while keeping live provider claims disabled until the integration is verified.
...
5. Smoke and E2E tests prove core routes work without live providers.
...
Ship URAI Spatial only if the demo/fallback shell is stable and all copy, APIs, and docs agree that live providers are not active. Provider rollout requires a separate implementation PR with consent, tests, route smoke coverage, and deployment evidence.
