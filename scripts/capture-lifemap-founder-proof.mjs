...
const receipt = { schemaVersion: 'urai-lifemap-founder-proof-2', repository: 'LifeLoggerAI/urai-spatial', pr: 860, exactHead, runId: process.env.GITHUB_RUN_ID || 'local', capturedAt: new Date().toISOString(), captures: [] }
let failed = false
...
async function shot(page, id, captureState, extra = {}) {
  ...
  const state = await root.count() ? await root.evaluate((node) => ({ source: node.getAttribute('data-life-map-source'), phase: node.getAttribute('data-life-map-phase'), mode: node.getAttribute('data-life-map-mode'), webgl: node.getAttribute('data-webgl-state'), privateMounted: node.getAttribute('data-private-memory-mounted') })) : {}
...
async function desktopJourney() {
  ...
  try {
    ...
    await page.waitForTimeout(360); await shot(page, 'mid-travel', 'travel')
    await page.waitForTimeout(560); await shot(page, 'approach', 'approach')
    await page.waitForTimeout(620); await shot(page, 'stable-arrival', 'arrival')
    await shot(page, 'selected-memory-arrival', 'selected-arrival', { memoryId: 'quiet-reset' })
    await page.getByRole('button', { name: 'Enter Focus' }).click({ force: true })
    ...
    await goto(page, '/life-map/?demo=1&memoryId=quiet-reset&manifestId=replay-recovery-thread&node=quiet-reset')
    await page.waitForTimeout(1450)
    await page.getByRole('button', { name: 'Overview' }).click({ force: true }); await stable(page); await shot(page, 'overview-reset', 'overview-reset')
    await page.getByRole('button', { name: /The Quiet Reset/i }).first().click({ force: true }); await page.waitForTimeout(1450)
    await page.keyboard.press('Escape'); await stable(page); await shot(page, 'escape-unwind', 'escape-unwind')
...
async function mobileAndReduced() {
  ...
  try {
    ...
    await mobile.page.getByRole('button', { name: /The Quiet Reset/i }).first().click({ force: true }); await mobile.page.waitForTimeout(1450); await shot(mobile.page, 'portrait-mobile-selected', 'mobile-selected', { memoryId: 'quiet-reset' })
  } finally { await mobile.context.close() }
  const reduced = await openPage({ reducedMotion: 'reduce' })
  try {
    await goto(reduced.page, '/life-map/?demo=1&manifestId=replay-recovery-thread&overview=1'); await reduced.page.getByRole('button', { name: /The Quiet Reset/i }).first().click({ force: true }); await stable(reduced.page); await shot(reduced.page, 'reduced-motion-arrival', 'reduced-motion-arrival', { memoryId: 'quiet-reset' })
...
async function privacyAndRecovery() {
  ...
  try { await fallback.page.goto(new URL('/life-map/?demo=1', base).toString(), { waitUntil: 'domcontentloaded' }); await fallback.page.locator('[data-testid="urai-life-map-authored-fallback"], [data-testid="urai-true-3d-life-map"]').first().waitFor({ state: 'visible', timeout: 45_000 }); await shot(fallback.page, 'no-webgl-fallback', 'no-webgl') } finally { await fallback.context.close() }
  const recovery = await openPage()
  try {
    await goto(recovery.page, '/life-map/?demo=1&memoryId=quiet-reset&manifestId=replay-recovery-thread&node=quiet-reset'); await recovery.page.waitForTimeout(1450)
    await recovery.page.locator('canvas').first().evaluate((canvas) => { const gl = canvas.getContext('webgl2') || canvas.getContext('webgl'); gl?.getExtension('WEBGL_lose_context')?.loseContext() })
    await recovery.page.locator('[data-webgl-state="lost"], [data-webgl-state="recovering"]').first().waitFor({ state: 'attached', timeout: 10_000 }); await shot(recovery.page, 'webgl-context-loss', 'context-lost', { memoryId: 'quiet-reset' })
    await recovery.page.locator('canvas').first().evaluate((canvas) => { const gl = canvas.getContext('webgl2') || canvas.getContext('webgl'); gl?.getExtension('WEBGL_lose_context')?.restoreContext() }).catch(() => {})
    await recovery.page.waitForTimeout(1200); await shot(recovery.page, 'webgl-recovered', 'context-recovered', { memoryId: 'quiet-reset' }); await shot(recovery.page, 'context-recovery-state-preserved', 'context-recovered-selected', { memoryId: 'quiet-reset' })
