...
for (const token of ['URAI_SPATIAL_SERVICE', 'URAI_SPATIAL_VERSION', 'spatialLaunchBoundary', 'assertSpatialFallbackMode', 'spatialDeferredCapabilities', 'spatialLiveProviderRequirements', 'requirementsBeforeLiveProviders']) {
  if (!route.includes(token)) fail(`${files.route} missing response token ${token}`)
}
for (const token of ['launchBoundary: "/api/system/launch-boundary"', 'launchBoundary: spatialLaunchBoundary', 'fallbackMode: assertSpatialFallbackMode()', 'deferredCapabilities: spatialDeferredCapabilities', 'requirementsBeforeLiveProviders: spatialLiveProviderRequirements']) {
  if (!systemContract.includes(token)) fail(`${files.systemContract} missing contract token ${token}`)
}
if (!capabilitiesRoute.includes('requirementsBeforeLiveProviders')) fail(`${files.capabilitiesRoute} must expose requirementsBeforeLiveProviders`)
if (!docs.includes('/api/system/launch-boundary')) fail(`${files.docs} must document /api/system/launch-boundary`)
