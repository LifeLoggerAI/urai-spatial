...
export function resolveLegacyRedirectTarget(location, fingerprintUrl) {
...
export async function verifyLegacyLiveBootstrap({
  ...
  if (first.status >= 300 && first.status < 400) {
    ...
    const target = resolveLegacyRedirectTarget(location, fingerprintUrl)
...
export function runLegacyLiveBootstrapSelfTest() {
  ...
  const fingerprintUrl = `${CANONICAL_ORIGIN}${FINGERPRINT_PATH}`
  assert.equal(resolveLegacyRedirectTarget('/home', fingerprintUrl).pathname, '/home')
  assert.throws(
    () => resolveLegacyRedirectTarget('https://user:pass@urai.app/home', fingerprintUrl),
