...
for (const item of routes) {
  ...
  const unsafeLinks = await page.locator('a[href^="/admin"], a[href^="/internal"], a[href*="console.firebase.google.com"]').count()
  const passwordInputs = await page.locator('input[type="password"]').count()
  const eventDisclosure = item.name !== 'event' || (await page.locator('[data-demo-data="synthetic-sample-only"]').count()) === 1
...
const manifest = {
  ...
  routes: results,
  video: path.relative(outDir, videoTarget),
  consoleErrors,
  failures,
}
