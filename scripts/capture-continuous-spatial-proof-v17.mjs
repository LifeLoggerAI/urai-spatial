...
async function chooseVisibleLifeMapStar(page) {
  const selected = page.getByRole('button', { name: 'Enter Focus', exact: true }).first()
  if (await selected.isVisible().catch(() => false)) return true
...
async function approachHomeOrb(page) {
  const context = page.locator('.home-world-context')
...
function homeReviewRoute({ id, query, expectedMode, expectedFixture, expectedOrbState, exercise = false }) {
  return {
    id,
    path: `/home/?homeAssetReview=1&${query}`,
    ready: '.urai-asset-home-world[data-home-primary-owner="asset-driven"] canvas',
    waitForScene: async (page) => {
      await waitForStableAnimationFrames(page)
      if (exercise) await approachHomeOrb(page)
    },
    verify: async (page) => {
      const runtime = page.locator('[data-urai-home-runtime="asset-driven-primary-with-procedural-degraded-fallback"]')
      const owner = page.locator('.urai-asset-home-world[data-home-primary-owner="asset-driven"]')
      const semanticNavigation = page.getByRole('navigation', { name: 'Accessible Home destinations' })
      const semanticButtons = semanticNavigation.getByRole('button')
      const disclosure = page.getByRole('status').filter({ hasText: 'Review candidate composition' })
      const fallback = page.locator('.urai-final-home-world, .urai-genesis-home__world')
      const controls = page.locator('.home-discreet-controls button')
      const canvas = await canvasEvidence(page, '.urai-asset-home-world canvas')
      const movementContextVisible = exercise
        ? await visibleElementCount(page.locator('.home-world-context')) === 1
        : true
      const sceneLabelRetired = await visibleElementCount(semanticButtons) === 0

      return {
        runtimeMounted: await runtime.count() === 1,
        activeHomeOwnerMounted: await owner.count() === 1,
        reviewCandidateDisclosed: await owner.getAttribute('data-home-asset-mode') === 'disclosed-review-candidate',
        personalizationModeCorrect: await owner.getAttribute('data-home-personalization-mode') === expectedMode,
        reviewFixtureCorrect: await owner.getAttribute('data-home-review-fixture') === expectedFixture,
        orbStateCorrect: await owner.getAttribute('data-home-orb-state') === expectedOrbState,
        disclosureVisible: await visibleElementCount(disclosure) === 1,
        fallbackSuppressed: await visibleElementCount(fallback) === 0,
        semanticNavigationAttached: await semanticNavigation.count() === 1 && await semanticButtons.count() === 3,
        sceneLabelRetired,
        discreetControlsVisible: await visibleElementCount(controls) === 2,
        movementReachedOrb: movementContextVisible,
...
const routes = [
  homeReviewRoute({
    id: 'home-review-private',
    query: 'homePrivateFixture=1',
    expectedMode: 'private-personalized',
    expectedFixture: 'safe-private',
    expectedOrbState: 'idle',
    exercise: true,
  }),
  homeReviewRoute({
    id: 'home-review-privacy',
    query: 'homeState=permission-limited',
    expectedMode: 'permission-limited',
    expectedFixture: 'none',
    expectedOrbState: 'privacy',
  }),
  homeReviewRoute({
    id: 'home-review-unavailable',
    query: 'homeState=unavailable',
    expectedMode: 'unavailable',
    expectedFixture: 'none',
    expectedOrbState: 'warning',
  }),
  {
    id: 'ground',
    path: '/ground/',
    ...
    verify: async (page) => {
      ...
      const controls = rail.locator('button[data-ground-destination]')
      const navigationPillsStyled = await controls.count() >= 2 && await controls.evaluateAll((nodes) => nodes.every((node) => {
        const style = getComputedStyle(node)
        const rect = node.getBoundingClientRect()
      ...
      const activeGroundLinkSuppressed = await visibleElementCount(rail.locator('a[aria-current="page"]')) === 0
      const groundPath = new URL(page.url()).pathname.replace(/\/$/, '')
      ...
      return {
        ...
        activeGroundLinkSuppressed,
        groundRouteOwned: groundPath === '/ground',
        canvasSized: canvas.canvasSized,
        ...canvas,
      }
    },
  },
  {
    ...
    verify: async (page) => {
      ...
      return {
        ...
        firstSpatialFrameMarked: await page.evaluate(() => performance.getEntriesByName('urai:first-spatial-frame').length > 0),
        spatialDocumentVisible: spatialVisible === 'true',
        unselectedActionControlsAbsent: await visibleElementCount(unselectedFocus) === 0 && await visibleElementCount(unselectedReplay) === 0,
        canvasSized: canvas.canvasSized,
        overlayOpacities,
        ...canvas,
      }
  ...
  },
  {
    ...
    verify: async (page) => {
      ...
      return {
        ...
        singleSelectedActionOwner: selectedMemoryControlsVisible && replayControlVisible,
        canvasSized: canvas.canvasSized,
        ...canvas,
      }
    },
...
async function captureNoWebGLFallback() {
  ...
  const page = await context.newPage()
  const url = new URL('/home/', base).toString()
  const screenshotName = `home-no-webgl-fallback-desktop-no-webgl-${exactHead.slice(0, 12)}.png`
  const capture = {
    route: 'home-no-webgl-fallback',
  ...
  page.on('requestfailed', (request) => {
    capture.requestFailures.push({ url: request.url(), error: request.failure()?.errorText || 'unknown' })
  })

  try {
    const response = await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 60_000 })
    capture.status = response?.status() ?? null
    await page.locator('.urai-home-spatial-world-final').first().waitFor({ state: 'visible', timeout: 45_000 })
    await waitForStableAnimationFrames(page)
    ...
    capture.verification = {
      runtimeAbsent: await page.locator('.urai-home-spatial-runtime-layer').count() === 0,
      fallbackOwnerVisible: await visibleElementCount(page.locator('.urai-genesis-home__world')) > 0,
      fallbackActionVisible: actionEvidence.some((action) => action.visible),
      fallbackInteractive: actionEvidence.some((action) => action.visible && action.href?.startsWith('/ground') === true && action.interactive),
    }
    await takeScreenshot(page, path.join(outputDir, screenshotName))
    capture.passed = capture.status === 200
      && verificationPassed(capture.verification)
