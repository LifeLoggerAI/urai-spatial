...
const receiptDirectory = resolve(
  process.cwd(),
  process.env.URAI_DEPLOY_RECEIPT_DIR || `artifacts/production-release-${targetSha || 'invalid'}`,
)
...
function validateAuthority() {
  if (confirmation !== CONFIRMATION) {
    fail('URAI_DEPLOY_CONFIRM must equal DEPLOY VERIFIED URAI')
  }
  if (!commitPattern.test(targetSha)) {
    fail('URAI_TARGET_SHA must be a full lowercase 40-character SHA')
  }
  if (!commitPattern.test(rollbackSha)) {
  ...
  if (firebaseProject !== CANONICAL_PROJECT) {
    fail(`URAI_FIREBASE_PROJECT must equal ${CANONICAL_PROJECT}`)
  }
  ...
  if (git('status', '--porcelain')) fail('Release workspace must be clean')
...
async function inspect(url, contract) {
  const response = await fetch(url, {
    ...
    headers: {
      ...
      'user-agent': 'urai-exact-static-release/1.0',
    },
  })
...
async function verifyLive() {
  ...
  for (const contract of contracts) {
    for (const url of variants(contract.path)) {
      results.push(await inspect(url, contract))
    }
  }
  return {
...
function assertStaticOutput() {
  const requiredFiles = [
    'urai-tier1/out/index.html',
    'urai-tier1/out/ground/index.html',
    'urai-tier1/out/life-map/index.html',
    'urai-tier1/out/focus/index.html',
    'urai-tier1/out/replay/index.html',
    'urai-tier1/out/mirror/index.html',
    'urai-tier1/out/passport/index.html',
    'urai-tier1/out/privacy-controls/index.html',
    'urai-tier1/out/status/index.html',
  ]
  for (const file of requiredFiles) readFileSync(resolve(process.cwd(), file))
}
