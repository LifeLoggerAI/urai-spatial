...
const routeFor = (file) => {
  ...
  const segments = directory.split('/').filter(Boolean).filter((segment) => !segment.startsWith('(') && !segment.startsWith('@'))
  return segments.length ? `/${segments.join('/')}` : '/'
}

const classified = (route) => publicExact.has(route) || prefixGroups.some((prefixes) => prefixes.some((prefix) => route === prefix.replace(/\/$/, '') || route.startsWith(prefix)))
const routes = [...new Set(walk(appRoot).filter((file) => /(?:^|\/)page\.(?:ts|tsx|js|jsx)$/.test(file.replaceAll(path.sep, '/'))).map(routeFor))].sort()
const failures = routes.filter((route) => !classified(route)).map((route) => `unclassified route: ${route}`)
