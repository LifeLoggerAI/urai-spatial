...
requireIncludesAny(
  homeScenePath,
  ['onClick={isHomeMode ? enterLifeMap : undefined}', 'data-testid="urai-sky-click-target"'],
  'home sky click target or canonical home click activation',
)

requireIncludes(
  homeScenePath,
  'onClick={isHomeMode ? enterLifeMap : undefined}',
  'silent root-stage home activation',
)

requireIncludes(
  homeScenePath,
  'if (silentHomeInvariantProof(mode) === null) return null',
  'silent home guidance guard',
)
...
requireIncludes(
  ...
  'deterministic LifeMap node test ids',
)

requireIncludes(
  'urai-tier1/scripts/tier-lock/tier-config.mjs',
  "{ route: '/ascent'",
  'ascent route tier coverage',
)
...
requireIncludes(
  ...
  'TierOneExperience',
  'source-of-truth canonical shell',
)

requireIncludes(
  'docs/URAI_SPATIAL_SOURCE_OF_TRUTH_LOCK.md',
  'HomeScene',
  'source-of-truth canonical scene',
)
