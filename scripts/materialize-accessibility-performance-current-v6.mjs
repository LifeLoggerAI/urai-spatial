...
await transformFile('urai-tier1/tests/accessibility-performance-embodied-exploration.spec.ts', (input) => replaceExact(
  ...
  "    await expect(home).toHaveAttribute('data-home-animation-owner', 'v226-rooted-living-memory-presence')",
  1,
  'current V226 Home animation owner',
))
...
await transformFile('urai-tier1/tests/accessibility-performance-evidence.spec.ts', (input) => {
  let source = replaceExact(
    ...
    1,
    'current visible Home Orb activator after companion hydration for target sizing',
  )
  source = replaceExact(
    source,
  ...
  source = replaceExact(
    ...
    await expect(page.locator('#urai-world-companion-menu')).toHaveAttribute('aria-hidden', 'true')`,
    1,
    'current Home semantic Orb activation after companion hydration with focus-return lifecycle',
  )
  return source
})
