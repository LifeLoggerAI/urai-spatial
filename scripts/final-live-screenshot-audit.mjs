...
const expectedCopy = new Map([
  ...
  ['/spatial', /URAI|Spatial|Life Map|Focus|Replay/i],
...
const summary = {
  ...
  screenshots: routeResults.map((result) => result.screenshot),
...
await fs.writeFile(path.join(outDir, 'audit-summary.md'), [
  ...
  '## Interactions',
  '',
  ...interactionResults.map((item) => `- ${item.ok ? 'PASS' : 'FAIL'} ${item.name}: ${item.currentUrl || item.error || 'no result'}`),
  '',
].join('\n'));
