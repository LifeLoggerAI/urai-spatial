...
try {
  ...
  const companionOwner = page.locator('.urai-world-companion[data-destination="home"]')
  assert(await page.locator('button[data-testid="home-semantic-orb"]').count() === 1, 'canonical semantic Orb control is not unique')
  assert(await companionOwner.count() === 1, 'canonical PersistentWorldCompanion Orb owner is not unique')
  assert(await page.locator('button[data-world-target="orb-controls"]').count() === 0, 'Home must not expose a duplicate closed Orb target')
  ...
  await page.locator('.urai-world-companion[data-destination="home"][data-hydrated="true"]').waitFor({ state: 'attached', timeout: 20_000 })
  ...
  const closeOrb = page.getByRole('button', { name: 'Close UrAi Orb companion', exact: true })
  assert(await closeOrb.count() === 1, 'open Home companion close control is not unique')
  assert(await closeOrb.isVisible() && await closeOrb.isEnabled(), 'open Home companion close control is not usable')
