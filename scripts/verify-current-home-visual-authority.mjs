...
for (const required of ['HomeWorldProductionV223.tsx','HomeVisualAuthority.tsx','HomeAtmosphericSky.tsx','HomeOrbReliquaryV286.tsx','HomeOrbGroundedV288.tsx','HomeLaunchSanctuaryV254.tsx']) if (!authority.runtimeAssets.includes(required)) fail(`runtimeAssets missing ${required}`)
if (authority.runtimeAssets.includes('HomeWorldProductionV225PolishV2.tsx')) fail('superseded V225PolishV2 cannot be current')
...
for (const token of ['cinematic-lived-world-threshold','home-visible-user-avatar','home-grounded-companion','home-life-map-sky-threshold','visible-sky-broad-interaction','continuous-lived-physical-world','data-home-canvas-owner="home-world-production-v223-cinematic-threshold-authority"']) if (!runtime.includes(token)) fail(`runtime missing ${token}`)

const renderer = await readFile(path.join(layoutRoot, authority.rendererOwner), 'utf8')
for (const token of ["import { HomeAtmosphericSky } from '@/spatial/assets/HomeAtmosphericSky'",'<HomeAtmosphericSky reducedMotion={reducedMotion}',"active={transition === 'life-map'}",'onLifeMap={onLifeMap}',"cameraCheckpoint: 'home-sky-ascent'","cameraCheckpoint: 'home-sky-ascent-complete'","cameraCheckpoint: 'ground-first-person-arrival'","router.prefetch('/ground/')","router.prefetch('/life-map/')",'home-visible-user-avatar','home-gold-companion','data-home-ground-entry="physical-world-surface"','data-home-life-map-entry="visible-sky-broad-interaction"','data-home-embodied-self="visible-cinematic-avatar"','event.point.clone()']) if (!renderer.includes(token)) fail(`renderer missing ${token}`)
...
for (const token of ["import { HomeOrbGroundedV288 } from '../assets/HomeOrbGroundedV288'",'<HomeOrbGroundedV288 />','object.visible = false','object.raycast = () => {}']) if (!visualAuthority.includes(token)) fail(`visual authority missing ${token}`)
...
for (const token of ['RetireLocalizedLifeMapGateways','name="home-sky-life-map-threshold"',"threshold: 'broad-visible-sky'",'localGroundPortal: false','event.ray.direction.y > .015','onClick={activateSky}']) if (!atmosphere.includes(token)) fail(`sky threshold missing ${token}`)
